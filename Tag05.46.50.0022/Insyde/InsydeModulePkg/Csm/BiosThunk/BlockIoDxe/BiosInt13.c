...
/**
  Call hooked INT13 function.

  @param  This          The Block Io private data structure of this device.
  @param  Regs          The registers set.

  @return The flag returned from INT 13.
 
**/
STATIC
UINTN
BiosInt13Main (
  IN      BIOS_BLOCK_IO_DEV       *This,
  ...
  )
{
  ...
  INT13Vecter = (UINT32 *)(UINTN)INT13_VECTER;
  OldINT13 = *(INT13Vecter);
  
  *(INT13Vecter) = This->Bios.INT13Vector;
  
  CarryFlag = This->LegacyBios->Int86 (This->LegacyBios, 0x13, Regs);
  ...
  )
{
  ...
  CarryFlag = BiosInt13Main (BiosBlockIoDev, &Regs);
  ...
  )
{
  ...
  ZeroMem (&Regs, sizeof (EFI_IA32_REGISTER_SET));
  ZeroMem (mLegacyDriverUnder1Mb, sizeof (BIOS_LEGACY_DRIVE));
  Regs.H.AH = 0x48;
  Regs.H.DL = Drive->Number;
  ...
  //
  if (Drive->Parameters.Fdpt != 0xFFFFFFFF) {
    PointerMath = (UINTN)EFI_SEGMENT (Drive->Parameters.Fdpt) << 4;
  ...
  )
{
  ...
  if ((BiosBlockIoDev->MbrReadError) && (Lba == 0)) {
    //
    // Abort the procedure due to previous attempt failure,
    // and it may be intrinsic device error.
    //
    return EFI_DEVICE_ERROR;
  }
  ...
  for (; BufferSize > 0;) {
    ...
    //
    AddressPacket->SegOffset = (UINT32) (((UINTN) TransferBuffer >> 4) << 16);
    AddressPacket->Lba  = (UINT64) Lba;
    ...
    if (CarryFlag != 0) {
      ...
      if (RetryCount > 3) {
        BiosBlockIoDev->MbrReadError = TRUE;
        return EFI_DEVICE_ERROR;
      } else {
        RetryCount++;
        continue;
