...
/**
  Enable IRQ 1 in 8259.

  @retval EFI_SUCCESS           Enable IRQ 1 in 8259 successfully.
  @retval Other                 failed to get Legacy8259 protocol instance or IRQ mask.
**/
EFI_STATUS
EnableIrq1In8259 (
  VOID
  )
{
  ...
  Status = Legacy8259->GetMask(Legacy8259, &Mask, NULL, NULL, NULL);
  if (EFI_ERROR (Status)) {
    return Status;
  }

  if ((Mask & (UINT16) BIT1) != 0) {
    Mask &= ~((UINT16) BIT1);
    Status = Legacy8259->SetMask(Legacy8259, &Mask, NULL, NULL, NULL);
  }
  ...
  )
{
  ...
  if (Ps2Policy != NULL) {
    ...
    Status = KeyboardUpdateStatusLights (BiosKeyboardPrivate, Command);
    if (EFI_ERROR (Status)) {
...
/**
  Update keyboard status lights by command.

  @param[in] BiosKeyboardPrivate   Bioskeyboard driver private structure.
  @param[in] Command               Keyboard status lights command

  @retval EFI_SUCCESS              Update keyboard status lights successfully.
  @retval EFI_DEVICE_ERROR         Failed to write data to KBC.
  @retval Other                    Failed to locate lagacy BIOS protocol.
**/
EFI_STATUS
KeyboardUpdateStatusLights (
  IN BIOS_KEYBOARD_DEV       *BiosKeyboardPrivate,
  IN UINT8                   Command
  )
{
