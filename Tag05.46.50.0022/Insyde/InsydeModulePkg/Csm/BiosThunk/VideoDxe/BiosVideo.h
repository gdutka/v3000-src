/** @file
  Header file for Bios Video Driver's Data Structures
...
MultiEdidFound (
  ...
  OUT UINT16          *ValidDDCPortNum,
  OUT UINT16          *VgaVendorID,
  OUT UINT8           *MaxNumOfDisplayDevIndex
  );
...
BOOLEAN
EFIAPI
IsMultiEdidChecksumChanged (
  IN VESA_BIOS_EXTENSIONS_INFO *VesaBiosExtensionsInfo
  );
...
GetIntersectedHigestResolution (
  ...
  IN  UINT32                                      LowerNativeX,
  IN  UINT32                                      LowerNativeY,
  OUT UINT32                                      *IntersectedHigestResolutionX,
  OUT UINT32                                      *IntersectedHigestResolutionY
  );
