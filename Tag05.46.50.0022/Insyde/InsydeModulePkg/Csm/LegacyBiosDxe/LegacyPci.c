...
typedef struct {
  H2O_FLASH_MAP_USB_OPTION_ROM_ID   OptionRomId;
  UINT8                             *RomImage;
  UINTN                             RomSize;
  BOOLEAN                           IsShadowed;
} FLASH_MAP_USB_OPROM_INFO;

typedef struct {
  BOOLEAN                           Initialized;
  //
  // USB option rom from OEM services
  //
  UINTN                             OptionRomCount;
  USB_OPROM_TABLE                   *OptionRomTable;
  BOOLEAN                           *OptionRomShadowed;

  //
  // USB option rom from flash map
  //
  UINTN                             FlashMapOptionRomCount;
  FLASH_MAP_USB_OPROM_INFO          *FlashMapOptionRomList;
} USB_OPROM_INFO;

USB_OPROM_INFO                      mUsbOpromInfo = {0};
EFI_HANDLE                          mUsbDevHandle = NULL;
EDKII_IOMMU_PROTOCOL                *mIoMmu;

/**
  Use PCI Handle, PCI vendor ID and device ID for the H2O Secerity checking mechanism.
  In this case, it will check if the option ROM image is available.

  @param[in]  PciHandle               PCI device handle of the dispatched option ROM.
  ...
  @retval EFI_UNSUPPORTED         Can't locate the H2OSecurityArch protocol.
                                  Can't get device path from PCI handle.

**/
EFI_STATUS
EFIAPI
H2OSecurityArchCheck (
  ...
  )
{
  ...
  if (FeaturePcdGet (PcdH2OPciOptionRomSecurityControlBySlot)) {
    //
    // Need device path if the Option ROM control is slot-based.
    //
  ...
  for (Index = 0; Index < NumberOfHandles; Index++) {
    ...
    if (!EFI_ERROR (Status)) {
      SecStatus = H2OSecArchProtocol->FileAuthentication(PciDevicePath, VendorId, DeviceId, TRUE);
  ...
  )
{
  ...
  //
  TempBridgesTable = AllocatePool (sizeof (BRIDGE_TABLE) * (HandleCount));
  ...
  TempSortedBridgeIndex = AllocatePool (sizeof (UINTN) * (HandleCount));
  ...
  //
  mBridges = AllocatePool (sizeof (BRIDGE_TABLE) * (mNumberOfBridges));
  if (mBridges == NULL) {
    Status = EFI_OUT_OF_RESOURCES;
    goto Done;
  }
  ...
  mSortedBridgeIndex = AllocatePool (sizeof (UINTN) * (mNumberOfBridges));
  if (mSortedBridgeIndex == NULL) {
    Status = EFI_OUT_OF_RESOURCES;
    goto Done;
  }
  ...
  )
{
  ...
  for (Index = 0; Index < HandleCount; Index++) {
    ...
    //
    // This device is not existing or maybe it has been hidden,
    // If it has been hidden, don't use the Interrupt Pin of the device to do anything, just skip it.
    //
    if (PciConfigHeader.Hdr.DeviceId == 0xffff) {
      continue;
    }
      ...
      if ((EFI_ERROR (Status)) && (PciConfigHeader.Hdr.ClassCode[2] == PCI_CLASS_MASS_STORAGE)) {
        ...
        for (MassStorageHandleIndex = 0; MassStorageHandleIndex < MassStorageHandleCount; MassStorageHandleIndex++) {
          if (MassStorageHandleBuffer[MassStorageHandleIndex] == HandleBuffer[Index]) {
            //
            // InstallLegacyIrqHandle function is only for IDE controller
            //
            if (PciConfigHeader.Hdr.ClassCode[1] == PCI_CLASS_MASS_STORAGE_IDE) {
              //
...
/**
  Get the pointer to the hardware PCI device path node in input device path.

  @param[in] DevicePath                Pointer to the device path

  @return A pointer to the hardware PCI device path node or NULL if not found.
**/
STATIC
PCI_DEVICE_PATH *
InternalGetPciDevicePath (
...
/**
  Get the Segment/Bus/Device/Function value for input handle.

  @param[in]  Handle                   The handle which contain a device path protocol instance
  @param[out] Bus                      Pointer to the output PCI bus value
  @param[out] Dev                      Pointer to the output PCI device value
  @param[out] Fun                      Pointer to the output PCI function value

  @retval EFI_SUCCESS                  Get the Bus/Device/Function value successfully.
  @retval EFI_NOT_FOUND                Failed to get PCI device path node.
  @retval Other                        Failed to get device path protocol or read PCI config register.
**/
STATIC
EFI_STATUS
InternalGetSegBusDevFun (
...
/**
  Install USB option ROM.

  @param[in] Private                   Pointer to legacy BIOS private instance
  @param[in] UsbDevHandle              USB device handle
  @param[in] RomImage                  Pointer to USB option ROM image

  @retval EFI_SUCCESS                  Install USB option ROM successfully
  @retval Other                        Failed to install USB option ROM
**/
STATIC
EFI_STATUS
InstallUsbOprom (
  IN LEGACY_BIOS_INSTANCE              *Private,
  IN EFI_HANDLE                        UsbDevHandle,
  IN UINT8                             *RomImage
  )
{
  EFI_STATUS                           Status;
...
/**
  Check if input board ID list support current board ID.

  @param[in] CurrentBoardId            Current board ID value
  @param[in] BoardIdList               Pointer to board ID list
  @param[in] BoardIdCount              Number of board ID in list

  @retval TRUE                         Board ID list support current board ID
  @retval FALSE                        Board ID list does not support current board ID
**/
STATIC
BOOLEAN
IsBoardIdSupported (
  IN H2O_BOARD_ID                      CurrentBoardId,
  IN UINT8                             *BoardIdList,
  ...
  )
{
  ...
  for (Index = 0; Index < BoardIdCount; Index++) {
    if (BoardIdList[Index] == 0xFF) {
      break;
    }

    if (BoardIdList[Index] == 0 ||
        BoardIdList[Index] == CurrentBoardId) {
      return TRUE;
    }
  }
...
/**
  Initialize USB option ROM info into module variable from flash device map.

  @retval EFI_SUCCESS                  Initialize USB option ROM info successfully.
  @retval EFI_NOT_FOUND                Failed to get USB option ROM table or skip by oem service return status.
  @retval EFI_OUT_OF_RESOURCES         Failed to allocate memory.
**/
EFI_STATUS
InitializeUsbOpromInfoFromFalshMap (
  VOID
  )
{
  EFI_STATUS                                   Status;
  ...
  UINT8                                        RegionCount;
  UINT8                                        Count;
  FLASH_MAP_USB_OPROM_INFO                     *FlashMapOpRomInfo;
  H2O_FLASH_MAP_USB_OPTION_ROM_ID              OptionRomId;
  UINT64                                       BaseAddress;
  UINT64                                       RegionOffset;
  UINT64                                       RegionSize;
  ...
  BOOLEAN                                      IsLegacyOprom;

  RegionCount = 0;
  Status = FdmGetNCount (&gH2OFlashMapRegionUsbOptionRomGuid, &RegionCount);
  if (EFI_ERROR(Status) || RegionCount == 0) {
    return EFI_NOT_FOUND;
  }

  FlashMapOpRomInfo = AllocateZeroPool (RegionCount * sizeof (FLASH_MAP_USB_OPROM_INFO));
  if (FlashMapOpRomInfo == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }

  Count       = 0;
  BoardId     = PcdGet64(PcdH2OBoardId);
  BaseAddress = FdmGetBaseAddr();
  if (BaseAddress == 0) {
    return EFI_NOT_FOUND;
  }

  for (Index = 1; Index <= RegionCount; Index++) {
    Status = FdmGetNAt (&gH2OFlashMapRegionUsbOptionRomGuid, Index, (UINT8 *) &OptionRomId, &RegionOffset, &RegionSize, &Attr);
    if (EFI_ERROR(Status)) {
      continue;
    }

    IsLegacyOprom = (BOOLEAN) ((OptionRomId.Flags & (BIT7 | BIT0)) == BIT7);
    if (!IsLegacyOprom ||
        !IsBoardIdSupported (BoardId, OptionRomId.Boards, sizeof(OptionRomId.Boards) / sizeof (OptionRomId.Boards[0]))) {
      continue;
    }

    FlashMapOpRomInfo[Count].RomImage = (UINT8 *) (UINTN) (BaseAddress + RegionOffset);
    FlashMapOpRomInfo[Count].RomSize  = (UINTN) RegionSize;
    CopyMem (&FlashMapOpRomInfo[Count].OptionRomId, &OptionRomId, sizeof (H2O_FLASH_MAP_USB_OPTION_ROM_ID));
    Count++;
  }
  if (Count == 0) {
    FreePool (FlashMapOpRomInfo);
    return EFI_NOT_FOUND;
  }

  mUsbOpromInfo.FlashMapOptionRomList  = FlashMapOpRomInfo;
  mUsbOpromInfo.FlashMapOptionRomCount = Count;

  return EFI_SUCCESS;
}

/**
  Initialize USB option ROM info into module variable from OEM service function.
...
STATIC
EFI_STATUS
InitializeUsbOpromInfoFromOemService (
  VOID
  )
{
  EFI_STATUS                           Status;
  ...
  Status = OemSvcInstallOptionRomTable (USB_OPROM, (VOID **) &OptionRomTable);
  ...
  OptionRomCount = 0;
  while (!CompareGuid(&OptionRomTable[OptionRomCount].FileName, &gZeroGuid)) {
    OptionRomCount++;
  }
  if (OptionRomCount == 0) {
    return EFI_NOT_FOUND;
  ...
  OptionRomShadowed = AllocateZeroPool (OptionRomCount * sizeof (BOOLEAN));
  if (OptionRomShadowed == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }

  mUsbOpromInfo.OptionRomTable    = OptionRomTable;
  mUsbOpromInfo.OptionRomCount    = OptionRomCount;
  mUsbOpromInfo.OptionRomShadowed = OptionRomShadowed;

  return EFI_SUCCESS;
}

/**
  Install USB option ROM by enumerating all UsbIo protocol.

  @param[in] Private                   Pointer to legacy BIOS private instance

  @retval EFI_SUCCESS                  Install USB option ROM successfully.
  @retval EFI_NOT_FOUND                Failed to get USB option ROM table.
  @retval Other                        Failed to get all UsbIo protocol.
**/
STATIC
EFI_STATUS
ShadowAllUsbOprom (
  ...
  )
{
  ...
  UINTN                                OptionRomIndex;
  VOID                                 *RomImage;
  UINTN                                RomSize;

  if (!mUsbOpromInfo.Initialized) {
    InitializeUsbOpromInfoFromOemService ();
    InitializeUsbOpromInfoFromFalshMap ();
    mUsbOpromInfo.Initialized = TRUE;
  }
  if (mUsbOpromInfo.OptionRomCount == 0 && mUsbOpromInfo.FlashMapOptionRomCount == 0) {
    return EFI_NOT_FOUND;
  }

  //
  // Shadow option ROMs for the UsbIo which has the matched option ROM file.
  //
  Status = gBS->LocateHandleBuffer (
                  ByProtocol,
  ...
  for (Index = 0; Index < HandleCount; Index++) {
    Status = gBS->HandleProtocol (HandleList[Index], &gEfiUsbIoProtocolGuid, &UsbIo);
    if (EFI_ERROR (Status)) {
      continue;
    }
    Status = UsbIo->UsbGetDeviceDescriptor (UsbIo, &UsbDescriptor);
    if (EFI_ERROR (Status)) {
      continue;
    }

    //
    // Check if there is a matched USB option ROM from OEM service
    //
    for (OptionRomIndex = 0; OptionRomIndex < mUsbOpromInfo.OptionRomCount; OptionRomIndex++) {
      if (mUsbOpromInfo.OptionRomTable[OptionRomIndex].ProductId == UsbDescriptor.IdProduct &&
          mUsbOpromInfo.OptionRomTable[OptionRomIndex].VendorId  == UsbDescriptor.IdVendor) {
        if (mUsbOpromInfo.OptionRomShadowed[OptionRomIndex]) {
          break;
        }

        mUsbOpromInfo.OptionRomShadowed[OptionRomIndex] = TRUE;
        Status = GetSectionFromAnyFv (
                   &mUsbOpromInfo.OptionRomTable[OptionRomIndex].FileName,
                   EFI_SECTION_RAW,
                   0,
                   &RomImage,
                   &RomSize
        ...
        InstallUsbOprom (Private, HandleList[Index], RomImage);
        FreePool (RomImage);
        break;
      }
    }
    if (OptionRomIndex < mUsbOpromInfo.OptionRomCount) {
      continue;
    }

    //
    // Check if there is a matched USB option ROM from flash device map
    //
    for (OptionRomIndex = 0; OptionRomIndex < mUsbOpromInfo.FlashMapOptionRomCount; OptionRomIndex++) {
      if (mUsbOpromInfo.FlashMapOptionRomList[OptionRomIndex].OptionRomId.ProductId == UsbDescriptor.IdProduct &&
          mUsbOpromInfo.FlashMapOptionRomList[OptionRomIndex].OptionRomId.VendorId  == UsbDescriptor.IdVendor) {
        if (mUsbOpromInfo.FlashMapOptionRomList[OptionRomIndex].IsShadowed) {
          break;
        }

        mUsbOpromInfo.FlashMapOptionRomList[OptionRomIndex].IsShadowed = TRUE;
        InstallUsbOprom (Private, HandleList[Index], mUsbOpromInfo.FlashMapOptionRomList[OptionRomIndex].RomImage);
        break;
      }
    }
  }
  ...
  )
{
  ...
  *Flags = NO_ROM;
  ...
  if (FeaturePcdGet (PcdH2OPciOptionRomSecurityControlSupported)) {
    Status = H2OSecurityArchCheck (PciHandle, PciConfigHeader.Hdr.VendorId, PciConfigHeader.Hdr.DeviceId);
  ...
  )
{
  ...
  if (PciHandle == NULL && mUsbDevHandle != NULL) {
    //
    // Install USB option ROM now. No need to check if it is in VGA progress because PCI ROM is isntalled before USB.
    //
    Status = InternalGetSegBusDevFun (mUsbDevHandle, &Segment, &Bus, &Device, &Function);
    if (EFI_ERROR (Status)) {
      return Status;
    }
  }
  ...
  if ((EndOpromShadowAddress = PcdGet32 (PcdEndOpromShadowAddress)) == 0) {
    EndOpromShadowAddress = Private->BiosStart - 1;
  }
  ...
  if ((OpromRevision >= 3) && (Private->Csm16PciInterfaceVersion >= 0x0300)) {
    ...
    if (EFI_ERROR (Status)) {
      if (ImageSize <= PcdGet32 (PcdOpromReservedMemorySize)) {
        //
        // Use OpromReservedMemory for PCI 3.0 OpRom if run out of conventional memory
        //
        PhysicalAddress = PcdGet32 (PcdOpromReservedMemoryBase);
      } else {
    ...
    if (RuntimeAddress + *RuntimeImageLength > EndOpromShadowAddress + 1) {
      DEBUG ((EFI_D_ERROR, "return LegacyBiosInstallRom(%d): EFI_OUT_OF_RESOURCES (no more space for OpROM)\n", __LINE__));
    ...
    if (InitAddress + ImageSize > EndOpromShadowAddress + 1) {
      DEBUG ((EFI_D_ERROR, "return LegacyBiosInstallRom(%d): EFI_OUT_OF_RESOURCES (no more space for OpROM)\n", __LINE__));
      return EFI_OUT_OF_RESOURCES;
    }
  ...
  if (Regs.X.BX != 0) {
    ...
    if (OpromRevision < 3) {
      ...
      if (InitAddress == PcdGet32 (PcdOpromReservedMemoryBase)) {
        gBS->SetMem ((VOID *)InitAddress, ImageSize, 0);
      } else {
  ...
  if (!IsOpRomTempFree) {
    if (OpromRevision < 3) {
      ...
      if (PhysicalAddress != 0) {
        if (PhysicalAddress == PcdGet32 (PcdOpromReservedMemoryBase)) {
          gBS->SetMem ((VOID *)(UINTN)PhysicalAddress, ImageSize, 0);
        } else {
          //
