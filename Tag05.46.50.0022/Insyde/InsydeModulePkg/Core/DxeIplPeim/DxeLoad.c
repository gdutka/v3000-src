  ...
  )
{
  ...
  if (BootMode == BOOT_ON_S3_RESUME) {
    ...
    if (EFI_ERROR (Status)) {
      ...
      DEBUG_OEM_SVC ((DEBUG_INFO, "OemKernelServices Call: OemSvcHookWhenRecoveryFail \n"));
      OemSvcStatus = OemSvcHookWhenRecoveryFail ();
      DEBUG_OEM_SVC ((DEBUG_INFO, "OemKernelServices OemSvcHookWhenRecoveryFail Status: %r\n", OemSvcStatus));
  ...
  if (FileHandle == NULL) {
    if (FeaturePcdGet (PcdH2OPeiCpDxeFvCorruptedSupported)) {
      H2O_PEI_CP_DXE_FV_CORRUPTED_DATA  CpDxeFvCorruptedData;

      CpDxeFvCorruptedData.Size        = sizeof (H2O_PEI_CP_DXE_FV_CORRUPTED_DATA);
      CpDxeFvCorruptedData.Status      = H2O_CP_TASK_NORMAL;
      CpDxeFvCorruptedData.ResetSystem = FALSE;
      CpDxeFvCorruptedData.FileHandle  = NULL;
      DEBUG_CP ((DEBUG_INFO, "Checkpoint Trigger: %g\n", &gH2OPeiCpDxeFvCorruptedGuid));
      H2OCpTrigger (&gH2OPeiCpDxeFvCorruptedGuid, &CpDxeFvCorruptedData);
      DEBUG_CP ((DEBUG_INFO, "Checkpoint Result: %x\n", CpDxeFvCorruptedData.Status));
      if ((CpDxeFvCorruptedData.Status == H2O_CP_TASK_SKIP_UPDATE) ||
          (CpDxeFvCorruptedData.Status == H2O_CP_TASK_UPDATE))
      {
        if (CpDxeFvCorruptedData.ResetSystem) {
          (*PeiServices)->ResetSystem ((CONST EFI_PEI_SERVICES **)PeiServices);
        }

        if (CpDxeFvCorruptedData.FileHandle != NULL) {
          FileHandle = CpDxeFvCorruptedData.FileHandle;
        }
      }

      if ((CpDxeFvCorruptedData.Status == H2O_CP_TASK_SKIP_UPDATE) ||
          (CpDxeFvCorruptedData.Status == H2O_CP_TASK_SKIP))
      {
        goto Skip;
      }
    }
