...
#include "InternalHiiStringLib.h"
  ...
  )
{
  ...
  for (Supported = AllocatedLanguages; *Supported != '\0'; ) {
    ...
    if (StringId == (EFI_STRING_ID)(0)) {
      Status = HiiString->NewString (HiiString, HiiHandle, &StringId, Language, NULL, String, NULL);
    } else {
      Status = HiiString->SetString (HiiString, HiiHandle, StringId, Language, String, NULL);
    }
  ...
  )
{
  ...
  //
  LanguageSize = 0;
  Status = HiiString->GetLanguages (HiiString, HiiHandle, &TempSupportedLanguages, &LanguageSize);
  ...
  //
  Status = HiiString->GetLanguages (HiiString, HiiHandle, SupportedLanguages, &LanguageSize);
  if (EFI_ERROR (Status)) {
    //
    // Free the buffer and return NULL if the supported languages can not be retrieved.
    //
