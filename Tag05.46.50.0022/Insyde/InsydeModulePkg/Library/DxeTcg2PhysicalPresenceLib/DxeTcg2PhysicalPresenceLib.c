  ...
  )
{
  ...
  case TCG2_PHYSICAL_PRESENCE_DISABLE:
    if (PcdGetBool (PcdH2OTpm2DisableOperationSupported)) {
      PpiFlags->PPFlags |= TCG2_BIOS_INFORMATION_FLAG_HIERARCHY_CONTROL_STORAGE_DISABLE;
      PpiFlags->PPFlags |= TCG2_BIOS_INFORMATION_FLAG_HIERARCHY_CONTROL_ENDORSEMENT_DISABLE;
      Status = Tpm2HierarchyDisable (PlatformAuth, PpiFlags);
      if (EFI_ERROR (Status)) {
        return TCG_PP_OPERATION_RESPONSE_BIOS_FAILURE;
      } else {
        return TCG_PP_OPERATION_RESPONSE_SUCCESS;
  ...
  )
{
  ...
  if (TpmPpCommand < TCG2_PHYSICAL_PRESENCE_STORAGE_MANAGEMENT_BEGIN) {
    ...
    StrnCatS (ConfirmText, (BufSize / sizeof (CHAR16)), TmpStr1, StrLen (TmpStr1));
    FreePool (TmpStr1);
  } else {
    if (CautionKey) {
    ...
    TmpStr1 = Tcg2PhysicalPresenceGetStringById (STRING_TOKEN (TCG_STORAGE_INQUERY));
    StrnCatS (ConfirmText, (BufSize / sizeof (CHAR16)), TmpStr1, StrLen (TmpStr1));
    FreePool (TmpStr1);
  }
  //
  ...
  )
{
  ...
  if (TcgPpData->PPRequest >= TCG2_PHYSICAL_PRESENCE_VENDOR_SPECIFIC_OPERATION) {
    ...
    if (!RequestConfirmed) {
      ...
      //
      DEBUG_OEM_SVC ((DEBUG_INFO, "OemKernelServices Call: OemSvcTpmUserConfirmDialog \n"));
      Status = OemSvcTpmUserConfirmDialog (TcgPpData->PPRequest, TPM_DEVICE_2_0, &RequestConfirmed);
      DEBUG_OEM_SVC ((DEBUG_INFO, "OemKernelServices OemSvcTpmUserConfirmDialog Status: %r\n", Status));
      if (EFI_ERROR (Status)) {
  ...
  )
{
  ...
  if (OperationRequest >= TCG2_PHYSICAL_PRESENCE_VENDOR_SPECIFIC_OPERATION) {
    ...
    if (EFI_ERROR (Status)) {
      Flags.PPFlags = PcdGet32 (PcdH2OTpm2PpFlagsDefault);
    }
