  ...
  )
{
  ...
  *EventAction = EventData.Event.Action;
  ...
  if ((EventData.Event.Action & H2O_SECURITY_EVENT_ACTION_STATUS_CODE) != 0) {
    ...
    StatusCodeData = AllocatePages (EFI_SIZE_TO_PAGES(Buffersize));
    ...
    FreePages (StatusCodeData, EFI_SIZE_TO_PAGES (Buffersize));
  }

  if ((EventData.Event.Action & H2O_SECURITY_EVENT_ACTION_LOG) != 0) {
    Buffersize = sizeof (H2O_SECURITY_EVENT_HOB) + (UINTN) EventData.Event.EventDataSize + DescSize;
    ...
    CopyMem (&SecurityEventHob->Event, &EventData, sizeof (H2O_SECURITY_EVENT));
    if (EventData.Event.EventDataSize > 0) {
      HobExtraData = (CHAR8 *) ((UINTN) SecurityEventHob + sizeof (H2O_SECURITY_EVENT_HOB));
      CopyMem (HobExtraData, EventData.EventData, EventData.Event.EventDataSize);
    }
    if (EventData.EventDesc != NULL && DescSize > 0) {
      HobExtraDesc = (CHAR8 *) ((UINTN) SecurityEventHob + sizeof (H2O_SECURITY_EVENT_HOB) + (UINTN) EventData.Event.EventDataSize);
      CopyMem (HobExtraDesc, EventData.EventDesc, DescSize - 1);
      HobExtraDesc[DescSize - 1] = 0;
    }
  }
