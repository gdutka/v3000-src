...
Done:
  ...
  return (BOOLEAN) (EFI_ERROR (Status) ? FALSE : TRUE);
  ...
  while ((DataSize > 0) && (DataSize >= CertList->SignatureListSize)) {
    ...
    if (CompareGuid (&CertList->SignatureType, &gEfiCertX509Guid)) {
      ...
      for (Index = 0; Index < CertCount; Index++) {
        ...
        //
        IsVerified = CryptoServices->AuthenticodeVerify (
                                       AuthData,
                                       AuthDataSize,
                                       RootCert,
                                       RootCertSize,
  ...
  )
{
  ...
  IsVerified = CryptoService->Pkcs7GetSigners (
                                AuthData,
                                AuthDataSize,
                                &SignerCerts,
                                &CertStackSize,
  ...
  for (Index = 0; Index < CertNumber; Index++) {
    ...
    //
    Status = IsCertHashFoundInDbx (Cert, CertSize, Signature, SignatureSize, &RevocationTime, &IsFound);
    if (EFI_ERROR (Status)) {
      //
  ...
  )
{
  ...
  VariableStatus = CommonGetVariableDataAndSize (L"SecureFlashCertData", &gSecureFlashInfoGuid, &CertSize, (VOID **)&CertList);
  if (EFI_ERROR (VariableStatus)) {
    goto Done;
  }

  for (Offset = 0; Offset + sizeof (EFI_SIGNATURE_LIST) < CertSize; Offset += SignatureList->SignatureListSize) {
    SignatureList = (EFI_SIGNATURE_LIST *)(CertList + Offset);
    ...
    if (!IsVerified && CompareMem (mImageDigest->Sha256Hash, (mImageDigest + 1)->Sha256Hash, mImageDigestSize) != 0) {
      IsVerified = CryptoServices->AuthenticodeVerify (
                                     AuthData,
                                     AuthDataSize,
                                     SignatureData->SignatureData,
  ...
  )
{
  ...
  //
  IsVerified = VerifyAuthenCodeSecureFlash (AuthData, AuthDataSize);
  if (!IsVerified) {
    //
  ...
  )
{
  ...
  BufferValid = IsSourceBufferValid (FileBuffer, FileSize, SourceBuffer, SourceSize);
  FreePool (FileBuffer);
  return BufferValid;
}
  ...
  )
{
  ...
  for (OffSet = SecDataDir->VirtualAddress;
    ...
    //
    IsVerified = VerifyCertPkcsSignedData (AuthData, AuthDataSize, &Action);
    if (IsVerified) {
      Action = EFI_IMAGE_EXECUTION_AUTH_SIG_PASSED | EFI_IMAGE_EXECUTION_INITIALIZED;
    } else {
      Action = EFI_IMAGE_EXECUTION_AUTH_SIG_FAILED | EFI_IMAGE_EXECUTION_INITIALIZED;
  ...
  )
{
  ...
  if (FeaturePcdGet (PcdH2OCustomizedSecureBootSupported)) {
    Status = CommonGetVariable (L"AuditMode", &gEfiGlobalVariableGuid, &DataSize, &AuditMode);
    if (EFI_ERROR (Status)) {
      DEBUG((DEBUG_INFO, "Not Found Variable AuditMode\n"));
    }
  }
  ...
  //
  DataSize  = sizeof (UINT8);
  Status = CommonGetVariable (EFI_SETUP_MODE_NAME, &gEfiGlobalVariableGuid, &DataSize, &SetupMode);
  DEBUG((DEBUG_INFO, "CommonGetVariable(SetupMode) : %r\n", Status));
  if (EFI_ERROR (Status)) {
    SetupMode = SETUP_MODE;
  }
  DataSize             = sizeof (UINT8);
  Status = CommonGetVariable (L"SecureFlashSetupMode", &gSecureFlashInfoGuid, &DataSize, &SecureFlashSetupMode);
  DEBUG((DEBUG_INFO, "CommonGetVariable(SecureFlashSetupMode) : %r\n", Status));
  if (EFI_ERROR (Status)) {
  ...
  DEBUG((DEBUG_INFO, "CommonGetVariable(SecureBootEnforce) : %r\n", Status));
  if (EFI_ERROR (Status)) {
    SecureBootEnforce = 0;
  }
  //
  ...
  if (AuditMode == 1 && SecureFlashSetupMode == SETUP_MODE) {
    if (SecureBootEnforce == 0) {
      return EFI_SUCCESS;
    } else {
  ...
  for (OffSet = SecDataDir->VirtualAddress;
    ...
    //
    IsVerified = VerifyCertPkcsSignedData (AuthData, AuthDataSize, &Action);
    if (!IsVerified && Action == EFI_IMAGE_EXECUTION_AUTH_SIG_FOUND) {
      //
...
Done:
  ...
  if (Status != EFI_SUCCESS) {
    ...
    // 2. EFI_IMAGE_EXECUTION_AUTH_SIG_FAILED    : The image is signed but corrupted - the recovered hash does not
