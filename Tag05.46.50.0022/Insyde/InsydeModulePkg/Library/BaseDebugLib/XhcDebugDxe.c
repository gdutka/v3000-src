...
STATIC XHC_DEBUG_PRIVATE        Instance;
STATIC XHC_DEBUG_PRIVATE        *mPrivate = NULL;
STATIC XHC_DEBUG_PRIVATE        *mPrivateHob = NULL;
  ...
  )
{
  ...
  if (!(HcToken & B_TOKEN_LMC)) {
    ...
    if (Private->DebugContextAddr != 0) {
      ...
      DebugContext = AllocateReservedPages (1);
  ...
  )
{
  ...
  if (mPrivate == NULL) {
    ...
    if (EFI_ERROR (Status) || mPrivate == NULL) {
      return;
    }
    mPrivateHob = GetFirstGuidHob (&mXhcDebugHobGuid);
    if (mPrivateHob != NULL) {
      CopyMem (&Instance, mPrivateHob, sizeof (XHC_DEBUG_PRIVATE));
    }
