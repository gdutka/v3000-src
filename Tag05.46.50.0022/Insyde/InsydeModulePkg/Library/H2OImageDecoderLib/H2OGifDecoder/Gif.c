...
/**
 ...
 @param [out]  ReadBits         Output read image bits

 @retval EFI_SUCCESS            Decode successfully.
 @retval EFI_UNSUPPORTED        Initial code size is not supported
**/
STATIC
EFI_STATUS
LZWDecoder (
  ...
  )
{
  ...
  //
  // InitCodeSize indicates the minimum number of bits required to represent the set of actual pixel values.
  // InitCodeSize can not be smaller than 2 by spec definition.
  // InitCodeSize should not be larger than 8 becuase color map length is 256.
  //
  InitCodeSize = *BufIn;
    ...
    if (Code == ClearCode) {
      ...
      if (Code < EntryIndex) {
        ...
        while (TmpCode != NONCODE) {
          if ((UINTN) (StackIndex + 1) >= H2O_GIF_OUT_STACK_MAX_LEN || TmpCode >= H2O_GIF_LZW_TABLE_MAX_LEN  || TmpCode >= EntryIndex) {
            break;
          }
      ...
      if (PrevCode != ClearCode && EntryIndex < H2O_GIF_LZW_TABLE_MAX_LEN && StackIndex >= 0) {
