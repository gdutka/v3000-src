/** @file
  The device manager HII related implementation
...
/**
  Pretend to add form package of device manager to notify PxeDummyDxe that entering device manager.
**/
VOID
DeviceManagerNotify (
  VOID
  )
{
  ...
  CopyGuid (&PackageList.PackageListGuid, &mDeviceManagerGuid);
  PackageList.PackageLength = 0;
  ...
  )
{
  ...
  if (Action == EFI_BROWSER_ACTION_FORM_OPEN) {
    ...
    // Update the device manage page,because the device option may changed.
    //
    if (QuestionId == DEVICE_MANAGER_KEY_HIDDEN_QUESTION) {
      DeviceManagerNotify ();
  ...
  if ((QuestionId < MAX_KEY_SECTION_LEN + NETWORK_DEVICE_LIST_KEY_OFFSET) && (QuestionId >= NETWORK_DEVICE_LIST_KEY_OFFSET)) {
    ...
    for (CurIndex = 0; CurIndex < mMacDeviceList.CurListLen; CurIndex ++) {
      if (mMacDeviceList.NodeList[CurIndex].QuestionId == QuestionId) {
          if (mSelectedMacAddrString != NULL) {
            FreePool (mSelectedMacAddrString);
          }
  ...
  } else if (QuestionId == DEVICE_MANAGER_KEY_VBIOS) {
    gDeviceManagerPrivate.VideoBios = Value->u8;
    CommonSetVariable (
      L"VBIOS",
      &gEfiGenericVariableGuid,
      EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE,
      sizeof (UINT8),
      &gDeviceManagerPrivate.VideoBios
      );
