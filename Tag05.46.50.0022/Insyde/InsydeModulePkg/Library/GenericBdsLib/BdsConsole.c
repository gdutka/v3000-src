...
/**
  ...
  @param  ExclusiveDevicePath      The console device path which will be removed
                                   ...
                                   parameter can be multi-instance.
  ...
  )
{
  ...
  ConVarGuid    = IsConVariable (ConVarName) ? &gEfiGlobalVariableGuid : &gEfiGenericVariableGuid;
  NewDevicePath = BdsLibGetVariableAndSize (
                    ConVarName,
                    ConVarGuid,
                    &DevicePathSize
                    );
  ...
  //
  DevicePathPtr = ExclusiveDevicePath;
  while (DevicePathPtr != NULL && NewDevicePath != NULL) {
    Instance = GetNextDevicePathInstance (&DevicePathPtr, &DevicePathSize);
    if (Instance == NULL) {
      break;
    }

    TempNewDevicePath = NewDevicePath;
    NewDevicePath = BdsLibDelPartMatchInstance (NewDevicePath, Instance);
    if (TempNewDevicePath != NewDevicePath) {
  ...
  //
  DevicePathPtr = CustomizedConDevicePath;
  while (DevicePathPtr != NULL) {
    Instance = GetNextDevicePathInstance (&DevicePathPtr, &DevicePathSize);
    if (Instance == NULL) {
      break;
    }

    if (!BdsLibMatchDevicePaths (NewDevicePath, Instance)) {
      //
  ...
  )
{
  ...
  Status      = EFI_SUCCESS;
  if (FeaturePcdGet (PcdH2OBdsCpConInConnectBeforeSupported) && StrCmp (ConVarName, L"ConIn") == 0) {
    TriggerCpConInConnectBefore ();
  }
  if (FeaturePcdGet (PcdH2OBdsCpConOutConnectBeforeSupported) && StrCmp (ConVarName, L"ConOut") == 0) {
...
Done:
  ...
  if (FeaturePcdGet (PcdH2OBdsCpConInConnectAfterSupported) && StrCmp (ConVarName, L"ConIn") == 0) {
    TriggerCpConInConnectAfter ();
  }
  if (FeaturePcdGet (PcdH2OBdsCpConOutConnectAfterSupported) && StrCmp (ConVarName, L"ConOut") == 0) {
