...
STATIC EFI_GUID       mAuthSizeGuid = {0x3556d42f, 0x866e, 0x440f, { 0xa5, 0x2e, 0x96, 0x31, 0xac, 0x2d, 0x65, 0x98 } };
  ...
  )
{
  ...
  LastAuthSize = 0;
  GuidHob = GetFirstGuidHob (&mAuthSizeGuid);
  if (GuidHob != NULL) {
    LastAuthSize = *(UINT16 *) GET_GUID_HOB_DATA (GuidHob);
  }

  while (LastAuthSize == 0) {
    LastAuthSize = SHA1_DIGEST_SIZE;
    ...
    for (Index = 0; Index < Pcrs.count; Index++) {
      ...
      if (DigestSize > LastAuthSize) {
        LastAuthSize = DigestSize;
      }
    }
  ...
  *AuthSize = LastAuthSize;
   if (GuidHob == NULL) {
     BuildGuidDataHob (&mAuthSizeGuid, &LastAuthSize, sizeof (UINT16));
   }
