...
STATIC EFI_GUID              mInternalBufferGuid = {
                               0x2f867eb9, 0xf2ac, 0x4fa0, 0xaa, 0x6, 0xba, 0x43, 0x3d, 0x3b, 0x63, 0xbb
                             };

STATIC UINT8                 *mInternalBuffer = NULL;
STATIC EFI_HANDLE            mInternalBufferHandle = NULL;
  ...
  )
{
  ...
  CopyMem ((VOID *)mInternalBuffer, (VOID *)Buffer, BufferSize);
  ...
  )
{
  ...
  while ((Tries--) > 0) {
    ...
    ZeroMem (mInternalBuffer, BufferSize);
    TransferSize = 0;
    ...
    CopyMem ((VOID *)Buffer, (VOID *)mInternalBuffer, BufferSize);
...
/**
  Constructor of TcgStorageOpalLib Library Instance.
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    mInternalBuffer = AllocateReservedPool(BUFFER_SIZE);
    if (mInternalBuffer == NULL) {
      return EFI_OUT_OF_RESOURCES;
    }
    mInternalBufferHandle = NULL;
    Status = gBS->InstallProtocolInterface (
                    &mInternalBufferHandle,
                    &mInternalBufferGuid,
...
/**
  Destructor of TcgStorageOpalLib Library Instance.
...
EFI_STATUS
EFIAPI
TcgStorageOpalLibDestructor (
  VOID
  )
{
  EFI_STATUS           Status;

  if (mInternalBufferHandle == NULL) {
    return EFI_SUCCESS;
  }
  ...
  if (mInternalBuffer != NULL) {
    Status = gBS->FreePool ((VOID *)mInternalBuffer);
  }
