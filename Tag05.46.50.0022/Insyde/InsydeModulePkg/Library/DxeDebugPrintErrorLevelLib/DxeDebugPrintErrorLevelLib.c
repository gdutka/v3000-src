/** @file
  Debug Print Error Level Library
...
/// every DEBUG() macro.
///
STATIC BOOLEAN           mGlobalErrorLevelInitialized = FALSE;
...
///
STATIC UINT32            mDebugPrintErrorLevel        = 0;
  ...
  )
{
  ...
  //
  // Set a flag to mark that the constructor is executed.
  //
  ...
  if (!mGlobalErrorLevelInitialized) {
    ...
    if (CurrentTpl <= TPL_CALLBACK) {
      ...
      if (Status != EFI_NOT_AVAILABLE_YET) {
        ...
        if (Hob != NULL) {
          if (GET_GUID_HOB_DATA_SIZE (Hob) == sizeof (UINT64)) {
