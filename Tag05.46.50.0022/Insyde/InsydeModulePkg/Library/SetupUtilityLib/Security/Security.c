...
STATIC UINT8                       mCurrentTpmDevice   = 0;
  ...
  )
{
  ...
  if (Password[0] != 0x00) {
    ...
    StrCpyS(SysPasswordDataInfo->InputString, SysPasswordDataInfo->StringLength, Password);
  ...
  )
{
  ...
  if (SuBrowser->SUCInfo->SupervisorPassword != NULL &&
      SuBrowser->SUCInfo->SupervisorPassword->Flag != CHANGE_PASSWORD) {
  ...
  )
{
  ...
  *PState = FALSE;
  if (Type != EFI_IFR_TYPE_STRING || SysPasswordDataInfo == NULL) {
    return  EFI_INVALID_PARAMETER;
  }
  Status = GetSetupUtilityBrowserData (&SuBrowser);
  if (EFI_ERROR (Status)) {
  ...
  case BROWSER_STATE_SET_PASSWORD:
    if (PcdGetBool (PcdSysPasswordSupportUserPswd)) {
      if ((SupervisorOrUser == USERPASSWORD_FLAG) &&
          (Password[0] != 0x00) &&
          (SuBrowser->SUCInfo->SupervisorPassword != NULL) &&
...
/**
 Check if question ID belong to the password question with callback flag or not.

 @param[in] QuestionId                VFR question ID.

 @retval TRUE                         Question ID belong to the password question with callback flag.
 @retval FALSE                        Question ID does not belong to the password question with callback flag.
**/
BOOLEAN
IsPasswordQuestionWithCallbackFlag (
  IN EFI_QUESTION_ID                        QuestionId
  )
{
  if (QuestionId == KEY_SUPERVISOR_PASSWORD           ||
      QuestionId == KEY_USER_PASSWORD                 ||
      QuestionId == KEY_SET_ALL_HDD_PASSWORD          ||
      QuestionId == KEY_SET_ALL_MASTER_HDD_PASSWORD   ||
      QuestionId == KEY_CHECK_STORAGE_PASSWORD        ||
      QuestionId == KEY_SET_STORAGE_PASSWORD          ||
      QuestionId == KEY_CHECK_MASTER_STORAGE_PASSWORD ||
      QuestionId == KEY_SET_MASTER_STORAGE_PASSWORD) {
    return TRUE;
  }
  return FALSE;
}
...
/**
  Set Config info according to TpmAlgHash and ActivePcrBanks

  @param[out] Tcg2ConfigInfo          TCG2 config.
  @param[in]  TpmAlgHash              TPM algorithms support.
  @param[in]  ActivePcrBanks          Bitmask of the ActivePcrBanks support.
**/
STATIC
VOID
SetTcg2ConfigInfo (
  ...
  )
{
  ...
  case TPM_ALG_SHA384:
    ...
    if (ActivePcrBanks & EFI_TCG2_BOOT_HASH_ALG_SHA384) {
  ...
  case TPM_ALG_SHA512:
    ...
    if (ActivePcrBanks & EFI_TCG2_BOOT_HASH_ALG_SHA512) {
  ...
  case TPM_ALG_SM3_256:
    ...
    if (ActivePcrBanks & EFI_TCG2_BOOT_HASH_ALG_SM3_256) {
...
/**
  This function checks whether the selected TPM exists,
  to determine if the TPM options should be suppressed by managing TpmDeviceOk and Tpm2DeviceOk.
  ...
  )
{
  ...
  //
  // Don't change TpmDevice if TPM auto detection is enabled.
  // Because when TPM auto detection is enabled, TpmDevice is more like a status instead of an option.
  //
  TpmDevice    = PcdGetBool (PcdTpmAutoDetection) ? mCurrentTpmDevice : KernelConfig->TpmDevice;
  TpmDeviceOk  = 0xFF;
  Tpm2DeviceOk = 0xFF;
  switch (TpmDevice) {
  ...
  case TPM_DEVICE_TCM:
    ...
    if (!EFI_ERROR (Status)) {
      //
      // TPM found. Display the related options in SCU.
      //
      TpmDeviceOk = 1;
    }
  ...
  KernelConfig->TpmDevice    = TpmDevice;
  KernelConfig->TpmDeviceOk  = TpmDeviceOk;
  KernelConfig->Tpm2DeviceOk = Tpm2DeviceOk;
...
VOID
PaserTpmAlgsHash(
  IN     TPMI_ALG_HASH     TpmAlgHash,
  IN OUT UINT32            *HashAlg 
  )
{
  switch (TpmAlgHash) {
  case TPM_ALG_SHA1:
    *HashAlg |= HASH_ALG_SHA1;
    break;
  case TPM_ALG_SHA256:
    *HashAlg |= HASH_ALG_SHA256;
    break;
  case TPM_ALG_SHA384:
    *HashAlg |= HASH_ALG_SHA384;
    break;
  case TPM_ALG_SHA512:
    *HashAlg |= HASH_ALG_SHA512;
    break;
  case TPM_ALG_SM3_256:
    *HashAlg |= HASH_ALG_SM3_256;
    break;
  }
}
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    ...
    for (Index = 0; Index < Pcrs.count; Index++) {
      if (!IsZeroBuffer (Pcrs.pcrSelections[Index].pcrSelect, Pcrs.pcrSelections[Index].sizeofSelect)) {
        ...
        PaserTpmAlgsHash(Pcrs.pcrSelections[Index].hash, &ActivePCRBanks);
      }
      PaserTpmAlgsHash(Pcrs.pcrSelections[Index].hash, &CapPcrsAlgs);
    }
    HiiSetString (HiiHandle, STRING_TOKEN (STR_TPM2_ACTIVE_HASH_ALGO_CONTENT), TempBuffer, NULL);
  }
  SupportAlgs = CapPcrsAlgs;

  Status = Tpm2GetCapabilitySupportedAlg (&AlgList);
  if (!EFI_ERROR(Status)) {
    CapAlgs = 0;
    for (Index = 0; Index < AlgList.count; Index++) {
      PaserTpmAlgsHash(AlgList.algProperties[Index].alg, &CapAlgs);
    }
    if (BitFieldCountOnes32 (CapAlgs, 0, 31) >= BitFieldCountOnes32 (CapPcrsAlgs, 0, 31)) {
      SupportAlgs = CapAlgs;
    }
  }
  ...
  if (SupportAlgs != 0) {
    ...
    for (Index = 0; Index < 32; Index++) {
      switch (SupportAlgs & ((UINT32)(1 << Index))) {
      ...
      if (HashAlg != TPM_ALG_ERROR) {
        AppendBufferWithTpmAlgHash (TempBuffer, sizeof(TempBuffer), HashAlg);
        SetTcg2ConfigInfo (&mTcg2ConfigInfo, HashAlg, ActivePCRBanks);
      }
    }
  ...
  //
  // BIOS supported algorithm
  //
  BiosSupportedHashAlg = PcdGet32 (PcdH2OTpm2BiosSupportedHashAlgorithmBitmap);
  if (BiosSupportedHashAlg == 0) {
    HiiSetString (HiiHandle, STRING_TOKEN (STR_BIOS_SUPPORTED_HASH_ALGO_CONTENT), L"[Unknown]", NULL);
  } else {
    FillBufferWithBootHashAlg (TempBuffer, sizeof(TempBuffer), BiosSupportedHashAlg);
    HiiSetString (HiiHandle, STRING_TOKEN (STR_BIOS_SUPPORTED_HASH_ALGO_CONTENT), TempBuffer, NULL);
  }
}
  ...
  )
{
  ...
  //
  // Record the current TpmDevice.
  //
  mCurrentTpmDevice = TpmDevice;
  ...
  if (KernelConfig->TpmHide == 1 || PcdGetBool (PcdTpmHide)) {
    //
    // Using the selected TpmDevice value (KernelConfig->TpmDevice) as the current TpmDevice for load default,
    // since TpmDevice is always TPM_DEVICE_NULL when TPM is hidden.
    //
    mCurrentTpmDevice = KernelConfig->TpmDevice;
    return;
  }
...
/**
  Update TCG storage data.

**/
STATIC
VOID
UpdateTcgStorageData (
  VOID
  )
{
  UINT32                                PpFlags;

  PpFlags = Tcg2PhysicalPresenceLibGetManagementFlags ();
  mPasswordConfig.BlockSidEnabled              = ((PpFlags & TCG2_BIOS_STORAGE_MANAGEMENT_FLAG_ENABLE_BLOCK_SID) == 0) ? 0 : 1;
  mPasswordConfig.PpRequiredForEnableBlockSid  = ((PpFlags & TCG2_BIOS_STORAGE_MANAGEMENT_FLAG_PP_REQUIRED_FOR_ENABLE_BLOCK_SID) == 0) ? 0 : 1;
  mPasswordConfig.PpRequiredForDisableBlockSid = ((PpFlags & TCG2_BIOS_STORAGE_MANAGEMENT_FLAG_PP_REQUIRED_FOR_DISABLE_BLOCK_SID) == 0) ? 0 : 1;
  mPasswordConfig.TcgStorageAction             = TCG2_PHYSICAL_PRESENCE_NO_ACTION;
}

/**
  Save the current TCG Sotarge Action settings.

  @retval EFI_SUCCESS             Save the current settings successfully
  @retval Other                   Fail to save the current settings or fail to get browser data.
**/
STATIC
EFI_STATUS
SaveTcgStorageAction (
  VOID
  )
{
  ...
  if (mPasswordConfig.TcgStorageAction == TCG2_PHYSICAL_PRESENCE_NO_ACTION) {
    return EFI_SUCCESS;
  }

  ReturnCode = Tcg2PhysicalPresenceLibSubmitRequestToPreOSFunction (mPasswordConfig.TcgStorageAction, 0);
...
/**
  Save the current settings of all varstores belong to Security VFR.
  ...
  )
{
  ...
  if (PcdGetBool (PcdH2OTpmSupported) || PcdGetBool (PcdH2OTpm2Supported) || PcdGetBool (PcdH2OTcmSupported)) {
  ...
  if (!EFI_ERROR (Status) && SetupUtilityApp != NULL && SetupUtilityApp->VfrDriverState == InitializeSetupUtility) {
    //
    // Check password
    //
  ...
  )
{
  ...
  ((KERNEL_CONFIGURATION *) SuBrowser->SCBuffer)->SupervisorFlag = (SuBrowser->SUCInfo->SupervisorPwdFlag) ? TRUE : FALSE;
  NewString = HiiGetString (
                HiiHandle,
                (SuBrowser->SUCInfo->SupervisorPwdFlag) ? STRING_TOKEN (STR_INSTALLED_TEXT) : STRING_TOKEN (STR_NOT_INSTALLED_TEXT),
                NULL
                );
  if (NewString != NULL) {
    HiiSetString (HiiHandle, STRING_TOKEN (STR_SUPERVISOR_PASSWORD_STRING2), NewString, NULL);
    FreePool (NewString);
  }

  if (PcdGetBool (PcdSysPasswordSupportUserPswd)) {
    ((KERNEL_CONFIGURATION *) SuBrowser->SCBuffer)->UserFlag = (SuBrowser->SUCInfo->UserPwdFlag) ? TRUE :FALSE;
    NewString = HiiGetString (
                  HiiHandle,
                  (SuBrowser->SUCInfo->UserPwdFlag) ? STRING_TOKEN (STR_INSTALLED_TEXT) : STRING_TOKEN (STR_NOT_INSTALLED_TEXT),
                  NULL
                  );
    if (NewString != NULL) {
      HiiSetString (HiiHandle, STRING_TOKEN (STR_USER_PASSWORD_STRING2), NewString, NULL);
      FreePool (NewString);
    }
  }
  ...
  )
{
  ...
  if (Action == EFI_BROWSER_ACTION_CHANGING) {
    //
    // Advance Setup only send CHANGING in "goto" opcode or "password" opcode with callback flag, doesn't send CHANGED
    //
    if ((QuestionId >= LABEL_STORAGE_PASSWORD_OPTION && QuestionId <= (LABEL_STORAGE_PASSWORD_OPTION + 16)) ||
        IsPasswordQuestionWithCallbackFlag (QuestionId)) {
  ...
  *ActionRequest = EFI_BROWSER_ACTION_REQUEST_NONE;
  ...
  case KEY_SET_ALL_MASTER_HDD_PASSWORD:
    ...
    case KEY_SET_MASTER_STORAGE_PASSWORD:
    case KEY_CHECK_MASTER_STORAGE_PASSWORD:
      Status = StoragePasswordCallback (
                 HiiHandle,
                 Type,
                 Value,
    ...
    case KEY_PSID_REVERT:
      Status = PsidRevertCallback (
                 HiiHandle,
                 Type,
                 Value,
                 mPasswordConfig.SelectedScuDataIndex
    ...
    case KEY_TPM_SELECT:
      UpdateTpmStateToken (HiiHandle, ((KERNEL_CONFIGURATION *)SuBrowser->SCBuffer)->TpmDevice, STRING_TOKEN (STR_TPM_STATUS_STRING2));
      break;
    ...
    case KEY_TPM2_PCR_BANKS_REQUEST_4:
  ...
  )
{
  ...
  FreePool (mSecurityCallBackInfo);
  ...
  )
{
  ...
  if (PcdGetBool (PcdH2OTpmSupported) || PcdGetBool (PcdH2OTpm2Supported) || PcdGetBool (PcdH2OTcmSupported)) {
    SuppressTpmOptionsByPolicy ((KERNEL_CONFIGURATION *)SuBrowser->SCBuffer);
    UpdateTpmStateToken (
      SuBrowser->SUCInfo->MapTable[SecurityHiiHandle].HiiHandle,
      ((KERNEL_CONFIGURATION *)SuBrowser->SCBuffer)->TpmDevice,
      STRING_TOKEN (STR_TPM_STATUS_STRING2)
      );
  }
