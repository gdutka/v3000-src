...
/**
  Using BVDT data (STR_MISC_BIOS_VERSION) to update STR_H2O_MISC_BIOS_VERSION string token
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    HiiSetString (
      HiiHandle,
      STRING_TOKEN (STR_H2O_MISC_BIOS_VERSION),
      Str,
      NULL
      );
  }
  ...
  HiiCreateTextOpCode (
    ...
    (EFI_STRING_ID)STRING_TOKEN(STR_H2O_MISC_BIOS_VERSION)
    );
