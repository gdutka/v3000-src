...
STATIC CONST UNICODE_WIDTH_ENTRY mUnicodeWidthTable[] = {
  //
  // General script area
  //
  {(CHAR16)0x1FFF,  1},
  ...
  */
   ...
   * Remove the above item if below is un-commented.
   *
  {(CHAR16)0x206F,  2},       // General punctuation. (See page7-154). 0x200-0x206F
  {(CHAR16)0x209F,  2},       // Superscripts and subscripts. 0x2070-0x209F
...
/**
  ...
  @param[in] UnicodeChar   A Unicode character.
  ...
  )
{
  ...
  if (UnicodeChar <= mUnicodeWidthTable[0].WChar) {
    return mUnicodeWidthTable[0].Width;
  }
  ...
  do {
    ...
    for (; (String[Index] != NARROW_CHAR) && (String[Index] != WIDE_CHAR) && (String[Index] != 0); Index++) {
      ...
      if (IncrementValue == 1) {
        ...
        //
        GlyphWidth = ConsoleLibGetGlyphWidth (String[Index]);
        if (GlyphWidth == 2) {
          Count++;
        }
      }
...
/**
  ...
  @retval The width of the query string.
  ...
  )
{
  ...
  return (GetStringDisplayWidth (String) + 1) * sizeof (CHAR16);
}
