/** @file
  The library instance provides security service of TCM measure boot.
  ...
  DxeTcmMeasureBootLibImageRead() function will make sure the PE/COFF image content
  read is within the image buffer.

  TcmMeasurePeImage() function will accept untrusted PE/COFF image and validate its
  data structure within this image buffer before use.

  TcmMeasureGptTable() function will receive untrusted GPT partition table, and parse
...
STATIC UINTN                             mMeasureGptCount = 0;
STATIC VOID                              *mFileBuffer;
STATIC UINTN                             mTcmImageSize;
//
// Measured FV handle cache
//
STATIC EFI_HANDLE                        mCacheMeasuredHandle  = NULL;
STATIC MEASURED_HOB_DATA                 *mMeasuredHobData     = NULL;
  ...
  )
{
  ...
  //
  // Replacing logic:
  // EventSize + sizeof (TCM_PCR_EVENT_HDR)
  //
  Status = SafeUint32Add (*EventSize, sizeof (TCM_PCR_EVENT_HDR), EventSize);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "EventSize would overflow!\n"));
    return EFI_BAD_BUFFER_SIZE;
  }
...
/**
  Return Tcg protocol pointer if TCM is ready for use.

  @retval Pointer to EFI_TCG_PROTOCOL

**/
EFI_TCG_PROTOCOL *
GetTcgProtocolIfTcmActivated (
  VOID
  )
{
  EFI_STATUS                          Status;
  ...
  if (EFI_ERROR (Status)) {
    //
    // TCG protocol is not installed. So, TCM is not present.
    // Don't do any measurement, and directly return EFI_SUCCESS.
    //
    DEBUG ((EFI_D_ERROR, "DxeTcmMeasureBootHandler - Tcg - %r\n", Status));
    return NULL;
  }
  ...
  if (EFI_ERROR (Status) || ProtocolCapability.TPMDeactivatedFlag) {
    //
    // TCM device doesn't work or activate.
    //
    DEBUG ((EFI_D_ERROR, "DxeTcmMeasureBootHandler - TPMDeactivatedFlag - %x\n", ProtocolCapability.TPMDeactivatedFlag));
    return NULL;
  }

  if ((ProtocolCapability.HashAlgorithmBitmap & HASH_ALG_SM3_256) == 0) {
    DEBUG ((DEBUG_INFO, "DxeTcmMeasureBootHandler - SM3 hash algorithm not supported\n"));
    return NULL;
  }
  ...
  )
{
  ...
  if (EndPosition > mTcmImageSize) {
    *ReadSize = (UINT32)(mTcmImageSize - FileOffset);
  }

  if (FileOffset >= mTcmImageSize) {
    *ReadSize = 0;
  }
...
/**
  Measure GPT table data into TCM log.
  ...
  )
{
  ...
  TcmEvent = (TCM_PCR_EVENT *) AllocateZeroPool (EventSize );
  if (TcmEvent == NULL) {
    FreePool (PrimaryHeader);
    FreePool (EntryPtr);
    return EFI_OUT_OF_RESOURCES;
  ...
  TcmEvent->PCRIndex   = 5;
  TcmEvent->EventType  = EV_EFI_GPT_EVENT;
  TcmEvent->EventSize  = EventSize - sizeof (TCM_PCR_EVENT_HDR);
  GptData = (EFI_GPT_DATA *) TcmEvent->Event;
...
/**
  Measure PE image into TCM log based on the authenticode image hashing in
  ...
  its caller function DxeTcmMeasureBootHandler().
  ...
  )
{
  ...
  TcmEvent = AllocateZeroPool (EventSize);
  if (TcmEvent == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }

  TcmEvent->EventSize = EventSize - sizeof(TCM_PCR_EVENT_HDR);
  ImageLoad           = (EFI_IMAGE_LOAD_EVENT *) TcmEvent->Event;
  ...
  switch (ImageType) {
    ...
    case EFI_IMAGE_SUBSYSTEM_EFI_BOOT_SERVICE_DRIVER:
      TcmEvent->EventType = EV_EFI_BOOT_SERVICES_DRIVER;
      TcmEvent->PCRIndex  = 2;
      break;
    case EFI_IMAGE_SUBSYSTEM_EFI_RUNTIME_DRIVER:
      TcmEvent->EventType = EV_EFI_RUNTIME_SERVICES_DRIVER;
      TcmEvent->PCRIndex  = 2;
      break;
    default:
      DEBUG ((
        EFI_D_ERROR,
        "TcmMeasurePeImage: Unknown subsystem type %d",
        ImageType
        ));
      goto Finish;
  }
  ...
  HashStatus = Sm3Update (Sm3Ctx, HashBase, HashSize);
  if (!HashStatus) {
    goto Finish;
  }
  ...
  //
  HashStatus = Sm3Final (Sm3Ctx, (UINT8 *) &TcmEvent->Digest);
  if (!HashStatus) {
    goto Finish;
  }
  ...
  )
{
      ...
      if (DevicePathType (DevicePathNode) == MEDIA_DEVICE_PATH &&
        ...
        if (((HARDDRIVE_DEVICE_PATH *) DevicePathNode)->MBRType == MBR_TYPE_EFI_PARTITION_TABLE_HEADER &&
          ...
          if (!EFI_ERROR (Status)) {
            ...
            // Measure GPT disk.
            //
            Status = TcmMeasureGptTable (GetTcgProtocolIfTcmActivated (), Handle);
            if (!EFI_ERROR (Status)) {
              //
              // GPT disk check done.
              //
  ...
  ImageContext.ImageRead = (PE_COFF_LOADER_READ_FILE) DxeTcmMeasureBootLibImageRead;
  ...
  if ((!ApplicationRequired) ||
    ...
    if (ApplicationRequired && ImageContext.ImageType == EFI_IMAGE_SUBSYSTEM_EFI_APPLICATION) {
      TrustedApp = TcmTrustedFileVerification (DevicePathNode);
    }
    //
    ...
    //
    // Measure PE image into TCM log.
    //
    Status = TcmMeasurePeImage (
               GetTcgProtocolIfTcmActivated (),
...
/**
  Register the security handler to provide TCM measure boot service.
...
EFI_STATUS
EFIAPI
DxeTcmMeasureBootLibConstructor (
