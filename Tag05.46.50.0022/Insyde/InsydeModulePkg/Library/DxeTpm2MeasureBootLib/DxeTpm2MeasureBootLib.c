...
/**
  Return Tcg2 protocol pointer if TPM is ready for use.

  @retval Pointer to EFI_TCG2_PROTOCOL

**/
EFI_TCG2_PROTOCOL *
GetTcg2ProtocolIfTpmPresent (
  VOID
  )
{
  EFI_STATUS                            Status;
  ...
  if (EFI_ERROR (Status)) {
    //
    // Tcg2 protocol is not installed. So, TPM is not present.
    //
  ...
  if (EFI_ERROR (Status) || !ProtocolCapability.TPMPresentFlag) {
    ...
    //
    DEBUG ((EFI_D_ERROR, "DxeTpm2MeasureBootHandler - TPMPresentFlag - %x\n", ProtocolCapability.TPMPresentFlag));
    return NULL;
  }
  ...
  )
{
      ...
      if (DevicePathType (DevicePathNode) == MEDIA_DEVICE_PATH &&
        ...
        if (((HARDDRIVE_DEVICE_PATH *) DevicePathNode)->MBRType == MBR_TYPE_EFI_PARTITION_TABLE_HEADER &&
          ...
          if (!EFI_ERROR (Status)) {
            ...
            // Measure GPT disk.
            //
            Status = Tcg2MeasureGptTable (GetTcg2ProtocolIfTpmPresent (), Handle);
  ...
  if ((!ApplicationRequired) ||
    ...
    if (ApplicationRequired && ImageContext.ImageType == EFI_IMAGE_SUBSYSTEM_EFI_APPLICATION) {
      TrustedApp = Tcg2TrustedFileVerification (DevicePathNode);
    }
    //
