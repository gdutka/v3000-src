...
STATIC EFI_SMM_REPORT_STATUS_CODE     mReportStatusCode = NULL;
STATIC EFI_SMM_STATUS_CODE_PROTOCOL   *mStatusCodeProtocol = NULL;
...
SmmInternalGetReportStatusCode (
  VOID
  )
{
  EFI_STATUS                    Status;
  ...
  )
{
  if ((ReportProgressCodeEnabled() && ((Type) & EFI_STATUS_CODE_TYPE_MASK) == EFI_PROGRESS_CODE) ||
    ...
    if (mReportStatusCode == NULL) {
      mReportStatusCode = SmmInternalGetReportStatusCode ();
      if (mReportStatusCode == NULL) {
...
BOOLEAN
EFIAPI
SmmReportStatusCodeExtractAssertInfo (
  IN EFI_STATUS_CODE_TYPE        CodeType,
  IN EFI_STATUS_CODE_VALUE       Value,
...
BOOLEAN
EFIAPI
SmmReportStatusCodeExtractDebugInfo (
  IN CONST EFI_STATUS_CODE_DATA  *Data,
...
/**
  ...
  SmmReportStatusCode()must actively prevent recusrsion.  If SmmReportStatusCode()
  is called while processing another any other Report Status Code Library function,
  then SmmReportStatusCode() must return immediately.
...
{
  return SmmInternalReportStatusCode (Type, Value, 0, &gEfiCallerIdGuid, NULL);
}
...
EFI_STATUS
EFIAPI
SmmReportStatusCodeWithExtendedData (
  IN EFI_STATUS_CODE_TYPE   Type,
  IN EFI_STATUS_CODE_VALUE  Value,
  ...
  )
{
  ...
  Status = SmmInternalReportStatusCode (Type, Value, Instance, CallerId, StatusCodeData);
...
EFI_STATUS
EFIAPI
SmmReportStatusCodeWithDevicePath (
  IN EFI_STATUS_CODE_TYPE            Type,
  IN EFI_STATUS_CODE_VALUE           Value,
...
{
  //
  // SMM doesn't support this function so return EFI_UNSUPPORTED directly.
  //
