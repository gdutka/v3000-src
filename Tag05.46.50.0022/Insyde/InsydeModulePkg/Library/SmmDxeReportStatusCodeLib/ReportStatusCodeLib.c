...
#include "InternalReportStatusCodeLib.h"
...
{
  if (InSmm () || !IsRuntimeDriver (ImageHandle)) {
    return EFI_SUCCESS;
  }

  return DxeReportStatusCodeLibConstructor (ImageHandle, SystemTable);
}
  ...
  )
{
  ...
  return DxeReportStatusCodeLibDestructor (ImageHandle, SystemTable);
}
...
{
  return gSmst == NULL ? DxeCodeTypeToPostCode (CodeType, Value, PostCode) : SmmCodeTypeToPostCode (CodeType, Value, PostCode);
}
...
{
  if (gSmst == NULL) {
    return DxeReportStatusCodeExtractAssertInfo (CodeType, Value, Data, Filename, Description, LineNumber);
  } else {
    return SmmReportStatusCodeExtractAssertInfo (CodeType, Value, Data, Filename, Description, LineNumber);
  }
}
...
{
  if (gSmst == NULL) {
    return DxeReportStatusCodeExtractDebugInfo (Data, ErrorLevel, Marker, Format);
  } else {
    return SmmReportStatusCodeExtractDebugInfo (Data, ErrorLevel, Marker, Format);
  }
}
...
{
  return gSmst == NULL ? DxeReportStatusCode (Type, Value) : SmmReportStatusCode  (Type, Value);
}
...
{
  return gSmst == NULL ? DxeReportStatusCodeWithDevicePath (Type, Value, DevicePath) : SmmReportStatusCodeWithDevicePath  (Type, Value, DevicePath);
}
...
{
  if (gSmst == NULL) {
    return DxeReportStatusCodeWithExtendedData (Type, Value, ExtendedData, ExtendedDataSize);
  } else {
    return SmmReportStatusCodeWithExtendedData (Type, Value, ExtendedData, ExtendedDataSize);
  }
}
...
/**
  ...
  DxeReportStatusCodeEx()must actively prevent recursion. If
  DxeReportStatusCodeEx() is called while processing another any
  other Report Status Code Library function, then
  DxeReportStatusCodeEx() must return EFI_DEVICE_ERROR immediately.
...
{
  if (gSmst == NULL) {
    return DxeReportStatusCodeEx (Type, Value, Instance, CallerId, ExtendedDataGuid, ExtendedData, ExtendedDataSize);
  } else {
    return SmmReportStatusCodeEx (Type, Value, Instance, CallerId, ExtendedDataGuid, ExtendedData, ExtendedDataSize);
  }
}
...
{
  return gSmst == NULL ? DxeReportProgressCodeEnabled () : SmmReportProgressCodeEnabled  ();
}
...
{
  return gSmst == NULL ? DxeReportErrorCodeEnabled () : SmmReportErrorCodeEnabled  ();
}
...
{
  return gSmst == NULL ? DxeReportDebugCodeEnabled () : SmmReportDebugCodeEnabled ();
}
