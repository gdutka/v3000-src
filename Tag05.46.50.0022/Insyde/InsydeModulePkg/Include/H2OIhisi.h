...
typedef enum {
  ...
  GetOemCommonTable,              // 60h = Pass common OEM tables (including the ME Counter table) See AP_COMMUNICATION_FUNCTION_0x60_TABLE for more information.
  WriteOa2Marker,                 // 61h
  WriteOa2PublicKey,              // 62h
  EraseOa2Marker,                 // 63h
  EraseOa2PublicKey,              // 64h
  GetTpmFirmwareFileName  = 0x68, // 68h = Get on-board descriptor TPM firmware file name. TBD What is the structure to hold the output.
  SetFlashConfiguration,          // 69h = Pass flash configuration information to the BIOS.
  SpecifyLogoType,                // 6Ah = Specify logo type.
                                  // 6Bh, 6Ch rserved for OEM.
  SignedLogoUpdate = 0x6D,        // 6Dh = Specify logo type.
  // 6Eh~EFh  Reserved
