...
#ifndef HAVE_FPRINTF
#define HAVE_FPRINTF	0		/* =1 EPRINTF/DPRINTF uses fprintf/printf, otherwise GdError or no-op*/
#endif

#ifndef HAVE_FLOAT
#define HAVE_FLOAT		0		/* =1 incl float, GdArcAngle*/
#endif
...
void	GdDrawImageEx(PSD psd, MWCOORD x, MWCOORD y, MWCOORD width, MWCOORD height, MWCOORD sx, MWCOORD sy, PMWIMAGEHDR pimage);
...
int		GdReadMouse(MWCOORD *px, MWCOORD *py, MWCOORD *pz, int *pb);
...
int		GdReadKeyboard(EFI_KEY_DATA *KeyData, MWKEYMOD *modifiers, MWSCANCODE *scancode);
...
PSD   GdLoadImageFromBuffer (IN EFI_IMAGE_INPUT      *Image);
PSD   GdLoadImageFromBuffer2(void *buffer, int size, int flags);
...
/* devlist.c*/
/* field offset*/
#define MWITEM_OFFSET(type, field)    ((UINTN)&(((type *)0)->field))
...
void	GdItemFree (void *ptr);
/* return base item address from list ptr*/
#define GdItemAddr(p,type,list)  ((type *)((UINTN)p - MWITEM_OFFSET(type,list)))
