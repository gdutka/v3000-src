/** @file
  Header file for Veas BIOS Extensions Data Structures
...
//
// *** Super VGA Information Block ***
// Use variable to record the first boot setting for use.
// It can reduce POST time.
//
#define VBIOS_CONFIGURATION_GUID {0XBBD1FD65, 0X5668, 0X4FB2, 0X89, 0X99, 0X23, 0X10, 0X95, 0X71, 0X7A, 0X07}

#define VBIOS_OEM_PREFER_MODE_INDEX      1
#define VBIOS_OEM_SCU_PREFER_MODE_INDEX  2
#define VBIOS_END_OF_MODE_INFO_INDEX     7
...
typedef struct {
  ...
  UINT32  OemProductNamePtr;  // VbeFarPtr to Product Name String
  UINT32  OemProductRevPtr;   // VbeFarPtr to Product Revision String
  UINT16  VbeModeNumber[8];   // [0] = Native Resolution of Panel
                              // [1] = OEM Defined Logo Resoultion
                              // [2] = OEM Defined SCU Resoultion
                              // [3] = 1024*768 , [4] = 800*600 , [5] = 640*480 , 
                              // [6] = Intersected highest resolution for multi-monitors
                              // [7] = VESA_BIOS_EXTENSIONS_END_OF_MODE_LIST(0xffff)
  UINT16  VendorId;
  UINT16  DeviceId;
  UINT16  SubsystemVendorID;
  UINT16  SubsystemID;
  UINT8   EdidChecksum;
  UINT8   MultiEdidChecksum[9];  // used to store multiple EDID, when multi-monitor attached
  UINT8   NumOfEdid;             // used to store number of EDID  
} VESA_BIOS_EXTENSIONS_INFO;
