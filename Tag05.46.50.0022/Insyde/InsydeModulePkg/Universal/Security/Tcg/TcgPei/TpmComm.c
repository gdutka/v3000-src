...
/**
  ...
  @param[in]  PeiServices        Describes the list of possible PEI Services.
  @param[in]  TpmHandle          TPM handle.  
  @param[out] TpmProperty        The value of TPM_CAP_PROPERTY.
                                 A value of TRUE indicates that the TPM has successfully installed an owner.
  ...
  )
{
  ...
  SendBuffer.Hdr.paramSize      = SwapBytes32 (TpmSendSize);  
  SendBuffer.Hdr.ordinal        = SwapBytes32 (TPM_ORD_GetCapability);
  SendBuffer.Capability         = SwapBytes32 (TPM_CAP_PROPERTY);
  SendBuffer.CapabilityFlagSize = SwapBytes32 (sizeof (TPM_CAP_PROP_OWNER));
  SendBuffer.CapabilityFlag     = SwapBytes32 (TPM_CAP_PROP_OWNER);
  Status = TisTpmCommand (PeiServices, TpmHandle, (UINT8 *)&SendBuffer, TpmSendSize, RecvBuffer, &TpmRecvSize);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  if (TpmProperty != NULL) {
    *TpmProperty = (BOOLEAN)RecvBuffer[sizeof(TPM_RSP_COMMAND_HDR) + sizeof(UINT32)];
  }
  return Status;
}
