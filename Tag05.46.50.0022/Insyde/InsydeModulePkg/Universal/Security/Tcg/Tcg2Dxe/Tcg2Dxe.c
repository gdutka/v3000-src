...
#define MSFT_INDEX_C0     0x01C101C0
...
EFI_HANDLE         mPlatformAuthHandle;
BOOLEAN            mTcgEventLogging = TRUE;
TPM2B_AUTH         mNewAuth = {0};
EFI_GUID           mTcg2UpdatedPlatformAuthGuid = {
  0xebab103e, 0x8906, 0x4dc8, 0xbd, 0xd1, 0xa0, 0xbc, 0x26, 0xb1, 0xba, 0x86
};
UINT8              mMsftIndexC0PolicyDigestSha256[0x20] = {
  0xcb, 0x45, 0xc8, 0x1f, 0xf3, 0x4b, 0xcf, 0x0a, 0xfb, 0x9e, 0x1a, 0x80, 0x29, 0xfa, 0x23, 0x1c,
  0x87, 0x27, 0x30, 0x3c, 0x09, 0x22, 0xdc, 0xce, 0x68, 0x4b, 0xe3, 0xdb, 0x81, 0x7c, 0x20, 0xe1
};

EFI_PHYSICAL_ADDRESS            mLasaTcg12 = 0;
EFI_PHYSICAL_ADDRESS            mLasaTcg2  = 0;
EFI_PHYSICAL_ADDRESS            mFinalLasa = 0;
  ...
  )
{
  ...
  if (PcdGet8 (PcdTrEEProtocolVersion) != 0) {
    //
  ...
  for (Index = 0; Index < sizeof(mTcg2EventInfo)/sizeof(mTcg2EventInfo[0]); Index++) {
    if ((mTcgDxeData.BsCap.SupportedEventLogs & mTcg2EventInfo[Index].LogFormat) != 0) {
      ...
      if ((PcdGet8(PcdTpm2AcpiTableRev) >= 4) ||
        ...
        //
        Status = PcdSet32S(PcdTpm2AcpiTableLaml, (UINT32)mTcgDxeData.EventLogAreaStruct[Index].Laml);
        ASSERT_EFI_ERROR (Status);
        Status = PcdSet64S(PcdTpm2AcpiTableLasa, mTcgDxeData.EventLogAreaStruct[Index].Lasa);
        ASSERT_EFI_ERROR (Status);
      }
...
/**
  Update TCG related SMBIOS table.
...
VOID
EFIAPI
Tcg2UpdateSmbiosEntry (
  IN EFI_EVENT                      Event,
  IN VOID                           *Context
  )
{
  EFI_STATUS                        Status;
  SMBIOS_TABLE_TYPE43               *Type43Record;
  ...
  while (TRUE) {
    ...
    if (Record->Type == SMBIOS_TYPE_TPM_DEVICE) {
      Type43Record = (SMBIOS_TABLE_TYPE43 *) Record;
      Type43Record->MajorSpecVersion = 0x02;
      Type43Record->MinorSpecVersion = 0x00;
      Type43Record->Characteristics  = 0x04;

      Status = Tpm2GetCapabilityFirmwareVersion (&FirmwareVersion1, &FirmwareVersion2);
      if (!EFI_ERROR (Status)) {
        Type43Record->FirmwareVersion1 = FirmwareVersion1;
        Type43Record->FirmwareVersion2 = FirmwareVersion2;
      }
      CopyMem ((VOID *)&Type43Record->VendorID, (VOID *)&mTcgDxeData.BsCap.ManufacturerID, sizeof (Type43Record->VendorID));
      break;
    }
  }
}
...
/**
  Set the authorization policy for platform hierarchy.
  ...
  )
{
  ...
  if ((PhPolicySha256 == NULL) || (Size != SHA256_DIGEST_SIZE)) {
    return EFI_INVALID_PARAMETER;
  }

  PlatformAuth = &mNewAuth;
  
  //
  // Prepare authorization session
  //
  if (PlatformAuth->size != 0) {
  ...
  //
  // Prepare authorization policy
  //
  ZeroMem (&AuthPolicy, sizeof(AuthPolicy));
  AuthPolicy.size = SHA256_DIGEST_SIZE;
  CopyMem (AuthPolicy.buffer, PhPolicySha256, AuthPolicy.size);

  return Tpm2SetPrimaryPolicy (TPM_RH_PLATFORM, (PlatformAuth->size == 0) ? NULL : &AuthSession, &AuthPolicy,  TPM_ALG_SHA256);
}

/**
  Ready to Boot Event notification handler.

  Uninstall ptotocol mTcg2UpdatedPlatformAuthGuid in this event notification handler.
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ZeroMem (PlatformAuth, sizeof(TPM2B_AUTH));
    ...
    if (EFI_ERROR(Status)) {
      DEBUG ((DEBUG_ERROR, "UninstallPlatmAuth(%r)\n", Status));
    }
  } else {
...
/**
  Replaces the default NULL platformAuth with a value from the RNG.
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG((EFI_D_ERROR, "Locate CryptoServicesProtocol(%r)\n", Status));
    return Status;
  }

  mNewAuth.size = SHA1_DIGEST_SIZE;
  ...
  if (EFI_ERROR (Status)) {
    ...
    for (Index = 0; Index < AlgList.count; Index++) {
      ...
      case TPM_ALG_SHA1:
        if (mNewAuth.size < SHA1_DIGEST_SIZE) {
          mNewAuth.size = SHA1_DIGEST_SIZE;
        }
        break;
      case TPM_ALG_SHA256:
        if (mNewAuth.size < SHA256_DIGEST_SIZE) {
          mNewAuth.size = SHA256_DIGEST_SIZE;
        }
        break;
      case TPM_ALG_SHA384:
        if (mNewAuth.size < SHA384_DIGEST_SIZE) {
          mNewAuth.size = SHA384_DIGEST_SIZE;
        }
        break;
      case TPM_ALG_SHA512:
        if (mNewAuth.size < SHA512_DIGEST_SIZE) {
          mNewAuth.size = SHA512_DIGEST_SIZE;
        }
        break;
  ...
  CryptoServiceProtocol->RandomSeed ((UINT8 *)&Time, sizeof(Time));
  CryptoServiceProtocol->RandomBytes (mNewAuth.buffer, mNewAuth.size);
  Status = Tpm2HierarchyChangeAuth (TPM_RH_PLATFORM, NULL, &mNewAuth);
  if (EFI_ERROR(Status)) {
    DEBUG((EFI_D_ERROR, "Tpm2HierarchyChangeAuth failed - %r\n", Status));
  } else {
    Handle = NULL;
...
/**
  Create TPM NV Index with handle 0x01C101C0.
  ...
  )
{
  ...
  NvIndex = MSFT_INDEX_C0;
  Status = Tpm2NvReadPublic (NvIndex, &NvPublic, &NvName);
  if (EFI_ERROR (Status) && (Status != EFI_NOT_FOUND)) {
    DEBUG ((EFI_D_ERROR, "CreateMsftIndexC0: Tpm2NvReadPublic failed -%r\n", Status));
    return Status;
  }

  if (Status == EFI_NOT_FOUND) {
    DEBUG ((EFI_D_ERROR, "TPM NV Index 0x%08x not exists, create the index -\n", NvIndex));

    ZeroMem (&AuthData, sizeof (AuthData));
    NvPublic.nvPublic.nvIndex  = NvIndex;
    NvPublic.nvPublic.dataSize = 0x80;
    ...
    NvPublic.nvPublic.attributes.TPMA_NV_PLATFORMCREATE = 1;
    NvPublic.nvPublic.attributes.TPMA_NV_POLICY_DELETE  = 1;

    NvPublic.nvPublic.authPolicy.size = sizeof (mMsftIndexC0PolicyDigestSha256);
    CopyMem ((VOID *)&NvPublic.nvPublic.authPolicy.buffer, (VOID *)&mMsftIndexC0PolicyDigestSha256, sizeof (mMsftIndexC0PolicyDigestSha256));
    ...
    Status = Tpm2NvDefineSpace (TPM_RH_PLATFORM, NULL, &AuthData, &NvPublic);
    if (EFI_ERROR (Status)) {
      DEBUG ((EFI_D_ERROR, "CreateMsftIndexC0: Tpm2NvDefineSpace(0x%08x): (%r)\n", NvIndex, Status));
      return Status;
    }
  }
...
/**
  ...
  Set specific platformPolicy, randomize the platformAuth and create TPM NV index if needed.

  @param[in] Event           A pointer to the Evenoked
...
{
  EFI_STATUS                              Status;
  UINT8                                   PhPolicySha256[SHA256_DIGEST_SIZE];

  //
  // Create TPM NV Index
  //
  if ((FeaturePcdGet (PcdH2OTpm2CreateMsftIndexC0Supported)) &&
      ((PcdGet32 (PcdDxeTpmPolicy) & SET_UP_MSFT_INDEX_C0) != 0)) {
    Status = CreateMsftIndexC0 ();
    if (EFI_ERROR (Status)) {
      DEBUG((EFI_D_ERROR, "CreateMsftIndexC0 failed -%r\n", Status));
    }
  }

  CopyMem(PhPolicySha256, PcdGetPtr (PcdTpm2PhPolicySha256), SHA256_DIGEST_SIZE);
  if (FeaturePcdGet(PcdH2ODxeCpUpdateTpm2PhPolicySha256Supported)) {
    H2O_DXE_CP_UPDATE_TPM2_PH_POLICY_SHA256_DATA  DxeCpUpdateTpm2PhPolicySha256Data;

    DxeCpUpdateTpm2PhPolicySha256Data.Size = sizeof (H2O_DXE_CP_UPDATE_TPM2_PH_POLICY_SHA256_DATA);
    DxeCpUpdateTpm2PhPolicySha256Data.Status = H2O_BDS_TASK_NORMAL;
    CopyMem(DxeCpUpdateTpm2PhPolicySha256Data.PhPolicySha256, PhPolicySha256, SHA256_DIGEST_SIZE);

    DEBUG_CP ((DEBUG_INFO, "Checkpoint Trigger: %g\n", &gH2ODxeCpUpdateTPM2PhPolicySha256Guid));
    H2OCpTrigger (&gH2ODxeCpUpdateTPM2PhPolicySha256Guid, &DxeCpUpdateTpm2PhPolicySha256Data);
    DEBUG_CP ((DEBUG_INFO, "Checkpoint Result: %x\n", DxeCpUpdateTpm2PhPolicySha256Data.Status));
    if (DxeCpUpdateTpm2PhPolicySha256Data.Status == H2O_CP_TASK_UPDATE) {
      CopyMem (PhPolicySha256, DxeCpUpdateTpm2PhPolicySha256Data.PhPolicySha256, SHA256_DIGEST_SIZE);
    }
  }
  if (!IsZeroBuffer (PhPolicySha256, SHA256_DIGEST_SIZE)) {
    Status = SetPlatformPolicy (PhPolicySha256, SHA256_DIGEST_SIZE);
    if (EFI_ERROR (Status)) {
      DEBUG((EFI_D_ERROR, "SetPlatformPolicy failed -%r\n", Status));
    }
  }

  if ((PcdGet32 (PcdDxeTpmPolicy) & SKIP_TPM_REVOKE_TRUST) == 0) {
    Status = RandomizePlatformAuth ();
    if (EFI_ERROR (Status)) {
      DEBUG((EFI_D_ERROR, "RandomizePlatformAuth failed -%r\n", Status));
    }
  }
}

/**
  Publishes EFI Tcg2 Protocol.
  ...
  )
{
  ...
  if (PcdGet8 (PcdTrEEProtocolVersion) == 0) {
    mTcgDxeData.BsCap.Size = sizeof(TREE_BOOT_SERVICE_CAPABILITY);
    mTcgDxeData.BsCap.ProtocolVersion.Major = 1;
    mTcgDxeData.BsCap.ProtocolVersion.Minor = 0;
    mTcgDxeData.BsCap.StructureVersion.Major = 1;
    mTcgDxeData.BsCap.StructureVersion.Minor = 0;
  ...
  if ((mTcgDxeData.BsCap.ProtocolVersion.Major > 0x01) || 
      ((mTcgDxeData.BsCap.ProtocolVersion.Major == 0x01) && ((mTcgDxeData.BsCap.ProtocolVersion.Minor > 0x00)))) {
    DEBUG ((EFI_D_ERROR, "!!!WARNING!!! SHA256 PCR Bank is inactive.\n", mTcgDxeData.BsCap.ActivePcrBanks));
  }

  if (mTcgDxeData.BsCap.TPMPresentFlag) {
    if (FeaturePcdGet (PcdH2OBdsCpInitSupported)) {
      //
      // Randomize platformAuth and set platformPolicy
      //
    ...
    //
    // Update SMBIOS Table Type 43 (TPM Device)
    //
    EfiCreateProtocolNotifyEvent (&gEfiSmbiosProtocolGuid, TPL_CALLBACK, Tcg2UpdateSmbiosEntry, NULL, &Registration);
  }
...
/**
  Tpm2 Interface Installation Event notification handler.
  Publishes EFI Tcg2 Protocol.
...
VOID
EFIAPI
Tpm2InterfaceInstallationEvent (
  IN      EFI_EVENT                 Event,
  IN      VOID                      *Context
  )
{
  ...
  Status = gBS->LocateProtocol (&gTpm2InterfaceGuid, NULL, (VOID **)&Interface);
  if (EFI_ERROR (Status)) {
    return;
  }
  ...
  if (!EFI_ERROR (Status)) {
    //
    // TCG2 protocol is ready already
    //
    gBS->CloseEvent (Event);
    return;
  }
  ...
  )
{
  ...
  SetMem ((VOID *)(UINTN)mLasaTcg12, PcdGet32 (PcdTcgLogAreaMinLen), 0xFF);
  ...
  SetMem ((VOID *)(UINTN)mLasaTcg2, PcdGet32 (PcdTcgLogAreaMinLen), 0xFF);
  ...
  SetMem ((VOID *)(UINTN)mFinalLasa, PcdGet32 (PcdTcg2FinalLogAreaLen), 0xFF);
  ...
  )
{
  ...
  Status = Tpm2RequestUseTpm ();
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "Tcg2Dxe: RequestUseTpm -%r\n", Status));
