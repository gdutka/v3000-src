/** @file
   EFI_PKCS7_VERIFY_PROTOCOL implement
  ...
  )
{
  ...
  //
  if ((TimeStampDb == NULL) || (*TimeStampDb == NULL)) {
    Status = FoundInRevokedDb ? EFI_SUCCESS : EFI_SECURITY_VIOLATION;
    goto _Exit;
  }
  ...
  )
{
  ...
  UINTN               TrustCertSize;
  UINTN               Index;
  UINT8               HashData[SHA512_DIGEST_SIZE];
  UINTN               HashSize;
  ...
  HashSize = sizeof (HashData);
  Status = HashDataUsingPkc7DataHashType (SignedData, SignedDataSize, InData, InDataSize, HashData, &HashSize);
  if (EFI_ERROR (Status)) {
  ...
  for (Index = 0; ; Index++) {
    ...
    // Verifying the PKCS#7 SignedData with the trusted certificate from AllowedDb
    //
    if (Pkcs7VerifyUsingHash (SignedData, SignedDataSize, TrustCert, TrustCertSize, HashData, HashSize)) {
      //
      // The SignedData was verified successfully by one entry in Trusted Database
      //
      Status = EFI_SUCCESS;
