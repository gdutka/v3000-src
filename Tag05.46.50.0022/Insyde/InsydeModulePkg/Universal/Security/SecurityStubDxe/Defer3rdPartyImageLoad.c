...
STATIC BOOLEAN                  mIsInFlashState           = FALSE;
...
/**
  Set the indicator if the system is in the flash mode.
...
{
  EFI_STATUS  Status;
  IMAGE_INFO  SecureFlashImageInfo;
  UINTN       Size;
  ...
  Status = CommonGetVariable (
             ...
             &Size,
             &SecureFlashImageInfo
             );
  if (!EFI_ERROR (Status) && SecureFlashImageInfo.FlashMode) {
    mIsInFlashState = TRUE;
  }
