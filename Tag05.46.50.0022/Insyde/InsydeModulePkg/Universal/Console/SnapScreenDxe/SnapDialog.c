  ...
  )
{
  ...
  if (ADiskName != NULL) {
    StrnCpyS (DiskName, sizeof (DiskName) / sizeof (CHAR16), ADiskName, 12);
  }

  if (AVolumeName != NULL) {
    StrnCpyS (VolumeName, sizeof (VolumeName) / sizeof (CHAR16), AVolumeName, 11);
  }
  ...
  if (AVolumeSize != NULL) {
    ...
    Index = 9 - Len;
    StrnCpyS (&VolumeSize[Index], sizeof (VolumeName) / sizeof (CHAR16) - Index, AVolumeSize, 9-Index);
  }
  ...
  StrCpyS (FsName, FsNameLen + 1, FsIndexStr);     // FS1
  StrCatS (FsName, FsNameLen + 1, FsSeparate);     // FS1:
  StrCatS (FsName, FsNameLen + 1, DiskTypeStr);    // FS1: USB
  StrCatS (FsName, FsNameLen + 1, DsSeparate);     // FS1: USB/
  StrCatS (FsName, FsNameLen + 1, DiskName);       // FS1: USB/JetFlash
  StrCatS (FsName, FsNameLen + 1, DsSeparate);     // FS1: USB/JetFlash     /
  StrCatS (FsName, FsNameLen + 1, VolumeName);     // FS1: USB/JetFlash     /MURPHY_128M
  ...
  StrCatS (FsName, FsNameLen + 1, VolumeSize);     // FS1: USB/JetFlash     /MURPHY_128M 120.8 MB
  ...
  )
{
  ...
  StrCpyS (MsgDialog->Message,  Width, MessageStr);
