...
STATIC EFI_EVENT                   mHiiRegistration;
STATIC EFI_IMAGE_START             mOrgImageStart;
STATIC BOOLEAN                     mReadyToBoot;
STATIC UINTN                       mNoRotateMode0;
  ...
  )
{
  ...
  if ((TextModeCount == NULL) || (TextModeData == NULL) || GraphicsOutput == NULL) {
  ...
  if (!EFI_ERROR (Status)) {
    ...
    for (ModeNumber = 0; ModeNumber < MaxMode; ModeNumber++) {
      if ((Private->ModeData[ModeNumber].Columns == Columns) &&
        ...
        if (Private->GraphicsOutput == NULL || (INT32)ModeNumber != Private->GraphicsOutput->Mode->Mode) {
          Status = GraphicsConsoleConOutSetMode (&Private->SimpleTextOutput, ModeNumber);
        } else {
          Private->SimpleTextOutput.Mode->Mode = (INT32)ModeNumber;
          Status = EFI_SUCCESS;
        }
        break;
      }
  ...
  )
{
  ...
  if (ModeNumber == 0) {
    ...
    if (mNoRotateMode0) {
      HorizontalResolution = Private->BackupMode0.GopHeight;
      VerticalResolution = Private->BackupMode0.GopWidth;
      ...
      ModeData->GopModeNumber = GopModeNumber;
      ModeData->DeltaX        = (HorizontalResolution - (ModeData->Columns * ModeData->GlyphWidth)) >> 1;
      ModeData->DeltaY        = (VerticalResolution - (ModeData->Rows * ModeData->GlyphHeight)) >> 1;
    }
  }
  ...
  )
{
  ...
  if (Private->GraphicsOutput != NULL) {
    ...
    Blt->Image.Bitmap = AllocateZeroPool ((UINTN) Blt->Width * Blt->Height * sizeof (EFI_GRAPHICS_OUTPUT_BLT_PIXEL));
  ...
  )
{
  ...
  StrCpyS (FontPackage->FontFamily, sizeof(L"ttf") / sizeof(CHAR16), L"ttf");
  ...
  )
{
  ...
  if (!mReadyToBoot || !IsWindows (ImageHandle, &VersionMS, &VersionLS)) {
    return mOrgImageStart (ImageHandle, ExitDataSize, ExitData);
  }

  mNoRotateMode0++;

  //
  // reset mode 0 to no rotate
  //
  if (mNoRotateMode0 == 1  && gST->ConOut->Mode->Mode == 0) {
    gST->ConOut->SetMode (gST->ConOut, 0);
  }
  Status = mOrgImageStart (ImageHandle, ExitDataSize, ExitData);

  mNoRotateMode0--;

  //
  // restore mode 0 to rotated mode
  //
  if (mNoRotateMode0 == 0 && gST->ConOut->Mode->Mode == 0) {
    gST->ConOut->SetMode (gST->ConOut, 0);
  }
...
VOID
EFIAPI
GraphicsConsoleReadyToBootCallback (
  IN EFI_EVENT  Event,
  IN VOID       *Context
  )
{
  ...
  )
{
  ...
  if (FeaturePcdGet (PcdH2ORotateScreenSupported) && FeaturePcdGet (PcdH2ORotateScreenRotateLogo)) {
    ...
    // hook image start
    //
    mOrgImageStart = gBS->StartImage;
    gBS->StartImage = GraphicsConsoleStartImage;
