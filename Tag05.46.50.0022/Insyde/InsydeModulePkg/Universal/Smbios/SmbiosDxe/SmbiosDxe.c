  ...
  )
{
  ...
  if (EFI_SIZE_TO_PAGES ((UINT32) EntryPointStructure->TableLength + PcdGet32(PcdH2OSmbiosTableExtraMemorySize)) > mPreAllocatedPages) {
    //
  ...
  )
{
  ...
  if (EFI_SIZE_TO_PAGES (Smbios30EntryPointStructure->TableMaximumSize + PcdGet32(PcdH2OSmbiosTableExtraMemorySize)) > mPre64BitAllocatedPages) {
    //
    // If new SMBIOS table size exceeds the previous allocated page,
