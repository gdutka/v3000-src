...
CHAR8                          mStrBuffer[MAX_BUFFER_LENGTH + 1];
  ...
  )
{
  ...
  if (VarStart != NULL) {
    ...
    while (((UINT8 *)DmiEntry + MinSize - 1) < VarEnd && DmiEntry->Type != 0xFF && 
           DmiEntry->Length >= MinSize && ((UINT8 *)DmiEntry + DmiEntry->Length - 1) < VarEnd) {

      if (DmiEntry->Valid == 0xFF && DmiEntry->Type == Type && DmiEntry->Offset == Offset) {
        if (DmiEntry->Length == MinSize) {
          Info->Data = NULL;
          Info->Length = 0x00;
        } else {
          Info->Data   = (CHAR8 *)DmiEntry->String;
  ...
  )
{
  ...
  if (DmiStart == NULL || *(UINT32*)DmiStart != DMI_UPDATE_STRING_SIGNATURE) {
  ...
  if (EFI_ERROR (Status)) {
    ...
    if ((VarSize < sizeof (DMI_UPDATE_STRING_SIGNATURE)) || (*(UINT32 *)VarStart != DMI_UPDATE_STRING_SIGNATURE)) {
...
/**
  ...
  Update Type24 SMBIOS Record.

  @param   SmbiosType24        Smbios type24 record
...
VOID
UpdateType24Record (
  IN SMBIOS_TABLE_TYPE24      *SmbiosType24
  )
{
  EFI_STATUS                      Status;
  KERNEL_CONFIGURATION            KernelConfig;

  SmbiosType24->HardwareSecuritySettings = (UINT8) (EfiHardwareSecurityStatusNotImplemented |
                                                   (EfiHardwareSecurityStatusDisabled << 2) |
                                                   (EfiHardwareSecurityStatusNotImplemented << 4) |
                                                   (EfiHardwareSecurityStatusDisabled << 6)); 

  Status = GetKernelConfiguration (&KernelConfig);
  if (EFI_ERROR (Status)) {
    return;
  }
  if (KernelConfig.PowerOnPassword == POWER_ON_PASSWORD) {
    SmbiosType24->HardwareSecuritySettings |= (UINT8) (EfiHardwareSecurityStatusEnabled << 6);
  }
  if (KernelConfig.SupervisorFlag) {
    SmbiosType24->HardwareSecuritySettings |= (UINT8) (EfiHardwareSecurityStatusEnabled << 2);
  }
...
/**
  ...
  Update SMBIOS Record.

  @param   SmbiosTableHeader        Smbios record
...
VOID
UpdateRecord (
  IN EFI_SMBIOS_TABLE_HEADER *SmbiosTableHeader
  )
{
  switch (SmbiosTableHeader->Type) {
  case EFI_SMBIOS_TYPE_HARDWARE_SECURITY:
    UpdateType24Record((SMBIOS_TABLE_TYPE24*) SmbiosTableHeader);
    break;
  ...
  )
{
  ...
  if (RecordData == NULL || (RecordLength == 1 && RecordData[0] == 0)) {
    //
    //  The record is empty.
  ...
  if (RecordStrings[0] != '\0') {
    ...
    for (StrIndex = 0; StrIndex < StrCount; ++StrIndex) {
      ...
      if (OffsetTable != NULL) {
        if (DataInfo.Length > MAX_BUFFER_LENGTH) {
          DEBUG((DEBUG_ERROR, "The length of this structure table(Type %d) is too big.\n", Hdr->Type));
          DEBUG((DEBUG_ERROR, "So this table(Type %d) will not add to system.\n", Hdr->Type));
          FreePool(Buffer);
          return;
        }
        CopyMem (mStrBuffer, DataInfo.Data, DataInfo.Length);
        mStrBuffer[DataInfo.Length] = '\0';
        DataInfo.Data = mStrBuffer;
        DEBUG_OEM_SVC ((DEBUG_INFO, "OemKernelServices Call: OemSvcDxeGetSmbiosReplaceString \n"));
        Status = OemSvcDxeGetSmbiosReplaceString(Hdr->Type, OffsetTable->Offset[StrIndex], &DataInfo.Length, &DataInfo.Data);
        DEBUG_OEM_SVC ((DEBUG_INFO, "OemKernelServices OemSvcDxeGetSmbiosReplaceString Status: %r\n", Status));
  ...
  UpdateRecord ((EFI_SMBIOS_TABLE_HEADER*) Buffer);
  if (PcdGetBool(PcdH2ODxeCpUpdateSmbiosRecordSupported)) {
    H2O_DXE_CP_UPDATE_SMBIOS_RECORD_DATA  DxeUpdateSmbiosRecordData;
    DxeUpdateSmbiosRecordData.SmbiosTableHeader = AllocateCopyPool (MAX_BUFFER_LENGTH, Buffer);
    DxeUpdateSmbiosRecordData.Size = sizeof (H2O_DXE_CP_UPDATE_SMBIOS_RECORD_DATA);
    DxeUpdateSmbiosRecordData.Status = H2O_CP_TASK_NORMAL;
    DEBUG_CP ((DEBUG_INFO, "Checkpoint Trigger: %g\n", &gH2ODxeCpUpdateSmbiosRecord));
    H2OCpTrigger (&gH2ODxeCpUpdateSmbiosRecord, &DxeUpdateSmbiosRecordData);
    DEBUG_CP ((DEBUG_INFO, "Checkpoint Result: %x\n", DxeUpdateSmbiosRecordData.Status));
    if (DxeUpdateSmbiosRecordData.Status == H2O_CP_TASK_UPDATE) {
      FreePool (Buffer);
      Buffer = (UINT8*) DxeUpdateSmbiosRecordData.SmbiosTableHeader;
      Hdr = (EFI_SMBIOS_TABLE_HEADER *) Buffer;
    } else if (DxeUpdateSmbiosRecordData.SmbiosTableHeader != NULL) {
      FreePool (DxeUpdateSmbiosRecordData.SmbiosTableHeader);
    }
    if (DxeUpdateSmbiosRecordData.Status == H2O_CP_TASK_SKIP) {
      goto UpdateSmbiosRecordDataSkip;
    }
  }
  //
  ...
  //
  DEBUG_OEM_SVC ((DEBUG_INFO, "OemKernelServices Call: OemSvcDxeUpdateSmbiosRecord \n"));  
  Status = OemSvcDxeUpdateSmbiosRecord((EFI_SMBIOS_TABLE_HEADER *)Buffer);
  DEBUG_OEM_SVC ((DEBUG_INFO, "OemKernelServices OemSvcDxeUpdateSmbiosRecord Status: %r\n", Status));
...
)
{
  ...
  if (DataInfo.Length > MAX_BUFFER_LENGTH) {
    DEBUG((DEBUG_ERROR, "The length of this structure table(Type 0) is too big.\n"));
    DEBUG((DEBUG_ERROR, "So this table(Type 0) will not to be add to system.\n"));
  ...
  )
{
  ...
  if (((Type0->BiosSize == 0) ||
      ((Type0->BiosSize == 0xFF) && (Type0->ExtendedBiosSize.Size == 0)))) {
    FlashSize = (UINTN) PcdGet32 (PcdFlashAreaSize);
    // FFh - size is 16MB or greater
    Type0->BiosSize = FlashSize >= SIZE_16MB ? 0xFF : (UINT8)((FlashSize >> 16) - 1);  
    ...
    if (Type0->BiosSize == 0xFF) {
      SizeUnit = FlashSize >= SIZE_1GB ? 1 : 0;
      BiosExtSize = (UINT16)(FlashSize / (SizeUnit ? SIZE_1GB : SIZE_1MB));
      // round up
      if ((FlashSize % (SizeUnit ? SIZE_1GB : SIZE_1MB)) != 0){
       BiosExtSize += 1;
      }
      Type0->ExtendedBiosSize.Unit = SizeUnit;
      Type0->ExtendedBiosSize.Size = BiosExtSize & 0x3FFF;
    } else {
      Type0->ExtendedBiosSize.Unit = 0;
      Type0->ExtendedBiosSize.Size = 0;
    }
  }
  ...
  if (Type0->Vendor == 0xFF) {
    UnicodeStrToAsciiStrS ((CHAR16 *)PcdGetPtr(PcdFirmwareVendor), BufPtr, (UINTN) (BufEnd - BufPtr - 1));
  } else {
  ...
  if (Type0->BiosVersion == 0xFF) {
    ...
    UnicodeStrToAsciiStrS (VersionStr, BufPtr, (UINTN) (BufEnd - BufPtr - 1));
  } else {
  ...
  if (Type0->BiosReleaseDate == 0xFF) {
    UnicodeStrToAsciiStrS ((CHAR16 *)PcdGetPtr(PcdFirmwareReleaseDateString), BufPtr, (UINTN) (BufEnd - BufPtr - 1));
  } else {
  ...
  if (PcdGetBool(PcdH2ODxeCpUpdateSmbiosRecordSupported)) {
    ...
    if (DxeUpdateSmbiosRecordData.Status == H2O_CP_TASK_SKIP) {
      goto UpdateSmbiosRecordDataSkipType0;
    }
  }
  //
  //  Transfer record buffer to OEM service, So OEM have chance to midify and verify record data
...
UpdateSmbiosRecordDataSkipType0:
  FreePool(Buffer);
