  ...
  )
{
  ...
  case EfiPlatformHookPrepareToScanRom:
    ...
    // OemServices
    //
    DEBUG_OEM_SVC ((DEBUG_INFO, "OemKernelServices Call: OemSvcInstallPciRomSwitchTextMode \n"));
    Status = OemSvcInstallPciRomSwitchTextMode (
               DeviceHandle,
               &ForceSwitchTextMode
               );
    DEBUG_OEM_SVC ((DEBUG_INFO, "OemKernelServices OemSvcInstallPciRomSwitchTextMode Status: %r\n", Status));
  ...
  case EfiPlatformHookShadowServiceRoms:
    ...
    if (BisLoaderImage != NULL) {
      FreePool (BisLoaderImage);
    }
    FreePool (ServiceRomTable);
    return EFI_SUCCESS;
