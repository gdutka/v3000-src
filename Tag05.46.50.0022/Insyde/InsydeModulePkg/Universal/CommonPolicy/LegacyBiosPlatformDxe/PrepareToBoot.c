  ...
  )  
{
  ...
  //
  SuperIoPtr = &(((EFI_TO_COMPATIBILITY16_BOOT_TABLE *) EfiToLegacy16BootTable)->SioData);
  ...
  // OemServices
  //
  DEBUG_OEM_SVC ((DEBUG_INFO, "OemKernelServices Call: OemSvcInstallLegacyBiosOemSlp \n"));
  Status = OemSvcInstallLegacyBiosOemSlp (
             &SlpStringLength,
             &SlpStringAddress
             );
  DEBUG_OEM_SVC ((DEBUG_INFO, "OemKernelServices OemSvcInstallLegacyBiosOemSlp Status: %r\n", Status));
