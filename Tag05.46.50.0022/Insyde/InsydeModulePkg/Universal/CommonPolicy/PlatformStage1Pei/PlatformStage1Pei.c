...
static EFI_PEI_PPI_DESCRIPTOR   mPpiList[2] = {
  {
  ...
  )
{
  ...
  if (Status == EFI_ALREADY_STARTED) {
    ...
    DEBUG_OEM_SVC ((DEBUG_INFO, "OemKernelServices Call: OemSvcEcInit \n"));
    OemSvcStatus = OemSvcEcInit ();
    DEBUG_OEM_SVC ((DEBUG_INFO, "OemKernelServices OemSvcEcInit Status: %r\n", OemSvcStatus));
