      ...
      if ((LegacyNormalMenuType == 0) && (BootOptionType == LEGACY_BOOT_DEV)) {
        ...
        case IN_AUTO:
          if (BootOptionType == LEGACY_BOOT_DEV) {
            ...
            CreateByOS = TRUE;
            //
            // AsciiStrnCmp () will call AsciiStrSize () in EFI_DEBUG mode to access in invalid memory if TempPtr doesn't have end of string.
            // Therefore, replace AsciiStrnCmp with CompareMem to check non-string data.
            //
            if (((BootOptionSize - Size == 2) && (CompareMem (TempPtr, "RC", 2) == 0)) ||
                (IsEfiShellBootOption (DevicePath) && (BootOptionSize - Size >= 2) && (CompareMem (TempPtr, "RC", 2) == 0))) {
              CreateByOS = FALSE;
            }
            Status = GetEfiOptionPriority (
                       This,
