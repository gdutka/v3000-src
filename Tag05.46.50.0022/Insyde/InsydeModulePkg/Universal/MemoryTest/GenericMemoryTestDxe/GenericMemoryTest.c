...
/**
  Helper funciton to set pattern to destination address

  @param[out] Desination       Pointer to Physical address.
...
VOID
SetPatternToAddress (
  OUT   EFI_PHYSICAL_ADDRESS  *Desination,
  IN    EFI_PHYSICAL_ADDRESS  Pattern
  )
{
  if (Desination != NULL) {
    *Desination = Pattern;
    ...
    for (Index = 0; Index < Private->PmtAddressLinePattern.Count; Index++) {
      ...
      if (InExtendedRange) {
        if (((UINTN) TestAddress) > (UINTN)IA32_MAX_LINEAR_ADDRESS) {
          ...
          if (WriteLoop) {
            //
            // Use function to workaound C6011: Dereferencing NULL pointer 'TestAddress'  issue.
            //
            SetPatternToAddress(TestAddress, AddressLinePattern[Index]);
          } else {
...
/**
  ...
  @param[in] BaseAddress  Base address of the memory range.
  @param[in] Length       Length of the memory range.
  @param[in] Capabilities Capabilities of the memory range.
  ...
  )
{
  ...
  *RequireSoftECCInit = FALSE;
  ...
  if (Private->PmtMemoryTest != NULL) {
    ...
    Private->MonoTestSize = (UINTN) Private->PmtMemoryPattern.Count * Private->PmtMemoryPattern.Width;
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "ConstructDIMMs(%r)\n", Status));
    }
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "Notify platform memory test driver test begin(%r)\n", Status));
    }
  }
  //
