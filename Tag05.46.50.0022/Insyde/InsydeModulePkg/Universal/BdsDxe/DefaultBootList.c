  ...
  )
{
  ...
  WorkingStringSize = StrSize (OptionalDataString);
  WorkingString = AllocateZeroPool (WorkingStringSize);
  if (WorkingString == NULL) {
  ...
  StrCpyS (WorkingString, WorkingStringSize / sizeof(CHAR16), CopiedString + 3);
  FreePool (CopiedString);
  ...
  )
{
  ...
  OptionalDataStringSize = StrSize (OptionalDataString);
  CopiedString = AllocateCopyPool (OptionalDataStringSize, OptionalDataString);
  if (CopiedString == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }
  ...
  AsciiString = AllocateZeroPool (OptionalDataStringSize);
  if (AsciiString == NULL) {
    FreePool (CopiedString);
    return EFI_OUT_OF_RESOURCES;
  }
  UnicodeStrToAsciiStrS (CopiedString + 2, AsciiString, OptionalDataStringSize);
  FreePool (CopiedString);
...
VOID
SwapMap (
  IN OUT HANDLE_PRIORITY_MAP   *PriorityMap,
  IN OUT HANDLE_PRIORITY_MAP   *PriorityMap2
  )
{
...
**/
EFI_STATUS
UpdateDefaultBootListFromBootGroup (
  ...
  )
{
  ...
  for (Index = 0; Index < HandleCount; Index++) {
    ...
    for (InnerIndex = 0; InnerIndex < BootOptionArrayLen; InnerIndex++) {
      InsertTailList (BootList, &BootOptionArray[InnerIndex].Link);
    }
  }
  ...
  )
{
  ...
  UpdateDefaultBootListFromPcd (BootList);
  UpdateDefaultBootListFromBootGroup (BootList);

  DEBUG_OEM_SVC ((DEBUG_INFO, "OemKernelServices Call: OemSvcUpdateDefaultBootList \n"));
  Status = OemSvcUpdateDefaultBootList (&BootList);
  DEBUG_OEM_SVC ((DEBUG_INFO, "OemKernelServices OemSvcUpdateDefaultBootList Status: %r\n", Status));
  if (Status == EFI_SUCCESS) {
    return Status;
  }
  ...
  Count = 0;
  for (Link = GetFirstNode (BootList); !IsNull (BootList, Link); Link = GetNextNode (BootList, Link)) {
    Count++;
  }
  ...
  for (Link = GetFirstNode (BootList); !IsNull (BootList, Link); Link = GetNextNode (BootList, Link)) {
    ...
    OemSvcStatus = OemSvcDxeUpdateDescriptionOfBootOption (LoadOption->DevicePath, NULL, NULL, &LoadOption->Description);
    ...
    BootOrder[Count++] = LoadOption->LoadOrder;
  }
