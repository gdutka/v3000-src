...
STATIC BDS_VARIABLE_INFO    mVariablesToBeLocked[] = {
                             {L"Setup",                                   &gSystemConfigurationGuid},
                             {SECURE_FLASH_INFORMATION_NAME,              &gSecureFlashInfoGuid},
                             {EFI_ADMINISTER_SECURE_BOOT_NAME,            &gEfiGenericVariableGuid},
                             {EFI_MEMORY_TYPE_INFORMATION_VARIABLE_NAME,  &gEfiMemoryTypeInformationGuid},
                             {L"TargetHddDevPath",                        &gEfiGenericVariableGuid}
                           };
  ...
  )
{
  ...
  Count = ARRAY_SIZE (mVariablesToBeLocked);
  for (Index = 0; Index < Count; Index++) {
    Status = VariableLock->RequestToLock (
                             VariableLock,
                             mVariablesToBeLocked[Index].VariableName,
                             mVariablesToBeLocked[Index].VarGuid
                             );
    ASSERT_EFI_ERROR( Status );
    if (EFI_ERROR (Status)) {
      return Status;
