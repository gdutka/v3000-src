  ...
  )
{
  ...
  //
  // All of the FHandles in the same folder are the same (because the FHandle
  // is for this folder). If we close any of these FHandles, it will cause the
  // FHandle for the folder is closed. Therefore, we only need close file handle
  // if the location is root.
  //
  if (FileContext->IsRoot && FileContext->FHandle != NULL) {
  ...
  )
{
  ...
  if (FileContext->IsLoadFile && !FileContext->IsBootLegacy) {
    ...
    while (!IsDevicePathEnd (FileContext->DevicePath)) {
      FileContext->DevicePath = NextDevicePathNode (FileContext->DevicePath);
    }
  ...
  )
{
  ...
  case DbtForm:
    return FORM_ID_DBT_ENROLL_SIGNATURE;

  case DbrForm:
    return FORM_ID_DBR_ENROLL_SIGNATURE;
  ...
  )
{
  ...
  if ((mSecureBootFsOptionMenu.MenuNumber != 0) && !IsListEmpty (&mSecureBootFsOptionMenu.Head)) {
    EmFreeMenu (&mSecureBootFsOptionMenu);
    mSecureBootFsOptionMenu.MenuNumber = 0;
  }
