...
#define TTF_GLYPH                       0x20
#define TTF_RENDER_GLYPH                0x10
...
/**
 ...
 @param[in] CharValue            Unicode character value, which identifies a glyph block.
 @param[in] GlyphInfoList        HII_GLYPH_INFO list head.
 @param[in] Cell                 Incoming character cell information.
...
/**
 ...
 @param[in]  CharValue           Unicode character value, which identifies a glyph block.
 @param[in]  GlyphInfoList       HII_GLYPH_INFO list head.
 @param[out] Cell                Buffer which stores output character cell information.

 @retval EFI_SUCCESS             Cell information is added to the GlyphInfoList.
 @retval EFI_NOT_FOUND           The character info specified by CharValue does not exist.
**/
EFI_STATUS
GetCell (
  IN  CHAR16                         CharValue,
  ...
  )
{
  ...
  SrcPitch = Cell->Width;
  DstPitch = Cell->AdvanceX;

  SrcX = 0;
  SrcY = 0;
  DstX = Cell->OffsetX;
  DstY = (INT16)(GlyphBaseLine - (Cell->OffsetY + Height));
  ...
  if ((DstX + Width) > DstPitch) {
    Width = DstPitch - DstX;
  }
  if ((DstY + Height) > FontSize) {
    Height = FontSize - DstY;
  }

  Src = Glyph + SrcY * SrcPitch + SrcX;
  Dst = GlyphBuffer + DstY * DstPitch + DstX;
  while (--Height >= 0) {
    CopyMem (Dst, Src, Width);
    Src += SrcPitch;
    Dst += DstPitch;
  }
}
  ...
  )
{
  ...
  EFI_STATUS                       Status;
  UINTN                            GlyphBaseLine;
  UINTN                            GlyphBufferLen;
  STATIC UINT8                     *GlyphBuffer = NULL;
  STATIC UINTN                     GlyphBufferMaxLen = 0;
  UINT16                           FontSize;

  ASSERT (StringInfo != NULL);
  ASSERT (GlyphBufferOut != NULL && Cell != NULL);
  ASSERT (Private != NULL && Private->Signature == HII_DATABASE_PRIVATE_DATA_SIGNATURE);
  ASSERT (IsTtfFontPackage (StringInfo->FontName));
  ...
  FontSize = StringInfo->FontSize;

  ASSERT (FontSize != 0);

  GlyphBufferLen = GlyphBufferMaxLen;
  Status = TrueTypeDecoder->GetGlyph (
                              TrueTypeDecoder,
                              Char,
                              FontSize,
                              StringInfo->FontStyle,
                              GlyphBuffer,
                              &GlyphBufferLen,
                              Cell,
                              &GlyphBaseLine
  ...
  if (Status == EFI_BUFFER_TOO_SMALL) {
    ...
    GlyphBufferMaxLen = GlyphBufferLen;
    GlyphBuffer = AllocateZeroPool (GlyphBufferMaxLen);
    Status = TrueTypeDecoder->GetGlyph (
                                TrueTypeDecoder,
                                Char,
                                FontSize,
  ...
  ASSERT (Cell->AdvanceX != 0);

  //
  // fill glyph buffer to advancex * fontsize
  //
  *GlyphBufferOut = AllocateZeroPool ((UINTN) Cell->AdvanceX * FontSize * sizeof (EFI_GRAPHICS_OUTPUT_BLT_PIXEL));
  if (*GlyphBufferOut == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }

  CopyTtfGlyph (*GlyphBufferOut, GlyphBuffer, Cell, FontSize, GlyphBaseLine);

  Cell->Width   = Cell->AdvanceX;
...
/**
 ...
 @param[in]  Private             HII database driver private data.
 @param[in]  Char                Character to retrieve.
 @param[in]  StringInfo          Points to the string font and color information
                                 or NULL  if the string should use the default
                                 system font and color.
 @param[out] GlyphBuffer         Buffer to store the retrieved bitmap data.
 @param[out] Cell                Points to EFI_HII_GLYPH_INFO structure.
 @param[out] Attributes          If not NULL, output the glyph attributes if any.
  ...
  )
{
  ...
  //
  if (StringInfo != NULL) {
    if (IsTtfFontPackage (StringInfo->FontName)) {
      return GetTtfGlyphBuffer (Private, Char, StringInfo, GlyphBuffer, Cell, Attributes);
    }
    ...
    for (Link = Private->DatabaseList.ForwardLink; Link != &Private->DatabaseList; Link = Link->ForwardLink) {
      ...
      for (Link1 = Node->PackageList->SimpleFontPkgHdr.ForwardLink;
        ...
        for (Index = 0; Index < SimpleFont->SimpleFontPkgHdr->NumberOfNarrowGlyphs; Index++, NarrowPtr++) {
          if (NarrowPtr->UnicodeWeight == Char) {
            *GlyphBuffer = (UINT8 *) AllocateZeroPool (EFI_GLYPH_HEIGHT);
            ...
            Cell->AdvanceX = Cell->Width;
            CopyMem (*GlyphBuffer, NarrowPtr->GlyphCol1, Cell->Height);
            if (Attributes != NULL) {
              *Attributes = (UINT8) (NarrowPtr->Attributes | NARROW_GLYPH);
            }
            return EFI_SUCCESS;
          }
        }
        ...
        for (Index = 0; Index < SimpleFont->SimpleFontPkgHdr->NumberOfWideGlyphs; Index++, WidePtr++) {
          if (WidePtr->UnicodeWeight == Char) {
            *GlyphBuffer    = (UINT8 *) AllocateZeroPool (EFI_GLYPH_HEIGHT * 2);
            ...
            Cell->Height   = EFI_GLYPH_HEIGHT;
            Cell->AdvanceX = Cell->Width;
            CopyMem (*GlyphBuffer                   , WidePtr->GlyphCol1, EFI_GLYPH_HEIGHT);
            CopyMem (*GlyphBuffer + EFI_GLYPH_HEIGHT, WidePtr->GlyphCol2, EFI_GLYPH_HEIGHT);
            if (Attributes != NULL) {
              *Attributes = (UINT8) (WidePtr->Attributes | EFI_GLYPH_WIDE);
            }
            return EFI_SUCCESS;
          }
        }
...
{
  UINT32                             GlyphWidth;
  EFI_NARROW_GLYPH                   ReplaceCharGlyphData =
                                       {0xFFFD, 0x00, {0xaa,0x55,0xaa,0x55,0xaa,0x55,0xaa,0x55,0xaa,0x55,0xaa,0x55,0xaa,0x55,0xaa,0x55,0xaa,0x55,0xaa}};
  ...
  GlyphWidth = ConsoleLibGetGlyphWidth (Char);
  ...
  if (GlyphWidth != 2) {
    *GlyphBuffer = (UINT8 *) AllocateZeroPool (EFI_GLYPH_HEIGHT);
    ...
    CopyMem (*GlyphBuffer, ReplaceCharGlyphData.GlyphCol1, Cell->Height);
    if (Attributes != NULL) {
      *Attributes = ReplaceCharGlyphData.Attributes | NARROW_GLYPH;
    }
    ...
    *GlyphBuffer    = (UINT8 *) AllocateZeroPool (EFI_GLYPH_HEIGHT * 2);
    ...
    Cell->AdvanceX = Cell->Width;
    CopyMem (*GlyphBuffer               , ReplaceCharGlyphData.GlyphCol1, Cell->Height);
    CopyMem (*GlyphBuffer + Cell->Height, ReplaceCharGlyphData.GlyphCol1, Cell->Height);
    if (Attributes != NULL) {
      *Attributes = ReplaceCharGlyphData.Attributes | EFI_GLYPH_WIDE;
    }
  }
...
/**
 ...
 @param[in]      GlyphBuffer    Buffer points to bitmap data of glyph.
 @param[in]      Foreground     The color of the "on" pixels in the glyph in the bitmap.
 @param[in]      Background     The color of the "off" pixels in the glyph in the bitmap.
 @param[in]      ImageWidth     Width of the whole image in pixels.
 @param[in]      RowWidth       The width of the text on the line, in pixels.
 @param[in]      RowHeight      The height of the line, in pixels.
 @param[in]      Transparent    If TRUE, the Background color is ignored and all
                                "off" pixels in the character's drawn wil use the
                                pixel value from BltBuffer.
 @param[in, out] Origin         On input, points to the origin of the to be
                                displayed character, on output, points to the next glyph's origin.
**/
VOID
NarrowGlyphToBlt (
...
//UINT8                         Alpha;
  ...
  RenderFlag = ((Attributes & TTF_RENDER_GLYPH) == TTF_RENDER_GLYPH);
  BltBuffer = *Origin - BaseLine * ImageWidth;
  FontSize = (UINT16)RowHeight;
  FgColor  = *(UINT32 *)&Foreground;
  FgColor &= 0x00FFFFFF;
  BgColor  = *(UINT32 *)&Background;
  BgColor &= 0x00FFFFFF;

  Width  = MIN (Cell->Width, RowWidth);
  Height = Cell->Height;
  ...
  SrcY = 0;
  DstX = Cell->OffsetX;
  DstY = BaseLine - (Cell->Height + Cell->OffsetY);
  ...
  Src = GlyphBuffer + SrcY * SrcPitch + SrcX;
  Dst = (UINT32 *)(BltBuffer + DstY * DstPitch + DstX);
  while (--Height >= 0) {
    register INTN     W = Width;
    register UINT8    *S = Src;
    register UINT32   *D = Dst;

    // background color
    SetMem32 (Dst, W << 2, BgColor);

    while (--W >= 0) {
      SrcColor = FgColor + ((*S++) << 24);
      if (RenderFlag) {
        BlendPixel ((EFI_GRAPHICS_OUTPUT_BLT_PIXEL *)D++, (EFI_GRAPHICS_OUTPUT_BLT_PIXEL *)&SrcColor);
      } else {
        *D++ = SrcColor;
      }
    }
...
/**
 ...
 @param[in]       Background     The color of the "off" pixels in the glyph in the bitmap.
 @param[in]       ImageWidth     Width of the whole image in pixels.
 @param[in]       BaseLine       BaseLine in the line.
 @param[in]       RowWidth       The width of the text on the line, in pixels.
 @param[in]       RowHeight      The height of the line, in pixels.
 @param[in]       Transparent    If TRUE, the Background color is ignored and all
                                 "off" pixels in the character's drawn wil use the
                                 pixel value from BltBuffer.
 @param[in]       Cell           Points to EFI_HII_GLYPH_INFO structure.
 @param[in]       Attributes     The attribute of incoming glyph in GlyphBuffer.
 @param[in, out]  Origin         On input, points to the origin of the to be
  ...
  )
{
  ...
  } else if ((Attributes & TTF_GLYPH) == TTF_GLYPH) {
    //
    // This character is TTF glyph.
    //
    TtfGlyphToBlt (
      GlyphBuffer,
      Foreground,
...
/**
 ...
 @param[in]  BufferIn            Buffer which stores the bitmap data of the found block.
 @param[in]  BufferLen           Length of BufferIn.
 @param[in]  InputCell           Buffer which stores cell information of the encoded bitmap.
 @param[out] GlyphBuffer         Output the corresponding bitmap data of the found
                                 block. It is the caller's responsiblity to free this buffer.
 @param[out] Cell                Output cell information of the encoded bitmap.
 @param[out] GlyphBufferLen      If not NULL, output the length of GlyphBuffer.
...
/**
 ...
 @param[in]  FontPackage         Hii string package instance.
 @param[in]  CharValue           Unicode character value, which identifies a glyph block.
 @param[out] GlyphBuffer         Output the corresponding bitmap data of the found
                                 block. It is the caller's responsiblity to free this buffer.
 @param[out] Cell                Output cell information of the encoded bitmap.
 ...
 @retval EFI_NOT_FOUND           The specified CharValue does not exist in current database.
 @retval EFI_OUT_OF_RESOURCES    The system is out of resources to accomplish the task.
**/
EFI_STATUS
FindGlyphBlock (
  ...
  )
{
  ...
  if (IsTtfFontPackage (FontPackage->FontPkgHdr->FontFamily)) {
    //
    // not yet support in TTF font package
    // Only do GetFontInfo check
    //
    ASSERT (GlyphBuffer == NULL && Cell == NULL && GlyphBufferLen == NULL);
    return TRUE;
  }
...
/**
 ...
 @param[in]  FontName            NULL-terminated string.
 @param[out] FontInfo            a new EFI_FONT_INFO which stores the FontName.
  ...
  )
{
  ...
  *FontInfo = (EFI_FONT_INFO *) AllocateZeroPool (FontInfoLen);
  ...
  StrCpyS ((*FontInfo)->FontName, StrLen(FontName) + 1, FontName);
  return EFI_SUCCESS;
}
...
/**
 ...
 @param[in]  Private             HII database driver private data.
 @param[out] FontInfo            Points to system default font output-related
                                 information. It's caller's responsibility to free this buffer.
 @param[out] FontInfoSize        If not NULL, output the size of buffer FontInfo.
  ...
  )
{
  ...
  StrCpyS (Info->FontInfo.FontName, sizeof(L"sysdefault") / sizeof(CHAR16), L"sysdefault");
...
/**
 ...
 @param[out] SystemInfo          If not NULL, points to system default font and color.

 @param[out] SystemInfoLen       If not NULL, output the length of default system info.
  ...
  )
{
  ...
  if (StringInfo != NULL && IsTtfFontPackage (StringInfo->FontInfo.FontName)) {
    return FALSE;
  }
...
/**
 ...
 @param[in]  FontInfo            Points to EFI_FONT_INFO structure.
 @param[in]  FontInfoMask        If not NULL, describes what options can be used
                                 to make a match between the font requested and
                                 the font available. The caller must guarantee
                                 this mask is valid.
 @param[in]  FontHandle          On entry, Points to the font handle returned by a
                                 previous  call to GetFontInfo() or NULL to start
                                 with the first font.
 @param[out] GlobalFontInfo      If not NULL, output the corresponding globa font info.
  ...
  )
{
  ...
  for (; Link != &Private->FontInfoList; Link = Link->ForwardLink) {
    ...
    if (IsTtfFontPackage (FontInfo->FontName))  {
      if (StrCmp (GlobalFont->FontInfo->FontName, L"ttf") == 0) {
...
/**
 ...
 @param[in] Char                 Unicode character
...
/**
 ...
 @param[in]   This                  A pointer to the EFI_HII_FONT_PROTOCOL instance.
 @param[in]   Flags                 Describes how the string is to be drawn.
 @param[in]   String                Points to the null-terminated string to be displayed.
 @param[in]   StringInfo            Points to the string output information,
                                    including the color and font.  If NULL, then the
                                    string will be output in the default system font
                                    and color.
 @param[in, out] Blt                If this points to a non-NULL on entry, this
 ...
 @param[in]      BltX               Specifies the offset from the left and top edge
                                    of the image of the first character cell in the
                                    image.
 @param[in]      BltY               Specifies the offset from the left and top edge
                                    of the image of the first character cell in the image.
 @param[out]     RowInfoArray       If this is non-NULL on entry, then on exit, this
                                    ...
                                    this buffer.
 @param[out]     RowInfoArraySize   If this is non-NULL on entry, then on exit it
                                    contains the number of elements in RowInfoArray.
 @param[out]     ColumnInfoArray    If this is non-NULL, then on return it will be
  ...
  )
{
  ...
  if (*Blt == NULL) {
    ...
    Image->Width  = 800;
    Image->Height = 600;
    Image->Image.Bitmap = AllocateZeroPool ((UINTN) Image->Width * Image->Height *sizeof (EFI_GRAPHICS_OUTPUT_BLT_PIXEL));
  ...
  if (GlyphBuf == NULL || Cell == NULL || Attributes == NULL) {
  ...
  if (SysFontFlag) {
    ...
    if (Status == EFI_NOT_FOUND) {
      ...
      if (IsTtfFontPackage (FontInfo->FontName)) {
        Height   = FontInfo->FontSize;
        BaseLine = FontInfo->FontSize;
      }
    } else {
      goto Exit;
    }
  ...
  while (*StringPtr != 0 && Index < StrLength) {
    ...
    if (Status == EFI_NOT_FOUND) {
      if ((Flags & EFI_HII_IGNORE_IF_NO_GLYPH) == EFI_HII_IGNORE_IF_NO_GLYPH) {
        ...
        //
        Status = GetReplaceCharGlyphBuffer (
                   *StringPtr,
                   FontInfo,
                   &GlyphBuf[Index],
                   &Cell[Index],
    ...
    if ((Attributes[Index] & TTF_GLYPH) == TTF_GLYPH) {
      Attributes[Index] |= TTF_RENDER_GLYPH;
    }
...
/**
 ...
 @param[in]      This                A pointer to the EFI_HII_FONT_PROTOCOL instance.
 @param[in]      Flags               Describes how the string is to be drawn.
 @param[in]      PackageList         The package list in the HII database to search
  ...
  )
{
  ...
  if (StringFontInfo != NULL && IsSystemFontInfo (Private, (EFI_FONT_DISPLAY_INFO *) StringInfo, NULL, NULL)) {
    ...
    StrCpyS (NewStringInfo->FontInfo.FontName, StrSize (StringFontInfo->FontName) / sizeof(CHAR16), StringFontInfo->FontName);
...
/**
 ...
 @param[in]  StringInfo          Points to the string font and color information
                                 or NULL if the string should use the default
                                 system font and color.
 @param[out] Blt                 Thus must point to a NULL on entry. A buffer will
                                 be allocated to hold the output and the pointer
                                 updated on exit. It is the caller's
                                 responsibility to free this buffer.
 @param[out] Baseline            Number of pixels from the bottom of the bitmap to
                                 the baseline.
  ...
  )
{
  ...
  ASSERT (FontInfo != NULL);
  if (EFI_ERROR (Status) || FontInfo == NULL) {
    goto Exit;
  }
  ...
  if (IsFontInfoExisted (Private, FontInfo, &InfoOut.FontInfoMask, LocalFontHandle, &GlobalFont)) {
    ...
    if (StringInfoOut != NULL) {
      ...
      *StringInfoOut   = (EFI_FONT_DISPLAY_INFO *) AllocateZeroPool (StringInfoOutLen);
      ...
      if (IsTtfFontPackage (FontInfo->FontName))  {
        (*StringInfoOut)->FontInfo.FontSize  = InfoOut.FontInfo.FontSize;
        (*StringInfoOut)->FontInfo.FontStyle = InfoOut.FontInfo.FontStyle;
      }
    }
...
/**
 According to font name to determine the package is weather true type font package.

 @param[in] FontName    A pointer to a Null-terminated Unicode string.

 @retval TRUE           Input font handle is true type font handle.
 @retval FALSE          Input font handle isn't true type font handle.
**/
BOOLEAN
IsTtfFontPackage (
  IN  CONST CHAR16      *FontName
  )
{
  if (FontName == NULL) {
    return FALSE;
...
/**
 Get True type decoder protocol.

 @retval TRUE_TYPE_DECODER_PROTOCOL *     Pointer to TRUE_TYPE_DECODER_PROTOCOL instance.
 @retval NULL                             Cannot find true type decoder protocol.
**/
TRUE_TYPE_DECODER_PROTOCOL *
GetTrueTypeDecoderProtocol (
  VOID
  )
{
  STATIC TRUE_TYPE_DECODER_PROTOCOL       *TrueTypeDecoder = NULL;
  EFI_STATUS                               Status;

  if (TrueTypeDecoder != NULL) {
    return TrueTypeDecoder;
  }

  Status = gBS->LocateProtocol (
                  &gTrueTypeDecoderProtocolGuid,
                  NULL,
                  (VOID**) &TrueTypeDecoder
                  );
  if (EFI_ERROR (Status)) {
    TrueTypeDecoder = NULL;
  }
