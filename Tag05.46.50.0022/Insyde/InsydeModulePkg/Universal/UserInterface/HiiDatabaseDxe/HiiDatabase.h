...
#define MAX_CONFIG_STRING_LENGTH           0x40000
...
#define BITMAP_LEN_1_BIT(Width, Height)  (((UINTN)(Width) + 7) / 8 * (Height))
#define BITMAP_LEN_4_BIT(Width, Height)  (((UINTN)(Width) + 1) / 2 * (Height))
#define BITMAP_LEN_8_BIT(Width, Height)  ((UINTN)(Width) * (Height))
#define BITMAP_LEN_24_BIT(Width, Height) ((UINTN)(Width) * (Height) * 3)
...
#define SUGGEST_STRING_BLOCK_BUFFER_SIZE(CurrentSize)  DivU64x32 ((MultU64x32 (CurrentSize, 6)), 5)

typedef struct {
  UINT8                                 *StringBlockAddr;
  UINTN                                 StringTextOffset;
} HII_STRING_BLOCK_INFO;
...
typedef struct _HII_STRING_PACKAGE_INSTANCE {
  ...
  UINT8                                 *StringBlock;
  UINTN                                 StringBlockSize;
  HII_STRING_BLOCK_INFO                 *StringBlockInfo;
  UINTN                                 StringBlockInfoCount;
...
typedef struct _HII_ANIMATION_PACKAGE_INSTANCE {
  EFI_HII_ANIMATION_PACKAGE_HDR         AnimationPkgHdr;
  UINT32                                AnimationBlockSize;
  UINT8                                 *AnimationBlock;
} HII_ANIMATION_PACKAGE_INSTANCE;
//
...
typedef struct _HII_DATABASE_PACKAGE_LIST_INSTANCE {
  ...
  UINT8                                 *DevicePathPkg;
  HII_ANIMATION_PACKAGE_INSTANCE        *AnimationPkg;
...
//  Type of table entry that used by ADT H2O_HII_BOARD_IMAGE_BLOCK_LIST.
//
#define H2O_HII_IMAGE_TABLE_ENTRY_SIGNATURE       SIGNATURE_32 ('h','b','i','e')

typedef struct _H2O_HII_IMAGE_TABLE_ENTRY {
  UINT32                Signature;
  INT32                 ImageBlockListNodeCount;
  LIST_ENTRY            *AssociateImageBlockList;
  LIST_ENTRY            TableEntry;
  H2O_BOARD_ID          ThisBoard;
  BOOLEAN               IsSlaveEntry;  //  Some entries might co-use same "AssociateImageBlockList". Only one entry is the master, the others are slaves within these group. (Exsists only when two or more boards co-use same image package.)
} H2O_HII_IMAGE_TABLE_ENTRY;

//
//  Insyde multi board image package extension.
//  Type of node that used by ADT H2O_HII_BOARD_IMAGE_BLOCK_LIST.
//
#define H2O_HII_IMAGE_BLOCK_NODE_SIGNATURE       SIGNATURE_32 ('h','i','i','n')

typedef struct _H2O_HII_IMAGE_BLOCK_NODE {
  UINT32                Signature;
  UINT32                ImageBlockSize;
  UINT8                 ImageBlockType;
  UINT8                 *ImageBlock;
  UINT8                 *ImageInstance;
  EFI_IMAGE_ID          VirtualImageId;
  LIST_ENTRY            ImageBlockNodeEntry;
} H2O_HII_IMAGE_BLOCK_NODE;
...
//  Insyde multi board image package extension.
//
//  For retrieving H2O_HII_IMAGE_TABLE_ENTRY and H2O_HII_IMAGE_BLOCK_NODE.
//
#define GET_H2O_HII_IMAGE_TABLE_ENTRY_FROM_THIS(a) \
  CR (a,\
      H2O_HII_IMAGE_TABLE_ENTRY, \
      TableEntry, \
      H2O_HII_IMAGE_TABLE_ENTRY_SIGNATURE \
      )

#define GET_H2O_HII_IMAGE_BLOCK_NODE_FROM_THIS(a) \
  CR (a,\
      H2O_HII_IMAGE_BLOCK_NODE, \
      ImageBlockNodeEntry, \
      H2O_HII_IMAGE_BLOCK_NODE_SIGNATURE \
      )
//
...
/**
 ...
 @retval     EFI_UNSUPPORTED        Input source is not a multi image package
                                    ...
                                    Some error occurred in any method of
                                    H2O_HII_BOARD_IMAGE_BLOCK_LIST.
**/
EFI_STATUS
