...
/**
 ...
 @param[in]  StringPackage      HII string package instance.
 @param[in]  FontId             Font identifer, which must be unique within the string package.
 @param[in]  DuplicateEnable    If true, duplicate HII_FONT_INFO which refers to
                                the same EFI_FONT_INFO is permitted. Otherwise it
                                is not allowed.
 @param[in]  GlobalFontInfo     Input a global font info which specify a
                                EFI_FONT_INFO.
 @param[out] LocalFontInfo      Output a local font info which refers to a
                                EFI_FONT_INFO.
...
/**
 ...
 @param[out]     StringDest     Buffer to store the string text. If it is NULL,
                                only the size will be returned.
 @param[in]      StringSrc      Points to current null-terminated string.
 @param[in, out] BufferSize     Length of the buffer.
 ...
 @retval EFI_BUFFER_TOO_SMALL   Buffer is insufficient to store the found string
                                text. BufferSize is updated to the required buffer size.
**/
EFI_STATUS
...
/**
 ...
 @param[in]  StringPackage      Hii string package instance.
 @param[in]  FontId             Font identifier which is unique in a string
                                package.
 @param[out] StringFontInfo     Buffer to record the output font info. It's
...
/**
 Sync new string block address into string block information which string block address of each string ID will be updated.
...
**/
EFI_STATUS
SyncNewStrBlkAddress (
  IN HII_STRING_PACKAGE_INSTANCE     *StringPackage,
  IN UINT8                           *NewStringBlock
  )
{
  UINTN                              Index;
  UINT8                              *NewStringBlockAddr;

  if (StringPackage == NULL || NewStringBlock == NULL) {
    return EFI_INVALID_PARAMETER;
  }

  for (Index = 0; Index < StringPackage->StringBlockInfoCount; Index++) {
    if (StringPackage->StringBlockInfo[Index].StringBlockAddr == 0) {
      continue;
    }

    NewStringBlockAddr = NewStringBlock + (StringPackage->StringBlockInfo[Index].StringBlockAddr - StringPackage->StringBlock);
    StringPackage->StringBlockInfo[Index].StringBlockAddr = NewStringBlockAddr;
  }
  ...
  )
{
  ...
  if (NewStrSize >= OldStrSize) {
    Increase   = TRUE;
    Difference = NewStrSize - OldStrSize;
  } else {
    Increase   = FALSE;
    Difference = OldStrSize - NewStrSize;
  }

  for (Index = StringId - 1; Index < StringPackage->StringBlockInfoCount; Index++) {
    if (StringPackage->StringBlockInfo[Index].StringBlockAddr == 0) {
      continue;
    }

    if (Increase) {
      StringPackage->StringBlockInfo[Index].StringBlockAddr += Difference;
    } else {
      StringPackage->StringBlockInfo[Index].StringBlockAddr -= Difference;
    }
  }
...
UpdateStrBlkInfo (
  IN HII_STRING_PACKAGE_INSTANCE     *StringPackage,
  IN EFI_STRING_ID                   StringId,
  IN UINT8                           *StringBlockAddr,
  IN UINTN                           StringTextOffset
  )
{
  UINT8                              *Buffer;
  UINTN                              Increment;

  if (StringPackage == NULL || StringBlockAddr == NULL ||
      StringId == 0 || StringId == (EFI_STRING_ID) (-1)) {
    return EFI_INVALID_PARAMETER;
  }

  while (StringId > StringPackage->StringBlockInfoCount) {
    Increment = 0x100;
    Buffer = (UINT8 *) AllocateZeroPool (sizeof (HII_STRING_BLOCK_INFO) * (StringPackage->StringBlockInfoCount + Increment));
    if (Buffer == NULL) {
      return EFI_OUT_OF_RESOURCES;
    }
    ...
    CopyMem (
      Buffer,
      StringPackage->StringBlockInfo,
      sizeof (HII_STRING_BLOCK_INFO) * StringPackage->StringBlockInfoCount
      );
    FreePool (StringPackage->StringBlockInfo);

    StringPackage->StringBlockInfo       = (HII_STRING_BLOCK_INFO *) Buffer;
    StringPackage->StringBlockInfoCount += Increment;
  }

  StringPackage->StringBlockInfo[StringId - 1].StringBlockAddr  = StringBlockAddr;
  StringPackage->StringBlockInfo[StringId - 1].StringTextOffset = StringTextOffset;
...
/**
 ...
 @param[out]  BlockType          Output the block type of found string block.
 @param[out]  StringBlockAddr    Output the block address of found string block.
 @param[out]  StringTextOffset   Offset, relative to the found block address, of
                                 the string text information.
 @param[out]  LastStringId       Output the last string id when StringId = 0 or StringId = -1.
 @param[out]  StartStringId      The first id in the skip block which StringId in the block.

 @retval EFI_SUCCESS             The string text and font is retrieved successfully.
 @retval EFI_NOT_FOUND           The specified text or font info can not be found out.
  ...
  )
{
  ...
  UINTN                                StringSize;
  CHAR16                               Zero;
  UINT8                                BackupBlockType;
  UINT8                                *BackupStringBlockAddr;
  UINTN                                BackupStringTextOffset;
  EFI_STRING_ID                        BackupStartStringId;
  ...
  if (StringId != (EFI_STRING_ID) (-1) && StringId != 0) {
    ...
    if (StringId > StringPackage->StringBlockInfoCount) {
      return EFI_NOT_FOUND;
    }

    if (StringPackage->StringBlockInfo[StringId - 1].StringBlockAddr == NULL) {
      return EFI_NOT_FOUND;
    }

    BackupBlockType        = *(StringPackage->StringBlockInfo[StringId - 1].StringBlockAddr);
    BackupStringBlockAddr  = StringPackage->StringBlockInfo[StringId - 1].StringBlockAddr;
    BackupStringTextOffset = StringPackage->StringBlockInfo[StringId - 1].StringTextOffset;
    BackupStartStringId    = 0;
    if (BackupBlockType == EFI_HII_SIBT_SKIP1 || BackupBlockType == EFI_HII_SIBT_SKIP2) {
      BackupStartStringId = StringId - 1;

      while (BackupStartStringId > 0 &&
             StringPackage->StringBlockInfo[BackupStartStringId].StringBlockAddr ==
             StringPackage->StringBlockInfo[StringId - 1].StringBlockAddr) {
        BackupStartStringId--;
      }

      BackupStartStringId += 2;
    }

    *BlockType        = BackupBlockType;
    *StringBlockAddr  = BackupStringBlockAddr;
    *StringTextOffset = BackupStringTextOffset;

    if (StartStringId != NULL) {
      *StartStringId  = BackupStartStringId;
    }
  ...
  while (*BlockHdr != EFI_HII_SIBT_END) {
    ...
    case EFI_HII_SIBT_STRING_SCSU:
      ...
      UpdateStrBlkInfo (StringPackage, CurrentStringId, BlockHdr, Offset);
    ...
    case EFI_HII_SIBT_STRINGS_SCSU:
      ...
      for (Index = 0; Index < StringCount; Index++) {
        ...
        UpdateStrBlkInfo (StringPackage, CurrentStringId, BlockHdr, (StringTextPtr - BlockHdr));
    ...
    case EFI_HII_SIBT_SKIP1:
      SkipCount = (UINT16)(UINT8)(*(BlockHdr + sizeof (EFI_HII_STRING_BLOCK)));
//    CurrentStringId = (UINT16) (CurrentStringId + SkipCount);
      while (SkipCount > 0) {
        UpdateStrBlkInfo (StringPackage, CurrentStringId, BlockHdr, 0);
        CurrentStringId++;
        SkipCount--;
      }
...
/**
 ...
 @param[in]      StringPackage    Hii string package instance.
 @param[in]      StringId         The string's id, which is unique within PackageList.
 @param[out]     String           Points to retrieved null-terminated string.
 @param[in, out] StringSize       On entry, points to the size of the buffer pointed
                                  to by String, in bytes. On return, points to the
                                  length of the string, in bytes.
 @param[out]     StringFontInfo   If not NULL, allocate a buffer to record the
                                  output font info. It's caller's responsibility to
                                  free this buffer.

 @retval EFI_SUCCESS            The string text and font is retrieved successfully.
 @retval EFI_NOT_FOUND          The specified text or font info can not be found out.
 @retval EFI_BUFFER_TOO_SMALL   The buffer specified by StringSize is too small to hold the string.
**/
EFI_STATUS
GetStringWorker (
  IN  HII_DATABASE_PRIVATE_DATA        *Private,
...
/**
 ...
 @param[in, out]  StringPackage           Hii string package instance.
 @param[in]       StartStringId           The first id in the skip block which StringId in the block.
 @param[in]       StringId                The string's id, which is unique within PackageList.
 @param[in, out]  BlockType               Output the block type of found string block.
 @param[in, out]  StringBlockAddr         Output the block address of found string block.
 @param[in]       FontBlock               whether this string block has font info.
  ...
  )
{
  ...
  if (StringPackage->StringBlockSize < NewBlockSize) {
    StringPackage->StringBlockSize = (UINTN) SUGGEST_STRING_BLOCK_BUFFER_SIZE(NewBlockSize);
    ...
    TempBuffer = AllocateCopyPool (
                   OldBlockSize - (OldStringAddr - StringPackage->StringBlock) - SkipLen,
  ...
  //
  SetMem (BlockPtr, NewUCSBlockLen, 0);
  *StringBlockAddr = BlockPtr;
  ...
  //
  if (StringBlock != StringPackage->StringBlock) {
    CopyMem (BlockPtr, OldStringAddr + SkipLen, OldBlockSize - (OldStringAddr - StringPackage->StringBlock) - SkipLen);
  } else {
    if (TempBuffer != NULL) {
      CopyMem (BlockPtr, TempBuffer, OldBlockSize - (OldStringAddr - StringPackage->StringBlock) - SkipLen);
      FreePool (TempBuffer);
    }
  }
...
/**
 ...
 @param[in]      StringFontInfo         Points to the input font info.
 ...
 @retval EFI_INVALID_PARAMETER  The specified StringFontInfo does not exist in current database.
 @retval EFI_OUT_OF_RESOURCES   The system is out of resources to accomplish the task.
**/
EFI_STATUS
SetStringWorker (
  ...
  )
{
  ...
  if (EFI_ERROR (Status) && (BlockType == EFI_HII_SIBT_SKIP1 || BlockType == EFI_HII_SIBT_SKIP2)) {
    ...
    FindStringBlock (Private, StringPackage, (EFI_STRING_ID) (-1), NULL, NULL, NULL, NULL, NULL);
  ...
  case EFI_HII_SIBT_STRINGS_SCSU_FONT:
    ...
    BlockSize = OldBlockSize + StringLength;
//  BlockSize -= AsciiStrSize ((CHAR8 *) StringTextPtr);
    ...
    if (StringPackage->StringBlockSize < BlockSize) {
      StringPackage->StringBlockSize = (UINTN) SUGGEST_STRING_BLOCK_BUFFER_SIZE (BlockSize);
      Block = AllocateZeroPool (StringPackage->StringBlockSize);
      if (Block == NULL) {
        return EFI_OUT_OF_RESOURCES;
      }

      CopyMem (Block, StringPackage->StringBlock, StringTextPtr - StringPackage->StringBlock);
      SyncNewStrBlkAddress (StringPackage, Block);
    } else {
      Block = StringPackage->StringBlock;

      TempBuffer = AllocateCopyPool (
                     OldBlockSize - (StringTextPtr - StringPackage->StringBlock) - AsciiStrSize ((CHAR8 *) StringTextPtr),
    ...
    SyncStrBlkInfoWhenSetStr (StringPackage, StringId + 1, StringLength + 1, AsciiStrSize ((CHAR8 *) StringTextPtr));
    ...
    *BlockPtr++ = 0;
    ...
    if (StringPackage->StringBlock != Block) {
      CopyMem (
        BlockPtr,
        StringTextPtr + AsciiStrSize ((CHAR8 *)StringTextPtr),
        TmpSize
  ...
  case EFI_HII_SIBT_STRINGS_UCS2_FONT:
    ...
    StringSize = 0;
    GetUnicodeStringTextOrSize (NULL, StringTextPtr, &StringSize);
    InputStringSize = StrSize (String);
    BlockSize = OldBlockSize + InputStringSize - StringSize;
    ...
    CopyMem (BlockPtr, String, InputStringSize);
    BlockPtr += InputStringSize;

    SyncStrBlkInfoWhenSetStr (StringPackage, StringId + 1, InputStringSize, StringSize);
...
/**
 ...
 @param[in]  This               A pointer to the EFI_HII_STRING_PROTOCOL instance.
 @param[in]  PackageList        Handle of the package list where this string will be added.
 @param[out] StringId           On return, contains the new strings id, which is
                                unique within PackageList.
 @param[in]  Language           Points to the language for the new string.
 @param[in]  LanguageName       Points to the printable language name to associate
                                ...
                                be ignored.
 @param[in]  String             Points to the new null-terminated string.
 @param[in]  StringFontInfo     Points to the new string's font information or
  ...
  )
{
  ...
  for (Link = PackageListNode->StringPkgHdr.ForwardLink;
    ...
    if ((NextStringId + 1) == (EFI_STRING_ID) (-1)) {
    ...
    if (HiiCompareLanguage (StringPackage->StringPkgHdr->Language, (CHAR8 *) Language)) {
      ...
      if (StringPackage->StringBlockSize < (OldBlockSize + Ucs2BlockSize)) {
        StringPackage->StringBlockSize = (UINTN) SUGGEST_STRING_BLOCK_BUFFER_SIZE (OldBlockSize + Ucs2BlockSize);
        StringBlock = (UINT8 *) AllocateZeroPool (StringPackage->StringBlockSize);
        if (StringBlock == NULL) {
          Status = EFI_OUT_OF_RESOURCES;
          goto Done;
        ...
        SyncNewStrBlkAddress (StringPackage, StringBlock);
      } else {
        StringBlock = StringPackage->StringBlock;
      }
      //
      ...
      if (NextStringId != 0) {
        UpdateStrBlkInfo (StringPackage, NextStringId + 1, BlockPtr, sizeof (EFI_HII_STRING_BLOCK));
      }
      SetMem (BlockPtr, sizeof (EFI_HII_SIBT_STRING_UCS2_BLOCK), 0);
      *BlockPtr = EFI_HII_SIBT_STRING_UCS2;
  ...
  if (MatchStringPackage != NULL) {
    ...
    AsciiStrCpyS (StringPackage->StringPkgHdr->Language, AsciiStrSize ((CHAR8 *) Language), (CHAR8 *) Language);
    ...
    StringPackage->StringBlockInfoCount = 0x100;
    ...
    UpdateStrBlkInfo (StringPackage, 1, BlockPtr, sizeof (EFI_HII_STRING_BLOCK));
    *BlockPtr = EFI_HII_SIBT_STRING_UCS2;
    ...
    for (Index = 2; Index <= *StringId - 1; Index ++) {
      UpdateStrBlkInfo (StringPackage, Index, BlockPtr, sizeof (EFI_HII_STRING_BLOCK));
      *BlockPtr = EFI_HII_SIBT_STRING_UCS2;
      BlockPtr += sizeof (EFI_HII_SIBT_STRING_UCS2_BLOCK);
    }
    //
  ...
  if (StringFontInfo == NULL) {
    ...
    //
    Ucs2BlockSize = (UINT32) (StringSize + sizeof (EFI_HII_SIBT_STRING_UCS2_BLOCK)
                              - sizeof (CHAR16));
    ...
    //
    BlockPtr  = StringBlock + OldBlockSize - sizeof (EFI_HII_SIBT_END_BLOCK);
    UpdateStrBlkInfo (StringPackage, *StringId, BlockPtr, sizeof (EFI_HII_STRING_BLOCK));
    SetMem (BlockPtr, sizeof (EFI_HII_STRING_BLOCK), 0);
    *BlockPtr = EFI_HII_SIBT_STRING_UCS2;
    BlockPtr  += sizeof (EFI_HII_STRING_BLOCK);
    CopyMem (BlockPtr, (EFI_STRING) String, StringSize);
    BlockPtr += StringSize;
    ...
    *BlockPtr = EFI_HII_SIBT_END;
    ...
    // _UCS2_FONT block.
    //
    Ucs2FontBlockSize = (UINT32) (StringSize + sizeof (EFI_HII_SIBT_STRING_UCS2_FONT_BLOCK) -
                                  sizeof (CHAR16));
    if (ReferFontInfoLocally (Private, StringPackage, StringPackage->FontId, FALSE, GlobalFont, &LocalFont)) {
      ...
      //
      if (StringPackage->StringBlockSize < (OldBlockSize + Ucs2FontBlockSize)) {
        StringPackage->StringBlockSize = (UINTN) SUGGEST_STRING_BLOCK_BUFFER_SIZE (OldBlockSize + Ucs2FontBlockSize);
      ...
      //
      BlockPtr  = StringBlock + OldBlockSize - sizeof (EFI_HII_SIBT_END_BLOCK);
      UpdateStrBlkInfo (StringPackage, *StringId, BlockPtr, sizeof (EFI_HII_STRING_BLOCK) + sizeof (UINT8));
      SetMem (BlockPtr, sizeof (EFI_HII_STRING_BLOCK), 0);
      *BlockPtr = EFI_HII_SIBT_STRING_UCS2_FONT;
      BlockPtr  += sizeof (EFI_HII_STRING_BLOCK);
      *BlockPtr = LocalFont->FontId;
      ...
      *BlockPtr = EFI_HII_SIBT_END;
      ...
      if (StringPackage->StringBlockSize < (OldBlockSize + FontBlockSize + Ucs2FontBlockSize)) {
        StringPackage->StringBlockSize = (UINTN) SUGGEST_STRING_BLOCK_BUFFER_SIZE (OldBlockSize + FontBlockSize + Ucs2FontBlockSize);
        StringBlock = (UINT8 *) AllocateZeroPool (StringPackage->StringBlockSize);
        if (StringBlock == NULL) {
          Status = EFI_OUT_OF_RESOURCES;
          goto Done;
...
/**
 ...
 @param[in]      This           A pointer to the EFI_HII_STRING_PROTOCOL instance.
 @param[in]      Language       Points to the language for the retrieved string.
 ...
 @param[in, out] StringSize     On entry, points to the size of the buffer pointed
                                to by  String, in bytes. On return, points to the
                                length of the string, in bytes.
 @param[out]     StringFontInfo If not NULL, points to the string's font
                                information.  It's caller's responsibility to free this buffer.

 @retval EFI_SUCCESS            The string was returned successfully.
 @retval EFI_NOT_FOUND          The string specified by StringId is not available.
 @retval EFI_NOT_FOUND          The string specified by StringId is available but not in the specified language.
...
/**
 ...
 @param[in] StringId            The string's id, which is unique within PackageList.
 @param[in] Language            Points to the language for the updated string.
 @param[in] String              Points to the new null-terminated string.
 @param[in] StringFontInfo      Points to the string's font information or NULL if
...
/**
 ...
 @param[in]      This           A pointer to the EFI_HII_STRING_PROTOCOL instance.
 @param[in]      PackageList    The package list to examine.
 @param[in, out] Languages      Points to the buffer to hold the returned
                                null-terminated ASCII string.
 @param[in, out] LanguagesSize  On entry, points to the size of the buffer pointed
 ...
 @retval EFI_NOT_FOUND          Could not find string package in specified packagelist.
**/
EFI_STATUS
EFIAPI
HiiGetLanguages (
  ...
  )
{
  ...
  for (Link = PackageListNode->StringPkgHdr.ForwardLink;
    ...
    if (ResultSize <= *LanguagesSize) {
      AsciiStrCpyS (Languages, *LanguagesSize, StringPackage->StringPkgHdr->Language);
...
/**
 ...
 @param[in]      This                   A pointer to the EFI_HII_STRING_PROTOCOL instance.
 @param[in]      PackageList            The package list to examine.
 @param[in]      PrimaryLanguage        Points to the null-terminated ASCII string that specifies
                                        the primary language. Languages are specified in the
                                        format specified in Appendix M of the UEFI 2.0 specification.
 @param[in, out] SecondaryLanguages     Points to the buffer to hold the returned null-terminated
                                        ...
                                        this is set to NULL.
 @param[in, out] SecondaryLanguagesSize On entry, points to the size of the buffer pointed
  ...
  for (Link = Private->DatabaseList.ForwardLink; Link != &Private->DatabaseList; Link = Link->ForwardLink) {
    ...
    if (HiiCompareLanguage (StringPackage->StringPkgHdr->Language, (CHAR8 *) PrimaryLanguage)) {
      ...
      if (ResultSize <= *SecondaryLanguagesSize) {
        AsciiStrCpyS (SecondaryLanguages, *SecondaryLanguagesSize, Languages);
      } else {
...
/**
 ...
 @param[in] Language1           Name of language 1 from StringPackage
 @param[in] Language2           Name of language 2 to be compared with language 1.
  ...
  )
{
  ...
  //
  //
  // Only get the length of Primary Language because only compare the Primary Language in Language1 to Language2
  //
  StrLen = 0;
  while (Language1[StrLen] != 0 && Language1[StrLen] != ';') {
    ++StrLen;
  }
  ++StrLen;

  if (StrLen < sizeof (Lan1Buffer)) {
    ZeroMem (Lan1Buffer, sizeof (Lan1Buffer));
    Lan1 = Lan1Buffer;
  } else {
  ...
  CopyMem (Lan1, Language1, StrLen);
  AsciiHiiToLower (Lan1);

  StrLen = AsciiStrSize (Language2);
  if (StrLen < sizeof (Lan2Buffer)) {
    ZeroMem (Lan2Buffer, sizeof (Lan2Buffer));
    Lan2 = Lan2Buffer;
  } else {
  ...
  for (Index = 0; Lan1[Index] != 0 && Lan1[Index] != ';'; Index++) {
    if (Lan1[Index] != Lan2[Index]) {
      ...
      //
      if (Lan1 != Lan1Buffer) FreePool (Lan1);
      if (Lan2 != Lan2Buffer) FreePool (Lan2);
      return FALSE;
    }
  }
