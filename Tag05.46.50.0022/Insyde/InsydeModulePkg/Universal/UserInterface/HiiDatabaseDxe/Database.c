...
/**
 ...
 @param[in]  Private            hii database private structure
 @param[out] DatabaseNode       HII_DATABASE_RECORD node which is used to store a
                                package list
...
/**
 ...
 @param[in] Handle              Pointer to a EFI_HII_HANDLE
...
/**
 ...
 @param[in] Private             HII Database driver private structure.
 @param[in] NotifyType          The type of change concerning the database.
 @param[in] PackageInstance     Points to the package referred to by the
                                notification.
 @param[in] PackageType         Package type
 @param[in] Handle              The handle of the package list which contains the
                                specified package.
  ...
  )
{
  ...
  //
  // Check if these is a matching registered notify type and register type.
  //
  ...
  if (Link == &Private->DatabaseNotifyList) {
    return EFI_SUCCESS;
  }
  ...
  case EFI_HII_PACKAGE_ANIMATIONS:
    BufferSize = ((HII_ANIMATION_PACKAGE_INSTANCE *) PackageInstance)->AnimationPkgHdr.Header.Length;
    HeaderSize = sizeof (EFI_HII_ANIMATION_PACKAGE_HDR);
    Buffer = (UINT8 *) AllocateZeroPool (BufferSize);
    ASSERT (Buffer != NULL);
    if (Buffer == NULL) {
      return EFI_OUT_OF_RESOURCES;
    ...
    CopyMem (
      Buffer,
      &((HII_ANIMATION_PACKAGE_INSTANCE *) PackageInstance)->AnimationPkgHdr,
      HeaderSize
      );
    CopyMem (
      Buffer + sizeof (EFI_HII_PACKAGE_HEADER),
    ...
    AnimationBlockSize = ((HII_ANIMATION_PACKAGE_INSTANCE *) PackageInstance)->AnimationBlockSize;
    if (AnimationBlockSize != 0) {
      CopyMem (
        Buffer + HeaderSize,
        ((HII_ANIMATION_PACKAGE_INSTANCE *) PackageInstance)->AnimationBlock,
        AnimationBlockSize
        );
    }
    Package = (EFI_HII_PACKAGE_HEADER *) Buffer;
...
/**
 ...
 @param[in]      PackageHdr     Pointer to a buffer stored with GUID package information.
 @param[in]      NotifyType     The type of change concerning the database.
 @param[in, out] PackageList    Pointer to a package list which will be inserted to.
 @param[out]     Package        Created GUID pacakge
...
/**
 ...
 @param[in]      Private        Hii database private structure.
 @param[in]      Handle         Identification of a package list.
 @param[in]      PackageList    Pointer to a package list which will be exported.
 @param[in]      UsedSize       The length of buffer be used.
 @param[in]      BufferSize     Length of the Buffer.
 @param[in, out] Buffer         Allocated space for storing exported data.
 @param[in, out] ResultSize     The size of the already exported content of  this
                                package list.
...
/**
 ...
 @param[in]      Private        Hii database private data.
 @param[in]      Handle         Handle of the package list which contains the to
                                be  removed GUID packages.
 @param[in, out] PackageList    Pointer to a package list that contains removing
                                packages.
...
/**
 ...
 @param[in]      PackageHdr     Pointer to a buffer stored with Form package
                                information.
 @param[in]      NotifyType     The type of change concerning the database.
 @param[in, out] PackageList    Pointer to a package list which will be inserted to.
 @param[out]     Package        Created Form package
...
/**
 ...
 @param[in]      Private        Hii database private structure.
 @param[in]      PackageHdr     Pointer to a buffer stored with String package information.
 @param[in]      NotifyType     The type of change concerning the database.
 @param[in, out] PackageList    Pointer to a package list which will be inserted to.
 @param[out]     Package        Created String package
  ...
  )
{
  ...
  AsciiStrCpyS (Language, LanguageSize, (CHAR8 *) PackageHdr + HeaderSize - LanguageSize);
  ...
  StringPackage->StringBlockSize = (UINTN) SUGGEST_STRING_BLOCK_BUFFER_SIZE(PackageHeader.Length - HeaderSize);
  StringPackage->StringBlock = (UINT8 *) AllocateZeroPool (StringPackage->StringBlockSize);
  if (StringPackage->StringBlock == NULL) {
    Status = EFI_OUT_OF_RESOURCES;
    goto Error;
  }

  StringPackage->StringBlockInfoCount = 0x500;
  StringPackage->StringBlockInfo      = (HII_STRING_BLOCK_INFO *) AllocateZeroPool (sizeof (HII_STRING_BLOCK_INFO) * StringPackage->StringBlockInfoCount);
  if (StringPackage->StringBlockInfo == NULL) {
    Status = EFI_OUT_OF_RESOURCES;
    goto Error;
  }
...
Error:
  ...
  if (StringPackage != NULL) {
    ...
    if (StringPackage->StringBlockInfo != NULL) {
      FreePool (StringPackage->StringBlockInfo);
    }
    if (StringPackage->StringPkgHdr != NULL) {
...
/**
 ...
 @param[in, out] PackageList        Pointer to a package list which will be adjusted.
  ...
  )
{
  ...
  for (Link = PackageList->StringPkgHdr.ForwardLink;
    ...
    if (StringPackage->MaxStringId < MaxStringId) {
      ...
      if (StringPackage->StringBlockSize < (OldBlockSize + Skip2BlockSize)) {
        StringPackage->StringBlockSize = (UINTN) SUGGEST_STRING_BLOCK_BUFFER_SIZE (OldBlockSize + Skip2BlockSize);
        StringBlock = (UINT8 *) AllocateZeroPool (StringPackage->StringBlockSize);
        if (StringBlock == NULL) {
          return EFI_OUT_OF_RESOURCES;
        }
        //
      ...
      *BlockPtr = EFI_HII_SIBT_END;
      if (StringPackage->StringBlock != StringBlock) {
        FreePool (StringPackage->StringBlock);
...
/**
 ...
 @param[in, out] PackageList    Pointer to a package list that contains removing packages.
  ...
  )
{
  ...
  while (!IsListEmpty (ListHead)) {
    ...
    FreePool (Package->StringBlockInfo);
    //
...
/**
 Internal function to initialize true type font database

 @param[in] Buffer              TTF file buffer
 @param[in] BufferSize          input buffer size

 @retval EFI_SUCCESS            Initialize true type font database successfully.
 @retval EFI_NOT_FOUND          Cannot find true type decoder protocol.
 @retval other                  Initialize true type data failure caused by ture type decoder protcol.
**/
STATIC
EFI_STATUS
InitTtfDatabase (
...
{
  TRUE_TYPE_DECODER_PROTOCOL       *TrueTypeDecoder;

  if (Buffer == NULL || BufferSize == 0) {
    return EFI_INVALID_PARAMETER;
  }

  TrueTypeDecoder = GetTrueTypeDecoderProtocol ();
  if (TrueTypeDecoder == NULL) {
    return EFI_UNSUPPORTED;
  }
  return TrueTypeDecoder->SetDataBase (
                            TrueTypeDecoder,
                            Buffer,
                            BufferSize
                            );
...
/**
 ...
 @param[in]      Private        Hii database private structure.
 @param[in]      PackageHdr     Pointer to a buffer stored with Font package information.
 @param[in]      NotifyType     The type of change concerning the database.
 @param[in, out] PackageList    Pointer to a package list which will be inserted to.
 @param[out]     Package        Created Font package
  ...
  )
{
  ...
  StrCpyS (FontInfo->FontName, (HeaderSize - sizeof (EFI_HII_FONT_PACKAGE_HDR)) / sizeof(CHAR16) + 1, FontPkgHdr->FontFamily);
  ...
  *Package = FontPackage;
  ...
  if (IsTtfFontPackage (FontInfo->FontName)) {
    InitTtfDatabase (FontPackage->GlyphBlock, PackageHeader.Length - HeaderSize);
  }
...
/**
 ...
 @param[in]      PackageHdr     Pointer to a buffer stored with Image package information.
 @param[in]      NotifyType     The type of change concerning the database.
 @param[in, out] PackageList    Pointer to a package list which will be inserted to.
 @param[out]     Package        Created Image package
  ...
  )
{
  ...
  UINT16                            CurrentSize;
  EFI_STATUS                        Status;
  UINT8                             *SourcePkgDataPtr;
  UINT8                             *ParsedPackageData;
  UINT32                            ParsedPackageDataLength;
  UINT32                            OriginalImagePackageLength;

  ParsedPackageData = NULL;
  ParsedPackageDataLength = 0;
  ...
  if (ImageInfoOffset != 0) {
    //
    //  Check if this is a multi image package.
    //  If so, let board id decide which package data will be installed.
    //  Call ClassifyMultiBoardImageBlock() to get package data for the board we are running on.
    //
    //  Ex.  This package contains images for Default 00
    //                                        board   0x04
    //                                        board   0x0B
    //
    //       If we are running on board 0x0B, then this function return image package data for board 0x0B.
    //
    //  +-----------------+                                                +------+
    //  |     |     |     |  ----> ClassifyMultiBoardImageBlock ()  ---->  |      |
    //  +-----------------+                                                +------+
    //
    SourcePkgDataPtr = (UINT8 *)PackageHdr + ImageInfoOffset;
    Status = ClassifyMultiBoardImageBlock(
               SourcePkgDataPtr,
               (VOID **)&ParsedPackageData,
               (VOID *)&ParsedPackageDataLength
               );
    ASSERT_EFI_ERROR (Status);
    if (EFI_ERROR (Status)) {
      if (ImagePackage->PaletteBlock != NULL) {
    ...
    //
    //  Update header size.
    //  [Header 12byte] + [ParsedPackageDataLength + EFI_HII_IIBT_END 1byte] + [PaletteSize].
    //
    ImagePackage->ImagePkgHdr.Header.Length = sizeof (EFI_HII_IMAGE_PACKAGE_HDR)
                                              + ParsedPackageDataLength + sizeof (UINT8) + PaletteSize;
    //
    //  Update PaletteInfoOffset only if Palette exists in this package.
    //
    if (ImagePackage->ImagePkgHdr.PaletteInfoOffset != 0) {
      ImagePackage->ImagePkgHdr.PaletteInfoOffset = ImagePackage->ImagePkgHdr.ImageInfoOffset
                                              + ParsedPackageDataLength + sizeof (UINT8);
    }
    ...
    if (ImagePackage->ImageBlock == NULL) {
      ...
      if (ParsedPackageData != NULL) {
        FreePool (ParsedPackageData);
      }
      return EFI_OUT_OF_RESOURCES;
    }
    //
    //  Insyde multi image package extension.
    //  Install image package belongs to current board.
    //
    CopyMem (
      ImagePackage->ImageBlock,
      ParsedPackageData,
      ParsedPackageDataLength
      );
    //
    //  Append a end block.
    //
    *(ImagePackage->ImageBlock + ParsedPackageDataLength) = EFI_HII_IIBT_END;
    //
    //  Update package list length.
    //
    OriginalImagePackageLength = ((EFI_HII_IMAGE_PACKAGE_HDR *)PackageHdr)->Header.Length;
    PackageList->PackageListHdr.PackageLength -= OriginalImagePackageLength;
    PackageList->PackageListHdr.PackageLength += ImagePackage->ImagePkgHdr.Header.Length;

    //
    //  Insyde multi image package extension.
    //  Data installed in hii completelly, so now ParsedPackageData is no longer required,free it.
    //
    if (ParsedPackageData != NULL) {
      FreePool (ParsedPackageData);
    }
...
/**
 ...
 @param[in, out] PackageList    Package List which contains the to be  removed
                                Image package.
...
/**
 This function insert a animation package to a package list node.

 @param[in]      PackageHdr     Pointer to a buffer stored with animation package information.
 @param[in]      NotifyType     The type of change concerning the database.
 @param[in, out] PackageList    Pointer to a package list which will be inserted to.
 @param[out]     Package        Created animation package

 @retval EFI_SUCCESS            Animation Package is inserted successfully.
 @retval EFI_OUT_OF_RESOURCES   Unable to allocate necessary resources for the new animation package.
 @retval EFI_INVALID_PARAMETER  PackageHdr is NULL or PackageList is NULL.
 @retval EFI_UNSUPPORTED        A animtion package with the same language already exists
                                in current package list.
**/
STATIC
EFI_STATUS
InsertAnimationPackage (
  IN     VOID                               *PackageHdr,
  IN     EFI_HII_DATABASE_NOTIFY_TYPE       NotifyType,
  IN OUT HII_DATABASE_PACKAGE_LIST_INSTANCE *PackageList,
  OUT    HII_ANIMATION_PACKAGE_INSTANCE     **Package
  )
{
  HII_ANIMATION_PACKAGE_INSTANCE    *AnimationPackage;
  UINT32                            AnimationInfoOffset;
  UINT32                            AnimationSize;

  if (PackageHdr == NULL || PackageList == NULL) {
    return EFI_INVALID_PARAMETER;
  }

  if (PackageList->AnimationPkg != NULL) {
    return EFI_INVALID_PARAMETER;
  }

  AnimationPackage = (HII_ANIMATION_PACKAGE_INSTANCE *) AllocateZeroPool (sizeof (HII_ANIMATION_PACKAGE_INSTANCE));
  if (AnimationPackage == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }

  CopyMem (&AnimationPackage->AnimationPkgHdr, PackageHdr, sizeof (EFI_HII_ANIMATION_PACKAGE_HDR));

  AnimationInfoOffset = AnimationPackage->AnimationPkgHdr.AnimationInfoOffset;
  AnimationSize = 0;
  AnimationPackage->AnimationBlock      = NULL;
  AnimationPackage->AnimationBlockSize  = 0;

  if (AnimationInfoOffset != 0) {
    AnimationSize = AnimationPackage->AnimationPkgHdr.Header.Length -
                    sizeof (EFI_HII_ANIMATION_PACKAGE_HDR);
    AnimationPackage->AnimationBlock = (UINT8 *) AllocateZeroPool (AnimationSize);
    if (AnimationPackage->AnimationBlock == NULL) {
      FreePool (AnimationPackage);
      return EFI_OUT_OF_RESOURCES;
    }
    CopyMem (
      AnimationPackage->AnimationBlock,
      (UINT8 *) PackageHdr + AnimationInfoOffset,
      AnimationSize
      );
  }
  AnimationPackage->AnimationBlockSize  = AnimationSize;
  PackageList->AnimationPkg             = AnimationPackage;
  *Package                              = AnimationPackage;
  if (NotifyType == EFI_HII_DATABASE_NOTIFY_ADD_PACK) {
    PackageList->PackageListHdr.PackageLength += AnimationPackage->AnimationPkgHdr.Header.Length;
  }
...
/**
 This function exports animation packages to a buffer.

 @param[in]      Private        Hii database private structure.
 @param[in]      Handle         Identification of a package list.
 @param[in]      PackageList    Pointer to a package list which will be exported.
 @param[in]      UsedSize       Address offset of RTC.
 @param[in]      BufferSize     Length of the Buffer.
 @param[in, out] Buffer         Allocated space for storing exported data.
 @param[in, out] ResultSize     The size of the already exported content of this package list.

 @retval EFI_SUCCESS            Animation Packages are exported successfully.
 @retval EFI_INVALID_PARAMETER  Any input parameter is invalid.
**/
STATIC
EFI_STATUS
ExportAnimationPackages (
  IN HII_DATABASE_PRIVATE_DATA          *Private,
  IN EFI_HII_HANDLE                     Handle,
  IN HII_DATABASE_PACKAGE_LIST_INSTANCE *PackageList,
  IN UINTN                              UsedSize,
  ...
  )
{
  UINTN                           PackageLength;
  EFI_STATUS                      Status;
  HII_ANIMATION_PACKAGE_INSTANCE  *Package;
  if (Private == NULL || PackageList == NULL || ResultSize == NULL) {
    return EFI_INVALID_PARAMETER;
  }
  if (BufferSize > 0 && Buffer == NULL ) {
  ...
  Package = PackageList->AnimationPkg;
  if (Package == NULL) {
    return EFI_SUCCESS;
  }
  PackageLength = Package->AnimationPkgHdr.Header.Length;
  if (PackageLength + *ResultSize + UsedSize <= BufferSize) {
    Status = InvokeRegisteredFunction (
               Private,
               EFI_HII_DATABASE_NOTIFY_EXPORT_PACK,
    ...
    ASSERT_EFI_ERROR (Status);
    ASSERT (Package->AnimationPkgHdr.Header.Length ==
            sizeof (EFI_HII_ANIMATION_PACKAGE_HDR) + Package->AnimationBlockSize);
    CopyMem (Buffer, &Package->AnimationPkgHdr, sizeof (EFI_HII_ANIMATION_PACKAGE_HDR));
    Buffer = (UINT8 *) Buffer + sizeof (EFI_HII_ANIMATION_PACKAGE_HDR);
    if (Package->AnimationBlockSize != 0) {
      CopyMem (Buffer, Package->AnimationBlock, Package->AnimationBlockSize);
    }
  }
  *ResultSize += PackageLength;
...
/**
 This function deletes Animation package from a package list node.

 @param[in]      Private      Hii database private data.
 @param[in]      Handle       Handle of the package list which contains the to be removed animation packages.
 @param[in, out] PackageList  Package List which contains the to be removed animation package.

 @retval EFI_SUCCESS            Animation Package(s) is deleted successfully.
 @retval EFI_INVALID_PARAMETER  Any input parameter is not valid.
**/
STATIC
EFI_STATUS
RemoveAnimationPackages (
  IN     HII_DATABASE_PRIVATE_DATA          *Private,
  IN     EFI_HII_HANDLE                     Handle,
  IN OUT HII_DATABASE_PACKAGE_LIST_INSTANCE *PackageList
  )
 {
  ...
  PackageList->PackageListHdr.PackageLength -= Package->AnimationPkgHdr.Header.Length;
  FreePool (Package->AnimationBlock);
  FreePool (Package);
  PackageList->AnimationPkg = NULL;
  return EFI_SUCCESS;
}
...
/**
 ...
 @param[in]      PackageHdr     Pointer to a buffer stored with Simple Font
                                package information.
 @param[in]      NotifyType     The type of change concerning the database.
 @param[in, out] PackageList    Pointer to a package list which will be inserted to.
 @param[out]     Package        Created Simple Font package
...
/**
 ...
 @param[in]      DevicePath     Pointer to a EFI_DEVICE_PATH_PROTOCOL protocol instance
 @param[in]      NotifyType     The type of change concerning the database.
 @param[in, out] PackageList    Pointer to a package list which will be inserted to.
...
/**
 ...
 @param[in]      Private        Hii database private data.
 @param[in]      Handle         Handle of the package list.
 @param[in, out] PackageList    Package List which contains the to be  removed
                                Device Path package.
...
/**
 ...
 @param[in]      NotifyType       The type of change concerning the database.
 @param[in]      DevicePath       Pointer to a EFI_DEVICE_PATH_PROTOCOL protocol instance
 @param[in, out] DatabaseRecord   Pointer to a database record contains  a package
...
/**
 ...
 @param[in]      PackageHdr     Pointer to a buffer stored with Keyboard Layout
                                package information.
 @param[in]      NotifyType     The type of change concerning the database.
 @param[in, out] PackageList    Pointer to a package list which will be inserted to.
 @param[out]     Package        Created Keyboard Layout package
...
/**
 ...
 @retval EFI_SUCCESS            Keyboard Layout Package(s) is deleted successfully.
 @retval EFI_INVALID_PARAMETER  Any input parameter is not valid.
**/
EFI_STATUS
...
/**
 ...
 @param[in]      Private          Hii database private structure.
 @param[in]      NotifyType       The type of change concerning the database.
 @param[in]      PackageList      Pointer to a package list.
 @param[in, out] DatabaseRecord   Pointer to a database record contains  a package
                                  list instance which will be inserted to.

 @retval EFI_SUCCESS            All incoming packages are inserted to current database.
 @retval EFI_OUT_OF_RESOURCES   Unable to allocate necessary resources for the new
                                Device path package.
 @retval EFI_INVALID_PARAMETER  Any input parameter is invalid.
**/
...
/**
 ...
 @param[in]      Handle         Identification of a package list.
 @param[in]      PackageList    Pointer to a package list which will be exported.
 @param[in, out] UsedSize       The length of buffer has been used by exporting
                                package lists when Handle is NULL.
 @param[in]      BufferSize     Length of the Buffer.
 @param[out]     Buffer         Allocated space for storing exported data.

 @retval EFI_SUCCESS            Keyboard Layout Packages are exported successfully.
 @retval EFI_INVALID_PARAMETER  Any input parameter is invalid.
**/
EFI_STATUS
ExportPackageList (
  ...
  )
{
  ...
  Status = ExportAnimationPackages (
             Private,
             Handle,
             PackageList,
             *UsedSize,
  ...
  //
  // double check the HII Package List size,
  // if package length isn't equal result size,
  // Maybe call this function in NEW_PACK callback
  //
  //
  if (NeedFixHeader) {
      ListHeader = (EFI_HII_PACKAGE_LIST_HEADER *)Buffer;
      if (ListHeader->PackageLength != (ResultSize + sizeof (EFI_HII_PACKAGE_HEADER))) {
        ListHeader->PackageLength = (UINT32)(ResultSize + sizeof (EFI_HII_PACKAGE_HEADER));
      }
  }
...
/**
 ...
 @param[in]  This               A pointer to the EFI_HII_DATABASE_PROTOCOL
                                instance.
 @param[in]  PackageList        A pointer to an EFI_HII_PACKAGE_LIST_HEADER
                                structure.
 @param[in]  DriverHandle       Associate the package list with this EFI handle.
                                If a NULL is specified, this data will not be associate
                                with any drivers and cannot have a callback induced.
 @param[out] Handle             A pointer to the EFI_HII_HANDLE instance.
...
/**
 ...
 @param[in] This                A pointer to the EFI_HII_DATABASE_PROTOCOL instance.
 @param[in] Handle              The handle that was registered to the data that is
  ...
  )
{
  ...
  for (Link = Private->DatabaseList.ForwardLink; Link != &Private->DatabaseList; Link = Link->ForwardLink) {
    ...
    if (Node->Handle == Handle) {
      ...
      Status = RemoveAnimationPackages (Private, Handle, PackageList);
      if (EFI_ERROR (Status)) {
        return Status;
      }
    ...
    if (Node->Handle == Handle) {
      ...
      while (PackageHeader.Type != EFI_HII_PACKAGE_END) {
        ...
        case EFI_HII_PACKAGE_IMAGES:
          ...
          break;
        case EFI_HII_PACKAGE_ANIMATIONS:
          Status = RemoveAnimationPackages (Private, Handle, OldPackageList);
          break;
        case EFI_HII_PACKAGE_SIMPLE_FONTS:
...
/**
 ...
 @param[in]      PackageType            Specifies the package type of the packages to list
                                        ...
                                        listed.
 @param[in]      PackageGuid            If PackageType is EFI_HII_PACKAGE_TYPE_GUID, then
 ...
 @param[out]     Handle                 An array of EFI_HII_HANDLE instances returned.
...
/**
 ...
 @param[in]      This           A pointer to the EFI_HII_DATABASE_PROTOCOL instance.
 @param[in]      Handle         An EFI_HII_HANDLE that corresponds to the desired
                                package list in the HII database to export or NULL
                                to indicate  all package lists should be exported.
 @param[in, out] BufferSize     On input, a pointer to the length of the buffer.
                                On output, the length of the buffer that is
                                required for the exported data.
 @param[out] Buffer             A pointer to a buffer that will contain the
...
/**
 ...
 @param[in]  This               A pointer to the EFI_HII_DATABASE_PROTOCOL instance.
 @param[in]  PackageType        Specifies the package type of the packages to list
                                or EFI_HII_PACKAGE_TYPE_ALL for all packages to be listed.
 @param[in]  PackageGuid        If PackageType is EFI_HII_PACKAGE_TYPE_GUID, then
                                this is the pointer to the GUID which must match
                                the Guid field of
                                EFI_HII_GUID_PACKAGE_GUID_HDR. Otherwise, it must be NULL.
 @param[in]  PackageNotifyFn    Points to the function to be called when the event
                                specified by NotificationType occurs.
 @param[in]  NotifyType         Describes the types of notification which this
                                function will be receiving.
 @param[out] NotifyHandle       Points to the unique handle assigned to the
...
/**
 ...
 @param[in]      This                   A pointer to the EFI_HII_DATABASE_PROTOCOL instance.
 @param[in, out] KeyGuidBufferLength    On input, a pointer to the length of the keyboard
                                        GUID  buffer. On output, the length of the handle
                                        buffer  that is required for the handles found.
 @param[out]     KeyGuidBuffer          An array of keyboard layout GUID instances returned.
...
/**
 ...
 @param[in]      This                   A pointer to the EFI_HII_DATABASE_PROTOCOL instance.
 @param[in]      KeyGuid                A pointer to the unique ID associated with a given
                                        keyboard layout. If KeyGuid is NULL then the
                                        current layout will be retrieved.
 @param[in, out] KeyboardLayoutLength   On input, a pointer to the length of the
                                        KeyboardLayout buffer.  On output, the length of
                                        the data placed into KeyboardLayout.
 @param[out]     KeyboardLayout         A pointer to a buffer containing the retrieved keyboard layout.
...
/**
 ...
 @param[in] This                A pointer to the EFI_HII_DATABASE_PROTOCOL instance.
 @param[in] KeyGuid             A pointer to the unique ID associated with a given keyboard layout.
...
/**
 ...
 @param[in]  This               A pointer to the EFI_HII_DATABASE_PROTOCOL instance.
 @param[in]  PackageListHandle  An EFI_HII_HANDLE that corresponds to the desired
                                package list in the HIIdatabase.
 @param[out] DriverHandle       On return, contains the EFI_HANDLE which was
                                registered with the package list in NewPackageList().

 @retval EFI_SUCCESS            The DriverHandle was returned successfully.
 @retval EFI_INVALID_PARAMETER  The PackageListHandle was not valid or DriverHandle was NULL.
 @retval EFI_NOT_FOUND          This PackageList handle can not be found in current database.
**/
EFI_STATUS
EFIAPI
