/** @file
  The driver health manager implementation
...
#define QUESTION_ID_CONFIG_REQUIRE_BASE     0x1000
  ...
  )
{
  ...
  Status = gBS->LocateProtocol (&gEfiDriverHealthProtocolGuid, NULL, (VOID **) &Instance);
  if (EFI_ERROR (Status)) {
    return;
  }
...
//EFI_HANDLE                  Handle;
  ...
  //
  // Publish HII data when there is a DriverHealthProtocol instance on system.
  //
  ...
  )
{
  ...
  if (mDriverHealthManagerHealthInfo == NULL) {
    mDriverHealthManagerHealthInfoCount = 0;
  }
  ...
  for (Index = 0; Index < mDriverHealthManagerHealthInfoCount; Index++) {
    ...
    case EfiDriverHealthStatusConfigurationRequired:
      ...
      HiiCreateGotoExOpCode (
        ...
        0,
//      0,
        (EFI_QUESTION_ID) (Index + QUESTION_ID_CONFIG_REQUIRE_BASE),
        0,
        &FormsetGuid,
        DevicePath
        );
