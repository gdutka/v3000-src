  ...
  )
{
  ...
  ExitDataSize  = 0;
  Status = gBdsServices->LaunchLoadOption (gBdsServices, Option, &ExitDataSize, &ExitData);
  DisableQuietBoot ();
  if (EFI_ERROR (Status)) {
    BdsLibBootFailed (Option, Status, ExitData, ExitDataSize);
  }
