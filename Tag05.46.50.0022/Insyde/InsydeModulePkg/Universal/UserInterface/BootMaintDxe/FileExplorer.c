  ...
  )
{
  ...
  if (Action == EFI_BROWSER_ACTION_FORM_OPEN) {
    if (!mBootMaintSendFormBySelf && QuestionId == KEY_VALUE_TRIGGER_FORM_OPEN_ACTION) {
      //
      // File explorer is imported by other VFR driver. Here to initialize boot maintenance manager.
      //
      Status = InitializeBM ();
      if (!EFI_ERROR (Status)) {
  ...
  if (Action == EFI_BROWSER_ACTION_CHANGED) {
    ...
    if (QuestionId >= FILE_OPTION_OFFSET && UpdateFileExplorer (Private, QuestionId)) {
