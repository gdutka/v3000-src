  ...
  )
{
  ...
  if (FeaturePcdGet (PcdSetMtcToCmos)) {
    WriteCmos32 ((UINT8) MonotonicCount, HighCount);
    Status = EFI_SUCCESS;
  } else {
    //
  ...
  )
{
  ...
  if (FeaturePcdGet (PcdSetMtcToCmos)) {
    if (EFI_ERROR (Status)) {
      ...
      *HighCount = ReadCmos32 ((UINT8) MonotonicCount);
      Status = EFI_SUCCESS;
    }
  }
  ...
  )
{
  ...
  POST_CODE (DXE_MTC_INIT); //PostCode = 0x4D, MTC Initial
  ...
  Status = GetEfiMtc (BufferSize, &HighCount);
  if (EFI_ERROR (Status)) {
    HighCount = 0;
  }
  //
