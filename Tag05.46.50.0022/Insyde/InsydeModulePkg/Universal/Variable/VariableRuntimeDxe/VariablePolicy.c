...
;******************************************************************************
;* Copyright (c)2021 - 2022, Insyde Software Corp. All Rights Reserved.Copyright (c) 2023, Insyde Software Corp. All Rights Reserved.Copyright (c) 2023, Insyde Software Corp. All Rights Reserved.
;*
  ...
  )
{
  ...
  if (NewSize > mCurrentTableSize) {
    ...
    mVariableModuleGlobal->PolicyTable      = mPolicyTable;
    mVariableModuleGlobal->CurrentTableSize = mCurrentTableSize;
  }
  ...
  mVariableModuleGlobal->CurrentTableUsage = mCurrentTableUsage;
  mVariableModuleGlobal->CurrentTableCount = mCurrentTableCount;
  // We're done here.
  ...
  )
{
  ...
  if (mProtectionDisabled || !AfterEndOfDxe ()) {
    ReturnStatus = EFI_SUCCESS;
    goto Exit;
  }
  ...
  if (ActivePolicy != NULL) {
    ...
    // Check for lock on state variable.
      ...
      if (!EFI_ERROR(Status) && Variable.CurrPtr != NULL) {
        StateVarSize = Variable.CurrPtr->DataSize;
        if (StateVarSize != sizeof(StateVar)) {
          ReturnStatus = EFI_ABORTED;
          goto Exit;
        }
        CopyMem (&StateVar, GetVariableDataPtr (Variable.CurrPtr), sizeof(StateVar));
...
/**
  Convert variable policy table to virtual address.
**/
VOID
...
/**
  ...
  @retval     EFI_UNSUPPORTED         System is not in SMM.
...
{
  ESAL_VARIABLE_GLOBAL  *VarGlobal;

  if (mSmst == NULL) {
    return EFI_UNSUPPORTED;
  }

  VarGlobal = mSmmVariableGlobal->ProtectedModeVariableModuleGlobal;
  mPolicyTable = VariableAllocateZeroBuffer (VarGlobal->CurrentTableSize, TRUE);
  if (mPolicyTable == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }

  CopyMem (mPolicyTable, VarGlobal->PolicyTable, VarGlobal->CurrentTableSize);
  mCurrentTableSize  = VarGlobal->CurrentTableSize;
  mCurrentTableUsage = VarGlobal->CurrentTableUsage;
  mCurrentTableCount = VarGlobal->CurrentTableCount;
