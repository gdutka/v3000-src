  ...
  if (StrCmp (VariableName, L"PlatformLang") == 0) {
    ...
    if ((mVariableModuleGlobal->PlatformLangCodes != NULL) && (mVariableModuleGlobal->LangCodes != NULL)) {
      ...
      if (BestPlatformLang != NULL) {
        ...
        Status = EmuUpdateVariable (L"Lang", &gEfiGlobalVariableGuid, BestLang, ISO_639_2_ENTRY_SIZE + 1, Attributes, &Variable);
    ...
    if ((mVariableModuleGlobal->PlatformLangCodes != NULL) && (mVariableModuleGlobal->LangCodes != NULL)) {
      ...
      if (BestLang != NULL) {
        ...
        if (BestPlatformLang != NULL) {
          ...
          Status = EmuUpdateVariable (L"PlatformLang", &gEfiGlobalVariableGuid, BestPlatformLang,
...
/**
  ...
  @param[in] VariableStore       Pointer to variable store address.

  @return UINTN       The offset of the first free variable space.ata into this region.
**/
STATIC
UINTN
GetCurrentLastOffset (
  IN  UINTN     VariableStore
  )
{
  VARIABLE_HEADER *VarHeader;

  VarHeader = (VARIABLE_HEADER*) ((UINT8*) VariableStore + sizeof (VARIABLE_STORE_HEADER));
  while (VarHeader->StartId == VARIABLE_DATA) {
    VarHeader = GetNextPotentialVariablePtr (VarHeader);
    if (VarHeader == NULL) {
      break;
  ...
  return (UINTN) VarHeader - VariableStore;
}
  ...
  )
{
  ...
  if ((Attributes & EFI_VARIABLE_NON_VOLATILE) != 0) {
    ...
    mVariableModuleGlobal->NonVolatileLastVariableOffset = GetCurrentLastOffset ((UINTN) Global->NonVolatileVariableBase);
    ...
    mVariableModuleGlobal->VolatileLastVariableOffset = GetCurrentLastOffset ((UINTN) Global->VolatileVariableBase);
  ...
  )
{
  ...
  Status = EmuUpdateVariable (VariableName, VendorGuid, Data, DataSize, Attributes, &Variable);
