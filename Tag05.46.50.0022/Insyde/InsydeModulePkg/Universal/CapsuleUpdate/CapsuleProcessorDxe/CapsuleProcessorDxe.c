  ...
  )
{
  ...
  LIST_ENTRY                             *Link;
  CAPSULE_FMP_ENTRY                      *CapsuleFmpEntry;
  UINTN                                  TempImageInfoSize;
  UINT32                                 TempDescriptorVersion;
  UINT8                                  TempDescriptorCount;
  UINTN                                  TempDescriptorSize;
  UINT32                                 TempPackageVersion;
  CHAR16                                 *TempPackageVersionName;
  ...
  *ImageInfoSize = mCapsuleFmp.CapsuleProcessorCount * sizeof (EFI_FIRMWARE_IMAGE_DESCRIPTOR);
  ...
  while (!IsNull (&mCapsuleFmp.CapsuleProcessorListHead, Link)) {
    ...
    if (CapsuleFmpEntry->GetImageInfo != NULL) {
      TempImageInfoSize = sizeof (EFI_FIRMWARE_IMAGE_DESCRIPTOR);
      TempPackageVersionName = NULL;
      Status = CapsuleFmpEntry->GetImageInfo (
                                  This,
                                  &TempImageInfoSize,               // ImageInfoSize
                                  &ImageInfo[Index],                // ImageInfo
                                  &TempDescriptorVersion,           // DescriptorVersion
                                  &TempDescriptorCount,             // DescriptorCount
                                  &TempDescriptorSize,              // DescriptorSize
                                  &TempPackageVersion,              // PackageVersion
                                  &TempPackageVersionName           // PackageVersionName
                                  );
    } else {
