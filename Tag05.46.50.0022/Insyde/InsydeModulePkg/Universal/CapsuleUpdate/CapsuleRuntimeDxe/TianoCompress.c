...
STATIC UINT8  mLevel[WNDSIZ + UINT8_MAX + 1];
STATIC UINT8  mText[WNDSIZ * 2 + MAXMATCH];
STATIC UINT8  mChildCount[WNDSIZ + UINT8_MAX + 1];
STATIC UINT8  mBuf[BLKSIZ];
...
STATIC NODE   mPosition[WNDSIZ + UINT8_MAX + 1];
STATIC NODE   mParent[WNDSIZ * 2];
STATIC NODE   mPrev[WNDSIZ * 2];
STATIC NODE   mNext[MAX_HASH_VAL + 1];
  ...
  )
{
  ...
  Node = NodeT + mLevel[NodeR];
  if (Node < 0 || Node >= sizeof (mText)) return ;
  NodeS           = Child (NodeR, mText[Node]);
  NodeT           = mPrev[NodeS];
  NodeU           = mNext[NodeS];
  mNext[NodeT]    = NodeU;
  mPrev[NodeU]    = NodeT;
