  ...
  )
{
  ...
  //
  // Check the system is whether in recovery mode.
  //
  Status = FtwLiteReadHeaderFromSpare (FtwLiteDevice, SpareHeader, sizeof (SpareHeader));
  if (EFI_ERROR (Status)) {
    FtwFreePool (FtwLiteDevice, (VOID **)&MyBuffer);
    return EFI_OUT_OF_RESOURCES;
  }
  InRecovery = FALSE;
  if (CompareGuid (PcdGetPtr (PcdSeamlessRecoverySignature), (EFI_GUID *)SpareHeader)) {
    InRecovery = TRUE;
  }


  //
  // If in recoveyr mode, try to keep the content of spare block
