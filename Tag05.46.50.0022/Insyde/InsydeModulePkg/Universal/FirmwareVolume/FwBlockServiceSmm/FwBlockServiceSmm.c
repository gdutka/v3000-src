  ...
  )
{
  ...
  //
  // Copy whole parameter buffer and data buffer to SMRAM to prevent from TOCTOU attack.
  //
  SmmFwbFunctionHeader = AllocateCopyPool (mFwbBufferSize - SMM_COMMUNICATE_HEADER_SIZE, CommBuffer);
  if (SmmFwbFunctionHeader == NULL) {
    return EFI_SUCCESS;
  }
  SmmFwbAccessFvb      = (SMM_FWB_COMMUNICATE_ACCESS_FVB *)SmmFwbFunctionHeader->Data;

  if (BufferOverlapSmram ((VOID *)SmmFwbAccessFvb->Address, SmmFwbAccessFvb->NumBytes)) {
  ...
  //
  CopyMem (CommBuffer, SmmFwbFunctionHeader, mFwbBufferSize - SMM_COMMUNICATE_HEADER_SIZE);
  FreePool (SmmFwbFunctionHeader);
  return EFI_SUCCESS;
}
