  ...
  )
{
  ...
  PartEntry = AllocatePool (ValidHeader->NumberOfPartitionEntries * ValidHeader->SizeOfPartitionEntry);
  if (PartEntry == NULL) {
  ...
  Status = DiskIo->ReadDisk (
                     ...
                     ValidHeader->NumberOfPartitionEntries * ValidHeader->SizeOfPartitionEntry,
                     PartEntry
                     );
  ...
  for (Index = 0; Index < ValidHeader->NumberOfPartitionEntries; Index++) {
    Entry = (EFI_PARTITION_ENTRY *) ((UINT8 *) PartEntry + Index * ValidHeader->SizeOfPartitionEntry);
