  ...
  )
{
  ...
  if (BlockIo->Media->MediaPresent ||
    ...
    //
    // Try for GPT, then UDF and El Torito ,and then legacy MBR partition types.
    // If the media supports a given partition type install child handles to
    // represent the partitions described by the media.
    //
    // Note: There are some El Torito (ex: RHEL-8.0) has MBR header and it may be
    // mistook to consider as a MRB partition. change the order to make UDF and El
    // Torito before legacy MBR to prevent from this issue.
    //
