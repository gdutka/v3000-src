/** @file
  PEI Module for Xhc Report Status Code
...
EFI_PEI_NOTIFY_DESCRIPTOR mNotifyOnMemoryDiscoveredPpiList = {
  ...
  )
{
  ...
  if (!EFI_ERROR(Status)) {
    ...
    Status = RscHandlerPpi->Register (XhcStatusCodeReportWorker);
  }
...
/**
  Entry point of Xhc Status Code PEIM.
  ...
  )
{
  ...
  Status = PeiServicesNotifyPpi (&mNotifyOnMemoryDiscoveredPpiList);
