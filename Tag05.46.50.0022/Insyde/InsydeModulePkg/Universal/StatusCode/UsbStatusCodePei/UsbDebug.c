  ...
  )
{
  ...
  //
  // Delay 30 MilliSecond
  //
  MicroSecondDelay (30000);
  //
  // Set EHCI ownership bit
  //
  ...
  DebugPtr->Sc = DebugSC;
  //
  // Sets debug port for 0xd0 to indicate waitting for USB cable plug
  //
  TestPostCode = 0xd0;
  Index = 0;
  ...
  //
  // Check PORTSC.CurrentConnectStatus
  //
  while ((CtrlPtr->PortSC[UsbDebugPortNum] & EUSBPORTSC_CCS) != EUSBPORTSC_CCS) {
    MicroSecondDelay (1000);
    if (!Index) {
      IoWrite8 ((UINT16)DebugIoConfig.Config.DebugPort, (UINT8)TestPostCode);
      if (TestPostCode == 0xd0) {
        TestPostCode = 0x0d;
      } else {
        TestPostCode = 0xd0;
      }
      Index = 500;
      if (DebugIoConfig.Config.Bypass) {
        Timeout --;
  ...
  //
  // Sets debug port for 0xd1 to indicate port 0 plugged
  //
  IoWrite8((UINT16)DebugIoConfig.Config.DebugPort, 0xd1);
  ...
  //
  // Delay 30 MilliSecond to make sure port reset complete
  //
  MicroSecondDelay (30000);
  //
  // Clean Reset bit
