...
STATIC XHC_DEBUG_PRIVATE        *mPrivate = NULL;
  ...
  )
{
  ...
  if (!(HcToken & B_TOKEN_LMC)) {
    ...
    if (Private->DebugContextAddr != 0) {
      ...
      DebugContext = AllocateRuntimePages (1);
...
/**
  ...
  Allocate IOMMU memory for DMAr
  ...
  )
{
  ...
  //
  // The common buffer Iommu->AllocateBuffer
  //
  ...
  if (!EFI_ERROR (Status)) {
    ...
    Private->DeviceAddrConverter = (UINT32)((UINTN)Ptr - (UINTN)DevAddr);
  } else {
    Private->Iommu->Unmap (
                      Private->Iommu,
                      Private->IommuMap
                      );
    Private->Iommu->FreeBuffer (
                      Private->Iommu,
                      1,
                      Ptr
                      );
...
/**
  ...
  Signal handlers for PciIo Protocol event
  ...
  )
{
  ...
  while (TRUE) {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      if (!EFI_ERROR (Status) && ((UsbClassCReg & 0xffffff00) == 0x0C033000)) {
        ...
        HcToken = mPrivate->HcToken;
        if (Bus != 0) {
          ValidateBridges (mPrivate);
        }
        if (Bus == (((UINT32)HcToken & B_TOKEN_XHCI_PCI_BUS) >> N_TOKEN_XHCI_PCI_BUS) &&
            Dev == (((UINT32)HcToken & B_TOKEN_XHCI_PCI_DEV) >> N_TOKEN_XHCI_PCI_DEV) &&
            Fun == (((UINT32)HcToken & B_TOKEN_XHCI_PCI_FUN) >> N_TOKEN_XHCI_PCI_FUN)) {
          mPrivate->HcToken = HcToken;
          Status = AllocateIommuMemory (mPrivate, Handle);
          if (!EFI_ERROR (Status)) {
            AsmWbinvd ();
            DetectXhci (mPrivate, FALSE, NULL);
          }
        }
      }
    }
...
/**
  ...
  Signal handlers for PciEnumerationComplete Protocol event
...
VOID
EFIAPI
PciEnumerationCompleteNotifyFunction (
  IN EFI_EVENT          Event,
  IN VOID               *Context
  )
{
    ...
    for (Index = 0; Index < HandleCount; Index ++) {
      ...
      if (!EFI_ERROR (Status) && ((UsbClassCReg & 0xffffff00) == 0x0C033000)) {
        ...
        if (Bus == (((UINT32)HcToken & B_TOKEN_XHCI_PCI_BUS) >> N_TOKEN_XHCI_PCI_BUS) &&
            ...
            Fun == (((UINT32)HcToken & B_TOKEN_XHCI_PCI_FUN) >> N_TOKEN_XHCI_PCI_FUN)) {
          mPrivate->HcToken = HcToken;
          Status = AllocateIommuMemory (mPrivate, HandleBuffer[Index]);
          if (!EFI_ERROR (Status)) {
            AsmWbinvd ();
            DetectXhci (mPrivate, FALSE, NULL);
            mPrivate->Flags = FLAGS_FORCE_STOP;
            return;
          }
        }
      }
  ...
  //
  // Registers the PciIo protocol notification once PciIo
  // not found in PciIoEnumerationComplete event 
  //
...
VOID
EFIAPI
PciEnumerationCompleteEndedNotifyFunction (
  IN EFI_EVENT          Event,
  IN VOID               *Context
  )
{
  ...
  )
{
  ...
  Status = AllocateIommuMemory (mPrivate, NULL);
  if (!EFI_ERROR (Status)) {
    AsmWbinvd ();
    DetectXhci (mPrivate, FALSE, NULL);
  } else {
    //
    // Registers the PciEnumerationComplete protocol notification
    //
    Status = gBS->CreateEvent (
                    EVT_NOTIFY_SIGNAL,
                    TPL_NOTIFY,
  ...
  )
{
  ...
  if (Private->Flags & FLAGS_FORCE_STOP) return EFI_ABORTED;
  ...
  )
{
  ...
  DbgContext = (DBG_CONTEXT*)(UINTN)mPrivate->DebugContextAddr;
  //
  // Save PCI index
  //
  ...
  for (StringPtr = OutputString; StringLength > 0; StringLength -= BufferLen, StringPtr += BufferLen) {
    ...
    DebugXhcWrite (mPrivate, BufferLen, StringPtr);
  }
  //
  // Restore PCI index
  //
  ...
  )
{
  ...
  mPrivate = AllocateRuntimeZeroPool (sizeof (XHC_DEBUG_PRIVATE));
  if (mPrivate == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }
  Private = GetFirstGuidHob (&mXhcDebugHobGuid);
  if (Private != NULL) {
    mPrivate->HcToken             = Private->HcToken;
    mPrivate->DebugContextAddr    = Private->DebugContextAddr;
    mPrivate->DeviceAddrConverter = Private->DeviceAddrConverter;
    mPrivate->Flags               = Private->Flags;
  } else {
    Status = DetectXhci (mPrivate, TRUE, NULL);
    if (EFI_ERROR (Status)) {
      return Status;
    }
  }
...
/**
  Called by unregister status code callback functions only available at boot time from
