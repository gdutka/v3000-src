/** @file
  Runtime DXE driver for Xhc Report Status Code
...
/**
  Convert status code value and extended data to readable ASCII string, send string to Xhc debug port
  ...
  )
{
  ...
  //
  // Call XhcDebugPrint function to do print.
  //
  if (CharCount > 0) {
    XhcDebugPrint ((CHAR8 *) Buffer);
  }
...
{
  mRscHandlerProtocol->Unregister (XhcStatusCodeReportWorker);
  XhcShutdownStatusCode ();
}
  ...
  )
{
  ...
  if (Status == EFI_SUCCESS) {
    Status = XhcInitializeStatusCode ();
    if (EFI_ERROR (Status)) {
      return ;
    }
    mRscHandlerProtocol->Register (XhcStatusCodeReportWorker, TPL_HIGH_LEVEL);
  ...
  )
{
  ...
  if (!EFI_ERROR(Status)) {
    XhcInitializeStatusCode ();
    Status = mSmmRscHandlerProtocol->Register (XhcStatusCodeReportWorker);
  }
  ASSERT_EFI_ERROR (Status);
  return Status;
}
...
/**
  Entry point of Xhc Status Code Driver.
