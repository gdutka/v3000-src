  ...
  )
{
  ...
  ByteCount = SectorNum * Private->Media[Lun].BlockSize;
  ...
  // From the above table, we could know 2.1 Mbytes per second is lowest one.
  ...
  Packet.Timeout          = MultU64x32 ((UINT64)((ByteCount / 2100000) + 31), 10000000);
  Packet.Cdb              = Cdb;
  Packet.CdbLength        = sizeof (Cdb);
  Packet.InDataBuffer     = DataBuffer;
  Packet.InTransferLength = *DataLength;
  ...
  )
{
  ...
  while (TRUE) {
    ...
    //
    // Some devices (e.g. Kingstone UFS) will fail to excute command if host not read
    // fDeviceInit flag. According to UFS 3.1 spec Section 13.1.3.3, the device shall
    // reset the fDeviceInit flag when the initialization is complete. Here will check
    // the flag to determine if the device is ready to accept any command.
    //
