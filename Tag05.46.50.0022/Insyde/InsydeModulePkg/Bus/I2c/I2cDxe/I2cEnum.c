...
{
  UINT16        *BusClockHertzTable16;
  UINT32        *BusClockHertzTable32;
  UINT32        TableSize;
  UINTN         Total;
  ...
  TableSize = PcdGetSize (PcdI2cBusSpeedTable);
  BusClockHertzTable16 = (UINT16*)PcdGetPtr (PcdI2cBusSpeedTable);
  if (BusClockHertzTable16 == NULL) {
    return EFI_NO_MAPPING;
  }
  ...
  if (TableSize <= 14) {
    ...
    // 2 + 2 + 2 + 8 = 14
    //
    for (Total = 0; BusClockHertzTable16[Total] != 0; Total++);
    if (I2cBusConfiguration >= Total) {
      return EFI_NO_MAPPING;
    }
    *BusClockHertz = (UINTN)BusClockHertzTable16[I2cBusConfiguration];
    return EFI_SUCCESS;
  }

  BusClockHertzTable32 = (UINT32 *) (UINTN) BusClockHertzTable16;
  for (Total = 0; (BusClockHertzTable16[Total] != 0) && Total < (TableSize / sizeof (UINT32)); Total++);
  if (I2cBusConfiguration >= Total) {
    return EFI_NO_MAPPING;
  }

  *BusClockHertz = (UINTN)BusClockHertzTable32[I2cBusConfiguration];
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  while (TRUE) {
    ...
    if (!EFI_ERROR (Status)) {
      //
      // The hid device information installed, go next event
      //
      continue;
    }
    //
    ...
    //
    // Install I2c Hid Device information if it is I2C HID device
    //
  ...
  )
{
  ...
  H2O_I2C_DEVICE_PCD            ZeroDevicePcd;
  ...
  for (TokenNumber = 0; (TokenNumber = LibPcdGetNextToken (&gI2cDeviceTokenSpaceGuid, TokenNumber)) != 0; ) {
    ...
    // check I2cDevicePcd
    //
    if (LibPcdGetExSize (&gI2cDeviceTokenSpaceGuid, TokenNumber) != sizeof (H2O_I2C_DEVICE_PCD)) {
      DEBUG((DEBUG_ERROR, "InitializeI2cEnum: gI2cDeviceTokenSpaceGuid TokenNumber %lld PCD data size isn't equal sizeof(H2O_I2C_DEVICE_PCD)\n", TokenNumber));
      continue ;
    }

    if (CompareGuid (&gZeroGuid, &I2cDevicePcd->Guid)) {
      ZeroMem (&ZeroDevicePcd, sizeof (H2O_I2C_DEVICE_PCD));
      if (CompareMem (I2cDevicePcd, &ZeroDevicePcd, sizeof (H2O_I2C_DEVICE_PCD)) == 0) {
        continue;
      }
    }
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    for (Index = 0; Index < DeviceHandleCount; Index++) {
      //
      // Uninstall I2cDeviceInfo in the handle
      //
      Status = gBS->HandleProtocol (
                      DeviceHandleBuffer[Index],
                      &gI2cHidDeviceInfoGuid,
