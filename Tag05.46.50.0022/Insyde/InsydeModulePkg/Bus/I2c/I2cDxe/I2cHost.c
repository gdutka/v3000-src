  ...
  )
{
  ...
  if (IsListEmpty (&I2cHostContext->RequestList)) {
  ...
  )
{
  ...
  //
  Entry = GetFirstNode(&I2cHostContext->RequestList);
  while (!IsNull (&I2cHostContext->RequestList, Entry)) {

    I2cRequest = I2C_REQUEST_FROM_ENTRY (Entry);

    //
    // Firstly retrieve next entry to avoid Entry.ForwardLink becomes garbage
    // when free the current entry.
    //
    Entry = GetNextNode(&I2cHostContext->RequestList, Entry);

    if (I2cRequest->MasterEvent != Event) {
      continue;
    }
    ...
    if ( NULL != I2cRequest->Status ) {
      *I2cRequest->Status = I2cRequest->MasterStatus;
    }
    //
    //  Notify the user of the I2C request completion
    //
    if ( NULL != I2cRequest->DeviceEvent ) {
      gBS->SignalEvent (I2cRequest->DeviceEvent);
    }
    //
    // Done with this request, remove the current request from list
    //
    gBS->CloseEvent (I2cRequest->MasterEvent);
...
/**
  ...
  @param[in] AsyncRequest       Async or sync request
  ...
  )
{
  ...
  if (AsyncRequest) {
    //
    // Get the I2cRequest from linking list for async request
    //
    ...
    //
    // Get teh I2cRequest from I2cHostContext for sync request
    //
    I2cRequest = I2cHostContext->SyncI2cRequest;
  }
  //
  ...
  if (I2cHostContext->I2cBusConfiguration != I2cBusConfiguration ) {
    ...
    //
    I2cBusConfigurationManagement->EnableI2cBusConfiguration (
  ...
  if (EFI_ERROR (Status)) {
    ...
    //
    if (AsyncRequest) {
      I2cRequest->MasterStatus = Status;
      gBS->SignalEvent (I2cRequest->MasterEvent);
    }
  ...
  I2cMaster->StartRequest (
               ...
               (!AsyncRequest) ? NULL : I2cRequest->MasterEvent,
               &I2cRequest->MasterStatus
               );
}
  ...
  )
{
  ...
  //
  // Allocate the request structure for async request
  //
  ...
  I2cRequest->DeviceEvent         = Event;
  I2cRequest->Status              = I2cStatus;
  I2cRequest->MasterStatus        = EFI_SUCCESS;
  ...
  if (I2cRequest->RequestPacket == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }
  ...
  if (Event != NULL) {
    //
    // Added the I2cRequest into queue list for async request
    //
    gBS->RaiseTPL ( TPL_I2C_SYNC );
    //
    // Insert new I2C request in the list
    //
    InsertHeadList (&I2cHostContext->RequestList, &I2cRequest->Link);
    //
    ...
    //
    // Set the I2cRequest into I2cHostContext for sync request
    //
    I2cHostContext->SyncI2cRequest = I2cRequest;
  }

  //
  // Start the first I2C request, then the subsequent of I2C request will continue
  //
  I2cHostRequestEnable (I2cHostContext, (BOOLEAN)(Event != NULL));
  if (Event != NULL) {
    //
  ...
  //
  Status = I2cRequest->MasterStatus;
  //
  // Free the pool for sync request
  //
  FreePool (I2cRequest->RequestPacket);
  FreePool (I2cRequest);
  //
