...
STATIC MOUSE_ID_TABLE IncompatibleMouse[] = {
  ...
  { 0x24AE, 0xffff },   // Rapoo Mouse
  { 0x214E, 0xffff },   // Swiftpoint Mouse
  { 0x0951, 0x1729 },   // HyperX Pulsefire FPS Pro Mouse
  ...
  )
{
  ...
  //
  // Set report protocol as default protocol
  //
  Protocol = REPORT_PROTOCOL;
  if (UsbMouseDevice->InterfaceDescriptor.InterfaceSubClass == SUBCLASS_BOOT) {
    //
    // Set boot protocol for the SUBCLASS_BOOT keyboard
    //
    ...
    for (Index = 0; IncompatibleMouse[Index].IdVendor != 0; Index ++) {
      if (IncompatibleMouse[Index].IdVendor == DeviceDescriptor.IdVendor &&
          (IncompatibleMouse[Index].IdProduct == DeviceDescriptor.IdProduct ||
           IncompatibleMouse[Index].IdProduct == 0xffff)) {
        Protocol = REPORT_PROTOCOL;
        break;
      }
    }
    if (Protocol == BOOT_PROTOCOL) {
      ...
      if (!EFI_ERROR (Status) && ConfigDesc.NumInterfaces > 1) {
        //
        // Apply report protocol to multiple interface device to workaround some
  ...
  //
  // Always set corresponding protocol for the USB mouse. Some of device must
