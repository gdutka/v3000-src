  ...
  )
{
  ...
  for (Index = 0; Index < Retry; Index ++) {
    ...
    if ((CmdResult == USB_MASS_CMD_SUCCESS) && !EFI_ERROR(Status)) {
      return (MediaChanged) ? EFI_MEDIA_CHANGED : EFI_SUCCESS;
    ...
    //
    // Retry for media changed until success or no media
    //
    ...
    //
    if (Status == EFI_NOT_READY || Status == EFI_ABORTED) {
  ...
  )
{
  ...
  //
  for (Index = 0, SpaceChar = 0, ManufactureString = (UINT8*)InquiryData + (UINTN)(((USB_BOOT_INQUIRY_DATA*)0)->VendorID); Index < 24; Index ++) {
  ...
  if (SpaceChar == 24 || Index < 24) {
    ...
    //
    CopyMem (ManufactureString, "Generic USB Storage     ", 24);
  }
  //
...
{   
  ...
  if ((Media->MediaId != OldMedia.MediaId) || 
    ...
    //
    // Update MediaId before reinstalling Block I/O Protocol.
    //
  ...
  )
{
  ...
  while (TotalBlock > 0) {
    ...
    //
    Count     = (UINT32)(((TotalBlock * BlockSize) < (USB_BOOT_IO_BLOCKS * 0x200)) ? TotalBlock : ((USB_BOOT_IO_BLOCKS * 0x200) / BlockSize));
    ...
    if (UsbMass->BlockIoMedia.LastBlock <= 0xffffffff) {
      ...
      *(UINT16*)Read10Cmd->TransferLen = SwapBytes16 ((UINT16)Count);
    } else {
      //
  ...
  )
{
  ...
  while (TotalBlock > 0) {
    ...
    if (UsbMass->BlockIoMedia.LastBlock <= 0xffffffff) {
      ...
      *(UINT16*)Write10Cmd->TransferLen = SwapBytes16 ((UINT16)Count);
    } else {
      //
