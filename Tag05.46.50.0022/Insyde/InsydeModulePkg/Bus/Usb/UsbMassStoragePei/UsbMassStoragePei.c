  ...
  )
{
  ...
  while (BlocksRemaining > 0) {
    ...
    if (PeiBlkIoDev->Media.LastBlock < 0xffffffff) {
      ...
      *(UINT16*)&Read10Cmd->TranLen0 = SwapBytes16 ((UINT16)SectorCount);
    } else {
      //
      ...
      *(UINT32*)Read16Cmd->TransferLen = SwapBytes32 ((UINT32)SectorCount);
    }
    ...
    Lba += SectorCount;
    BufferPtr       += (SectorCount * BlockSize);
  ...
  )
{
  ...
  while (BlocksRemaining > 0) {
    ...
    if (PeiBlkIoDev->Media.LastBlock < 0xffffffff) {
      ...
      *(UINT16*)&Write10Cmd->TranLen0 = SwapBytes16 ((UINT16)SectorCount);
    } else {
      //
      ...
      *(UINT32*)Write16Cmd->TransferLen = SwapBytes32 ((UINT32)SectorCount);
    }
    ByteCount                = SectorCount * BlockSize;
    Timeout                  = (UINT16) (SectorCount * 2000);
    //
