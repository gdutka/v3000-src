...
EFI_SMM_USB_DISPATCH2_PROTOCOL  *mUsbDispatch;
EFI_KBC_EMULATOR_PROTOCOL       *mKbcEmulator;
EFI_SMM_CPU_PROTOCOL            *mSmmCpu;
EFI_USB_CORE_SMM_USB_DISPATCH   mUsbLegacySwSmiCallback;
BOOLEAN                         mInSmram;
...
EFI_USB_CORE_PROTOCOL           mUsbCoreProtocol =
{
  UsbGetDescriptor,
  UsbSetDescriptor,
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    mSmmBase->InSmm (mSmmBase, &mInSmram);
  } else {
    mInSmram = FALSE;
  }
  if (!mInSmram) {
    ...
    mPrivate->PciExpressBaseAddress = PcdGet64 (PcdPciExpressBaseAddress);
    //
    ...
    InitializeListHead(&mPrivate->SmmUsbCorePatchListHead);
    InitializeListHead(&mPrivate->UsbSmiRegistrationListHead);
    InitializeListHead(&mPrivate->ExtendMemoryPoolListHead);
    //
    ...
    if (mPrivate->AcpiTimer == 0) {
      //
      // Setup the AcpiSdtProtocol and register the notification if not found
      //
      Status = gBS->LocateProtocol (
      ...
      if (EFI_ERROR (Status)) {
        ...
        AcpiSdtProtocolNotifyFunction (0, 0);
      }
    }
    //
    // Setup the hook of system reset for shutdown procedures
    //
    Status = gBS->LocateProtocol (
    ...
    if (!EFI_ERROR (Status)) {
      //
      // Setup the hook of system reset
      //
      ResetNotifyProtocolFunction (0, 0);
    } else {
      //
      // Register the notification of ResetNotify
      //
      Status = gBS->CreateEvent (
                      EVT_NOTIFY_SIGNAL,
                      TPL_CALLBACK,
    ...
    ModuleRegistration (ImageHandle);
    //
    // Search for list to find out is needed to patch SMM UsbCore
    //
    while (!IsListEmpty(&mPrivate->SmmUsbCorePatchListHead)) {
      SmmUsbCorePatchToken = (SMM_USBCORE_PATCH_TOKEN*)GetFirstNode(&mPrivate->SmmUsbCorePatchListHead);
      *(SmmUsbCorePatchToken->ConvertedAddress) = (VOID*)(UINTN)&mUsbCoreProtocol;
      RemoveEntryList((LIST_ENTRY*)SmmUsbCorePatchToken);
    }
    //
    ...
    InsertAddressConvertTable (
      ACT_LINKING_LIST,
      &mPrivate->UsbSmiRegistrationListHead,
      1
      );
    InsertAddressConvertTable (
      ACT_LINKING_LIST,
      &mPrivate->ExtendMemoryPoolListHead,
      1
      );
  }
  return Status;
  ...
  )
{
  ...
  if (!DevicePath) {
    ...
    //
    mPrivate->UsbLegacySwSmiCallback = mUsbLegacySwSmiCallback = Callback;
    return EFI_SUCCESS;
  }
  ...
  )
{
  EFI_STATUS                    Status;
  USB_SMI_TOKEN                 *UsbSmiToken;
  USB_SMI_REGISTRATION_TOKEN    *UsbSmiRegistrationToken;
  USB_SMI_REGISTRATION_TOKEN    *UsbSmiRegistrationLink;
  ...
  if (mPrivate->ProceedAddressPatching) {
    ...
    //
    // Root periodic timer registration check
    //
    if (mPrivate->ProceedUsbSmiToken->Callback == NULL) {
    ...
    if (mPrivate->ProceedUsbSmiToken->Handle == NULL) {
      ...
      //
      Status = AllocateBuffer(
                 sizeof (USB_SMI_REGISTRATION_TOKEN),
                 ALIGNMENT_32 | SMM_ADDRESS,
                 (VOID **)&UsbSmiRegistrationToken
                 );
      if (Status != EFI_SUCCESS || UsbSmiRegistrationToken == NULL) {
        //
      ...
      UsbSmiToken = &UsbSmiRegistrationToken->UsbSmiToken;
      CopyMem (
        UsbSmiToken,
      ...
      if (EFI_ERROR (Status)) {
        //
        // Free the instance on error condition
        //
        FreeBuffer (
          sizeof (USB_SMI_REGISTRATION_TOKEN),
          UsbSmiRegistrationToken
          );
        //
        // Set ProceedUsbSmiToken as NULL to stop the registration procedure
        //
        mPrivate->ProceedUsbSmiToken = NULL;
        //
        // Inspect the registered record to reuse them if error occurred
        //
        if (!IsListEmpty (&mPrivate->UsbSmiRegistrationListHead)) {
          UsbSmiRegistrationLink = (USB_SMI_REGISTRATION_TOKEN*)&mPrivate->UsbSmiRegistrationListHead;
          if (!IsInUsbMemory (UsbSmiRegistrationLink->Link.ForwardLink, sizeof (USB_SMI_REGISTRATION_TOKEN)) || !IsInUsbMemory (UsbSmiRegistrationLink->Link.BackLink, sizeof (USB_SMI_REGISTRATION_TOKEN))) return EFI_SECURITY_VIOLATION;
          do {
            UsbSmiRegistrationLink = (USB_SMI_REGISTRATION_TOKEN*)GetFirstNode((LIST_ENTRY*)UsbSmiRegistrationLink);
            if (!IsInUsbMemory (UsbSmiRegistrationLink->Link.ForwardLink, sizeof (USB_SMI_REGISTRATION_TOKEN)) || !IsInUsbMemory (UsbSmiRegistrationLink->Link.BackLink, sizeof (USB_SMI_REGISTRATION_TOKEN))) return EFI_SECURITY_VIOLATION;
            if (CompareMem (UsbSmiRegistrationLink->UsbSmiToken.DevicePath, UsbSmiToken->DevicePath, GetDevicePathSize ((EFI_DEVICE_PATH_PROTOCOL*)UsbSmiToken->DevicePath)) == 0) {
              //
              // SMI registration found, reuse it
              //
              UsbSmiRegistrationLink->UsbSmiToken.Context  = UsbSmiToken->Context;
              UsbSmiRegistrationLink->UsbSmiToken.Callback = UsbSmiToken->Callback;
              mPrivate->ProceedUsbSmiToken = &UsbSmiRegistrationLink->UsbSmiToken;
              break;
            }
          } while (!IsNodeAtEnd(&mPrivate->UsbSmiRegistrationListHead, (LIST_ENTRY*)UsbSmiRegistrationLink));
        }
        return EFI_SUCCESS;
      }
      //
      // Insert the token into link list
      //
      InsertTailList (&mPrivate->UsbSmiRegistrationListHead, (LIST_ENTRY*)UsbSmiRegistrationToken);
      //
      ...
      //
      Status = mUsbDispatch->UnRegister (
                               mUsbDispatch,
                               mPrivate->ProceedUsbSmiToken->Handle
                               );
      if (!EFI_ERROR (Status)) {
        UsbSmiRegistrationToken = (USB_SMI_REGISTRATION_TOKEN*)(((UINT8*)mPrivate->ProceedUsbSmiToken) - (UINTN)&(((USB_SMI_REGISTRATION_TOKEN*)0)->UsbSmiToken));
        if (!IsInUsbMemory (UsbSmiRegistrationToken->Link.ForwardLink, sizeof (USB_SMI_REGISTRATION_TOKEN)) || !IsInUsbMemory (UsbSmiRegistrationToken->Link.BackLink, sizeof (USB_SMI_REGISTRATION_TOKEN))) return EFI_SECURITY_VIOLATION;
        RemoveEntryList ((LIST_ENTRY*)UsbSmiRegistrationToken);
        FreeBuffer (
          sizeof(USB_SMI_REGISTRATION_TOKEN),
          UsbSmiRegistrationToken
          );
        ...
        RemoveAddressConvertTable (
          ACT_INSTANCE_POINTER,
          &UsbSmiRegistrationToken->UsbSmiToken.Context
          );
      }
    }
    ...
    // Called from CSM16
    //
    if (mUsbLegacySwSmiCallback == NULL) mUsbLegacySwSmiCallback = mPrivate->UsbLegacySwSmiCallback;
    if (mUsbLegacySwSmiCallback != NULL) mUsbLegacySwSmiCallback (NULL, NULL, NULL, NULL);
  }
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  for (Index = 0; Index < Count; Index ++) {
    if (UsbHid[Index]->SyncLED != NULL) {
      ...
      //
      SyncLED = UsbHid[Index]->SyncLED;
      Status = AddressConvert (
                 ((mInSmram) ? SMM_ADDRESS : NON_SMM_ADDRESS),
                 (VOID*)(UINTN)SyncLED,
                 (VOID**)&SyncLED
                 );
      if (Status != EFI_NOT_FOUND) SyncLED (0, UsbHid[Index]->Instance);
    }
  }
  //
  ...
  )
{
  ...
  ImageGuid = EfiGetNameGuidFromFwVolDevicePathNode ((MEDIA_FW_VOL_FILEPATH_DEVICE_PATH *)LoadedImage->FilePath);
  if (ImageGuid == NULL) {
    return EFI_NOT_FOUND;
  }
  for (Index = 0, ImageAttrib = &mPrivate->ImageAttrib[0]; Index < mPrivate->NumImageAttrib; Index ++, ImageAttrib ++) {
    ...
    //
    if (CompareGuid (&ImageAttrib->ImageGuid, ImageGuid)) break;
  }
  IsInSmm (&InSmm);
  if (Index == mPrivate->NumImageAttrib) {
    ...
    CopyGuid (&ImageAttrib->ImageGuid, ImageGuid);
  ...
  )
{
  ...
  CPU_SAVE_STATE_PACKAGE2     *CpuSaveStatePackage2;
  UINTN                       Width;
  VOID                        *Buffer;
  UINTN                       Index;
  UINT32                      CpuIndex;
  ...
  if (CpuSaveStatePackage->Signature != CPU_SAVE_STATE_SIGNATURE &&
      CpuSaveStatePackage->Signature != CPU_SAVE_STATE_SIGNATURE2) {
    return EFI_INVALID_PARAMETER;
  }
  if (mSmst == NULL) mSmst = mPrivate->Smst;
  if (CpuSaveStatePackage->Signature == CPU_SAVE_STATE_SIGNATURE) {
    CpuIndex  = CpuSaveStatePackage->CpuIndex;
    Direction = CpuSaveStatePackage->Direction;
    DataSize  = CpuSaveStatePackage->DataSize;
    Data      = CpuSaveStatePackage->Data;
  } else {
    CpuSaveStatePackage2 = (CPU_SAVE_STATE_PACKAGE2*)CpuSaveStatePackage;
    CpuIndex  = CpuSaveStatePackage2->CpuIndex;
    Direction = CpuSaveStatePackage2->Direction;
    DataSize  = CpuSaveStatePackage2->DataSize;
    Data      = CpuSaveStatePackage2->Data;
  }
  if (CpuIndex >= mSmst->NumberOfCpus) {
    return EFI_NOT_FOUND;
  }
  for (Index = 0; Index < DataSize; Index ++) {
    Register = Data[Index * 2];
    Buffer   = &Data[(Index * 2) + 1];
    Width    = sizeof (UINT32);
    ...
    if (mSmmCpu == NULL) {
      mSmmCpu = mPrivate->SmmCpu;
      if (mSmmCpu == NULL) return EFI_NOT_FOUND;
    }
    if (Direction) {
      //
      // Data out
...
/**
  ...
  Toggle the switch of the SMI sources
  ...
  )
{
  ...
  if (mPrivate->PeriodicTimerSource == PERIODIC_SMI_TIMER || IsListEmpty(&mPrivate->PeriodicTimerProviderListHead)) return;
  ...
  do {
    ...
    if (OnOff) {
      ProviderToken->Usb3Hc->RegisterEvent (
      ...
      ProviderToken->Usb3Hc->UnregisterEvent (
                               ProviderToken->Usb3Hc,
                               NULL
                               );
    }
  } while (!IsNodeAtEnd(&mPrivate->PeriodicTimerProviderListHead, (LIST_ENTRY*)ProviderToken));
}
  ...
  )
{
  ...
  if (CallbackEvent != LEGACY_FREE_HC_EXIT_BOOT_SERVICE_EVENT && CallbackEvent != LEGACY_FREE_HC_ACPI_ENABLE_EVENT) {
    ...
    // off SMI_BY_EVENT before IRQ timer polling
    //
    SwitchSmiSource (FALSE);
    ...
    do {
      ...
      Callback = LegacyFreeHcCallbackToken->Callback;
	  Status = EFI_SUCCESS;
      if (mInSmram) {
        Status = AddressConvert (
                   SMM_ADDRESS,
      ...
      if (Status != EFI_NOT_FOUND) Callback (CallbackEvent, LegacyFreeHcCallbackToken->Context);
      //
      // Safety check
      //
    ...
    //
    // Turn on SMI_BY_EVENT after IRQ timer polling
    //
    SwitchSmiSource (TRUE);
...
/**
  ...
  Signal handlers for address patch for security at ExitBootServices/LegacyBoot event
  ...
  )
{
  ...
  if (!IsListEmpty(&mPrivate->LegacyFreeHcCallbackListHead)) {
    //
    // Startup periodic timer source and set the type to SMI timer
    //
  ...
  // Patching the boot service address into SMM address through UsbSwSmi
  //
  SecurityPatchNotifyFunction (0, NULL);
}
  ...
  )
{
  ...
  if (mPrivate->CurrMode != USB_CORE_RUNTIME_MODE) {
    ...
    if (!IsListEmpty(&mPrivate->LegacyFreeHcCallbackListHead)) {
      //
      // Startup periodic timer source and set the type to SMI timer before entering legacy mode
      //
      SwitchPeriodicTimerSource ((Type) ? PERIODIC_SMI_TIMER : PERIODIC_IRQ_TIMER);
    }
  } else {
    //
  ...
  )
{
  ...
  //
  // Fill the root periodic timer instance for periodic SMI source
  //
  mPrivate->RootPeriodicTimerHc.GetCapability 	  = RootPeriodicTimerGetCapability;
  mPrivate->RootPeriodicTimerHc.InterruptTransfer = RootPeriodicTimerInterruptTransfer;
  mPrivate->RootPeriodicTimerHc.CancelIo          = RootPeriodicTimerCancelIo;
  mPrivate->RootPeriodicTimerHc.QueryIo           = RootPeriodicTimerQueryIo;
  mPrivate->RootPeriodicTimerHc.RegisterEvent     = RootPeriodicTimerRegisterEvent;
  mPrivate->RootPeriodicTimerHc.UnregisterEvent   = RootPeriodicTimerUnregisterEvent;
}
...
/**
  ...
  Signal handlers for AcpiSdtProtocol installed event
...
{
  if (Table != NULL && mPrivate->AcpiTimer == 0 && Table->Signature == EFI_ACPI_3_0_FIXED_ACPI_DESCRIPTION_TABLE_SIGNATURE) {
    mPrivate->AcpiTimer = ((EFI_ACPI_3_0_FIXED_ACPI_DESCRIPTION_TABLE*)Table)->PmTmrBlk;
    //
    // Don't unregister the notification in the callback because AcpiSdt driver
    // in MdeModulePkg unable to handle it and will cause CPU exception 
    //
  }
  return EFI_SUCCESS;
}
...
/**
  ...
  Signal handlers for ResetNotifyProtocol installed event
...
/**
  This routine is called to properly shutdown the USB periodicSMI before system reset.
...
{
  //
  // Remove USB periodic SMI before system reset
  //
  ...
  )
{
  ...
  EFI_USB_IO_GET_DEVICE_DESCRIPTOR      UsbGetDeviceDescriptorFunc;
  ...
  //
  if (UsbIo) {
    UsbGetDeviceDescriptorFunc = UsbIo->UsbGetDeviceDescriptor;
    if (mInSmram) {
      Status = AddressConvert (
                 SMM_ADDRESS,
