  ...
  )
{
  ...
  if (UsbKeyboardDevice->HidRemappingKeyTable) {
    mUsbCore->InsertAddressConvertTable (
                ACT_INSTANCE_BODY,
                UsbKeyboardDevice->HidRemappingKeyTable,
                UsbKeyboardDevice->HidRemappingKeyCount * sizeof (HID_REMAPPING_KEY)
                );
    mUsbCore->InsertAddressConvertTable (
                ACT_INSTANCE_POINTER,
                &UsbKeyboardDevice->HidRemappingKeyTable,
...
free_kbdevice:
  if (UsbKeyboardDevice->HidRemappingKeyTable != NULL) {
    mUsbCore->FreeBuffer (
                UsbKeyboardDevice->HidRemappingKeyCount * sizeof (HID_REMAPPING_KEY),
                UsbKeyboardDevice->HidRemappingKeyTable
                );
  ...
  )
{
  ...
  // console devices.
  // NOTE: Directly install gEfiConsoleInDeviceGuid protocol to make it binding on ConSplitter.
  //
  Status = gBS->InstallMultipleProtocolInterfaces (
                  &mFakeDev.Handle,
                  &gEfiDevicePathProtocolGuid,
