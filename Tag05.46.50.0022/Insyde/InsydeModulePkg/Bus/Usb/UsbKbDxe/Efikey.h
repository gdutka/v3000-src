...
typedef struct {
  ...
  UINTN                                 NsKeyBufferSize;
  //
  // Consumer usage code convert table to supporting consumer device
  //
  HID_REMAPPING_KEY                     *HidRemappingKeyTable;
  UINTN                                 HidRemappingKeyCount;
} USB_KB_DEV;

#define USB_KB_DEV_BUFFER_SIZE          (sizeof(USB_KB_DEV) + ((MAX_KEY_ALLOWED + 1) * (sizeof(USB_KEY) + sizeof(EFI_KEY_DATA) + sizeof(EFI_KEY_DATA))))
...
extern EFI_USB_CORE_PROTOCOL            *mUsbCore;
...
#define USBKBD_VALID_JP_KEYCODE(key) (((UINT8) (key) >= 0x87) && ((UINT8) (key) <= 0x8B))
//
...
//
#define NUMBER_OF_VALID_JP_USB_KEYCODE                5
#define NUMBER_OF_VALID_MODIFIER_USB_KEYCODE          8
#define NUMBER_OF_VALID_NON_MODIFIER_USB_KEYCODE      0x62
#define NUMBER_OF_VALID_USB_KEYCODE                   0x6F
