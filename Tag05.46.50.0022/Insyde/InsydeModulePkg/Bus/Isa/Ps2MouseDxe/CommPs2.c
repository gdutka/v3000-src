  ...
  )
{
  ...
  *KeyboardEnable = FALSE;
  ...
  //
  // Mcafee will binding IsaIo after reconnect,
  // it will cause can't find IsaIo with SimpleTextIn
  // so need read KBC status to check keyboard status directly
  //
  Status = Out8042Command (IsaIo, READ_CMD_BYTE);
  if (EFI_ERROR (Status)) {
    return Status;
...

{
  ...
  while (1) {
    ...
    case PS2_READ_DATA_BYTE:
      ...
      if (MouseDev->DataPackageSize == 0x04) {
      	Count = 1;
        Status = PS2MouseRead (MouseDev->IsaIo, &Data, &Count, State);
        if (EFI_ERROR (Status)) {
          return EFI_NOT_READY;
    ...
    case PS2_PROCESS_PACKET:
      ...
      RelativeMovementZ = Packet[3];
      RelativeMovementZ = RelativeMovementZ & 0x0F;
      ...
      MouseDev->State.RelativeMovementZ += RelativeMovementZ;
  ...
  )
{
  ...
  //
  Status = WaitOutputFull (IsaIo, (UINTN) PcdGet32 (PcdPS2MouseBATTimeOut));
  if (EFI_ERROR (Status)) {
    return Status;
  }
...
/**
  I/O work flow to cleanup output buffer empty in given time.

  @param IsaIo       Pointer to instance of EFI_ISA_IO_PROTOCOL
  @param MaxTryTime  Mouse max try time

  @retval EFI_TIMEOUT  try time to max
  @retval EFI_SUCCESS  output buffer is empty
**/
EFI_STATUS
CleanupOutputBuffer (
  IN EFI_ISA_IO_PROTOCOL                  *IsaIo
  )
{
  ...
  TryTime = 0;
  Status = EFI_SUCCESS;
  while (!EFI_ERROR (Status) && TryTime < MOUSE_MAX_TRY) {
    Status = In8042Data (IsaIo, &Data);
    TryTime ++;
  }

  if (TryTime == MOUSE_MAX_TRY) {
    return EFI_TIMEOUT;
  }
...
/**
  Issue command to get mouse's device ID.

  @param IsaIo  Pointer to instance of EFI_ISA_IO_PROTOCOL
  @param PackageSize value of PackageSize
  ...
  )
{
  ...
  // Send auxiliary command to set mouse resolution
  //
  Status = Out8042AuxCommand (IsaIo, GET_DEVID, FALSE);
  if (EFI_ERROR (Status)) {
    return Status;
  }
