  ...
  )
{
  ...
  *Weekday = (UINT8) ((Time->Day + (13 * Month - 1) / 5 + Year + Year / 4 - Year / 100 + Year / 400) % 7);
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  RegisterB.Data = RtcRead (RTC_ADDRESS_REGISTER_B);
  RegisterB.Data = RegisterB.Data | FixedPcdGet8 (PcdInitialValueRtcRegisterB);
  ...
  mRecordTick = GetPerformanceCounter ();
  ...
  )
{
  ...
  if (!EfiAtRuntime ()) {
    //
    // For performance concern, return record time within 90 ms.
    //
    CurrentTick = GetPerformanceCounter ();
    if (GetTimeInNanoSecond (CurrentTick - mRecordTick) < 90000000) {
      //
