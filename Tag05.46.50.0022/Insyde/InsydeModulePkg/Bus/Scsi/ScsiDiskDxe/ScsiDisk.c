...
UFS_STORAGE_SECURITY_COMMAND_SEND_FUNCTION_TABLE     mSendOpTable[] = {
  { SEND_WP_ENABLE                 , UfsWriteProtectionEnable              }, \
  { SEND_WP_DISABLE                , UfsWriteProtectionDisable             }, \
  { SEND_RPMB_WR_REQ               , UfsRpmbWriteRequest                   }, \
  { SEND_RPMB_KEY_PRG              , UfsRpmbProgramAuthKey                 }, \
  { 0                              , NULL                                  }
  };

UFS_STORAGE_SECURITY_COMMAND_RCV_FUNCTION_TABLE      mRcvOpTable[] = {
  { RCV_IS_RPMB_PROGRAMMED         , RcvIsRpmbKeyProgrammed                },\
  { 0                              , NULL                                  }
  };
...
/**
  Execute Mode Select(10) SCSI command on a specific SCSI target.
  ...
  If UfsHostData is NULL, then ASSERT().
  ...
  @param[in]      UfsHostData          Pointer to UFS_HOST_DATA structure
  @param[in]      Lun                  The target Logical Unit Number
  @param[in]      Timeout              The length of timeout period.
  @param[in, out] SenseData            A pointer to output sense data.
  @param[in, out] SenseDataLength      The length of output sense data.
  @param[out]     HostAdapterStatus    The status of Host Adapter.
  @param[out]     TargetStatus         The status of the target.
  @param[in, out] DataBuffer           Sense 10 command data.
  ...
  )
{
  ...
  if (SenseDataLength   == NULL || 
      HostAdapterStatus == NULL || 
      TargetStatus      == NULL ||
      DataLength        == NULL) {
  ...
  ZeroMem (Cdb, UFS_SCSI_OP_LENGTH_TEN);
  ...
  //
  // Fill Cdb for Mode Select (10) Command
  //
  Cdb[0]                          = EFI_SCSI_OP_MODE_SEL10;
  //
  // PAGE FORMAT is in Cdb[1] bit4 of (bit7..0)
  //
  Cdb[1]                          |= BIT4;
  Cdb[7]                          = (UINT8) (*DataLength >> 8);
  Cdb[8]                          = (UINT8) (*DataLength);

  CommandPacket.CdbLength         = UFS_SCSI_OP_LENGTH_TEN;
...
/**
  Determine if UFS write protect function should be supported.
...
**/
BOOLEAN
DetermineUfsWriteProtectSupport (
  ...
  )
{
  ...
  if (UfsDevice->Lun != UFS_LUN_0 &&
      UfsDevice->Lun != UFS_LUN_1 &&
      UfsDevice->Lun != UFS_LUN_2) {
    RetVal = FALSE;
  }
...
/**
  Check whether RPMB is ready.

  @param[in]     This                          Pointer to the SSCP
  @param[in,out] PayloadBuffer                 A pointer to a destination PayloadBuffer
  @param[in,out] PayloadBufferSize             Size in bytes of the payload data buffer
...
**/
EFI_STATUS
RcvIsRpmbKeyProgrammed (
  IN     EFI_STORAGE_SECURITY_COMMAND_PROTOCOL        *This,
  IN OUT VOID                                         *PayloadBuffer,
  IN OUT UINTN                                        *PayloadBufferSize
  )
{
  EFI_STATUS                    Status;
  ...
  Status = ScsiDiskDevice->ScsiIo->GetDeviceLocation (
  ...
  if (Lun != RPMB_LU && Lun != UFS_WLUN_RPMB) {
    return EFI_UNSUPPORTED;
  }
  
  if (*PayloadBufferSize < sizeof (UINT8)) {
    *PayloadBufferSize = sizeof (UINT8);
    return EFI_WARN_BUFFER_TOO_SMALL;
  }

  //
  // To determine if RPMB auth key is programmed by reading data from RPMB
  //
  BlockIo      = &ScsiDiskDevice->BlkIo;
  BlockIoMedia = &ScsiDiskDevice->BlkIoMedia;
  ...
  if (!EFI_ERROR (Status)) {
    //
    // RPMB is ready for use
    //
   	*(UINT8 *)PayloadBuffer = 1;
  } else if (Status == EFI_NOT_READY) {
    //
    // RPM auth key is not programed
    //
    *(UINT8 *)PayloadBuffer = 0;
  } else {
    //
    // General fail
    //
...
/**
  Assign the authentication key for RPMB data write.

  @param[in] This                          Pointer to the SSCP
  @param[in] Packet                        Pointer to H2O SSCP packet
...
UfsRpmbWriteRequest (
  IN     EFI_STORAGE_SECURITY_COMMAND_PROTOCOL        *This,
  IN     H2O_STORAGE_SECURITY_COMMAND_SEND_HEADER     *Packet
  )
{
  UINT8                 *AuthKey;
  
  if (Packet->MessageLength != RPMB_PACKET_KEY_MAC_SIZE) {
    DEBUG ((EFI_D_ERROR, "RPMB Write Request - incorrect key length\n"));
    return EFI_INVALID_PARAMETER;
  }

  ZeroMem (mAuthKey, RPMB_PACKET_KEY_MAC_SIZE);
  
  AuthKey = (UINT8 *)(Packet + 1);
  CopyMem (mAuthKey, AuthKey, RPMB_PACKET_KEY_MAC_SIZE);
  DEBUG ((EFI_D_INFO, "RPMB Write Request (EFI_SUCCESS)\n"));
  return EFI_SUCCESS;
}

/**
  Program the authentication key on the RPMB.
...
**/
EFI_STATUS
UfsRpmbProgramAuthKey (
  IN     EFI_STORAGE_SECURITY_COMMAND_PROTOCOL        *This,
  IN     H2O_STORAGE_SECURITY_COMMAND_SEND_HEADER     *Packet
  )
{
  EFI_STATUS                    Status;
  RPMB_DATA_PACKET              CmdPacket;
  RPMB_DATA_PACKET              ResultPacket;
  UINT8                         *AuthKey;
  ...
  if (Packet->MessageLength != RPMB_PACKET_KEY_MAC_SIZE) {
    DEBUG ((EFI_D_ERROR, "RpmbProgramAuthKey - incorrect key length\n"));
    return EFI_INVALID_PARAMETER;
  }

  Timeout   = SCSI_CMD_TIMEOUT;
  ScsiDiskDevice    = SCSI_DISK_DEV_FROM_STORSEC (This);
  //
  // Zero out frame
  //
  ZeroMem(&CmdPacket, sizeof(CmdPacket));

  //
  // Fill in the request message type: 0x0001
  // (Set RPMB authentication key request)
  //
  CmdPacket.ReqResp[0] = (UINT8)(RPMB_KEY_PROGRAMM_REQUEST & 0xFF00);
  CmdPacket.ReqResp[1] = (UINT8)(RPMB_KEY_PROGRAMM_REQUEST & 0x00FF);

  //
  // Fill in the authentication key
  //
  AuthKey = (UINT8 *)(Packet + 1);  
  CopyMem (CmdPacket.KeyMAC, AuthKey, RPMB_PACKET_KEY_MAC_SIZE);
  SenseDataLength = 0;
  Status = ScsiSecurityProtocolOutCommand (
               ...
               &HostAdapterStatus,
               &TargetStatus,
               SECURITY_PROTOCOL_ID_UFS,
               SECURITY_PROTOCOL_SPECIFC_UFS_RPMB,
               FALSE,
               sizeof(RPMB_DATA_PACKET),
               (VOID*)&CmdPacket
             );
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "RpmbProgramAuthKey: COMMAND (Security Protocol Out) RPMB_KEY_PROGRAMM_REQUEST (%r)\n", Status));
    return Status;
  }
  ...
  //
  // Fill in the request message type: 0x0005
  // (Get RPMB result register request)
  //
  CmdPacket.ReqResp[0] = (UINT8)(RPMB_RESULT_READ_REQUEST & 0xFF00);
  CmdPacket.ReqResp[1] = (UINT8)(RPMB_RESULT_READ_REQUEST & 0x00FF);
  SenseDataLength = 0;
  Status = ScsiSecurityProtocolOutCommand (
               ScsiDiskDevice->ScsiIo,
               Timeout,
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "RpmbProgramAuthKey: COMMAND (Security Protocol Out) RPMB_RESULT_READ_REQUEST (%r)\n", Status));
    return Status;
  }
  ...
  // Zero out frame
  //
  ZeroMem(&ResultPacket, sizeof(ResultPacket));
  TransferSize = sizeof(ResultPacket);
  ...
  if ((ResultPacket.Result[1] & 0x7F) != 0) {
    DEBUG((DEBUG_ERROR, "RpmbProgramAuthKey: Result / Response Information\n"));  
    DEBUG((DEBUG_ERROR, "RpmbProgramAuthKey: Result[0]  = 0x%x\n", ResultPacket.Result[0]));
    DEBUG((DEBUG_ERROR, "RpmbProgramAuthKey: Result[1]  = 0x%x\n", ResultPacket.Result[1]));
    DEBUG((DEBUG_ERROR, "RpmbProgramAuthKey: ReqResp[0] = 0x%x\n", ResultPacket.ReqResp[0]));
    DEBUG((DEBUG_ERROR, "RpmbProgramAuthKey: ReqResp[1] = 0x%x\n", ResultPacket.ReqResp[1]));
...
/**
  Set fPowerOnWPEn bit to enable power on write protect.
  if Force is set, bLUWriteProtect is forced to be configured to desired.
  CAUTION! Force configuration may lead to data loss!
  
  @param[in] Enable               Enable power-on WP
  @param[in] Force                Forcibly change WP configuration

  @retval EFI_SUCCESS             Operation succeeded
  @retval EFI_INVALID_PARAMETER   Unsupported LUN input
  @retval EFI_WRITE_PROTECTED     Power-on WP has been enabled and can't be modified until not power-cycle
  @retval Other                   Operation failed

**/
EFI_STATUS
UfsWriteProtectionPOWP (
  IN      EFI_HANDLE              ChildHandle,
  IN      BOOLEAN                 Enable,
  IN      BOOLEAN                 Force,
  IN      UINT64                  Lun  
  )
{
  ...
  UINT8  bLUWriteProtect = 0;

  //
  // Firstly, check if ExtScsiPassThru Protocol parent handle exists. If existence,
  // check its attribute, logic or physical.
  //
  UfsDeviceConfig = GetDevConfig(ChildHandle);
  if (UfsDeviceConfig == NULL) {
    return EFI_NOT_FOUND;
  }
  
  WpType = Enable ? LU_PWN_ON_WP : LU_NO_WP;
  Status = UfsDeviceConfig->RwUfsFlag (
                            UfsDeviceConfig,
                            TRUE,
                            QUERY_FLAG_IDN_FPOWERONWPEN,
                            &PowerOnWPEn
                            );
  if (EFI_ERROR(Status)) {
    return Status;
  ...
  bLUWriteProtect = ConfigDesc.Unit[Lun].bLUWriteProtect;
  //
  // fPowerOnWPEn is set and can't be reversed until next power-cycle
  //
  if (PowerOnWPEn) {
    //
    // Check if the LU is power-on WP already
    //
    if (ConfigDesc.Unit[Lun].bLUWriteProtect == WpType) {
      return EFI_SUCCESS;
    }
    return EFI_WRITE_PROTECTED;
  }
    
  if (Force && ConfigDesc.Unit[Lun].bLUWriteProtect != WpType) {
    //
    // Enforce the LU to be configured to required WP type.
    // CAUTION!! MAY LEAD TO DATA LOSS!!
    //
    ConfigDesc.Unit[Lun].bLUWriteProtect = WpType;
    Status = UfsDeviceConfig->RwUfsDescriptor (
                                UfsDeviceConfig,
                                FALSE,
                                QUERY_DESC_IDN_CONFIGURATION,
  ...
  if (ConfigDesc.Unit[Lun].bLUWriteProtect != WpType) {
    DEBUG ((DEBUG_ERROR, "bLUWriteProtect(%x) is not the desire(%x)\n", ConfigDesc.Unit[Lun].bLUWriteProtect, Enable));
    return EFI_UNSUPPORTED;
  }
  ...
  if (Enable) {
    ...
    if (EFI_ERROR (Status) || !PowerOnWPEn) {
      DEBUG ((DEBUG_ERROR, "Set fPowerOnWPEn enable failed, (fPowerOnWPEn = (%x))\n", PowerOnWPEn));
      return EFI_DEVICE_ERROR;
    }
  }
  return Status;
...
/**
  Execute Mode Sense(10) command to read Control mode page of specific LUN,
  set WP bit and SWP bit in the command argument, and execute Mode Select(10) command
  to modified the WP bit and SWP bit of specific LUN to enable or disable Software
  Write Protection.

  @param[in] Enable               Enable SWP
...
**/
EFI_STATUS
UfsWriteProtectionSWP (
  IN      EFI_SCSI_IO_PROTOCOL    *ScsiIo,
  IN      BOOLEAN                 Enable
  )
{
  UINT8               Buffer[MODE_PARAMETER_HEADER_SIZE + CONTROL_MODE_PAGE_SIZE];
  UINT32              ByteCount;
  UINT64              Timeout;
  EFI_STATUS          Status;
  UINT8               Retry;
  ...
  //
  // DBD always set to 1 for Mode Sense 10 command
  //
  DBDField        = 1;
  Status          = EFI_UNSUPPORTED;
  ByteCount       = MODE_PARAMETER_HEADER_SIZE + CONTROL_MODE_PAGE_SIZE;
  SenseDataLength = 0;

  Timeout   = SCSI_CMD_TIMEOUT;
  MaxRetry  = 2;
  for (Retry = 0; Retry < MaxRetry; Retry++) {
   Status  = ScsiModeSense10Command (
               ScsiIo,
               Timeout,
               NULL,
               &SenseDataLength,
               ...
               &ByteCount,
               DBDField,
               UFS_PAGE_CONTROL_CURRENT_VALUES,
               UFS_PAGE_CODE_CONTROL
               );
    if (!EFI_ERROR (Status)) {
      break;
    }
  ...
  //
  // Set/clear software write protect (SWP) bit in Control Page
  //
  if (Enable) {
    Buffer[12] |= BIT3;
  } else {
  ...
  for (Retry = 0; Retry < MaxRetry; Retry++) {
    Status = ScsiModeSelect10Command (
               ScsiIo,
               Timeout,
               NULL,
               &SenseDataLength,
  ...
  )
{
  ...
  if (DetermineUfsWriteProtectSupport(ScsiDiskDevice, ScsiDiskDevice->Handle)) {
    if (Packet->Control == SOFTWARE_WP) {
      Status = UfsWriteProtectionSWP (ScsiDiskDevice->ScsiIo, TRUE);
    } else if (Packet->Control == POWER_ON_WP) {
       Status = ScsiDiskDevice->ScsiIo->GetDeviceLocation (
                                          ScsiDiskDevice->ScsiIo,
                                          &Target,
                                          &Lun
       ...
       Status = UfsWriteProtectionPOWP (ScsiDiskDevice->Handle, TRUE, (BOOLEAN)Packet->Identifier, Lun);
    } else {
...
/**
  Disable write protection of the device.
  ...
  )
{
  ...
  if (DetermineUfsWriteProtectSupport(ScsiDiskDevice, ScsiDiskDevice->Handle)) {
    if (Packet->Control == SOFTWARE_WP) {
      Status = UfsWriteProtectionSWP (ScsiDiskDevice->ScsiIo, FALSE);
    } else if (Packet->Control == POWER_ON_WP) {
       Status = ScsiDiskDevice->ScsiIo->GetDeviceLocation (
                                          ScsiDiskDevice->ScsiIo,
                                          &Target,
       ...
       Status = UfsWriteProtectionPOWP (ScsiDiskDevice->Handle, FALSE, (BOOLEAN)Packet->Identifier, Lun);
    } else {
      Status = EFI_INVALID_PARAMETER;
    }
  }
...
/**
  Check sense key to find if write protected.

  @param  SenseData    The pointer of EFI_SCSI_SENSE_DATA
  @param  SenseCounts  The number of sense key

  @retval TRUE  It is write protected.
  @retval FALSE It is NOT write protected.

**/
BOOLEAN
ScsiDiskIsWriteProtected (
  ...
  )
{
  ...
  for (Index = 0; Index < SenseCounts; Index++) {
    //
    // Sense Key is EFI_SCSI_SK_DATA_PROTECT (0x7)
    // Additional Sense Code is EFI_SCSI_ASC_WRITE_PROTECTED (0x27)
    //
    if ((SensePtr->Sense_Key == EFI_SCSI_SK_DATA_PROTECT) &&
        (SensePtr->Addnl_Sense_Code == EFI_SCSI_ASC_WRITE_PROTECTED)) {
      return TRUE;
    }
  ...
  )
{
  ...
  //
  // If NewTpl is below the current TPL level, then the system behavior is indeterminate.
  // To raise TPL level to TPL_CALLBACK only if the current level is below TPL_CALLBACK.
  //
  ...
  )
{
  ...
  if (SecurityProtocolId == H2O_SECURITY_PROTOCOL_ID) {
    BufferSize = PayloadBufferSize;
    Status     = EFI_UNSUPPORTED;
    Count = ARRAY_SIZE(mRcvOpTable);
    for (Index = 0; Index < Count ; Index++) {
      if (SecurityProtocolSpecificData == mRcvOpTable[Index].OpNum) {
        Status = mRcvOpTable[Index].SubFunction (This, AlignedBuffer, &BufferSize);
        break;
      }
    }
    goto Done;
  ...
  )
{
  ...
  BOOLEAN             AlignedBufferAllocated;
  UINT8               Index;
  H2O_STORAGE_SECURITY_COMMAND_SEND_HEADER     *Packet;
  UINT8               Count;
  ...
  if (SecurityProtocolId == H2O_SECURITY_PROTOCOL_ID) {
    Packet = (H2O_STORAGE_SECURITY_COMMAND_SEND_HEADER *) PayloadBuffer;
    if (Packet->HeaderSize  != sizeof (H2O_STORAGE_SECURITY_COMMAND_SEND_HEADER)) {
      Status = EFI_UNSUPPORTED;
      goto Done; 
    }
    Count = ARRAY_SIZE(mSendOpTable);
    for (Index = 0; Index < Count; Index++) {
      if (SecurityProtocolSpecificData == mSendOpTable[Index].OpNum) {
        Status = mSendOpTable[Index].SubFunction (This, Packet);
        break;
      }
    }
    goto Done; 
  ...
  )
{
  ...
  *Action = ACTION_READ_CAPACITY;
  ...
  if (ScsiDiskIsWriteProtected (SenseData, NumberOfSenseKeys)) {
    DEBUG ((EFI_D_VERBOSE, "ScsiDisk: ScsiDiskIsWriteProtected!\n"));
    *Action = ACTION_NO_ACTION;
    return EFI_WRITE_PROTECTED;
  }
...
/**
  Search protocol database and find device config protocol for the
  parent UFS controller of input child handle
  ...
  )
{
  ...
  for (Index = 0; Index < HandleCount; Index++) {
    Status = EfiTestChildHandle (HandleBuffer[Index], ChildHandle, &gEfiExtScsiPassThruProtocolGuid);
    if (!EFI_ERROR (Status)) {
      Status = gBS->HandleProtocol (HandleBuffer[Index], &gEfiUfsDeviceConfigProtocolGuid, (VOID **)&Interface);
      if (!EFI_ERROR (Status)) {
