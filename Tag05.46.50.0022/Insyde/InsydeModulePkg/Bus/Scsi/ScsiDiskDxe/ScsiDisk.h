...
#define SCSI_CMD_TIMEOUT           10000000
#define MODE_PARAMETER_HEADER_SIZE               0x8
#define CONTROL_MODE_PAGE_SIZE                   0xC
#define SECURITY_PROTOCOL_ID_UFS                0xEC
#define SECURITY_PROTOCOL_SPECIFC_UFS_RPMB      0x01
...
//
// Size of nonce in RPMB data packet
//
#define RPMB_NONCE_SIZE                         16
//
// RPMB HMAC size
//
#define RPMB_PACKET_STUFF_SIZE                  196
#define RPMB_PACKET_KEY_MAC_SIZE                32
#define RPMB_PACKET_HASH_SIZE                   (512 - RPMB_PACKET_STUFF_SIZE - RPMB_PACKET_KEY_MAC_SIZE)
//
// RPMB request message types
//
#define RPMB_KEY_PROGRAMM_REQUEST               0x0001
#define RPMB_READ_COUNTER_REQUEST               0x0002
#define RPMB_DATA_WRITE_REQUEST                 0x0003
#define RPMB_DATA_READ_REQUEST                  0x0004
#define RPMB_RESULT_READ_REQUEST                0x0005
...
typedef struct  {
  ...
  UINT8  Reserved14;
  UINT8  Reserved15;
} CONFIGURABLE_UNIT_DESCRIPTOR;

typedef struct  {
  CONFIGURATION_HEADER          Header;
  CONFIGURABLE_UNIT_DESCRIPTOR  Unit[8];
} CONFIGURATION_DESCRIPTOR;



typedef struct _RPMB_DATA_PACKET {
  UINT8        StuffBytes[196];
  UINT8        KeyMAC[32];       // Either the auth key to be programmed or the MAC authenticating this packet or series of packets
  UINT8        Data[256];        // The data input or output
  UINT8        Nonce[16];        // Random 128-bit number generated by host
  UINT8        WriteCounter[4];  // 32-bit counter
  UINT8        Address[2];       // The half-sector address to operate on
  UINT8        BlockCount[2];    // The count of half-sector blocks to read/write
  UINT8        Result[2];        // The result of the operation
  UINT8        ReqResp[2];       // The type of request or response
} RPMB_DATA_PACKET;
...
enum {
  ...
  QUERY_FLAG_IDN_FPOWERONWPEN     = 0x03,
...
//
enum {
 	UFS_PAGE_CONTROL_CURRENT_VALUES     = 0x0,
 	UFS_PAGE_CONTROL_CHANGEABLE_VALUES  = 0x1,
 	UFS_PAGE_CONTROL_DEFAULT_VALUES     = 0x2,
 	UFS_PAGE_CONTROL_SAVED_VALUES       = 0x3,
};
...
//
enum {
 	UFS_PAGE_CODE_CONTROL           = 0x0A,
 	UFS_PAGE_CODE_RW_ERROR_RECOVERY = 0x01,
 	UFS_PAGE_CODE_CACHING           = 0x08,
 	UFS_PAGE_CODE_ALL_PAGES         = 0x3F,
};
...
typedef
EFI_STATUS
(* UFS_STORAGE_SECURITY_COMMAND_SEND_FUNCTION) (
...
typedef
EFI_STATUS
(* UFS_STORAGE_SECURITY_COMMAND_RCV_FUNCTION) (
  IN     EFI_STORAGE_SECURITY_COMMAND_PROTOCOL        *This,
...
typedef struct {
  UINT8                                               OpNum;
  UFS_STORAGE_SECURITY_COMMAND_SEND_FUNCTION          SubFunction;
} UFS_STORAGE_SECURITY_COMMAND_SEND_FUNCTION_TABLE;

typedef struct {
  UINT8                                               OpNum;
  UFS_STORAGE_SECURITY_COMMAND_RCV_FUNCTION           SubFunction;
} UFS_STORAGE_SECURITY_COMMAND_RCV_FUNCTION_TABLE;
...
#define SCSI_DISK_DEV_FROM_SCSIIO(a) CR (a, SCSI_DISK_DEV, ScsiIo, SCSI_DISK_DEV_SIGNATURE)
