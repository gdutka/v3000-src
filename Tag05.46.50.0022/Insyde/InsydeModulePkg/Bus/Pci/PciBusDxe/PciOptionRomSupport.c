  ...
  )
{
  ...
  //
  SizeInDWord = (sizeof (PCI_EXPANSION_ROM_HEADER) + sizeof (UINT32) - 1) / sizeof (UINT32);
  RomHeader = AllocatePool (SizeInDWord * sizeof (UINT32));
  ...
  RomBarAddress = 0; 
  if (PcdGetBool (PcdH2OOptionRomBarReserved)) { 
  ...
  if (!EFI_ERROR (RetStatus)) {
    DEBUG ((EFI_D_INFO, " OptionROM found 0x%02x/0x%02x/0x%02x, ImageAddr=0x%lx ......\n", PciDevice->BusNumber, PciDevice->DeviceNumber, PciDevice->FunctionNumber, (UINTN) RomInMemory));
  }
