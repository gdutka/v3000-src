  ...
  )
{
  ...
  if ((*Offset + Count * (UINTN)(1ULL << Width)) - 1 >= PciIoDevice->PciBar[BarIndex].Length) {
    return EFI_INVALID_PARAMETER;
  }
  ...
  )
{
  ...
  if (PciIoDevice->IsPciExp) {
    if ((*Offset + Count * (UINTN)(1ULL << Width)) - 1 >= PCI_EXP_MAX_CONFIG_OFFSET) {
    ...
    *Offset       = (*Offset) | ExtendOffset;
    ...
    if ((*Offset + Count * (UINTN)(1ULL << Width)) - 1 >= PCI_MAX_CONFIG_OFFSET) {
      return EFI_UNSUPPORTED;
    }
