...
EFI_HANDLE                                    gPciHostBridgeHandles[PCI_MAX_HOST_BRIDGE_NUM];
  ...
  )
{
  ...
  //
  //
  DEBUG ((EFI_D_INFO, " PciEnumerator - start\n"));
  Status = PciEnumerator (Controller);
  DEBUG ((EFI_D_INFO, " PciEnumerator - end with status %r\n", Status));
  ...
  //
  DEBUG ((EFI_D_INFO, " StartPciDevices - start\n"));
  StartPciDevices (Controller);
  DEBUG ((EFI_D_INFO, " StartPciDevices - end with status %r\n", Status));
