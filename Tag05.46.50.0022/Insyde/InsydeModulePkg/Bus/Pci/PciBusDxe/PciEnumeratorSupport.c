  ...
  )
{
  ...
  if (!EFI_ERROR (Status) && (Pci->Hdr).VendorId != 0xffff) {
    ...
    //
    DEBUG ((EFI_D_INFO, " %02x/%02x/%02x Present, VID 0x%04x, DID 0x%04x\n", Bus, Device, Func, (Pci->Hdr).VendorId, (Pci->Hdr).DeviceId));
    Status = PciRootBridgeIo->Pci.Read (
                                    PciRootBridgeIo,
                                    EfiPciWidthUint32,
                                    Address,
...
/**
  ...
  @retval   EFI_SUCCESS     - The Pci device should be skipped.
  @retval   EFI_NOT_FOUND   - There is no PciSkipTable, or the device is not found in PciSkipTable.
  @retval   EFI_ABORTED     - The Pci device is found in PciSkipTable, but it's unskipped by the UnSkipPciDevice OemService.
  ...
  )
{
  ...
  PCI_SKIP_TABLE                            *PciSkipTable     = NULL;
  PCI_SKIP_TABLE                            *TempPciSkipTable = NULL;
  EFI_STATUS                                Status;
  ...
  // OemServices
  //
  DEBUG_OEM_SVC ((DEBUG_INFO, "OemKernelServices Call: OemSvcInstallPciSkipTable \n"));
  Status = OemSvcInstallPciSkipTable (
	     &PciSkipTableCount,
             &TempPciSkipTable
	     );
  DEBUG_OEM_SVC ((DEBUG_INFO, "OemKernelServices OemSvcInstallPciSkipTable Status: %r\n", Status));

  if (!EFI_ERROR (Status) || PciSkipTableCount == 0 || TempPciSkipTable == NULL) {
    return EFI_NOT_FOUND;
  }
      PciSkipTable = AllocatePool (PciSkipTableCount * sizeof (PCI_SKIP_TABLE));
      if (PciSkipTable == NULL) {
        return EFI_OUT_OF_RESOURCES;
      }
      CopyMem (
        PciSkipTable,
        TempPciSkipTable,
        PciSkipTableCount * sizeof (PCI_SKIP_TABLE)
        );
    if (TempPciSkipTable != NULL) {
      FreePool(TempPciSkipTable);
      TempPciSkipTable = NULL;
    }
  //
  // Loop through table of video option rom descriptions
  //
  for (TableIndex = 0; PciSkipTable[TableIndex].VendorId != 0xffff; TableIndex++) {
    //
    // See if the PCI device specified by PciHandle matches at device in mPciSkipTable
    //
    if (VendorId != PciSkipTable[TableIndex].VendorId ||
        DeviceId != PciSkipTable[TableIndex].DeviceId) {
      continue;
    }
    //
    // To handle a special case:
    // more than 1 PCI devices which has the same VID/DID exist on the system but not all of them has to be skipped
    // So provide an OEM service, let project owner to decide if this device has to be skipped or not.
    // if EFI_SUCCESS return, means this device should NOT be skipped.
    // Otherwise, this device has to be skip.
    // (OemServices)
    //
    DEBUG_OEM_SVC ((DEBUG_INFO, "OemKernelServices Call: OemSvcUnSkipPciDevice \n"));
    ...
    DEBUG_OEM_SVC ((DEBUG_INFO, "OemKernelServices OemSvcUnSkipPciDevice Status: %r\n", Status));

    if (!EFI_ERROR (Status)) {
      //
      // The Pci device should NOT be skipped.
      // i.e, SkipPci should return ERROR.
      //
      FreePool(PciSkipTable);
      return EFI_ABORTED;
    }

    //
    // The Pci device was found in PciSkipTable, and it should be skipped.
    //
    FreePool(PciSkipTable);
  ...
  //
  // The Pci device was NOT found in PciSkipTable, SkipPci returns ERROR.
  //
  FreePool(PciSkipTable);
  ...
  )
{
  ...
  for (Device = 0; Device <= PCI_MAX_DEVICE; Device++) {
    ...
    for (Func = 0; Func <= PCI_MAX_FUNC; Func++) {
      ...
      if (!EFI_ERROR (Status)) {
      	//
      	// To check PciSkip table, if the Pci device is one of entry in PciSkip,
      	//   return EFI_SUCCESS and skip add the device to Pci Root Bridge.
      	//
          ...
          if (Status == EFI_SUCCESS ) {
            DEBUG ((EFI_D_INFO, " %02x/%02x/%02x was skipped, VID 0x%04x, DID 0x%04x\n", StartBusNumber, Device, Func, Pci.Hdr.VendorId, Pci.Hdr.DeviceId));
            continue;
          }
        }
        ...
        DEBUG ((EFI_D_INFO, " %02x/%02x/%02x PciSearchDevice Status %r, VID 0x%04x, DID 0x%04x\n", StartBusNumber, Device, Func, Status, Pci.Hdr.VendorId, Pci.Hdr.DeviceId));
        //
  ...
  )
{
  ...
  if (!IS_PCI_BRIDGE (Pci)) {
    ...
    if (IS_CARDBUS_BRIDGE (Pci)) {
      ...
      DEBUG ((EFI_D_INFO, " %02x/%02x/%02x PCI device found\n", Bus, Device, Func));
      //
      ...
      DEBUG ((EFI_D_INFO, " GatherDeviceInfo - 0x%lx\n", PciIoDevice));
    }

  } else {
    DEBUG ((EFI_D_INFO, " %02x/%02x/%02x PCI bridge found\n", Bus, Device, Func));
    //
    ...
    DEBUG ((EFI_D_INFO, " GatherPpbInfo - bridge 0x%02x/0x%02x/0x%02x at 0x%lx\n", Bus, Device, Func, PciIoDevice));
    //
  ...
  if (FeaturePcdGet (PcdH2OBdsCpPciEnumUpdateDevInfoSupported)) {
    H2O_BDS_CP_PCI_ENUM_UPDATE_DEV_INFO_DATA           PciEnumUpdateDevInfoData;
    H2O_PCI_IO_DEVICE                                  H2OPciIoDevice;

    PciEnumUpdateDevInfoData.Size                 = sizeof (H2O_BDS_CP_PCI_ENUM_COMPLETE_AFTER_DATA);
    PciEnumUpdateDevInfoData.Status               = H2O_CP_TASK_NORMAL;
    PciEnumUpdateDevInfoData.PciDevice            = &H2OPciIoDevice;
    ...
    H2OCpTrigger (&gH2OBdsCpPciEnumUpdateDevInfoGuid, &PciEnumUpdateDevInfoData);
    DEBUG_CP ((DEBUG_INFO, "Checkpoint Result: %x\n", PciEnumUpdateDevInfoData.Status));
    if (PciEnumUpdateDevInfoData.Status == H2O_CP_TASK_UPDATE) {
  ...
  if (gFullEnumeration) {
    ...
    if (PcdGetBool (PcdH2OPciOptionRomSupported) && !IS_CARDBUS_BRIDGE (Pci)) {
...
/**
  Parse PCI option ROM bar information and fill them into PCI device instance.

  @param[in] PciIoDevice  PCI device instance.
  @param[in] Device       BOOLEAN value to indicate this is a PCI device or not.

  @retval EFI_SUCCESS     Parse option ROM bar information successfully.
  @retval EFI_NOT_FOUND   input PCI device doesn't have option ROM.
**/
STATIC
EFI_STATUS
PciParseOptionRomBar (
  IN  PCI_IO_DEVICE      *PciIoDevice,
  IN  BOOLEAN            Device
  )
{
  ...
  PciIoDevice->OptionRomBar.BarType   = PciBarTypeMem32;
  PciIoDevice->OptionRomBar.Length    = (~(Value & 0xFFFFF800)) + 1;
  if (PciIoDevice->OptionRomBar.Length < (SIZE_4KB)) {
    PciIoDevice->OptionRomBar.Alignment = (SIZE_4KB - 1);
  } else {
    PciIoDevice->OptionRomBar.Alignment = PciIoDevice->OptionRomBar.Length - 1;
  }
  PciIoDevice->OptionRomBar.Offset    = Offset;
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  if (PcdGetBool (PcdH2OOptionRomBarReserved) && PcdGetBool (PcdH2OPciOptionRomSupported)) {
    PciParseOptionRomBar (PciIoDevice, TRUE);
  }
  //
  ...
  )
{
  ...
  if (PcdGetBool (PcdH2OOptionRomBarReserved) && PcdGetBool (PcdH2OPciOptionRomSupported)) {
    PciParseOptionRomBar (PciIoDevice, FALSE);
  }
  PciIo = &PciIoDevice->PciIo;
  ...
  )
{
  ...
  if ((Command & EFI_PCI_COMMAND_IO_SPACE) != 0) {
    Attributes |= (UINT64)EFI_PCI_IO_ATTRIBUTE_IO;
  }

  if ((Command & EFI_PCI_COMMAND_MEMORY_SPACE) != 0) {
    Attributes |= (UINT64)EFI_PCI_IO_ATTRIBUTE_MEMORY;
  }

  if ((Command & EFI_PCI_COMMAND_BUS_MASTER) != 0) {
    Attributes |= (UINT64)EFI_PCI_IO_ATTRIBUTE_BUS_MASTER;
  }
  ...
  if (Option == EFI_SET_SUPPORTS) {
    ...
    if (IS_PCI_LPC (&PciIoDevice->Pci)) {
        Attributes |= (UINT64)EFI_PCI_IO_ATTRIBUTE_ISA_MOTHERBOARD_IO;
        Attributes |= (UINT64)(mReserveIsaAliases ? EFI_PCI_IO_ATTRIBUTE_ISA_IO : \
    ...
    if (IS_PCI_BRIDGE (&PciIoDevice->Pci) || IS_CARDBUS_BRIDGE (&PciIoDevice->Pci)) {
      ...
      //
      Attributes |= (UINT64)EFI_PCI_IO_ATTRIBUTE_IDE_SECONDARY_IO;
      Attributes |= (UINT64)EFI_PCI_IO_ATTRIBUTE_IDE_PRIMARY_IO;
      ...
      if (IS_PCI_VGA (&PciIoDevice->Pci)) {
        Attributes |= (UINT64)EFI_PCI_IO_ATTRIBUTE_VGA_MEMORY;
        Attributes |= (UINT64)(mReserveVgaAliases ? EFI_PCI_IO_ATTRIBUTE_VGA_IO : \
                                                    EFI_PCI_IO_ATTRIBUTE_VGA_IO_16);
      }
    }

    PciIoDevice->Supports = Attributes;
    PciIoDevice->Supports &= (UINT64)( (PciIoDevice->Parent->Supports) | \
    ...
    if (!PciIoDevice->EmbeddedRom) {
      Attributes |= (UINT64)EFI_PCI_IO_ATTRIBUTE_EMBEDDED_ROM;
    }
    ...
    if (!EFI_ERROR (Status)) {
      ...
      if ((Data32 & EFI_PCIE_CAPABILITY_DEVICE_CAPABILITIES_2_ARI_FORWARDING) != 0) {
        ...
        if ((Data32 & EFI_PCIE_CAPABILITY_DEVICE_CONTROL_2_ARI_FORWARDING) == 0) {
          ...
          //
          // Save the AFE setting into boot script for S3 resume
          //
          ParentPciIo->GetLocation (ParentPciIo, &Seg1, &Bus1, &Dev1, &Fun1);
          BootScriptPciAddress = S3_BOOT_SCRIPT_LIB_PCI_ADDRESS (Bus1, Dev1, Fun1, Bridge->PciExpressCapabilityOffset + EFI_PCIE_CAPABILITY_DEVICE_CONTROL_2_OFFSET);
          S3BootScriptSavePciCfgWrite (
            S3BootScriptWidthUint32,
            BootScriptPciAddress,
            1,
