  ...
  )
{
  ...
  for (Index = 0; Index < mNumberOfPciRomImages; Index++) {
    if (mRomImageTable[Index].Seg  == Seg &&
      ...
      //
      DEBUG_CODE (
        if (mRomImageTable[Index].RomSize != 0 && RomSize != mRomImageTable[Index].RomSize) {
          DEBUG ((EFI_D_INFO, "Update %02x/%02x/%02x/%02x option ROM size from 0x%x bytes to 0x%x bytes\n",
                  Seg, Bus, Dev, Func, mRomImageTable[Index].RomSize, RomSize));
        }
      );
      break;
    }
