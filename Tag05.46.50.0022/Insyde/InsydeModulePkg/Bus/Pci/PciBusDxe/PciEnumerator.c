  ...
  )
{
  ...
  if (FeaturePcdGet (PcdH2OBdsCpPciEnumBeginSupported)) {
    H2O_BDS_CP_PCI_ENUM_BEGIN_DATA      PciEnumBeforeData;

    PciEnumBeforeData.Size                 = sizeof (H2O_BDS_CP_PCI_ENUM_BEGIN_DATA);
    PciEnumBeforeData.Status               = H2O_CP_TASK_NORMAL;
    DEBUG_CP ((DEBUG_INFO, "Checkpoint Trigger: %g\n", &gH2OBdsCpPciEnumBeginGuid));
    H2OCpTrigger (&gH2OBdsCpPciEnumBeginGuid, &PciEnumBeforeData);
    DEBUG_CP ((DEBUG_INFO, "Checkpoint Result: %x\n", PciEnumBeforeData.Status));

    if (PciEnumBeforeData.Status == H2O_CP_TASK_SKIP) {
      return EFI_SUCCESS;
    }
  }
  //
  ...
  DEBUG ((EFI_D_INFO, " PciEnumerator - gFullEnumeration = %d - start\n", gFullEnumeration));
  //
  ...
  //
  DEBUG ((EFI_D_INFO, " NotifyPhase - EfiPciHostBridgeBeginEnumeration\n"));
  ...
  //
  DEBUG ((EFI_D_INFO, " PciHostBridgeEnumerator - start\n"));
  Status = PciHostBridgeEnumerator (PciResAlloc);
  DEBUG ((EFI_D_INFO, " PciHostBridgeEnumerator - end with status %r\n", Status));
  ...
  //
  DEBUG ((EFI_D_INFO, " PciHostBridgeResourceAllocator - start\n"));
  Status = PciHostBridgeResourceAllocator (PciResAlloc);
  DEBUG ((EFI_D_INFO, " PciHostBridgeResourceAllocator - end with status %r\n", Status));
  ...
  //
  DEBUG ((EFI_D_INFO, " NotifyPhase - EfiPciHostBridgeEndEnumeration\n"));
  ...
  // Process P2C
  //
  DEBUG ((EFI_D_INFO, " PciHostBridgeP2CProcess - start\n"));
  Status = PciHostBridgeP2CProcess (PciResAlloc);
  DEBUG ((EFI_D_INFO, " PciHostBridgeP2CProcess - end with status %r\n", Status));
  ...
  //
  DEBUG ((EFI_D_INFO, " PciHostBridgeDeviceAttribute - start\n"));
  Status = PciHostBridgeDeviceAttribute (PciResAlloc);
  DEBUG ((EFI_D_INFO, " PciHostBridgeDeviceAttribute - end with status %r\n", Status));
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  if (FeaturePcdGet (PcdH2OBdsCpPciEnumCompleteBeforeSupported)) {
    H2O_BDS_CP_PCI_ENUM_COMPLETE_BEFORE_DATA      PciEnumCompleteBeforeData;

    PciEnumCompleteBeforeData.Size                 = sizeof (H2O_BDS_CP_PCI_ENUM_COMPLETE_BEFORE_DATA);
    PciEnumCompleteBeforeData.Status               = H2O_CP_TASK_NORMAL;
    DEBUG_CP ((DEBUG_INFO, "Checkpoint Trigger: %g\n", &gH2OBdsCpPciEnumCompleteBeforeGuid));
    H2OCpTrigger (&gH2OBdsCpPciEnumCompleteBeforeGuid, &PciEnumCompleteBeforeData);
    DEBUG_CP ((DEBUG_INFO, "Checkpoint Result: %x\n", PciEnumCompleteBeforeData.Status));

    if (PciEnumCompleteBeforeData.Status == H2O_CP_TASK_SKIP) {
      return EFI_SUCCESS;
    }
  }
  //
  // According to PI 1.3 specification defines the EFI_PCI_ENUMERATION_PROTOCOL shall be installed on the same handle as the host bridge protocol.
  //
  ...
  DEBUG ((EFI_D_INFO, " PciEnumerator - gFullEnumeration = %d - End, gEfiPciEnumerationCompleteProtocolGuid be Installed\n", gFullEnumeration));
  if (FeaturePcdGet (PcdH2OBdsCpPciEnumCompleteAfterSupported)) {
    H2O_BDS_CP_PCI_ENUM_COMPLETE_AFTER_DATA      PciEnumCompleteAfterData;

    PciEnumCompleteAfterData.Size                 = sizeof (H2O_BDS_CP_PCI_ENUM_COMPLETE_AFTER_DATA);
    PciEnumCompleteAfterData.Status               = H2O_CP_TASK_NORMAL;
    DEBUG_CP ((DEBUG_INFO, "Checkpoint Trigger: %g\n", &gH2OBdsCpPciEnumCompleteAfterGuid));
    H2OCpTrigger (&gH2OBdsCpPciEnumCompleteAfterGuid, &PciEnumCompleteAfterData);
    DEBUG_CP ((DEBUG_INFO, "Checkpoint Result: %x\n", PciEnumCompleteAfterData.Status));
  }
  return EFI_SUCCESS;
}
  ...
  for (Device = 0; Device <= PCI_MAX_DEVICE; Device++) {
    for (Func = 0; Func <= PCI_MAX_FUNC; Func++) {
      ...
      if (!EFI_ERROR (Status)   &&
        ...
        BootScriptPciAddress = S3_BOOT_SCRIPT_LIB_PCI_ADDRESS (StartBusNumber, Device, Func, 0x18);
        ...
        BootScriptPciAddress = S3_BOOT_SCRIPT_LIB_PCI_ADDRESS (StartBusNumber, Device, Func, 0x1A);
        S3BootScriptSavePciCfgWrite (
...
/**
  ...
  @param Config           Output buffer holding new constructed ACPI resource requestor
  ...
  )
{
  ...
  for (Index = 0; Index < gPciHostBridgeNumber; Index++) {
    if (HostBridgeHandle == gPciHostBridgeHandles[Index]) {
      return TRUE;
    }
  }
  ...
  )
{
  ...
  if (Index < PCI_MAX_HOST_BRIDGE_NUM) {
    gPciHostBridgeHandles[Index] = HostBridgeHandle;
