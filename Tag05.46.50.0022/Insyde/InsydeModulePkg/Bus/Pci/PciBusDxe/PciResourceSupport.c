  ...
  )
{
  ...
  //
  Aperture = MAX (Aperture, PaddingAperture);
  //
...
/**
  Find resource for input PCI resource node from free resource.

  @param[in] FreeResource     Pointer to the header of free PCI resource.
  @param[in] Node             PCI resource node for given PCI device.

  @retval EFI_SUCCESS         Find free resource for given PCI device successfully.
  @retval EFI_NOT_FOUND       Cannot find free resource for given PCI device.
**/
STATIC
EFI_STATUS
UseSpaceFromFreeResource (
  IN PCI_AVAILABLE_SPACE_DESCRIPTOR   *FreeResource,
  IN PCI_RESOURCE_NODE                *Node
  )
{
  LIST_ENTRY                          *Link;
  PCI_AVAILABLE_SPACE_DESCRIPTOR      *AvailableSpace;
  PCI_AVAILABLE_SPACE_DESCRIPTOR      *NewAvailableSpace;
  UINT64                              BaseAddress;
  UINT64                              EndAddress;

  if (IsListEmpty (&FreeResource[Node->ResourceUsage].Link)) {
    return EFI_NOT_FOUND;
  }

  for (Link = GetFirstNode (&FreeResource[Node->ResourceUsage].Link);
       !IsNull (&FreeResource[Node->ResourceUsage].Link, Link);
       Link = GetNextNode (&FreeResource[Node->ResourceUsage].Link, Link)) {
    AvailableSpace = AVAILABLE_SPACE_FROM_LINK (Link);
    BaseAddress    = ALIGN_VALUE (AvailableSpace->BaseAddress, Node->Alignment + 1);
    EndAddress     = BaseAddress + Node->Length;
    if (AvailableSpace->EndAddress < EndAddress) {
      continue;
    }
    if (BaseAddress == AvailableSpace->BaseAddress) {
      AvailableSpace->BaseAddress = EndAddress;
    } else if (EndAddress == AvailableSpace->EndAddress) {
       AvailableSpace->EndAddress = BaseAddress;
    } else {
      NewAvailableSpace = AllocateZeroPool (sizeof (PCI_AVAILABLE_SPACE_DESCRIPTOR));
      if (NewAvailableSpace == NULL) {
        return EFI_OUT_OF_RESOURCES;
      }

      NewAvailableSpace->BaseAddress = EndAddress;
      NewAvailableSpace->EndAddress  = AvailableSpace->EndAddress;
      AvailableSpace->EndAddress     = BaseAddress;
      InsertHeadList (Link, &NewAvailableSpace->Link);
    }
...
/**
  Add resource to free resource link list
  ...
  )
{
  ...
  if (FreeResource == NULL || Node == NULL) {
    return;
  }
  //
  // Have not AvailableSpace.
  //
  BaseAddress = Node->Offset;
  //
  // Update AvailableSpace List.
  //
  if (BaseAddress > FreeResource[Node->ResourceUsage].EndAddress) {
    AvailableSpace = AllocateZeroPool (sizeof (PCI_AVAILABLE_SPACE_DESCRIPTOR));
    if (AvailableSpace == NULL) {
      return;
    }
    AvailableSpace->BaseAddress = FreeResource[Node->ResourceUsage].EndAddress;
    AvailableSpace->EndAddress  = BaseAddress;
    InsertHeadList (&FreeResource[Node->ResourceUsage].Link, &AvailableSpace->Link);
  }
  FreeResource[Node->ResourceUsage].EndAddress = BaseAddress + Node->Length;
}

/**
  Free all of resource in free resource link list

  @param[in] FreeResource     Pointer to the header of free PCI resource.
**/
STATIC
VOID
FreeFreeResourceList (
  IN PCI_AVAILABLE_SPACE_DESCRIPTOR  *FreeResource
  )
{
  UINTN                              Index;
  ...
  for (Index = PciResUsageTypical; Index <= PciResUsagePadding; Index++) {
    while (!IsListEmpty (&FreeResource[Index].Link)) {
      AvailiableSpaceLink = FreeResource[Index].Link.ForwardLink;
      AvailableSpace      = AVAILABLE_SPACE_FROM_LINK (AvailiableSpaceLink);

      RemoveEntryList (AvailiableSpaceLink);
      if (AvailableSpace != NULL) {
        FreePool (AvailableSpace);
      }
    }
  }
}
...
/**
  ...
  @param[in] Bridge      PCI resoure node for given bridge device.
  @param[in] OnRoot      BOOLEAN value to indicate the bridge is on root or not.
  ...
  )
{
  UINT64                           Aperture[2];
  LIST_ENTRY                       *CurrentLink;
  PCI_AVAILABLE_SPACE_DESCRIPTOR   FreeResource[2];
  PCI_RESOURCE_NODE                *Node;
  EFI_STATUS                       Status;
  ...
  if (OnRoot) {
    ZeroMem (FreeResource, sizeof (FreeResource));
    InitializeListHead (&FreeResource[PciResUsageTypical].Link);
    InitializeListHead (&FreeResource[PciResUsagePadding].Link);
  }
  //
  // Assume the bridge is aligned
  //
  for ( CurrentLink = GetFirstNode (&Bridge->ChildList)
    ...
    if (OnRoot && Node->ResourceUsage == PciResUsageTypical) {
      Status = UseSpaceFromFreeResource (FreeResource, Node);
      if (!EFI_ERROR (Status)) {
        continue;
      }
    }
    ...
    if (OnRoot && Node->ResourceUsage == PciResUsageTypical) {
      AddFreeResource (FreeResource, Node);
    }
  }
  ...
  if (OnRoot) {
    FreeFreeResourceList (FreeResource);
  }
}

/**
  Get IO/Memory resource information for given PCI device.
  ...
  )
{
  ...
  if (PcdGetBool (PcdH2OOptionRomBarReserved) && PcdGetBool (PcdH2OPciOptionRomSupported) && PciDev->OptionRomBar.BarType == PciBarTypeMem32) {
    Node = CreateResourceNode (
             PciDev,
             PciDev->OptionRomBar.Length,
             PciDev->OptionRomBar.Alignment,
             OPTION_ROM_BAR,
             PciBarTypeMem32,
             PciResUsageTypical
             );
  ...
  )
{
  ...
  while (CurrentLink != NULL && CurrentLink != &Bridge->ChildList) {
    ...
    if (IS_PCI_BRIDGE (&Temp->Pci)) {
      ...
      //
      // Recursively create resoure map on this bridge
      //
      CreateResourceMap (
        Temp,
        IoBridge,
  ...
  // Also, will find the better memory allocation on the bus of the Root bridge.
  //
  CalculateResourceAperture (Mem32Node,  Bridge->Parent == NULL);
  CalculateResourceAperture (PMem32Node, Bridge->Parent == NULL);
  CalculateResourceAperture (Mem64Node,  Bridge->Parent == NULL);
  CalculateResourceAperture (PMem64Node, Bridge->Parent == NULL);
  CalculateResourceAperture (IoNode,     Bridge->Parent == NULL);
  ...
  )
{
  ...
  if (FeaturePcdGet (PcdPciDegradeResourceForOptionRom) && H2OGetBootType () != EFI_BOOT_TYPE) {
    //
      ...
      if (PciIoDevice->RomSize != 0) {
        if (!IsListEmpty (&Mem64Node->ChildList)) {
          ...
          while (ChildNodeLink != &Mem64Node->ChildList) {
            ...
            if ((ResourceNode->PciDev == PciIoDevice && ResourceNode->Bar < PCI_MAX_BAR) &&
                (ResourceNode->Virtual || ResourceNode->Bar == OPTION_ROM_BAR || !PciIoDevice->PciBar[ResourceNode->Bar].BarTypeFixed)
                ) {
  ...
  if (sizeof (UINTN) <= 4) {
    ...
    //
    if(!(GetPciHostBridgeDecodeAttributes & EFI_BRIDGE_MEM64_DECODE_SUPPORTED)) {
        MergeResourceTree (
          Mem32Node,
          Mem64Node,
          TRUE
        ...
        DEBUG_CODE (
          {
            DEBUG ((EFI_D_INFO, " PCI device: 0x%02x/0x%02x/0x%02x, BarOffset_0x%02x, Type_%d, Address_0x%lx\n", \
                    ...
                    Node->Bar == OPTION_ROM_BAR ? Node->PciDev->OptionRomBar.Offset : (Node->PciDev->PciBar[Node->Bar]).Offset,
                    Node->Bar == OPTION_ROM_BAR ? Node->PciDev->OptionRomBar.BarType : (Node->PciDev->PciBar[Node->Bar]).BarType,
                    Base + Node->Offset));
          }
        );
      ...
      DEBUG_CODE (
        {
          DEBUG ((EFI_D_INFO, " <PCI Bridge>: 0x%02x/0x%02x/0x%02x, BarOffset_0x%02x, Type_%d, Address_0x%lx\n", \
                  Node->PciDev->BusNumber, Node->PciDev->DeviceNumber, Node->PciDev->FunctionNumber, \
                  Node->Bar == OPTION_ROM_BAR ? Node->PciDev->OptionRomBar.Offset :(Node->PciDev->PciBar[Node->Bar]).Offset,  \
                  Node->Bar, Base + Node->Offset));
        }
      );
      ProgramPpbAperture (Base, Node);
    }
  ...
  )
{
  ...
  ASSERT (Node->Bar < PCI_MAX_BAR || Node->Bar == OPTION_ROM_BAR);
  ...
  if (Node->Bar == OPTION_ROM_BAR) {
    if (PcdGetBool (PcdH2OOptionRomBarReserved) && PcdGetBool (PcdH2OPciOptionRomSupported)) {
      PciIo->Pci.Write (
                   PciIo,
                   EfiPciIoWidthUint32,
                   Node->PciDev->OptionRomBar.Offset,
                   1,
                   &Address
                   );
      BootScriptPciAddress = S3_BOOT_SCRIPT_LIB_PCI_ADDRESS (Bus, Device, Function, Node->PciDev->OptionRomBar.Offset);
      ...
      Node->PciDev->OptionRomBar.BaseAddress = Address;
    }
    return;
  }
...
/**
  ...
  @param OptionRomBase  Base address for Optional Rom.
