/** @file
  Support routines for memory allocation routines for Windows tool modules
...
#ifdef __GNUC__
  return (VOID *) malloc(Pages *EFI_PAGE_SIZE);
#else
  return (VOID *) LocalAlloc(LPTR, Pages * EFI_PAGE_SIZE);
#endif
}
...
#else
  return (VOID *)LocalAlloc(LPTR, AllocationSize);
#endif
}
