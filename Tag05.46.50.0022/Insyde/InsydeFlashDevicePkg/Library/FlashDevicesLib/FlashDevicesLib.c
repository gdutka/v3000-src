  ...
  )
{
  ...
  if (PcdTokenNumber != 0){
    ...
    if (FlashDeviceInstance != NULL){
      goto Done;
    }
  }

  for (PcdTokenNumber = LibPcdGetNextToken(&gH2OFlashDeviceGuid, 0);
       PcdTokenNumber != 0;
       PcdTokenNumber = LibPcdGetNextToken (&gH2OFlashDeviceGuid, PcdTokenNumber)) {
    if (PcdTokenNumber == PcdToken (PcdCommonvidCommondidSpi) || PcdTokenNumber == PcdToken (PcdActiveFlashDeviceId) ){
      //
      // Do nothing if Common Spi or ActiveFlashDeviceId
      //
    } else {
      FlashDeviceInstance = RecognizeFlashDeviceByPcdEx (&PcdTokenNumber);
      if (FlashDeviceInstance != NULL){
  ...
  PcdTokenNumber = PcdToken (PcdCommonvidCommondidSpi);
  FlashDeviceInstance = RecognizeFlashDeviceByPcdEx (&PcdTokenNumber);
