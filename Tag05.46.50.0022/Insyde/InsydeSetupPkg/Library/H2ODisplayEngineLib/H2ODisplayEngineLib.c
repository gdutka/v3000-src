  ...
  )
{
  ...
  if ((DateTimeItem == YearItem   && EfiTime->Year   == 0xFF) ||
      (DateTimeItem == MonthItem  && EfiTime->Month  == 0xFF) ||
      (DateTimeItem == DayItem    && EfiTime->Day    == 0xFF) ||
      (DateTimeItem == HourItem   && EfiTime->Hour   == 0xFF) ||
      (DateTimeItem == MinuteItem && EfiTime->Minute == 0xFF) ||
      (DateTimeItem == SecondItem && EfiTime->Second == 0xFF)) {
    return EFI_SUCCESS;
  }
  ...
  //
  // If changing year or month makes day invalid, decrease day value until it is valid.
  //
  ...
  )
{
  ...
  LogoResolution = OemLogoResolutionBuffer;
  ...
  if (Horizontal == 0xFFFF && Vertical == 0xFFFF) {
    ...
    for (Count = 0; Count < OEM_LOGO_RESOLUTION_DISPLAY_ENGINE_MAX_NUM; Count++) {
