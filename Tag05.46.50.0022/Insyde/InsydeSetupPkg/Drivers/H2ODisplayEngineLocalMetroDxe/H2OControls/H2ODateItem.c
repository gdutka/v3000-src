  ...
  )
{
  ...
  MinValue = 1;
  MaxValue = 12;
  Month    = (EfiTime->Month < MinValue || EfiTime->Month > MaxValue) ? (UINT8) MinValue : EfiTime->Month;
  ListStr = CreateStrArrayByNumRange (MinValue, MaxValue, 1, L"%02d");
  UnicodeSPrint (Str, sizeof (Str), L"0x%p", ListStr);
  UiSetAttribute (ChildControl, L"list", Str);
  UnicodeSPrint (Str, sizeof (Str), L"%d", Month - 1);
  UiSetAttribute (ChildControl, L"currentindex", Str);
  FreeStringArray (ListStr, (MaxValue - MinValue + 1));
  ...
  MinValue = 1;
  if (Month == 2 && IsLeapYear (EfiTime->Year)) {
    MaxValue = 29;
  } else {
    MaxValue = (UINT32) DayOfMonth[Month - 1];
  }
  ListStr = CreateStrArrayByNumRange (MinValue, MaxValue, 1, L"%02d");
  UnicodeSPrint (Str, sizeof (Str), L"0x%p", ListStr);
  UiSetAttribute (ChildControl, L"list", Str);
  ...
  )
{
  ...
  if (StrCmp (Name, L"efitime") == 0) {
    ...
    CopyMem (&This->EfiTime, EfiTime, sizeof(EFI_TIME));
    if (This->EfiTime.Year  == 0xFF ||
        This->EfiTime.Month == 0xFF ||
        This->EfiTime.Day   == 0xFF) {
      This->EfiTime.Year  = PcdGet16 (PcdRealTimeClockYearMin);
      This->EfiTime.Month = 1;
      This->EfiTime.Day   = 1;
    }
    H2ODateItemNumberPickerUpdate (This, &This->EfiTime);
  } else {
