...
//    L"</Control>"
//    L"<Label padding='15,0,15,8' width='1492' textcolor='0xFFFFFFFF' font-size='30' name='TitleName'/>"
//  L"</HorizontalLayout>";
CHAR16 *mFormTitlePanelChilds = NULL;
#endif

STATIC
VOID
H2OFormTitlePanelUpdateVfcfOrder (
  IN UI_CONTROL                        *Order
  )
{
  UINTN                                Index;
  ...
  for (Index = 0; Index < Order->ItemCount; Index++) {
    ...
    if (STR_MATCH(Child->Name, NameLen, L"formset-image")) {
      ...
      UiSetAttribute (Child, L"text", (gFB->CurrentQ == NULL) ? L"" : GetCurrentFormSetTitle ());
    } else if (STR_MATCH(Child->Name, NameLen, L"form-title")) {
      //
      // TODO: VFCF should support to dynamicaly hide form-title and str0.
      //
      if (IsRootPage ()) {
        UiSetAttribute (Child, L"visibility", L"false");
      } else {
        UiSetAttribute (Child, L"visibility", L"true");
        UiSetAttribute (Child, L"text", gFB->CurrentP->PageTitle);
      }
    } else if (PREFIX_STR_MATCH(Child->Name, NameLen, L"str")) {
      if (IsRootPage ()) {
        UiSetAttribute (Child, L"visibility", L"false");
      } else {
        UiSetAttribute (Child, L"visibility", L"true");
      ...
      H2OFormTitlePanelUpdateVfcfOrder (Child);
    }
  }
}
