  ...
  if (Control->VScrollBar != NULL) {
    ...
    if (Direction == VK_PRIOR) {
      ...
      if (SelectIndex == 0) {
        ...
        // 3. If first item can not fit in ListView, scroll to the top of first item.
        if (Index == (INT32) Control->ItemCount) {
          Child = Control->Items[0];
          Control->Manager->GetControlRect (Control->Manager, Child, &ChildRc);
          if (ChildRc.top <= ListRc.top) {
            dy = MAX (ChildRc.top - ListRc.top, ListRc.top - ListRc.bottom);
          }
        }
      }
    }
    ...
    if (Direction == VK_NEXT) {
      ...
      if (SelectIndex == 0) {
        ...
        // 3. If last item can not fit in ListView, scroll to the bottom of last item.
        if (Index == -1) {
          Child = Control->Items[Control->ItemCount - 1];
          Control->Manager->GetControlRect (Control->Manager, Child, &ChildRc);
          if (ChildRc.bottom >= ListRc.bottom) {
            dy = MIN (ChildRc.bottom - ListRc.bottom, ListRc.bottom - ListRc.top);
          }
        }
      }
    }
  ...
  )
{
  ...
  if (StrCmp (Name, L"layout") == 0) {
    ...
    } else if (StrCmp (Value, L"horizontal") == 0) {
      This->Layout = UiListViewHorizontalLayout;
    } else {
...
VOID
UiListViewSetPositionCellSpanning (
  UI_CONTROL                   *Control,
  CONST RECT                   *Pos
  )
{
  ...
  INT32                         Range;
  INT32                         MaxItemBottom;
  INT32                         PreviousMarginBottom;
  INT32                         PreviousMarginRight;
  RECT                          Rc;
  SIZE                          Available;
  SIZE                          ItemSize;
  SIZE                          ItemAvailable;
  ...
  for (Index = 0; Index < Control->ItemCount; Index++) {
    ...
    Size = Item->Class->EstimateSize (Item, ItemAvailable);
    ...
    if (((X - Rc.left) + ItemSize.cx + GetControlMarginRight (Item) + MAX((GetControlMarginLeft (Item) - PreviousMarginRight), 0)) > Available.cx) {
      Y = EndY + This->CellHeight;
      X = Rc.left;
      PreviousMarginBottom = EndY - MaxItemBottom;
      PreviousMarginRight  = 0;
    }

    ItemRc.left   = X + ((GetControlMarginLeft (Item) > PreviousMarginRight ) ? (GetControlMarginLeft (Item) - PreviousMarginRight) : 0);
    ItemRc.top    = Y + ((GetControlMarginTop (Item)  > PreviousMarginBottom) ? (GetControlMarginTop (Item) - PreviousMarginBottom) : 0);
    ItemRc.right  = ItemRc.left + ItemSize.cx;
    ItemRc.bottom = ItemRc.top  + ItemSize.cy;
    MaxItemBottom = MAX (MaxItemBottom, ItemRc.bottom);
    EndY          = MAX (EndY, ItemRc.bottom + GetControlMarginBottom (Item));

    mSetPositionLevel++;
    CONTROL_CLASS (Item)->SetPosition (Item, &ItemRc);
    mSetPositionLevel--;
    X = ItemRc.right + GetControlMarginRight (Item) + This->CellWidth;
    PreviousMarginRight = GetControlMarginRight (Item);
  }
  ...
  )
{
  ...
  case UiListViewVerticalLayout:
    UiVerticalLayoutSetPosition (Control, Pos);
    break;

  case UiListViewHorizontalLayout:
    UiHorizontalLayoutSetPosition (Control, Pos);
    break;

  case UiListViewCellSpanningLayout:
    if (IsDraggingItem (This)) {
      GetWindowRect (Control->Items[This->CurSel]->Wnd, &Rc);
      CopyRect (&This->MoveItemOrgRc, &Rc);
    }
    UiListViewSetPositionCellSpanning (Control, Pos);
    break;
  ...
  if (This->CurSel >= 0 && This->CurSel < (INT32)Control->ItemCount) {
    //
    // If current selection is not in display field, clear selected state.
    //
    Item = Control->Items[This->CurSel];
    Control->Manager->GetControlRect (Control->Manager, Control, &Rc);
    Control->Manager->GetControlRect (Control->Manager, Item   , &ItemRc);
    if ((ItemRc.top    >= Rc.top && ItemRc.top    < Rc.bottom) ||
        (ItemRc.bottom >= Rc.top && ItemRc.bottom < Rc.bottom)) {
      CONTROL_CLASS_SET_STATE (Item, UISTATE_SELECTED, 0);
    } else {
      CONTROL_CLASS_SET_STATE (Item, 0, UISTATE_SELECTED);
    }
  }
}
  ...
  )
{
  ...
  case UiListViewVerticalLayout:
  case UiListViewCellSpanningLayout:
    Size = UiVerticalEstimateSize (Control, AvailableSize);
    break;

  case UiListViewHorizontalLayout:
    Size = UiHorizontalEstimateSize (Control, AvailableSize);
    break;
  ...
  )
{
  ...
  case VK_RETURN:
    if (Control->ItemCount != 0 && This->CurSel >= 0 && This->CurSel < (INT32)Control->ItemCount) {
      if (CONTROL_CLASS_GET_STATE (Control->Items[This->CurSel]) & UISTATE_SELECTED) {
  ...
  )
{
  ...
  ((UI_CONTROL_CLASS *)CURRENT_CLASS)->EstimateSize = UiListViewEstimateSize;
