  ...
  )
{
  ...
  case H2O_DISPLAY_ENGINE_EVT_TYPE_ABS_PTR_MOVE:
    ...
    if (SelectedControl != NULL) {
      if (SelectedControl == QuestionPanel->SelectedControl) {
        ...
        if (Control == NULL ||
            IfrNumericIsEmptyStr (Control->Flags, Control->Text.String) ||
            !IfrNumericIsValueInRange (Control->Flags, Control->HiiValue.Value.u64, Control->Minimum, Control->Maximum)) {
          Status = SendShutDNotify ();
          break;
        }
      ...
      if ((IS_DISPLAYED_IN_HEX (Control) && StrSize (UpdatedString) < sizeof (HEX_NUMBER_PREFIX_STRING)) ||
          !IfrNumericIsEditStrValid (Control->Flags, UpdatedString, Control->Minimum, Control->Maximum)) {
        FreePool (UpdatedString);
        break;
      }

      ZeroMem (&HiiValue, sizeof (EFI_HII_VALUE));
      CreateValueAsString (&HiiValue, (UINT16) StrSize (UpdatedString), (UINT8 *) UpdatedString);
  ...
  case H2O_DISPLAY_ENGINE_EVT_TYPE_CHANGING_Q:
    ...
    if (!IfrNumericIsEditStrValid (Control->Flags, UpdatedString, Control->Minimum, Control->Maximum)) {
      break;
    }

    SafeFreePool ((VOID **) &Control->Text.String);
    Control->Text.String        = AllocateCopyPool(StrSize (UpdatedString), UpdatedString);
    Control->HiiValue.Value.u64 = IfrNumericConvertStrToValue (Control->Flags, UpdatedString);
