  ...
  )
{
  ...
  if (Question->Operand == EFI_IFR_DATE_OP) {
    if (Question->HiiValue.Value.date.Month == 0xFF ||
        Question->HiiValue.Value.date.Day   == 0xFF ||
        Question->HiiValue.Value.date.Year  == 0xFF) {
      return CatSPrint (
               NULL,
               L"%c%s%c",
    ...
    if (Question->HiiValue.Value.time.Hour   == 0xFF ||
        Question->HiiValue.Value.time.Minute == 0xFF ||
        Question->HiiValue.Value.time.Second == 0xFF) {
      return CatSPrint (
               NULL,
               L"%c%s%c",
               LEFT_NUMERIC_DELIMITER,
