  ...
  )
{
  ...
  case HotKeyEnter:
    ...
    if (HotKey->HotKeyTargetQuestionId == 0) {
      SetupPagePanelProcessOpenQHotKey (SetupPagePanel, NULL);
    } else {
  ...
  )
{
  ...
  Panel->Vertical        = (VfcfPanelInfo->PanelId == SETUP_MENU_PANEL_ID) ? FALSE : TRUE;
  ...
  Panel->BorderColorAttribute = Panel->ColorAttribute;
  Status = GetPropertyFromVfrAndPanel (
             Panel->VfcfPanelInfo,
             NULL,
  ...
  if (!EFI_ERROR (Status)) {
    Panel->BorderColorAttribute &= ~0x0F;
    Panel->BorderColorAttribute |= PropValue.Value.Color.Attribute;
  }
...
/**
  Reset all console device to initial status.

  @param[in] ConsoleDevListHead    Pointer to console device linked list
**/
STATIC
VOID
ResetConDevStatus (
  IN LIST_ENTRY                                  *ConsoleDevListHead
  )
{
  LIST_ENTRY                                     *Link;
  H2O_FORM_BROWSER_CONSOLE_DEV_NODE              *ConDevNode;

  Link = GetFirstNode (ConsoleDevListHead);
  while (!IsNull (ConsoleDevListHead, Link)) {
    ConDevNode = H2O_FORM_BROWSER_CONSOLE_DEV_NODE_FROM_LINK (Link);
    Link = GetNextNode (ConsoleDevListHead, Link);
...
/**
  Remove all panel from list

  @param[in] PanelListHead        Pointer to panel linked list
**/
STATIC
VOID
RemovePanelList (
  ...
  )
{
  ...
  if (PanelListHead == NULL) {
    return;
  }

  if (IsListEmpty (PanelListHead)) {
    return;
  }

  Link = GetFirstNode (PanelListHead);
  while (!IsNull (PanelListHead, Link)) {
    Panel = H2O_LTDE_PANEL_FROM_LINK (Link);
    Link  = GetNextNode (PanelListHead, Link);
    FreePanel (Panel);
    //
  ...
  )
{
  ...
  if (mDEPrivate->Layout != NULL &&
      mDEPrivate->Layout->LayoutId != Layout->LayoutId) {
    //
    // Layout is different. Reset console device status and remove all panel list.
    //
    ResetConDevStatus (&mDEPrivate->ConsoleDevListHead);
    RemovePanelList (&mDEPrivate->PanelListHead);
  }
  ...
  while (!IsNull (&Layout->PanelListHead, Link)) {
    ...
    Panel = GetPanel (VfcfPanelInfo->PanelId);
    if (Panel == NULL) {
      Panel = CreatePanel ();
    }
    if (Panel != NULL) {
  ...
  )
{
  ...
  while (!IsNull (&Private->PanelListHead, PanelLink)) {
    ...
    switch (Panel->VfcfPanelInfo->PanelId) {

    case HELP_TEXT_PANEL_ID:
    case QUESTION_PANEL_ID:
//      FreePanel (Panel);
      break;
    }
  }
  ...
  )
{
  ...
  if (mDEPrivate->DEStatus == DISPLAY_ENGINE_STATUS_AT_POPUP_DIALOG) {
    if (QuestionPanel->Data != NULL && 
        ((H2O_LTDE_QUESTION_PANEL_PRIVATE_DATA *) QuestionPanel->Data)->Dialog.DialogType == ((H2O_DISPLAY_ENGINE_EVT_OPEN_D *) Notify)->Dialog.DialogType) {
      QuestionPanelUpdateField (&((H2O_LTDE_QUESTION_PANEL_PRIVATE_DATA *) QuestionPanel->Data)->Dialog, QuestionPanel);
  ...
  )
{
  ...
  case H2O_DISPLAY_ENGINE_EVT_TYPE_REL_PTR_MOVE:
    ...
    Status = TransferToTextModePosition (RealMouseX, RealMouseY, &InputData.CursorX, &InputData.CursorY);
    break;
