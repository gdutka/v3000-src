...
/**
  ...
  @param [in] *IfrGuidPcd     Point to H2O_IFR_GUID_PCD.
  @param [out]*Value          Point to value that store Pcd value
...
EFI_STATUS
ProcessOpCodeH2OPcd (
  IN  H2O_IFR_GUID_PCD     *IfrGuidPcd,
  ...
  )
{
  ...
  Status = mIfrPiPcdInfo->GetInfo (&IfrGuidPcd->PcdGuid, IfrGuidPcd->PcdTokenNumber, &PcdInformation);
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "The PCD is unsupported -- PCD GUID = %g, TokenNumber = %x\n", IfrGuidPcd->PcdGuid, IfrGuidPcd->PcdTokenNumber));
    return Status;
  }
  ...
  switch(PcdInformation.PcdType) {
    case EFI_PCD_TYPE_8:
      ...
      Value->Value.u8 = LibPcdGetEx8(&IfrGuidPcd->PcdGuid, IfrGuidPcd->PcdTokenNumber);
      break;
    case EFI_PCD_TYPE_16:
      Value->Type = EFI_IFR_TYPE_NUM_SIZE_16;
      Value->Value.u16 = LibPcdGetEx16(&IfrGuidPcd->PcdGuid, IfrGuidPcd->PcdTokenNumber);
      break;
    case EFI_PCD_TYPE_32:
      Value->Type = EFI_IFR_TYPE_NUM_SIZE_32;
      Value->Value.u32 = LibPcdGetEx32(&IfrGuidPcd->PcdGuid, IfrGuidPcd->PcdTokenNumber);
      break;
    case EFI_PCD_TYPE_64:
      Value->Type = EFI_IFR_TYPE_NUM_SIZE_64;
      Value->Value.u64 = LibPcdGetEx64(&IfrGuidPcd->PcdGuid, IfrGuidPcd->PcdTokenNumber);
      break;
    case EFI_PCD_TYPE_BOOL:
      Value->Type = EFI_IFR_TYPE_NUM_SIZE_8;
      Value->Value.b = LibPcdGetExBool(&IfrGuidPcd->PcdGuid, IfrGuidPcd->PcdTokenNumber);
      break;
    case EFI_PCD_TYPE_PTR:
      Value->Type = EFI_IFR_TYPE_BUFFER;
      DataPtr = LibPcdGetExPtr(&IfrGuidPcd->PcdGuid, IfrGuidPcd->PcdTokenNumber);
  ...
  )
{
  ...
  Result->Value.b = CollationMetaiMatch (String[0], String[1]);
  ...
  )
{
  ...
  case EFI_IFR_TYPE_TIME:
    RetVal = Value->Value.u32 & 0xffffff;
    break;
  ...
  while (!IsNull (&Expression->OpCodeListHead, Link)) {
    ...
    case EFI_IFR_GUID_OP:
      if (CompareGuid (&mH2OIfrExtGuid, (EFI_GUID *)(((UINT8 *)(OpCode->GuidOpCodeData)) + sizeof (EFI_IFR_OP_HEADER)))) {
        if ((((H2O_IFR_GUID_PCD *)((UINT8 *)(OpCode->GuidOpCodeData)))->Function) == H2O_IFR_EXT_PCD) {
          Status = ProcessOpCodeH2OPcd((H2O_IFR_GUID_PCD *)((UINT8 *)(OpCode->GuidOpCodeData)), Value);
          if (EFI_ERROR(Status)) {
            return Status;
          }
        }
    ...
    case EFI_IFR_TO_UPPER_OP:
      ...
      if (OpCode->Operand == EFI_IFR_TO_LOWER_OP) {
        CollationStrLwr (StrPtr);
      } else {
        CollationStrUpr (StrPtr);
      }
