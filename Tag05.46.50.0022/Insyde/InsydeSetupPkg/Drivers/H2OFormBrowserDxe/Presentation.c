...
UINT8              mOldSecondTime = 60;

#define CHECK_VALUE_OF_SECOND                TRUE
#define NO_CHECK_VALUE_OF_SECOND             FALSE
...
//mHiiPackageListUpdated = FALSE;
  ...
  if (mHiiPackageListUpdated) {
    ...
    // To do: check whether need set gUserInput->Action
    gUserInput->Action        = BROWSER_ACTION_NONE;
    mRefreshFormSet = TRUE;
  }

  CopyMem (&Statement->Statement.HiiValue, &Statement->HiiValue, sizeof (EFI_HII_VALUE));
  if (Statement->TextTwo != 0) {
    Statement->Statement.TextTwo = GetString (Statement->TextTwo, gCurrentSelection->FormSet->HiiHandle);
  }  
}
  ...
  )
{
  ...
  CheckIfGetRtc = (UINTN) Context;
  if (CheckIfGetRtc == CHECK_VALUE_OF_SECOND) {
    Status = gRT->GetTime (&NewTime, NULL);
    if (!EFI_ERROR (Status)) {
      if (NewTime.Second != mOldSecondTime) {
        mOldSecondTime = NewTime.Second; //update previous Time
      } else {
        return;
      }
    }
  ...
  )
{
  ...
  FORM_BROWSER_REFRESH_EVENT_NODE        *EventNode;
  BOOLEAN                                RefreshTimeOpcodeFlag;

  Private = &mFBPrivate;
  Private->FB.CurrentQ = NULL;
  Private->FB.CurrentP = NULL;
  MinRefreshInterval   = 0;
  RefreshTimeOpcodeFlag = FALSE;
  ...
  if (MinRefreshInterval != 0) {
    Status = gBS->CreateEvent (
                    ...
                    TPL_CALLBACK,
                    RefreshIntervalProcess,
                    (RefreshTimeOpcodeFlag) ? (VOID*)(UINTN)CHECK_VALUE_OF_SECOND : (VOID*)(UINTN)NO_CHECK_VALUE_OF_SECOND,
    ...
    if (!EFI_ERROR (Status)) {
      Status = gBS->SetTimer (
                      RefreshIntervalEvent, 
                      TimerPeriodic, 
                      (RefreshTimeOpcodeFlag) ? 100 * ONE_MILLISEC : MinRefreshInterval * ONE_SECOND
                      ); 
    }
    EventNode = AllocateZeroPool (sizeof (FORM_BROWSER_REFRESH_EVENT_NODE));
    ASSERT (EventNode != NULL);
  ...
  Private->FB.CurrentP = &Form->PageInfo;
  if (Private->FB.CurrentQ == NULL) {
    for (Index = 0; Index < Private->FB.CurrentP->NumberOfStatementIds; Index++) {
      Statement = GetStatementByStatementId (Form, Form->PageInfo.StatementIds[Index]);
      if (Statement == NULL) {
        continue;
      }
      if (Statement->Statement.Selectable) {
        Private->FB.CurrentQ = &Statement->Statement;
        break;
      } else if (Private->FB.CurrentQ == NULL) {
        Private->FB.CurrentQ = &Statement->Statement;
      }
    }
  }
  ...
  Status = FBGetHotKeyInfo (
             ...
             Form,
             (Private->FB.CurrentQ == NULL) ? NULL : FORM_BROWSER_STATEMENT_FROM_H2O_STATEMENT(Private->FB.CurrentQ),
             &HotKeyInfoCount,
             &HotKeyInfo
             );
  if (!EFI_ERROR (Status)) {
  ...
  Private->FB.CurrentP->HotKeyInfo = Private->HotKeyInfo;
}
  ...
  )
{
  ...
  ZeroMem (&gDisplayFormData, sizeof (FORM_DISPLAY_ENGINE_FORM));
  ...
  )
{
  ...
  if ((Action & BROWSER_ACTION_SUBMIT) == BROWSER_ACTION_SUBMIT) {
    ...
    if (EFI_ERROR (Status)) {
      PopupErrorMessage(BROWSER_SUBMIT_FAIL, NULL, NULL, NULL);
    }
  }
  ...
  )
{
  ...
  while (!IsNull (&Form->StatementListHead, Link)) {
    ...
    Status = FBCallCallbackFn (
               ...
               QuestionId,
               HiiValue->Type,
               (HiiValue->Type == EFI_IFR_TYPE_BUFFER) ? Statement->HiiValue.BufferLen : sizeof (EFI_IFR_TYPE_VALUE),
               TypeValue,
               &ActionRequest,
               (IS_IMPORT_STATEMENT (Statement)) ? &Statement->ImportInfo->SrcFormSet->Guid : &FormSet->Guid,
               (IS_IMPORT_STATEMENT (Statement)) ? Statement->ImportInfo->SrcForm->FormId : Form->FormId
               );
  ...
  )
{
  ...
  if (Statement == NULL || ((Statement->QuestionFlags & EFI_IFR_FLAG_CALLBACK) != EFI_IFR_FLAG_CALLBACK) || ConfigAccess == NULL) {
  ...
  )
{
  ...
  do {
    ...
    if (Selection->Form->SuppressExpression != NULL) {
      LoadFormSetConfig (Selection, Selection->FormSet);
    ...
    if (((Selection->Handle != mCurrentHiiHandle) ||
      ...
      if (ConfigAccess != NULL) {
        ...
        //
        if (ConfigAccess->Callback != NULL) {
          ZeroMem (&Value, sizeof (Value));
          ConfigAccess->Callback(ConfigAccess, EFI_BROWSER_ACTION_FORM_OPEN, 0, 0, &Value, &DummyActionRequest);
        }
    ...
    //
    if (ConfigAccess != NULL && ConfigAccess->Callback != NULL) {
      ZeroMem (&Value, sizeof (Value));
      ConfigAccess->Callback(ConfigAccess, EFI_BROWSER_ACTION_FORM_CLOSE, 0, 0, &Value, &DummyActionRequest);
    }
