...
UINT32                 mStatementIndex;
UINT32                 mExpressionOpCodeIndex;
EFI_QUESTION_ID        mUsedQuestionId;
extern LIST_ENTRY      gBrowserStorageList;
EFI_GUID               mH2OIfrExtGuid = H2O_IFR_EXT_GUID;
  ...
  )
{
  ...
  Statement->StatementId = mStatementIndex;
  mStatementIndex++;
  ...
  )
{
  ...
  if (Statement->Storage == NULL) {
    DEBUG ((EFI_D_ERROR, "CreateQuestion() Storage is NULL"));
    return NULL;
  }
  ...
  if ((Statement->Storage->Type == EFI_HII_VARSTORE_NAME_VALUE) ||
    ...
    Statement->VariableName = GetString (Statement->VarStoreInfo.VarName, FormSet->HiiHandle);
  ...
  )
{
  ...
  if (Expression == NULL) {
    DEBUG ((EFI_D_INFO, "CreateExpression() Expression is NULL"));
    return NULL;
  }
...
/**
  Delete invalid browser storage by checking HII handle.
**/
VOID
  ...
  )
{
  ...
  while (!IsNull (&gBrowserStorageList, Link)) {
    ...
    if (BrowserStorage->Type == EFI_HII_VARSTORE_NAME_VALUE ||
        BrowserStorage->Type == EFI_HII_VARSTORE_BUFFER) {
      for (Index = 0, Found = FALSE; HiiHandles[Index] != NULL; Index++) {
        if (HiiHandles[Index] == BrowserStorage->HiiHandle) {
          Found = TRUE;
          break;
        }
      }
      if (!Found) {
        ...
        FBFreePool ((VOID **) &BrowserStorage->EditBuffer);
        FBFreePool ((VOID **) &BrowserStorage->Buffer);
        FBFreePool ((VOID **) &BrowserStorage->Name);
        FBFreePool ((VOID **) &BrowserStorage->ConfigHdr);
        if (BrowserStorage->NameValueListHead.ForwardLink != NULL &&
            BrowserStorage->NameValueListHead.BackLink    != NULL &&
            !IsListEmpty(&BrowserStorage->NameValueListHead)) {
          NameValueLink = GetFirstNode (&BrowserStorage->NameValueListHead);
          while (!IsNull (&BrowserStorage->NameValueListHead, NameValueLink)) {
            NameValueNode = NAME_VALUE_NODE_FROM_LINK (NameValueLink);
            NameValueLink = GetNextNode (&BrowserStorage->NameValueListHead, NameValueLink);
  ...
  )
{
  ...
  if (BrowserStorage == NULL) {
    ...
    //
    // Always update ConfigHdr string in case device path is changed.
    //
    FBFreePool ((VOID **) &BrowserStorage->ConfigHdr);
    InitializeConfigHdr (FormSet, BrowserStorage);
  }
  ...
  )
{
  ...
  if (Storage->Type == EFI_HII_VARSTORE_BUFFER ||
    ...
    Question->BlockName = AllocateCopyPool ((StrLength + 1) * sizeof (CHAR16), RequestElement);
  } else {
    StrLength = UnicodeSPrint (RequestElement, 30 * sizeof (CHAR16), L"&%s", Question->VariableName);
  }
  ...
  //
  if (StrLength > FormsetStorage->SpareStrLen) {
    //
  ...
  FormsetStorage->SpareStrLen -= StrLength;
  ...
  // Append <RequestElement> to <ConfigRequest>
  //
  if (StrLength > ConfigInfo->SpareStrLen) {
    //
    // Old String buffer is not sufficient for RequestElement, allocate a new one
    //
    MaxLen = StringSize / sizeof (CHAR16) + CONFIG_REQUEST_STRING_INCREMENTAL;
  ...
  ConfigInfo->SpareStrLen -= StrLength;
  return EFI_SUCCESS;
}
...
/**
  ...
  @param  [in] Expression                Pointer of the Expression
  ...
  )
{
  ...
  while (!IsListEmpty (&Expression->OpCodeListHead)) {
    ...
    FBFreePool ((VOID **) &OpCode->ValueList);
    FBFreePool ((VOID **) &OpCode->ValueName);
  ...
  )
{
  ...
  while (!IsListEmpty (&Statement->OptionListHead)) {
    ...
    // Free Property List
    //
    while (!IsListEmpty (&Option->PropertyListHead)) {
      PropertyLink = GetFirstNode (&Option->PropertyListHead);
      Property = H2O_PROPERTY_INFO_NODE_FROM_LINK (PropertyLink);
  ...
  // Free Property List
  //
  while (!IsListEmpty (&Statement->PropertyListHead)) {
    Link = GetFirstNode (&Statement->PropertyListHead);
    Property = H2O_PROPERTY_INFO_NODE_FROM_LINK (Link);
    RemoveEntryList (&Property->Link);
    FreePool (Property);
  }

  FBFreePool ((VOID **) &Statement->Expression);
  FBFreePool ((VOID **) &Statement->VariableName);
  FBFreePool ((VOID **) &Statement->BlockName);
  FBFreePool ((VOID **) &Statement->BufferValue);
  FBFreePool ((VOID **) &Statement->ImportInfo);
  FBFreePool ((VOID **) &Statement->VfrImportInfo);
  ...
  )
{
  ...
  // Free Property List
  //
  while (!IsListEmpty (&Form->PropertyListHead)) {
    Link = GetFirstNode (&Form->PropertyListHead);
    Property = H2O_PROPERTY_INFO_NODE_FROM_LINK (Link);
    RemoveEntryList (&Property->Link);
    FreePool (Property);
  }
  ...
  )
{
  ...
  if (FormSet->DisplayLink.ForwardLink != NULL && FormSet->DisplayLink.BackLink != NULL && !IsListEmpty (&FormSet->DisplayLink)) {
    RemoveEntryList (&FormSet->DisplayLink);
  }
  RemoveHiiHandle (FormSet->HiiHandle);
  ...
  // Free Property List
  //
  while (!IsListEmpty (&FormSet->PropertyListHead)) {
    Link = GetFirstNode (&FormSet->PropertyListHead);
    Property = H2O_PROPERTY_INFO_NODE_FROM_LINK (Link);
    RemoveEntryList (&Property->Link);
    FreePool (Property);
  }

  DestroyLinkStatements (FormSet);
...
/**
  Check if the opcode is the EFI_IFR_GUID_OP and ExtendOpCode is H2O_IFR_EXT_PCD
...
STATIC
BOOLEAN
IsH2oExpressionPcdOpCode (
  IN UINT8 *OpCodeData
  )
{
  H2O_IFR_GUID_PCD *PcdOp;
  BOOLEAN IsPcdOpCode;

  PcdOp = (H2O_IFR_GUID_PCD *)OpCodeData;
  IsPcdOpCode = FALSE;
  if  ((PcdOp->Header.OpCode == EFI_IFR_GUID_OP) &&
       CompareGuid((CONST GUID *)&PcdOp->Guid, &mH2OIfrExtGuid) &&
       (PcdOp->Function == H2O_IFR_EXT_PCD))  {
    IsPcdOpCode = TRUE;
  }
  return  IsPcdOpCode;
}
...
CountOpCodes (
  IN  FORM_BROWSER_FORMSET  *FormSet,
  IN OUT  UINT32            *NumberOfStatement,
  IN OUT  UINT32            *NumberOfExpression
  )
{
  ...
  while (Offset < FormSet->IfrBinaryLength) {
    ...
    if (IsExpressionOpCode (((EFI_IFR_OP_HEADER *) OpCodeData)->OpCode)  || IsH2oExpressionPcdOpCode(OpCodeData)) {
      ExpressionCount++;
    } else {
      StatementCount++;

      if (((EFI_IFR_OP_HEADER *) OpCodeData)->OpCode == EFI_IFR_GUID_OP) {
        if (CompareGuid (&mH2OIfrExtGuid, (EFI_GUID *)(OpCodeData + sizeof (EFI_IFR_OP_HEADER))) &&
          ((EFI_IFR_GUID_LABEL *) OpCodeData)->ExtendOpCode == H2O_IFR_EXT_LINK) {
          //
          // For link opcode
          //
          StatementCount += 100;
  ...
  )
{
  ...
  UINT8                   Width;
  UINT32                  NumberOfStatement;
  UINT32                  NumberOfExpression;
  ...
  INTN                    ConditionalExprCount;
  UINT32                  ExpressionOpCodeIndex;
  BOOLEAN                 InUnknownScope;
  UINT8                   UnknownDepth;
  H2O_PROPERTY_INFO       *CurrentProperty;
  H2O_IFR_GUID_LINK       *CurrentLink;
  LINK_DATA               LinkData;
  H2O_VFR_TARGET_INFO     *CurrentTargetInfo;
  ...
  InitializeListHead (&FormSet->PropertyListHead);
  InitializeListHead (&FormSet->LinkStatementListHead);
  ...
  while (OpCodeOffset < FormSet->IfrBinaryLength) {
    ...
    if (IsExpressionOpCode (Operand) || IsH2oExpressionPcdOpCode(OpCodeData)) {
      ExpressionOpCode = &FormSet->ExpressionBuffer[ExpressionOpCodeIndex];
      ExpressionOpCodeIndex++;
      ...
      case EFI_IFR_GUID_OP:
        ExpressionOpCode->GuidOpCodeData = (VOID *)OpCodeData;
        break;
      ...
      case EFI_IFR_SET_OP:
        ...
        if ((ExpressionOpCode->VarStorage != NULL) &&
          ...
          ExpressionOpCode->ValueName = GetString (ExpressionOpCode->VarStoreInfo.VarName, FormSet->HiiHandle);
    ...
    case EFI_IFR_STRING_OP:
      ...
      CurrentStatement->HiiValue.BufferLen = CurrentStatement->StorageWidth + sizeof (CHAR16);
      InitializeRequestElement (FormSet, CurrentStatement, CurrentForm);
      break;
    ...
    case EFI_IFR_ONE_OF_OPTION_OP:
      ...
      InitializeListHead (&CurrentOption->PropertyListHead);
      ...
      //
      ParentStatement->NumberOfOptions++;
    ...
    case EFI_IFR_GUID_OP:
      if (CompareGuid (&gEfiIfrTianoGuid, (EFI_GUID *)(OpCodeData + sizeof (EFI_IFR_OP_HEADER)))) {
        ...
        case EFI_IFR_EXTEND_OP_LABEL:
          if (CurrentForm != NULL && HaveTargetInfo (&FormSet->Guid, CurrentForm->FormId, ((EFI_IFR_GUID_LABEL *) OpCodeData)->Number)) {
            CreateFakeStatementByVfrLabel ((EFI_IFR_GUID_LABEL *) OpCodeData, FormSet, CurrentForm);
          }
          break;
        ...
        case EFI_IFR_EXTEND_OP_BANNER:
          ...
          if (FormSet->SubClass == EFI_FRONT_PAGE_SUBCLASS) {
            CopyMem (
              &FormSet->BannerData.Banner[((EFI_IFR_GUID_BANNER *) OpCodeData)->LineNumber][
      ...
      } else if (CompareGuid (&mH2OIfrExtGuid, (EFI_GUID *)(OpCodeData + sizeof (EFI_IFR_OP_HEADER)))) {
        ...
        case H2O_IFR_EXT_PROPERTY:
          CurrentProperty = AllocateZeroPool (sizeof (H2O_PROPERTY_INFO));
          if (CurrentProperty == NULL) {
            return EFI_OUT_OF_RESOURCES;
          }
          CurrentProperty->Signature     = H2O_PROPERTY_INFO_NODE_SIGNATURE;
          CurrentProperty->ValueType     = ((H2O_IFR_GUID_PROPERTY *)OpCodeData)->ValueType;
          CopyMem (&CurrentProperty->H2OValue, &((H2O_IFR_GUID_PROPERTY *)OpCodeData)->H2OValue, sizeof (H2O_VALUE));
          CurrentProperty->IdentifierStr = (CHAR8 *)OpCodeData + ((H2O_IFR_GUID_PROPERTY *)OpCodeData)->IdentifierOffset;
          CurrentProperty->ValueStr      = (CHAR8 *)OpCodeData + ((H2O_IFR_GUID_PROPERTY *)OpCodeData)->ValueOffset;
          //
          // Get ScopeOpcode from top of stack
          //
          PopScope (&ScopeOpCode);
          PushScope (ScopeOpCode);
          //
          // Add Property by ScopeOpCode
          //
          switch (ScopeOpCode) {

          case EFI_IFR_FORM_SET_OP:
            ImageId = &FormSet->ImageId;
            InsertTailList (&FormSet->PropertyListHead, &CurrentProperty->Link);
            break;
          ...
          case EFI_IFR_FORM_MAP_OP:
            ASSERT (CurrentForm != NULL);
            if (CurrentForm != NULL) {
              InsertTailList (&CurrentForm->PropertyListHead, &CurrentProperty->Link);
            }
            break;

          case EFI_IFR_ONE_OF_OPTION_OP:
            InsertTailList (&CurrentOption->PropertyListHead, &CurrentProperty->Link);
            break;

          default:
            if (CurrentTargetInfo != NULL) {
              InsertTailList (&CurrentTargetInfo->PropertyListHead, &CurrentProperty->Link);
            } else if (CurrentStatement != NULL) {
              InsertTailList (&CurrentStatement->PropertyListHead, &CurrentProperty->Link);
            } else {
        ...
        case H2O_IFR_EXT_LINK:
          CurrentLink = (H2O_IFR_GUID_LINK *)OpCodeData;
          ZeroMem (&LinkData, sizeof (LINK_DATA));
          break;

        case H2O_IFR_EXT_IMPORT:
          ASSERT (CurrentForm != NULL);
          if (CurrentForm == NULL) {
            return EFI_LOAD_ERROR;
          }
          CurrentStatement = CreateFakeStatementByVfrImport ((H2O_IFR_GUID_IMPORT *)OpCodeData, FormSet, CurrentForm);
          //
          // Update Scope Stack
          //
          if (CurrentStatement != NULL) {
            Operand = CurrentStatement->Operand;
            if (Scope != 0) {
              Status = PopScope (&ScopeOpCode);
              Status = PushScope (CurrentStatement->Operand);
            }
          }
          break;

        case H2O_IFR_EXT_LINK_TARGET:
          ASSERT (CurrentForm != NULL);
          if (CurrentForm == NULL) {
            return EFI_LOAD_ERROR;
          }
          CurrentTargetInfo = CreateVfrTargetInfoByVfrLinkTarget ((H2O_IFR_GUID_LINK_TARGET *) OpCodeData, CurrentForm);
          break;

        case H2O_IFR_EXT_IMPORT_TARGET:
          ASSERT (CurrentForm != NULL);
          if (CurrentForm == NULL) {
            return EFI_LOAD_ERROR;
          }
          CurrentTargetInfo = CreateVfrTargetInfoByVfrImportTarget ((H2O_IFR_GUID_IMPORT_TARGET *) OpCodeData, CurrentForm);
          break;

        case H2O_IFR_EXT_EXT_FLAGS:
          ASSERT (CurrentStatement != NULL);
          if (CurrentStatement == NULL) {
            return EFI_LOAD_ERROR;
          }
          CurrentStatement->ExtFlags = ((H2O_IFR_GUID_EXT_FLAGS *) OpCodeData)->ExtFlags;
          break;

        case H2O_IFR_EXT_INCLUDE_FORM_SET:
          ASSERT (CurrentLink != NULL);
          if (CurrentLink == NULL) {
            return EFI_LOAD_ERROR;
          }
          ParseLinkOpCodeInsertGuid (((H2O_IFR_GUID_INCLUDE_FORM_SET *)OpCodeData)->Function, &((H2O_IFR_GUID_INCLUDE_FORM_SET *)OpCodeData)->IncludeGuid, &LinkData);
          break;

        case H2O_IFR_EXT_EXCLUDE_FORM_SET:
          ASSERT (CurrentLink != NULL);
          if (CurrentLink == NULL) {
            return EFI_LOAD_ERROR;
          }
          ParseLinkOpCodeInsertGuid (((H2O_IFR_GUID_EXCLUDE_FORM_SET *)OpCodeData)->Function, &((H2O_IFR_GUID_EXCLUDE_FORM_SET *)OpCodeData)->ExcludeGuid, &LinkData);
          break;
    ...
    case EFI_IFR_END_OP:
      ...
      case EFI_IFR_GUID_OP:
        if (CurrentLink != NULL) {
          ...
          // End of Link Op Code
          //
          ParseLinkOpCodeEnd (&LinkData, FormSet, CurrentForm);
          CurrentLink = NULL;
      ...
      default:
        if (IsExpressionOpCode (ScopeOpCode) || IsH2oExpressionPcdOpCode(OpCodeData)) {
          if (InScopeDisable && CurrentForm == NULL) {
            //
            // This is DisableIf expression for Form, it should be a constant expression
            //
