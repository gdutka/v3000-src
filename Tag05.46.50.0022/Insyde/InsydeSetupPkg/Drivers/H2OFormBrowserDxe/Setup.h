...
typedef struct _FB_VFR_IMPORT_INFO {
  EFI_IFR_SUBTITLE              IfrSubtitleOpcode;
...
#define IS_VFR_IMPORT_FAKE_STATEMENT(Statement)      ((BOOLEAN) ((Statement) != NULL && (Statement)->VfrImportInfo != NULL))
...
struct _FORM_BROWSER_FORM {
  ...
  LIST_ENTRY           PropertyListHead;
  LIST_ENTRY           TargetInfoListHead;   // List of import/link target info
};
...
struct _FORM_BROWSER_FORMSET {
  ...
  EFI_QUESTION_ID                 *ExpressionQIdMapList[2];
  UINTN                           ExpressionQIdMapListCount;
};
