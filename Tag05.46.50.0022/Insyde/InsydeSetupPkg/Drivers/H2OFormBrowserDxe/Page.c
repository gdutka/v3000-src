...
/**
 ...
 @param[in, out] Form              Specified Form
**/
VOID
DestroyPage (
  ...
  )
{
  ...
  if (Private == NULL || FormSet == NULL || Form == NULL || Page == NULL) {
    return;
  }
  ...
  Page->FormId               = Form->FormId;

  Page->Attribs = 0;
  if (EvaluateExpressionList(Form->SuppressExpression, TRUE, FormSet, Form) == ExpressSuppress) {
    Page->Attribs |= H2O_ATTRIB_P_SUPPRESSED;
  }

  CopyMem (&Page->FormsetGuid, &FormSet->Guid, sizeof (EFI_GUID));
}
