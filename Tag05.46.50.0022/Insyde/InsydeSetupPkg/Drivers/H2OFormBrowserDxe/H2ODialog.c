  ...
  )
{
  ...
  if ((MaximumStringSize == 0) || (UserInputStringBuffer == NULL) || KeyValue == NULL) {
    return EFI_SUCCESS;
  }
  ...
  )
{
  ...
  if (NumberOfLines == 0 || EventKey == NULL || SelectIndex == NULL || String == NULL) {
    return EFI_INVALID_PARAMETER;
  }
  ...
  )
{
  ...
  BodyStringArray[0] = (ConfirmStrings   == NULL) ? L"" : ConfirmStrings;
  BodyStringArray[1] = (ShowStringBuffer == NULL) ? L"" : ShowStringBuffer;
