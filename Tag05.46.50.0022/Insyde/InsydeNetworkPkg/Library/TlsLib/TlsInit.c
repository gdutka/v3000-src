  ...
  )
{
  ...
  //
  // Clean up the X509Store->objs stack storage which will be used to store Cert information if it's not empty.
  //
  if (sk_X509_OBJECT_num(X509Store->objs) != 0){
    sk_X509_OBJECT_zero(X509Store->objs);
  }
