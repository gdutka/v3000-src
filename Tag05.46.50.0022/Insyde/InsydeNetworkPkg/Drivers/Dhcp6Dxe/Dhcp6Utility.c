...
#define EFI_DHCP6_PACKET_HEADER_SIZE ((sizeof(((EFI_DHCP6_PACKET *)0)->Size)) + (sizeof(((EFI_DHCP6_PACKET *)0)->Length)))
  ...
  )
{
  ...
  if (  (*PacketCursor < Packet->Dhcp6.Option)
     || (*PacketCursor >= Packet->Dhcp6.Option + (Packet->Size - sizeof (EFI_DHCP6_HEADER) - EFI_DHCP6_PACKET_HEADER_SIZE)))
  {
    return EFI_INVALID_PARAMETER;
  }
  ...
  //
  Status = SafeUint32Sub (Packet->Size, Packet->Length, &Length);
  if (!EFI_ERROR(Status)) {
    Status = SafeUint32Sub (Length, EFI_DHCP6_PACKET_HEADER_SIZE, &Length);
  }
  if (EFI_ERROR(Status)) {
    return EFI_INVALID_PARAMETER;
  }
