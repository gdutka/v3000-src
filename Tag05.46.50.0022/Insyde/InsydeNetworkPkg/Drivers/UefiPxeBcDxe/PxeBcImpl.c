...
/**
  This function is signaled when PXE error occurred.
...
VOID
EFIAPI
PxeFinishEventDummyFunction (
  IN     EFI_EVENT                Event,
  IN     VOID                     *Context
  );

/**
  Get the agent handle of a protocol.

  @param[in]   Handle           The handle for the protocol interface that is being queried.
  @param[in]   Protocol         The published unique identifier of the protocol.
  @param[out]  ParentHandle     The agent handle.
...
**/
EFI_STATUS
GetAgentHandleByDriver (
  IN     EFI_HANDLE                  Handle,
  IN     EFI_GUID                    *ProtocolName,
     OUT EFI_HANDLE                  *ParentHandle
  )
{
  EFI_STATUS                  Status;
  EFI_OPEN_PROTOCOL_INFORMATION_ENTRY  *OpenInfoEntries;
  UINTN                       OpenInfoEntriesCounter;
  UINTN                       Index;
  ...
  Status = gBS->OpenProtocolInformation (
                  ...
                  ProtocolName,
                  &OpenInfoEntries,
                  &OpenInfoEntriesCounter
                  );
  if (EFI_ERROR(Status)) {
    return EFI_NOT_FOUND;
  }
  for (Index = 0; Index < OpenInfoEntriesCounter; Index++) {
    if (OpenInfoEntries[Index].ControllerHandle != Handle) {
      continue;
    }
    if ((OpenInfoEntries[Index].Attributes & EFI_OPEN_PROTOCOL_BY_DRIVER) != EFI_OPEN_PROTOCOL_BY_DRIVER) {
      continue;
    }
    *ParentHandle = OpenInfoEntries[Index].AgentHandle;
    return EFI_SUCCESS;
  }
  return EFI_NOT_FOUND;
}
...
/**
  Get NIC's MAC address and convert to an unicode string.

  @param[in]   UndiHandle       The UNDI of a NIC.
  @param[out]  MacString        MAC address in unicode string.
  ...
  )
{
  ...
  Status = NetLibGetMacAddress (UndiHandle, &MacAddr, &AddressSize);
  if (EFI_ERROR (Status)) {
    return Status;
  }

  //
  // MacString format "XX-XX-XX-XX-XX-XX"
  //   6 set hex value and 5 frame char and 1 end of string
  //
  StringSize   = sizeof (CHAR16) * ((AddressSize * 2) + AddressSize - 1 + 1);
  TmpMacString = AllocateZeroPool (StringSize);
  if (TmpMacString == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }
  UnicodeSPrint (
    TmpMacString, StringSize,
  ...
  *MacString = TmpMacString;
  return EFI_SUCCESS;
}

/**
  Get the device/driver handle of a device path.

  @param[in]   Protocol         The published unique identifier of the protocol.
  @param[in]   DevicePath       The device path of NIC.
  @param[out]  DeviceHandle     The device handle of NIC.
  @param[out]  DriverHandle     The agent handle of NIC.
...
**/
EFI_STATUS
GetParentDevicePathDriverHandle (
  ...
  )
{
  ...
  EFI_DEVICE_PATH             *DevicePathPtr;
  EFI_HANDLE                  TmpDeviceHandle;
  EFI_HANDLE                  TmpDriverHandle;

  TmpDeviceHandle = NULL;
  TmpDriverHandle = NULL;
  DevicePathPtr = NULL;
  TmpDevicePath = DuplicateDevicePath (DevicePath);
  if (TmpDevicePath == NULL) {
    return EFI_NOT_FOUND;
  }
  EndDevicePath = TmpDevicePath;
  DevicePath    = TmpDevicePath;
  while (IsDevicePathEnd (EndDevicePath)) {
    DevicePathPtr = EndDevicePath;
    EndDevicePath = NextDevicePathNode (EndDevicePath);
  }
  ...
  *DriverHandle = TmpDriverHandle;
  return EFI_SUCCESS;
}

/**
  Display PXE device information.

  @param[in]   VirtualNic       Virtual NIC structure.
  @param[in]   UsingIpv6        PXE is using IPv6.
...
**/
VOID
PxeDisplayBootInformation (
  IN     PXEBC_VIRTUAL_NIC           *VirtualNic,
  ...
  )
{
  ...
  CHAR16                      *DriverName;
  CHAR16                      *DeviceHandleString;
  CHAR16                      *MacString;
  EFI_DEVICE_PATH             *DrvicePath;
  H2O_PXE_DEVICE_INFO_PROTOCOL H2oPxeDeviceInfo;
  VOID                        *InformationDisplayEnablePtr;
  BOOLEAN                     InformationDisplayEnable;

  DriverHandle = NULL;
  DriverName   = NULL;
  DrvicePath   = NULL;
  MacString    = NULL;
  DeviceHandleString = NULL;
  InformationDisplayEnable = FALSE;
  Private = VirtualNic->Private;

  Status = GetMacString (Private->Controller, &MacString);
  if (EFI_ERROR (Status)) {
    MacString = NULL;
  }

  Status = GetParentDevicePathDriverHandle (
  ...
  if (DeviceHandle != NULL) {
    ...
    if (!EFI_ERROR (Status)) {
      DeviceHandleString = ConvertDevicePathToText (
                             DrvicePath,
                             TRUE,
                             TRUE
                             );
...
EXIT:
  InformationDisplayEnablePtr = PcdGetExPtr (&gInsydeTokenSpaceGuid, PcdH2oPxeDisplayDeviceInformation);
  if (InformationDisplayEnablePtr != NULL) {
    InformationDisplayEnable = *((BOOLEAN *) InformationDisplayEnablePtr);
  }

  ZeroMem (&H2oPxeDeviceInfo, sizeof (H2O_PXE_DEVICE_INFO_PROTOCOL));
  H2oPxeDeviceInfo.PxeBootHandle        = VirtualNic->Controller;
  H2oPxeDeviceInfo.UndiControllerHandle = Private->Controller;
  H2oPxeDeviceInfo.DeviceHandle         = DeviceHandle;
  H2oPxeDeviceInfo.DeviceInfo           = DeviceHandleString;
  H2oPxeDeviceInfo.UndiControllerInfo   = DriverName;
  H2oPxeDeviceInfo.MacString            = MacString;
  H2oPxeDeviceInfo.DisplayEnables       = InformationDisplayEnable;

  Status = gBS->InstallProtocolInterface (
                  &VirtualNic->Controller,
                  &gH2oPxeDeviceInfoProtocolGuid,
                  EFI_NATIVE_INTERFACE,
                  (VOID *) &H2oPxeDeviceInfo
                  );
  ...
  if (H2oPxeDeviceInfo.DisplayEnables) {
    if (H2oPxeDeviceInfo.ProjectHookDeviceInformation == NULL) {
      AsciiPrint (
        "PXE boot %a\n  boot from device : %s\n  MAC Address : %s\n  Controller Driver Name : %s\n",
        (UsingIpv6) ? "IPv6" : "IPv4",
        (DeviceHandleString != NULL) ? DeviceHandleString : L"",
        (MacString != NULL) ? MacString : L"",
        (DriverName != NULL) ? DriverName : L"DRIVER NAME of UNDI"
        );
    } else {
      AsciiPrint ("%s\n", H2oPxeDeviceInfo.ProjectHookDeviceInformation);
    }
  }

  if (DeviceHandleString != NULL) {
    FreePool (DeviceHandleString);
  }
  if (MacString != NULL) {
    FreePool (MacString);
  }
  ...
  )
{
  ...
  /// Insyde modify, to keep UsingIpv6 in Mode
  BOOLEAN                 UsingIpv6;
...
{
/// Insyde modify, to handle cancel key to exit PXE processing.
//  EFI_INPUT_KEY       Key;
//  EFI_STATUS          Status;
//
//  //
  ...
  )
{
  ...
  //
  // Install gH2ONetworkPxeRunningGuid protocol to indicate PXE running
  //
  InsydeNetworkLibPxeStart (&Private->Image);

  Status = gBS->LocateProtocol (&gEfiConsoleControlProtocolGuid, NULL, (VOID **) &ConsoleControl);
  if (!EFI_ERROR (Status)) {
    Status = ConsoleControl->GetMode(ConsoleControl, &ScreenMode, NULL, NULL);
    if (!EFI_ERROR(Status) && ScreenMode == EfiConsoleControlScreenGraphics) {
      //
      // Fill black to clean graphic screen then switch to Text Mode.
      //
  ...
  //
  // If first entry, relocate the cursor to (0, 0)
  //
  if (!Private->CursorRelocation) {
    Private->CursorRelocation = TRUE;
  ...
  CheckMediaRetryTimes = H2OGetNetworkProtocolRetryTimes();

  for (Index = 0; Index <= CheckMediaRetryTimes; Index++) {
    ...
    // Media check retry
    //
    if (CheckMediaRetryTimes == H2O_NETWORK_BOOT_INFINITELY_RETRY){
        AsciiPrint ("  Network media is not present, retry media connection infinitely.....\n");
      } else if (Index < CheckMediaRetryTimes) {
        AsciiPrint ("  Network media is not present, retry media connection for %d time(s).\n", Index + 1);
      } else if (Index == CheckMediaRetryTimes) {
        break;
      }
      gBS->Stall (NETWORK_MEDIA_CHECK_STALL_TIME);
  }
  if (MediaStatus != EFI_SUCCESS) {
    AsciiPrint ("\n  Error: Could not detect network connection.\n");
    gBS->Stall (NETWORK_ERROR_SHOW_STALL_TIME);
    InsydeNetworkLibPxeEnd (Private->Image);
    return EFI_NO_MEDIA;
  }
  ...
  //
  // Start Pxe Base Code to initialize PXE boot, retry if it fails.
  //
  for (Index = 0; Index <= CheckMediaRetryTimes; Index++) {
    ...
    if (Status != EFI_SUCCESS &&
      ...
      PxeBc->Stop (PxeBc);
      if (CheckMediaRetryTimes == H2O_NETWORK_BOOT_INFINITELY_RETRY){
        AsciiPrint ("  Connect to server fail, retry connection infinitely.....\n");
      } else if (Index < CheckMediaRetryTimes) {
        AsciiPrint ("  Connect to server fail, retry connection for %d time(s).\n", Index + 1);
      } else if (Index == CheckMediaRetryTimes) {
        //
        // Stall so that user can see the error message.
        //
        gBS->Stall (NETWORK_ERROR_SHOW_STALL_TIME);
      }
    } else {
  ...
  //
  // If PXE error, signal event to oem
  //
  if (EFI_ERROR (Status) && (Status != EFI_BUFFER_TOO_SMALL) && (Status != EFI_ABORTED)) {
    EventStatus = gBS->CreateEventEx (
                    EVT_NOTIFY_SIGNAL,
                    TPL_NOTIFY,
                    PxeFinishEventDummyFunction,
                    NULL,
                    &gEfiNetworkEventPxeFailed,
                    &PxeFinishEvent
                    );
    if (!EFI_ERROR (EventStatus)) {
      gBS->SignalEvent (PxeFinishEvent);
      gBS->CloseEvent (PxeFinishEvent);
    }
  }

  //
  // Uninstall gH2ONetworkPxeRunningGuid protocol to indicate PXE running finished
  //
  InsydeNetworkLibPxeEnd (Private->Image); 
