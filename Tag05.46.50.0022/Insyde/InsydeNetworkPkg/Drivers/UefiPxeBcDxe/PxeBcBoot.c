  ...
  )
{
  ...
  *Type     = 0;
  ...
  ZeroMem (MenuArray, sizeof (MenuArray));
  ...
  )
{
  ...
  *BufferSize = 0;
  ...
  //
  // Handle the case of Pxe server is not also TFTP server.
  // If ServerIp.Addr[0] is not zero, check if the boot file exists, if not, check the other Ip.
  //
  while (TRUE) {
    ...
    if (Private->ServerIp.Addr[0] == 0) {
      ...
      GetTheOtherServerIp = TRUE;
    } else {
      GetTheOtherServerIp = FALSE;
    }
    ...
    if (*BufferSize !=  0 || GetTheOtherServerIp){
      break;
    } else {
