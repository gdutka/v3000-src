  ...
  )
{
  ...
  if (Snp->Mode->State == EfiSimpleNetworkInitialized) {
    //
    // If SNP is already initialized, it might not be called by BIOS, may be called by Linux grub,
    // so we need Shutdown/Stop and then Start/Initialize.
    //
  ...
  if (!EFI_ERROR (Status)) {
    // CP to config MAC address
    // gH2OBdsCpNetworkUpdateMacAddrGuid
    if (FeaturePcdGet (PcdH2OBdsCpNetworkUpdateMacAddrSupported)) {
      BdsNetworkUpdateMacAddrData.Size      = sizeof (H2O_BDS_CP_NETWORK_UPDATE_MAC_ADDR_DATA);
      BdsNetworkUpdateMacAddrData.NicHandle = ControllerHandle;
      BdsNetworkUpdateMacAddrData.Status    = H2O_CP_TASK_NORMAL;
      ZeroMem (&BdsNetworkUpdateMacAddrData.NicMacAddr, sizeof (EFI_MAC_ADDRESS));
      Status = H2OCpTrigger  (&gH2OBdsCpNetworkUpdateMacAddrGuid, (VOID *) &BdsNetworkUpdateMacAddrData);
      if (BdsNetworkUpdateMacAddrData.Status == H2O_CP_TASK_UPDATE) {
        Snp->StationAddress (Snp, FALSE, &BdsNetworkUpdateMacAddrData.NicMacAddr);
      }
    }
  }
  ...
  )
{
  ...
  if (!IsConfigUpdate) {
    ...
    if (MnpDeviceData->ConfiguredChildrenNumber == 1) {
      ...
      //
      Status = MnpStartSnp (MnpDeviceData->ControllerHandle, MnpDeviceData->Snp);
      if (EFI_ERROR (Status)) {
