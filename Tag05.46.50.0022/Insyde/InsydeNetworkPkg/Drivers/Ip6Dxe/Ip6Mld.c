  ...
  )
{
  ...
  if (Ip6FindMac (&IpSb->MldCtrl, &Group->Mac) == 1) {
    ...
    if (EFI_ERROR (Status) && (Status != EFI_NOT_FOUND) && (Status != EFI_DEVICE_ERROR)) {
      return Status;
    }
  }
  ...
  if (Status == EFI_DEVICE_ERROR) {
    /// This function should be pass if MNP report EFI_DEVICE_ERROR, maybe it means the LAN is not workable like USB LAN hot plug out.
    Status = EFI_SUCCESS;
  } else if (Group->SendByUs) {
