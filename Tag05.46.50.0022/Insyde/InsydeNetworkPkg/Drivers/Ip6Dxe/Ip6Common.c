  ...
  )
{
  ...
  EFI_STATUS                   Status;
  IP6_CONFIG_VARIABLE          *Ip6ConfigVar;
  UINTN                        Ip6ConfigVarSize;
  ...
  //
  // Get the previously recorded Mac Address to check if the Nic is changed.
  // If yes, use current Mac Address to create the Ipv6 interface id.
  //
  ...
  if (Status == EFI_NOT_FOUND || 
      CompareMem (&Ip6ConfigVar->PreviousMacAddress, &IpSb->Ip6ConfigInstance.InterfaceInfo.HwAddress, sizeof(EFI_MAC_ADDRESS)) != 0) {

    FreePool (Ip6ConfigVar);
    //
