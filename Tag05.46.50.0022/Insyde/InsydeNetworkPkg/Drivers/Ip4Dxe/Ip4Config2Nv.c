  ...
  )
{
  ...
  IfrNvData->Configure = (BOOLEAN)((Policy == Ip4Config2PolicyDhcp) ||
                                   (Policy == Ip4Config2PolicyStatic && IpSb->DefaultInterface->Configured));
  mIp4UserConfiguredOrg = IfrNvData->Configure;
  if (!IfrNvData->Configure) {
    goto Exit;
  }
...
/**
  Free Ip4NvData and set all entries to NULL or 0.

  @param[in]  Ip4NvData   Pointer to Ip4 configure data.

**/
VOID
FreeIp4NvData (
  IN  IP4_CONFIG2_NVDATA    *Ip4NvData
  )
{
  if (Ip4NvData == NULL) {
    return;
  ...
  ZeroMem (Ip4NvData, sizeof(IP4_CONFIG2_NVDATA));
}
  ...
  )
{
  ...
  if (!mIp4UserConfiguredOrg && !IfrFormNvData->Configure) {
    return EFI_SUCCESS;
  }

  if (IfrFormNvData->Configure == FALSE) {
    //
    // Run to here means user change configured from enabled to disabled.
    // Clear all the settings so that the correct info can be saved.
    //
    ZeroMem (IfrFormNvData, sizeof(IP4_CONFIG2_IFR_NVDATA));
    SetUserConfig = FALSE;
  }

  FreeIp4NvData (Ip4NvData);
  ...
  if (IfrFormNvData->DhcpEnable == TRUE) {
    ...
    if (SetUserConfig) {
      ...
      //
      // Set user configured info into Ip4NvData.
      //
