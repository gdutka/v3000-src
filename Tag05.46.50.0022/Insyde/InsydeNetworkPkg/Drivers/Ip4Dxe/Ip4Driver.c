  ...
  )
{
  ...
  if (NetworkLockerCallback (ControllerHandle, IP4)) {
  ...
  )
{
  ...
  *Service = NULL;
  ...
  //
  // Enhance configuration variable to use root DevicePath as name, not MAC.
  // To avoid it will use too much space to record many UNDI cards.
  // If MacAddressSize is 0, it will use MAC string as variable name.
  //
  Status = GetUniqueStringByDevicePath (IpSb->Controller, &IpSb->MacString);
  if (!EFI_ERROR (Status)) {
    RemoveUnnecessaryConfigureVariable (&gEfiIp4Config2ProtocolGuid, IP4_CONFIG2_VARIABLE_ATTRIBUTE);
  }
