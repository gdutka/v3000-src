  ...
  )
{
  ...
  //
  // The name server information in the form of DNS device path node is optional, 
  // and is used only when the BootURI contains the server name or FQDN.
  //
  if (Private->UsingIpv6) {
    //
    ...
    Status = HttpUrlGetIp6 (
               (Private->FilePathUri == NULL) ? Private->BootFileUri : Private->FilePathUri,
  ...
  // EFI_SUCCESS: IP address -> Can not contain DNS device path.
  // Others: Server Name or FQDN -> contain DNS device path.
  //

  //
  // Update the DNS node with DNS server IP list if existed.
  //
  if ((Private->UrlBootNic == NULL) && (Private->DnsServerIp != NULL) && (EFI_ERROR (Status))) {
  ...
  //
  CurrentUri = (Private->FilePathUri == NULL) ? Private->BootFileUri : Private->FilePathUri;
  if (CurrentUri == NULL) {
    if (TmpIpDevicePath != NULL) {
      FreePool (TmpIpDevicePath);
    }
  ...
  Length = sizeof (EFI_DEVICE_PATH_PROTOCOL) + AsciiStrSize (CurrentUri);
  Node = AllocatePool (Length);
  if (Node == NULL) {
    if (TmpIpDevicePath != NULL) {
      FreePool (TmpIpDevicePath);
  ...
  SetDevicePathNodeLength (Node, Length);
  CopyMem ((UINT8*) Node + sizeof (EFI_DEVICE_PATH_PROTOCOL), CurrentUri, AsciiStrSize (CurrentUri));
  ...
  if (Private->UrlBootNic != NULL) {
    ...
    Status = gBS->ReinstallProtocolInterface (
                    Private->UrlBootNic->Controller,
                    &gEfiDevicePathProtocolGuid,
                    Private->UrlBootNic->DevicePath,
                    NewDevicePath
                    );
    if (EFI_ERROR (Status)) {
      return Status;
    }
    FreePool (Private->UrlBootNic->DevicePath);
    Private->UrlBootNic->DevicePath = NewDevicePath;
  } else if (!Private->UsingIpv6) {
    //
    // Reinstall the device path protocol of the child handle.
    //
    Status = gBS->ReinstallProtocolInterface (
...
//
// Don't update DevicePath if HttpBoot is not complete.
//
//  //
