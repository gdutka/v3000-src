  ...
  )
{
  ...
  if (Private->BootFileUri == NULL) {
    ...
    if (Status == EFI_NO_MAPPING) {
      //
      // Network stack is connecting but not ready yet.
      // Wait 1000 ms for network stack establishment.
      //
      gBS->Stall (EFI_TIMER_PERIOD_MILLISECONDS (1000));
      Status = HttpBootDiscoverBootInfo (Private);
    }
    if (EFI_ERROR (Status)) {
  ...
  //
  // Update DevicePath to append URL, Modify original HttpBootDxe driver.
  // This version will update device path later, after FILE get complete.
  //
  if (!EFI_ERROR (Status)) {
  ...
  )
{
  ...
  EFI_DEVICE_PATH_PROTOCOL      *LoadFileDevicePath;
  ...
  //
  // Check media status before HTTP boot start, retry if it fails.
  //
  ...
  Private->UrlBootNic = NULL;
  if ((VirtualNic != Private->Ip4Nic) && (VirtualNic != Private->Ip6Nic)) {
    Status = gBS->HandleProtocol (
                    VirtualNic->Controller,
                    &gEfiDevicePathProtocolGuid,
                    (VOID **) &LoadFileDevicePath
                    );
    if (!EFI_ERROR (Status)) {
      FilePath = LoadFileDevicePath;
      Private->UrlBootNic = VirtualNic;
      UsingIpv6 = VirtualNic->IsIPv6;
    }
  }

  //
  // Initialize connection and connect to the HTTP server, retry if it fails.
  //
  for (Index = 0; Index <= CheckMediaRetryTimes; Index++) {
    //
