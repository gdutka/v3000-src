...
HTTP_BOOT_FORM_CALLBACK_INFO           mFormCallbackInfo;
  ...
  )
{
  ...
  if (NetworkLockerCallback (ControllerHandle, HTTPBOOT4)) {
  ...
  )
{
  ...
  if (NetworkLockerCallback (ControllerHandle, HTTPBOOT6)) {
    return EFI_UNSUPPORTED;
  }
...
/**
  This is the function when unload handle for HttpBootDxe driver.
...
EFI_STATUS
EFIAPI
HttpBootDxeDriverUnload (
  IN EFI_HANDLE        ImageHandle
  )
{
  HttpBootConfigLinkFormUnload (&mFormCallbackInfo);
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  //
  // Install form for SCU\Boot\Link
  //
  Status = HttpBootConfigLinkFormInit (&mFormCallbackInfo);
  if (EFI_ERROR (Status)) {
    return Status;
  }
