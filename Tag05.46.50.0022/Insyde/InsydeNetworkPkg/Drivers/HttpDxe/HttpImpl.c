  ...
  )
{
  ...
  if (HttpMsg->Data.Response != NULL) {
    ...
    /// Update TICKS_PER_SECOND to TICKS_PER_MS for RedFish request from RedFish.
  ...
  if (!HttpInstance->UseHttps) {
    ...
    TimeoutValue = PcdGet32 (PcdHttpIoTimeout);
////////////////////////////////////
/// Modification for RedFish Start
////////////////////////////////////
    //
    ...
    //
    HttpInstance->CallbackData.Wrap = (VOID *) Wrap;
    HttpInstance->CallbackData.ParseData = Fragment.Bulk;       /// HttpMsg->Body;
    HttpInstance->CallbackData.ParseDataLength = Fragment.Len;  /// HttpMsg->BodyLength;
    ...
    Status = HttpParseMessageBody (
               HttpInstance->MsgParser,
               (UINTN) Fragment.Len,            /// HttpMsg->BodyLength,
               (CHAR8 *) Fragment.Bulk          /// HttpMsg->Body
               );
    if (EFI_ERROR (Status)) {
      goto Error2;
    }
    ...
    //
    if (HttpInstance->NextMsg != NULL) {
      HttpMsg->BodyLength = HttpInstance->NextMsg - (CHAR8 *) Fragment.Bulk;  /// HttpMsg->Body;
    }
////////////////////////////////////
/// Modification for RedFish End
////////////////////////////////////
