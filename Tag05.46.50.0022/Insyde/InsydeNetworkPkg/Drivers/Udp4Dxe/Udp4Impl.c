  ...
  )
{
  ...
  Len = sizeof (IP4_ICMP_ERROR_HEAD) + (EFI_IP4_HEADER_LEN (IpHdr) -
        (UINT32)sizeof (IP4_HEAD)) + ICMP_ERROR_PACKET_LENGTH;
  ...
  //
  HeaderLength = EFI_IP4_HEADER_LEN (IpHdr);
  CopyMem (&IcmpErrHdr->IpHead, IpHdr, HeaderLength);

  //
  // Copy the UDP header.
  //
  HeaderLength = EFI_IP4_HEADER_LEN (IpHdr);
  Ptr = (UINT8 *) &IcmpErrHdr->IpHead + HeaderLength;
