...
//[-start-171020-IB14630127-remove]//
...
//[-end-170824-IB14630122-modify]//
    ...
    If(LEqual(TBTS, 0x01)) {
      If(LEqual(RPN0, 0x01)) {
        Acquire(OSUM, 0xFFFF)
        \_GPE.TINI(DTBT_CONTROLLER,RPS0, RPT0)
        Release(OSUM)
      }
      If(LEqual(RPN1, 0x01)) {
        Acquire(OSUM, 0xFFFF)
        \_GPE.TINI(DTBT_CONTROLLER,RPS1, RPT1)
        Release(OSUM)
      }
    }
...
#if H2O_BOARD_NAME_SUPPORTED(BoardIdCoffeeLakeSUdimm)
    If (H2O_BOARD_SUPPORTED(0x1A)) {
      Store(H2O_BOARD_SUPPORTED(0x1A), Local0)
    }   
#endif

#if H2O_BOARD_SUPPORTED(0x10)
    If (H2O_BOARD_NAME_SUPPORTED(BoardIdCoffeeLakeSSodimm)) {
      Store(H2O_BOARD_NAME_SUPPORTED(BoardIdCoffeeLakeSSodimm), Local0)
    }
#endif
...
//
// ENVM - Enable Voltage margining (NVS variable) if premium PMIC is connected
//
If(LEqual(ENVM,1))
{
  Scope(\_SB){
...
Scope(\_SB.PCI0.I2C0) {
  //
  // MAXIM Microchips for all except Whiskeylake boards
  //
  if (LNotEqual(PLID, 0x14) && LNotEqual(PLID, 0x15)) {
  //
  ...
  //
    Device (PA01){
      ...
      Method (_CRS, 0, NotSerialized) {
        Name (RBUF, ResourceTemplate (){
          I2cSerialBus (0x10, ControllerInitiated, 400000, AddressingMode7Bit, "\\_SB.PCI0.I2C0", 0x00, ResourceConsumer, ,)
        })
      ...
      //
      // _DSM: will return power rail configuration information to the driver.
      //
      Method (_DSM, 4, NotSerialized)
      {
        ...
        // 1 channel exposed to the driver.
        If (LEqual (Arg2, One)) {
          If (LEqual (Arg1, Zero)) {
        ...
        // Indicate that 1 channel is exposed
        If (LEqual (Arg2, 0x02)){
          If (LEqual (Arg1, Zero)){
            Name (CBUF, Package (0x01){
              0x01                       // Number of Channels Above
            })
      ...
      } //_DSM Method Scope End
    } // PowerMeter Device Scope End
  }
}
