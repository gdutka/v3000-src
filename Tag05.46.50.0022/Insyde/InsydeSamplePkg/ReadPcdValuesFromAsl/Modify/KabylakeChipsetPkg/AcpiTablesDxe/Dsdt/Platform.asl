...
//
// Temporarily hard-coded here, will use Platform GNVS instead.
//
Name (H2OE, FixedPcdGet8 (PcdH2OPort80DebugEnable))
Name (P8XE, 0 )
Name (PFDP, FixedPcdGet8 (PcdPlatformDebugPort))

OperationRegion (PRTD, SystemIO, PFDP, 4)
...
Scope(\_SB.PCI0)
{
  ...
  Method(_INI,0, Serialized)
  {
    ...
    Store(\_SB.PCDE(gInsydeTokenSpaceGuid,PcdTestString), Local0)
...
If(LEqual(RTVM,1))
{
  Scope(\_SB){ 
      ...
      Method(VMON, 0, Serialized)
      {
          ...
          // Enable voltage margining on entering D3 - program PMIC V085ACNT[7:6]= 01
      ...
      Method(VMOF, 0, Serialized)
      {
          ...
          // Disable voltage margining on entering D0 - program PMIC V085ACNT[7:6]= 00
...
Scope(\_GPE)
{
  ...
  Method(PGWA, 0, Serialized)
  {
    ...
    If(LGreaterEqual(TBSE, 21))
    {
      Store(\_SB.PCI0.GPCB(), Local0) // MMIO Base address
      Add(Local0, ShiftLeft(TBTD(TBSE), 15), Local0) // Device no
      Add(Local0, ShiftLeft(TBTF(TBSE), 12), Local0) // Function no
  ...
  Method(TBFF, 0, Serialized)// Check for FFFF in TBT
  {
    ADBG("TBFF")
  ...
  } // End of Method(TBFF, 0, Serialized)
  ...
  Method(DRPM)// Disable root port MSI
  {
    ADBG("DRPM")

    Store(\_SB.STDC(\_SB.PCI0.GPCB(), 0x00, TBTD(TBSE), TBTF(TBSE), 0x05), Local7)
    Store(MMRP(), Local6)
    Add(Local7, Local6, Local0)

    OperationRegion (MMMS, SystemMemory, Local0, 0x2)
  ...
  Method(DMSI)
  {
    ...
    If(LEqual(Local1, 0x25))
    {
      ...
      ADBG("0x25->P2TB")
      Store(0x25, P2TB) // Write Disable MSI Finished
      ...
      While(LGreater(Local2, 0))
      {
        ...
        If(LEqual(Local1, 0xFFFFFFFF))// Device gone
        {
          ADBG("DMSiF")
          Return(0)// Trigger continuation of regular flow
        }
        If(LEqual(Local1, 0x24)) // Done
        {
          break
        }
        Sleep(50)
  ...
  Method(RSMI)// Disable MSI on root port
  {
    ADBG("RSMI")

    Store(TFPS(), Local0)
    Add(MMTB(), 0, Local1)
    OperationRegion(PXVD,SystemMemory,Local1,0x550) 
    Field(PXVD,DWordAcc, NoLock, Preserve)
    {
      VIDI, 32,
      Offset(0x548),
    ...
    Store(TB2P, Local2)
    If(LNotEqual(Local2, 0xFFFFFFFF))// HR is here continue normal flow
    {
      ADBG("RSMi 1")
      Return(1)
    }
    ...
    If(LNot(CNCT()))// Check for disconnect
    {
      ADBG("RSMi 2")
      Return(2)
    }
    ...
    // Poling for PCIe domain and GPIO_3 deasserted
    Store(0, Local3)
    While(LOr(FFTB(), CNCT()))
    {
      Sleep(5)
      CLNE()// Clean GPIO event
      Add(Local3, 1, Local3)
      If(LGreater(Local3, 20))// 100 ms
      {
        Sleep(1000)
        ADBG("RSMi PCIe down")
      }
      If(LGreater(Local3, 25))// 5100 ms
      {
        ADBG("RSMi Oops")
        // Restore Force Power
        If(Local0)
...
If(LAnd(LEqual(TBTS, 1),LEqual(TBSE, 1)))
{
  Scope(\_SB.PCI0.RP01)
  {
    Device(HRUS)// Host router Upstream port BusX:Device0x00:Function0x00
    {
      ...
      Device(HRDP) // Host router Downstream port BusX+1:Device0x01:Function0x00
      {
        Name(_ADR, 0x00010000)
        Method(_PRW, 0) { Return(GPRW(0x69, 4)) }  // can wakeup from S4 state

        Device(TDUS) // TBT Device Upstream port BusX+3:Device0x00:Function0x00
        {
          Name(_ADR, 0x00000000)
          Method(_PRW, 0) { Return(GPRW(0x69, 4)) }  // can wakeup from S4 state

          Device(TDDP) // TBT Device Downstream port BusX+4:Device0x01:Function0x00
          {
            Name(_ADR, 0x00010000)
            Method(_PRW, 0) { Return(GPRW(0x69, 4)) }  // can wakeup from S4 state

            Device(PXST) // DGPU Device BusX+5:Device0x00:Function0x00
            {
              Name(_ADR, 0x00000000)
              Method(_PRW, 0) { Return(GPRW(0x69, 4)) }  // can wakeup from S4 state
            }
...
If(LAnd(LEqual(TBTS, 1),LEqual(TBSE, 2)))
{
  Scope(\_SB.PCI0.RP02)
  {
...
If(LAnd(LEqual(TBTS, 1),LEqual(TBSE, 3)))
{
  Scope(\_SB.PCI0.RP03)
  {
    Device(HRUS)// Host router Upstream port
...
If(LAnd(LEqual(TBTS, 1),LEqual(TBSE, 4)))
{
  Scope(\_SB.PCI0.RP04)
  {
    Device(HRUS)// Host router Upstream port
...
If(LAnd(LEqual(TBTS, 1),LEqual(TBSE, 5)))
{
  Scope(\_SB.PCI0.RP05)
  {
    Device(HRUS)// Host router Upstream port BusX:Device0x00:Function0x00
...
If(LAnd(LEqual(TBTS, 1),LEqual(TBSE, 6)))
{
  Scope(\_SB.PCI0.RP06)
  {
    Device(HRUS)// Host router Upstream port
...
If(LAnd(LEqual(TBTS, 1),LEqual(TBSE, 7)))
{
  Scope(\_SB.PCI0.RP07)
  {
    Device(HRUS)// Host router Upstream port
...
If(LAnd(LEqual(TBTS, 1),LEqual(TBSE, 8)))
{
  Scope(\_SB.PCI0.RP08)
  {
    Device(HRUS)// Host router Upstream port
...
If(LAnd(LEqual(TBTS, 1),LEqual(TBSE, 10)))
{
  Scope(\_SB.PCI0.RP10)
  {
    Device(HRUS)// Host router Upstream port
...
If(LAnd(LEqual(TBTS, 1),LEqual(TBSE, 11)))
{
  Scope(\_SB.PCI0.RP11)
  {
    Device(HRUS)// Host router Upstream port
...
If(LAnd(LEqual(TBTS, 1),LEqual(TBSE, 12)))
{
  Scope(\_SB.PCI0.RP12)
  {
    Device(HRUS)// Host router Upstream port
...
If(LAnd(LEqual(TBTS, 1),LEqual(TBSE, 13)))
{
  Scope(\_SB.PCI0.RP13)
  {
    Device(HRUS)// Host router Upstream port
...
If(LAnd(LEqual(TBTS, 1),LEqual(TBSE, 14)))
{
  Scope(\_SB.PCI0.RP14)
  {
    Device(HRUS)// Host router Upstream port
...
If(LAnd(LEqual(TBTS, 1),LEqual(TBSE, 15)))
{
  Scope(\_SB.PCI0.RP15)
  {
    Device(HRUS)// Host router Upstream port
...
If(LAnd(LEqual(TBTS, 1),LEqual(TBSE, 16)))
{
  Scope(\_SB.PCI0.RP16)
  {
    Device(HRUS)// Host router Upstream port
...
If(LAnd(LEqual(TBTS, 1),LEqual(TBSE, 17)))
{
  Scope(\_SB.PCI0.RP17)
  {
    Device(HRUS)// Host router Upstream port
...
If(LAnd(LEqual(TBTS, 1),LEqual(TBSE, 18)))
{
  Scope(\_SB.PCI0.RP18)
  {
    Device(HRUS)// Host router Upstream port
...
If(LAnd(LEqual(TBTS, 1),LEqual(TBSE, 19)))
{
  Scope(\_SB.PCI0.RP19)
  {
    Device(HRUS)// Host router Upstream port
...
If(LAnd(LEqual(TBTS, 1),LEqual(TBSE, 20)))
{
  Scope(\_SB.PCI0.RP20)
  {
    Device(HRUS)// Host router Upstream port
...
If(LAnd(LEqual(TBTS, 1),LEqual(TBSE, 21)))
{
  Scope(\_SB.PCI0.PEG0)
  {
    Device(HRUS)// Host router Upstream port BusX:Device0x00:Function0x00
...
If(LAnd(LEqual(TBTS, 1),LEqual(TBSE, 22)))
{
  Scope(\_SB.PCI0.PEG1)
  {
    Device(HRUS)// Host router Upstream port
...
If(LAnd(LEqual(TBTS, 1),LEqual(TBSE, 23)))
{
  Scope(\_SB.PCI0.PEG2)
  {
    Device(HRUS)// Host router Upstream port
