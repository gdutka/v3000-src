  ...
  )
{
  ...
  ATA_DEVICE_SERVER_SECURITY_STATE_DATA  SecurityCheckStateData;  
  ...
  for (DevInfoTableIndex = 0; DevInfoTableIndex < NumOfDevInfoTable; DevInfoTableIndex++) {
    if (DevInfoPtr[DevInfoTableIndex].DeviceType != OPAL_DEVICE_TYPE_NVME &&
        DevInfoPtr[DevInfoTableIndex].DeviceType != OPAL_DEVICE_TYPE_TG) {
      continue;
    }
    ...
    ///
    P2pBase = PCI_LIB_ADDRESS (DevInfoPtr[DevInfoTableIndex].P2pBridgeBus, DevInfoPtr[DevInfoTableIndex].P2pBridgeDevice, DevInfoPtr[DevInfoTableIndex].P2pBridgeFunction, 0x0);
    RpBase = PCI_LIB_ADDRESS (DevInfoPtr[DevInfoTableIndex].Bus, DevInfoPtr[DevInfoTableIndex].Device, DevInfoPtr[DevInfoTableIndex].Function, 0x0);    
    MemoryBase = DevInfoPtr[DevInfoTableIndex].BarAddr;
    MemoryLength = 0;
    ConfigureRootPortForPcieNand (P2pBase, Bus, (UINT32) MemoryBase, (UINT32) MemoryLength);

    ///
    /// Enable PCIE decode for RootPort
    ///
    NvmeCmdSt = PciRead8 (P2pBase + NVME_PCIE_PCICMD);
    PciWrite8  (P2pBase + NVME_PCIE_PCICMD,  0x6);
    ...
    if ((BaseClassCode != PCI_CLASS_MASS_STORAGE) ||
      ...
      NvmeContext.Nbar = DevInfoPtr[DevInfoTableIndex].BarAddr;
      NvmeContext.PciBase = PCI_LIB_ADDRESS (Bus, Device, Function, 0x0);
      NvmeContext.NvmeInitWaitTime = 0;
      NvmeContext.Nsid = DevInfoPtr[DevInfoTableIndex].NvmeNamespaceId;
      NvmeContext.Remapped = FALSE;  
      ...
      DevInfoNvme = AllocateZeroPool(sizeof(OPAL_DEVICE_NVME));
      DevInfoNvme->BarAddr = DevInfoPtr[DevInfoTableIndex].BarAddr;
      CopyMem(DevInfoNvme->Password, DevInfoPtr[DevInfoTableIndex].Password, DevInfoPtr[DevInfoTableIndex].PasswordLength);
      DevInfoNvme->PasswordLength = DevInfoPtr[DevInfoTableIndex].PasswordLength;
      DevInfoNvme->NvmeNamespaceId = DevInfoPtr[DevInfoTableIndex].NvmeNamespaceId;
      OpalDev.Device = (OPAL_DEVICE_COMMON *) DevInfoNvme;
      OpalDev.Context = &NvmeContext;

      Session.Sscp = &OpalDev.Sscp;
      Session.MediaId = 0;
      SupportAhciServerPassword = OpalUpdateAhciServePasswordAttribute(Session);
      ZeroMem ((VOID*)&SecurityCheckStateData, sizeof(ATA_DEVICE_SERVER_SECURITY_STATE_DATA));
      Status = SecurityReceiveData (
                 &OpalDev.Sscp,
                 0,
                 TRUSTED_COMMAND_TIMEOUT_NS,
                 TCG_SECURITY_PROTOCOL_ATA_DEVICE_SERVER_PASS,
                 SwapBytes16(TCG_SP_SPECIFIC_PROTOCOL_LIST),
                 sizeof(ATA_DEVICE_SERVER_SEND_CMD_DATA),
                 &SecurityCheckStateData,
                 0
                 );
      

      if (DevInfoPtr[DevInfoTableIndex].DeviceType != OPAL_DEVICE_TYPE_TG) {
        OpalGetSupportedAttributesInfo(&Session, &SupportedAttributes, &DevInfoNvme->OpalBaseComId);      
        UnlockOpalPassword (&OpalDev);
         ...
         if (EFI_ERROR(Status)) {
           AtaSecuritySendData->Byte1 = MASTER_PSW;
           Status = SecuritySendData(
                      &OpalDev.Sscp,
                      0,
                      TRUSTED_COMMAND_TIMEOUT_NS,
