...
//#include <Guid/AmdFarbicMmioVariable.h>
...
AMD_MCM_INFO_PROTOCOL       *gAmdMcmInfoProtocol;

#define PCIEX_LENGTH       PcdGet32 (PcdH2OPciExpressSize)

#define IommuCapId                   0x0F
#define MAX_ROOT_BRIDGE_NUM  PcdGet8 (PcdH2OMaxRbNum)

UINTN mDefaultBusLength = 0x100;
  ...
  )
{
  ...
  AMD_MCM_INFO_DATAS                  *AmdMcmRbInfo;
  UINTN                               SocketNum;
  UINTN                               DieNum;
  UINTN                               RbNum;
  ACPI_HID_DEVICE_PATH                *TempAcpiDp;
  EFI_HANDLE                          Handle;
  ...
  GetSystemInfoFromFabric (&SocketNum, &DieNum, &RbNum);
  ...
  mDefaultBusLength = (PCIEX_LENGTH >> 20) / RootBridgeCount;

  AmdMcmRbInfo = AllocateZeroPool (sizeof(AMD_MCM_INFO_DATAS));
  AmdMcmRbInfo->NumberOfSockets = SocketNum;
  AmdMcmRbInfo->NumberOfDies = DieNum;
  AmdMcmRbInfo->NumberOfRootBridge = RootBridgeCount;
  AmdMcmRbInfo->RbInfo = AllocateZeroPool (sizeof(AMD_ROOT_BRIDGE_INFO) * MAX_ROOT_BRIDGE_NUM);

  Status = gDS->GetMemorySpaceDescriptor (PcdGet64 (PcdPciExpressBaseAddress), &GcdMemorySpaceDescriptor);
  if (Status == EFI_SUCCESS) {
    if ((GcdMemorySpaceDescriptor.Attributes & EFI_MEMORY_RUNTIME) != EFI_MEMORY_RUNTIME) {
      Status = gDS->SetMemorySpaceAttributes (
                      GcdMemorySpaceDescriptor.BaseAddress,
  ...
  for (Index = 0; Index < RootBridgeCount; Index++) {
    ...
    TempAcpiDp = (ACPI_HID_DEVICE_PATH *)RootBridge->DevicePath;
    TempAcpiDp->UID = (UINT32)Index;
    AmdMcmRbInfo->RbInfo[Index].RbUid = TempAcpiDp->UID;

    AmdMcmRbInfo->RbInfo[Index].BusBase = RootBridges[Index].Bus.Base;
    AmdMcmRbInfo->RbInfo[Index].BusLimit = RootBridges[Index].Bus.Limit;
    //
    ...
    AmdMcmRbInfo->RbInfo[Index].IoBase = RootBridges[Index].Io.Base;
    AmdMcmRbInfo->RbInfo[Index].IoLimit = RootBridges[Index].Io.Base;
    if (RootBridges[Index].Io.Base <= RootBridges[Index].Io.Limit) {
      AmdMcmRbInfo->RbInfo[Index].IoLimit = RootBridges[Index].Io.Limit;
      //
    ...
    AmdMcmRbInfo->RbInfo[Index].MmioBase32   = RootBridges[Index].Mem.Base;
    AmdMcmRbInfo->RbInfo[Index].MmioLimit32  = RootBridges[Index].Mem.Limit;
    AmdMcmRbInfo->RbInfo[Index].PMmioBase32  = RootBridges[Index].PMem.Base;
    AmdMcmRbInfo->RbInfo[Index].PMmioLimit32 = RootBridges[Index].PMem.Limit;
    AmdMcmRbInfo->RbInfo[Index].MmioBase64   = RootBridges[Index].MemAbove4G.Base;
    AmdMcmRbInfo->RbInfo[Index].MmioLimit64  = RootBridges[Index].MemAbove4G.Limit;
    AmdMcmRbInfo->RbInfo[Index].PMmioBase64  = RootBridges[Index].PMemAbove4G.Base;
    AmdMcmRbInfo->RbInfo[Index].PMmioLimit64 = RootBridges[Index].PMemAbove4G.Limit;
    ...
    AmdMcmRbInfo->RbInfo[Index].RbExist = 0x0F;
    //
  ...
  DEBUG_CODE (
    DEBUG ((DEBUG_INFO, "AMD MCM INFO Socket Num %x Die Num %x Root Bridge Num %x\n", AmdMcmRbInfo->NumberOfSockets, AmdMcmRbInfo->NumberOfDies, AmdMcmRbInfo->NumberOfRootBridge));
    for (Index = 0; Index < MAX_ROOT_BRIDGE_NUM; Index ++) {
      DEBUG ((DEBUG_INFO, "Root Brideg %x Bus Base %x Bus Limit %x\n", Index, AmdMcmRbInfo->RbInfo[Index].BusBase, AmdMcmRbInfo->RbInfo[Index].BusLimit));
      DEBUG ((DEBUG_INFO, "I/O Base %x Limit %x\n", AmdMcmRbInfo->RbInfo[Index].IoBase, AmdMcmRbInfo->RbInfo[Index].IoLimit));
      DEBUG ((DEBUG_INFO, "32 bit MMIO Base %x Limit %x\n", AmdMcmRbInfo->RbInfo[Index].MmioBase32, AmdMcmRbInfo->RbInfo[Index].MmioLimit32));
      DEBUG ((DEBUG_INFO, "32 bit Prefetch MMIO Base %x Limit %x\n", AmdMcmRbInfo->RbInfo[Index].PMmioBase32, AmdMcmRbInfo->RbInfo[Index].PMmioLimit32));
      DEBUG ((DEBUG_INFO, "64 bit MMIO Base %lx Limit %lx\n", AmdMcmRbInfo->RbInfo[Index].MmioBase64, AmdMcmRbInfo->RbInfo[Index].MmioLimit64));
      DEBUG ((DEBUG_INFO, "64 bit Prefetch MMIO Base %lx Limit %lx\n", AmdMcmRbInfo->RbInfo[Index].PMmioBase64, AmdMcmRbInfo->RbInfo[Index].PMmioLimit64));
      DEBUG ((DEBUG_INFO, "ACPI UID %x Root Bridge Exist %x\n", AmdMcmRbInfo->RbInfo[Index].RbUid, AmdMcmRbInfo->RbInfo[Index].RbExist));
    }
  );

  Handle = NULL;
  Status = gBS->AllocatePool (EfiBootServicesData,
                sizeof(AMD_MCM_INFO_PROTOCOL),
                (VOID **)&gAmdMcmInfoProtocol);
  gAmdMcmInfoProtocol->AmdMcmInfoDatas = AmdMcmRbInfo;
  Status = gBS->InstallMultipleProtocolInterfaces (
                  &Handle,
                  &gAmdMcmInfoProtocolGuid,
                  gAmdMcmInfoProtocol,
                  NULL
                  );
   DEBUG ((DEBUG_INFO, "Install AMD MCM info %r\n", Status));
...
//[-start-230817-IB20840016-modify]//
  for (Link = GetFirstNode (&HostBridge->RootBridges), Descriptor = Resources
       ; !IsNull (&HostBridge->RootBridges, Link) && (Descriptor != NULL)
       ; Link = GetNextNode (&HostBridge->RootBridges, Link)
       ) {
    RootBridge = ROOT_BRIDGE_FROM_LINK (Link);
  ...
  )
{
  ...
  for (Link = GetFirstNode (&HostBridge->RootBridges)
    ...
    if (RootBridgeHandle == RootBridge->Handle) {
      ...
      RootBridge->ResAllocNode[TypeBus].Length  = mDefaultBusLength;//Descriptor->AddrLen;
