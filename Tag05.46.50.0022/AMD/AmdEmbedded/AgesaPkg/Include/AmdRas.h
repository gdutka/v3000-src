 ...
 *
 * @details Provide generic MSR and error section definition accorss programs
 *
 */
 ...
 *             Bit 7:0 - Socket
 *             Bit 15:8 - Channel
 *             Bit 16 - DIMM 0, training error chip select on this DIMM
 *             Bit 17 - DIMM 1, training error chip select on this DIMM
 *             Bit 31:18 - reserved
...
/**
 * @brief The PMIC is reporting that there is an error.
 *
 * @details The format of the data section of the PMIC error record:
 *          DATA A in APOB_ERROR_LOG structure or DataParam1 in ERROR_LOG_PARAMS structure
 *             Bit 7:0    - Socket
 *             Bit 15:8   - Channel
 *             Bit 16     - DIMM
 ...
 *          DATA A in APOB_ERROR_LOG structure or DataParam1 in ERROR_LOG_PARAMS structure
 *             NodeId of GCD that reported the error.
 *             GPU socketId = nodeId >> 1, socketId range is 4-7.
 *          DATA B in APOB_ERROR_LOG structure or DataParam2 in ERROR_LOG_PARAMS structure
...
typedef enum _UMC_EXT_ERROR_CODE_NUM {
  ...
  UMC_DCQSRAMECCERR,                        ///< An ECC error occured on a DCQ SRAM.
  UMC_AESSRAMECCERR,                        ///< An ECC error occured on a AES SRAM.
...
typedef union {
  struct {
     ...
     UINT64       InstanceIdHi:4;           ///< The Hi value instance ID of this IP.
 ...
 *          MCA_STATUS, MCA_ADDR, MCA_MISC0, MCA_CONFIG, MCA_IPID, MCA_SYND, MCA_DESTATUS, MCA_DEADDR, MCA_MISC1.
 */
...
typedef struct _MCA_BANK_ERROR_INFO {
  ...
  MCA_SYNDX_MSR         McaSynd1Msr;
  MCA_SYNDX_MSR         McaSynd2Msr;
} MCA_BANK_ERROR_INFO;
...
/**
 * @brief The maximum number of machine check banks visible to a logical core on RS (Stones).
 *
 * @details The number of visible error reporting banks may vary from core to core,
...
/**
 * @brief The number of Core MCA Bank (on SSP, GN and RS).
 *
...
typedef struct _RAS_MCA_ERROR_INFO_V2 {
  ...
  UINTN                 McaBankCount;     ///< For SSP/GN/BA This is the max bank number support for the processor. For RS or later, this is the Error bank count from the processor
 ...
 *
 *          The RAS_NBIO_ERROR_INFO structure is no longer used in RS (Stones)
 ...
 *          The NBIO and NBIF Errors structure has been deprecated from RS (Stones)
 *          because these errors have been reported through MCA instead.
 */
...
typedef union {
  struct {                                     ///< PMIC_ERR_VALID_BIT
    UINT64       ErrStatusValid:1;             ///< [0] Error Status Valid
    UINT64       SocketIdValid:1;              ///< [1] Socket ID Valid
    UINT64       ChannelValid:1;               ///< [2] Channel Valid
    UINT64       DimmValid:1;                  ///< [3] Dimm Valid
    UINT64       :60;                          ///< [63:4] Reserved
  } Field;
  UINT64  Value;
} PMIC_ERR_VALID_BIT;

///
/// PMIC Error Record structure
///
typedef struct {
    PMIC_ERR_VALID_BIT      ValidBits;
    UINT64                  ErrStatus;         ///< Error Status
    UINT16                  SocketId;          ///< Socket Number
    UINT16                  Channel;           ///< Channel Number
    UINT16                  Dimm;              ///< Dimm Number
 ...
 *          The SATA Error Record structure has been deprecated from RS (Stones)
 *          because these errors have been reported through MCA instead.
 */
 ...
 *          The FCH A-Link Error Record structure has been deprecated from RS (Stones)
 *          because these errors have been reported through MCA instead.
 */
 ...
 *          The USB Error Record structure has been deprecated from RS (Stones)
 *          because these errors have been reported through MCA instead.
 */
...
///S-Link Error Source definition
typedef struct {
    UINT32                  Length;
    SLINK_ERROR_VALID_BIT   ValidationBits;
    UINT8                   CcixSourceId;
    UINT8                   CcixPortId;
    UINT8                   Reserved[2];
