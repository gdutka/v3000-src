...
*/
/**
 * @file AmdPspCommonServiceProtocol.h
 *
...
typedef
BOOLEAN
(EFIAPI *FP_GET_FTPM_CONTROLAREA) (
...
typedef
VOID
(EFIAPI *FP_SWITCH_PSP_MMIO_DECODE) (
...
typedef
BOOLEAN
(EFIAPI *FP_CHECK_PSP_DEVICE_PRESENT) (
  VOID
  );
...
typedef
VOID
(EFIAPI *FP_ACQUIRE_PSPSMIREGMUTEX) (
  VOID
  );
...
typedef
VOID
(EFIAPI *FP_RELEASE_PSPSMIREGMUTEX) (
  VOID
  );
...
typedef
EFI_STATUS
(EFIAPI *FP_READ_C2P_MESSAGE_REGISTER) (
  IN         UINTN        Socket,
  IN         UINTN        Die,
...
/**
 * @brief  AMD_PSP_COMMON_SERVICE_PROTOCOL prototype
 *
 * @details Defines AMD_PSP_COMMON_SERVICE_PROTOCOL, which publish the common PSP service across all programs
 */
