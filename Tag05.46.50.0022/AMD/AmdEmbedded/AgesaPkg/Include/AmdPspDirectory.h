...
typedef enum _PSP_DIRECTORY_ENTRY_TYPE {
  ...
  PSP_NV_DATA                     = 0x04,           ///< PSP entry points to fTPM data region in SPI space
  ...
  EXTERNAL_PREMIUM_CHIPSET_MP1_FW = 0x2F,           ///< PSP entry points to External Premium Chipset MP0 FW image
  ...
  PSP_SEV                         = 0x39,           ///< PSP entry points to SEV binary
  PSP_DIR_LV2                     = 0x40,           ///< PSP entry points to Level 2 PSP DIR
  PSP_PHY                         = 0x42,           ///< PSP entry points to PHY binary
  EXTERNAL_PREMIUM_CHIPSET_PSP_BL = 0x46,           ///< PSP entry points to External Premium Chipset PSP Boot Loader
  ...
  MSMU_BINARY_0                   = 0x5A,           ///< PSP entry points to MSMU
  PSP_MPIO                        = 0x5D,           ///< PSP entry points to MPIO Offchip Firmware
  AMD_SCS_BINARY                  = 0x5F,           ///< Software Configuration Settings Data Block
  AMD_SFFS_BINARY                 = 0x63,           ///< Secure Coprocessor Enforced System Firmware
                                                    //Provides an authenticated mechanism to enable and customize system level firmware/software features.

  PSP_RIB                         = 0x76,           ///< PSP entry points to RIB FW
  ...
  MPM_FW_1                        = 0x85,           ///< AMF Firmware part 1: Lx core can start execution only from SRAM,
                                                    ///< also part 1 FW will be used to initialize MPM TLB's so that DRAM can be mapped to Lx
  MPM_FW_2                        = 0x86,           ///< AMF Firmware part 2: Bulk of MPM functionality will be implemented in this part
  MPM_PERSISTENT_STORAGE          = 0x87,           ///< Persistent storage exclusively for AIM-T needs
  MPM_WLAN_FW                     = 0x88,           ///< WLAN firmware copied to MPM memory by MFD and then MPM will send this to WLAN
  PSP_MMPDMA                      = 0x8C,           ///< PSP entry points to TigerFish MMPDMA FW.
  PSP_GMI                         = 0x91,           ///< PSP entry points to MPDMA Page Migration FW
  PSP_PM                          = 0x92,           ///< PSP entry points to GMI FW
  PMF_BINARY                      = 0x99,           ///< Binary that contains PMF policy
