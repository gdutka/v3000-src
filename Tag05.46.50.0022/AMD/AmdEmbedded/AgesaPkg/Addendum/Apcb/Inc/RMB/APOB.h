...
//[-start-240806-IB20840077-modify]//
#if defined(BUILD_TARGET_IS_RS) && (BUILD_TARGET_IS_RS == 1)
//[-end-240806-IB20840077-modify]//
...
typedef enum {
  SLINK_ADDR_SUCCESS = 0,            // The requested SLink mapping was successfully created
  SLINK_ADDR_OUT_OF_MAPS = 1,        // This region was not routed due to a shortage of DRAM map registers
  SLINK_ADDR_INVALID_SOCKET = 2,     // The APCB passed in a 3rd and/or 4th region descriptor attached to the same socket
  SLINK_ADDR_INVALID_NBIO_MAP = 3,   // The APCB passed in 2 even or 2 odd numbered NBIOs for the same socket, whether it is in the same descriptor or 2 different descriptors.
  SLINK_ADDR_INVALID_ALIGNMENT = 4,  // The requested alignment was not achievable.
  SLINK_ADDR_INVALID_SIZE = 5,       // The requested region was too large for the current configuration.
} DF_SLINK_REGION_INIT_STATUS;
...
typedef struct {
  APOB_TYPE_HEADER     ApobTypeHeader;      ///< APOB Type Header
  SLINK_ADDR_MAP_INFO  SLinkInfo[8];        ///< S-Link address map information
} APOB_SYSTEM_SLINK_INFO_TYPE_STRUCT;
...
/**
 * @brief Jedec_DDR5_SPD5xx8_Hub_1v0.pdf, DIMM hub register
 *        contents
 *
 */
typedef struct {
  UINT8  MR12;      ///< DIMM Hub MR12
  UINT8  MR13;      ///< DIMM Hub MR13
...
typedef struct {
  ...
  APOB_MEM_D5_HUB_REG_STRUCT                       MemDdr5HubReg;             ///< DDR5 DIMM hub register contents
...
typedef struct {
  ...
  APOB_SYSTEM_SLINK_INFO_TYPE_STRUCT        SystemSLinkInfo;
  APOB_DF_DXIO_PHY_FW_OVERRIDE_TABLE_STRUCT DfDxioPhyFwOverrideInfo;
  APOB_SYSTEM_NPS_INFO_TYPE_STRUCT          SystemNpsInfo;
} APOB_GLOBAL_DATA;
