...
/**
  ...
  @param[in]  IoWidth           - The size of the requested access, has to
                                  be one type from EFI_SMM_IO_WIDTH.
  ...
  )
{
  ...
  //
  if (AccessMask != SECURE_POLICY_RESOURCE_ATTR_READ &&
      AccessMask != SECURE_POLICY_RESOURCE_ATTR_WRITE) {
  ...
  //
  if (IoWidth == SMM_IO_UINT8) {
    IoSize = sizeof (UINT8);
  } else if (IoWidth == SMM_IO_UINT16) {
    IoSize = sizeof (UINT16);
  } else if (IoWidth == SMM_IO_UINT32) {
  ...
  for (i = 0; i < PolicyRoot->Count; i++) {
    ...
    if ((IoDescriptor[i].Attributes & SECURE_POLICY_RESOURCE_ATTR_STRICT_WIDTH) &&
      ...
      if (IoDescriptor[i].Attributes & AccessMask) {
        ...
        //
        // Someone is trying to access something against policy.
        //
        DEBUG ((DEBUG_ERROR, "%a Strict width access matches an entry of Security Policy.\n", __FUNCTION__));
        MEMLOG ("IO %x Strict width access matches an entry of Security Policy.", IoAddress);
        FoundMatch = TRUE;
      }
      ...
      if (IoDescriptor[i].Attributes & AccessMask) {
        ...
        // Someone is trying to access something against policy.
        //
        DEBUG ((DEBUG_ERROR, "%a Access matches an entry of the Security Policy.\n", __FUNCTION__));
        MEMLOG ("IO %x access matches an entry of the Security Policy.", IoAddress);
        FoundMatch = TRUE;
      }
  ...
  if ((FoundMatch && PolicyRoot->AccessAttr == SMM_SUPV_ACCESS_ATTR_DENY) ||
    ...
    //
    DEBUG ((DEBUG_ERROR, "%a Rejecting IO access based on policy walk through: Index: %d, AccessAttr: 0x%x.\n",
      __FUNCTION__, i, PolicyRoot->AccessAttr));
    MEMLOG ("Rejecting IO access based on policy walk through.");
    Status = EFI_ACCESS_DENIED;
  }
  ...
  )
{
  ...
  for (i = 0; i < PolicyRoot->Count; i++) {
    ...
    if (MsrAddress >= MsrDescriptor[i].MsrAddress &&
      ...
      if (MsrDescriptor[i].Attributes & AccessMask) {
        ...
        //
        DEBUG ((DEBUG_ERROR, "%a Access matches an entry of the Security Policy\n", __FUNCTION__));
        MEMLOG ("MSR %x access matches an entry of the Security Policy.", MsrAddress);
        FoundMatch = TRUE;
      }
  ...
  if ((FoundMatch && PolicyRoot->AccessAttr == SMM_SUPV_ACCESS_ATTR_DENY) ||
    ...
    // 2. did not find a matching policy, reject access if this is an allow list
    //
    DEBUG ((DEBUG_ERROR, "%a Rejecting MSR access based on policy walk through: Index: %d, AccessAttr: 0x%x.\n",
      __FUNCTION__, i, PolicyRoot->AccessAttr));
    MEMLOG ("Rejecting MSR access based on policy walk through.");
    Status = EFI_ACCESS_DENIED;
  }
  ...
  )
{
  ...
  for (i = 0; i < PolicyRoot->Count; i++) {
    ...
    if (InstructionIndex == InstrDescriptor[i].InstructionIndex) {
      ...
      if (InstrDescriptor[i].Attributes & SECURE_POLICY_RESOURCE_ATTR_EXECUTE) {
        ...
        //
        DEBUG ((DEBUG_ERROR, "%a Execution matches an entry of the Security Policy\n", __FUNCTION__));
        MEMLOG ("Instruction %x execution matches an entry of the Security Policy.", InstructionIndex);
        FoundMatch = TRUE;
      }
  ...
  if ((FoundMatch && PolicyRoot->AccessAttr == SMM_SUPV_ACCESS_ATTR_DENY) ||
    ...
    // 2. did not find a matching policy, reject access if this is an allow list
    //
    DEBUG ((DEBUG_ERROR, "%a Rejecting Instruction access based on policy walk through: Index: %d, AccessAttr: 0x%x.\n",
      __FUNCTION__, i, PolicyRoot->AccessAttr));
    MEMLOG ("Rejecting Instruction access based on policy walk through.");
    Status = EFI_ACCESS_DENIED;
  }
