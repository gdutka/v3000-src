...
#include "AMDSmramSaveStateMap.h"

// EFER register LMA bit
#define EFER_ADDRESS_LMA_BIT BIT10
  ...
  )
{
  ...
  if (mSmmSaveStateRegisterLma == EFI_SMM_SAVE_STATE_REGISTER_LMA_32BIT) {
    ...
    if (NeedSysCall () == TRUE) {
      DEBUG ((DEBUG_ERROR, "ReadSaveStateRegisterByIndex CpuIndex = %x RegisterIndex = %x Width = %x Buffer = %p\n",
        CpuIndex, RegisterIndex, Width, Buffer));
  ...
  )
{
  ...
  if (Register == EFI_SMM_SAVE_STATE_REGISTER_IO) {
    ...
    if (NeedSysCall () == TRUE){
      SaveStateIODWord = (UINT32) SysCall (SMM_SC_SVST_READ, CpuIndex, (UINTN)&(CpuSaveState->x64.IO_DWord), sizeof(CpuSaveState->x64.IO_DWord));
    } else {
  ...
  )
{
  ...
  LMAValue = (UINT32)AsmReadMsr64 (EFER_ADDRESS) & EFER_ADDRESS_LMA_BIT;
  ...
  )
{
  ...
  if (mSmmSaveStateRegisterLma == EFI_SMM_SAVE_STATE_REGISTER_LMA_32BIT) {
    ...
    if ((AmdCpuState->x64.EFER & EFER_ADDRESS_LMA_BIT) == 0) {
...
{
  return SupervisorGetSmiHandlerSize ();
}
