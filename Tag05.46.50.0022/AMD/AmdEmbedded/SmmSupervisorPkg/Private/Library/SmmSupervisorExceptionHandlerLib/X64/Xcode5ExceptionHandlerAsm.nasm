...
CommonInterruptEntryFixup:    DQ  0
HookAfterStubHeaderEndFixup:  DQ  0
...
%rep  32
    push    byte %[Vector]
    push    rax
    mov     rax, qword [CommonInterruptEntryFixup]
    jmp     rax
...
@VectorNum:
    ...
    push    rax
    mov     rax, qword [HookAfterStubHeaderEndFixup]
JmpAbsoluteAddress:
    jmp     rax
...
HasErrorCode:
    ...
    push    0             ; clear EXCEPTION_HANDLER_CONTEXT.ExceptionHandled
...
;; UINT64  Cr0, Cr1, Cr2, Cr3, Cr4, Cr8;
    ...
    push    rax
    mov     rax, cr4
    ;or      rax, 0x208    ;Skip CR4 write for two reason, 1. SmmPgCfgLock is set, 2, already set in File: ..\SmmSupervisorPkg\SmiEntry\SmmSupvEntry.nasm
    ;mov     cr4, rax
    push    rax
    mov     rax, cr3
    push    rax
...
;Update ds, es, ss with selectors for 64-bit. Processor sets ss to NULL, and doesn't change ds and es.
...
;;BIT 33: SmmPgCfgLock. Read-write. Reset: 0. 1=SMM page config locked.
    mov     ecx, 0xc0010015
    rdmsr
    bt      edx, 1
    jc      SKIP_CR_POP
...
DoReturn:
    ...
    cmp     qword [rax + 0x10], LONG_MODE_CS
    je      .0
...
; Fix up CommonInterruptEntry address
    ...
    mov    [CommonInterruptEntryFixup], rax
; Fix up HookAfterStubHeaderEnd
    lea    rax, [HookAfterStubHeaderEnd]
    mov    [HookAfterStubHeaderEndFixup], rax
...
;-------------------------------------------------------------------------------------
;  TransferToDefaultExceptionHandler (IN EFI_EXCEPTION_TYPE  ExceptionType, IN EFI_SYSTEM_CONTEXT SystemContext)
;-------------------------------------------------------------------------------------
extern ASM_PFX(mDefaultInterruptHandler)
global ASM_PFX(TransferToDefaultExceptionHandler)
ASM_PFX(TransferToDefaultExceptionHandler):
    push    rbp
    push    rax	; maintain stack alignment

    mov     rbp, rsp    ; Save stack for return through call gate.

    ; Set ds, es to CPL3.
    mov     eax, LONG_MODE_DS_R3
    mov     ds, eax
    mov     es, eax

    push    LONG_MODE_DS_R3 ; CPL3 SS
    push    r8
    push    LONG_MODE_CS_R3 ; Push new CPL3 CS
    push    qword [mDefaultInterruptHandler]

    db   48h
    retf            ;Switch to CPL3 stack, CPL3 code, and transfer to mDefaultInterruptHandler.

global ASM_PFX(ReturnToCpl0ExceptionHandler)
ASM_PFX(ReturnToCpl0ExceptionHandler):
    ; Set ds, es, ss to CPL0.
    mov     eax, PROTECT_MODE_DS
    mov     ds, eax
    mov     es, eax
    mov     ss, eax

    mov     rsp, rbp    ; restore CPL0 stack pointer.
    pop     rax
    pop     rbp
    ret

global ASM_PFX(GetRsp)
ASM_PFX(GetRsp):
    mov rax, rsp
    ret

global ASM_PFX(FlushTlbMemory)
ASM_PFX(FlushTlbMemory):
    invlpg [rcx]
    ret