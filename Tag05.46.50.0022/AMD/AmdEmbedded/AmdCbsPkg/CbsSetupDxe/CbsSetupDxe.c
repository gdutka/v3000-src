  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    DEBUG ((EFI_D_INFO, "CBS Recovery: Status = 0x%x, ApcbSyncSignature=0x%x\n", Status, ApcbSyncSignature));
  ...
  if ((ApcbRecoveryFlag) || (ApcbSyncSignature == 0x56434552)) {
    DEBUG ((EFI_D_INFO, "CBS Recovery: APCB Recovery enabled\n"));
    CbsWriteDefalutValue(Private->IfrData);
    CbsComboIdentify(Private->IfrData);
...
//[-start-220906-IB14740262-add]//
  ImageHandle = 0;
//[-end-220906-IB14740262-add]//
  DEBUG ((EFI_D_ERROR, "InitializeCbsVarDefault .......\n"));
  Status = gRT->GetVariable (
              VariableName,
              &gCbsSystemConfigurationGuid,
              NULL,
...
//          if (RecordOffset <= CbsVariableSize) {
//            if (RecordOffset > 32) {
//              LibAmdMemCopy ((VOID *)&tempBuffer[32],
//                             (VOID *)&IfrData[32],
//                             (RecordOffset - 32),
  ...
  )
{
  ...
  //
  // Dummy call for one time dynamic update
  //
