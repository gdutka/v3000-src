...
sub generateUniFileStr
{
  ...
  if ($thisOption->{'Type'} =~ /^SECTION$/i) {
    ...
    &outputStr($uniFileStr, $sectionNameToken, $thisOption->{'Name'}, $uniExistHash, 0, $ibvStr);
    ...
    if ($thisOption->{'Input_type'} =~ /^Text$/i) {
      ...
      &outputStr($uniFileStr, $optionNameToken, $optionNameStr, $uniExistHash, 0, $ibvStr);
    } else {
      $optionNameToken = &genStrToken($thisOption->{'Name'});
      $optionNameStr = $thisOption->{'Name'};
      $optionNameStr =~s/GRA\d/GRA/g;
      $optionVariableName = $thisOption->{'Variable_name'};
      &outputStr($uniFileStr, $newOptionNameToken, $optionNameStr, $uniExistHash, 1, $ibvStr);
    }
    ...
    &outputStr($uniFileStr, $optionHelpToken, $optionHelpStr, $uniExistHash, 0, $ibvStr);
    ...
    if ($thisOption->{'Input_type'} =~ /^Fixed$/i) {
      foreach my $choice (@{$thisOption->{'OPTION_ITEM'}}) {
        ...
        &outputStr($uniFileStr, $optionItemToken, $choice->{'Name'}, $uniExistHash, 0, $ibvStr);
      }
    }
  }
}
...
sub generateKeyStr
{
  ...
  if (($thisOption->{'Type'} =~ /^OPTION$/i) && (($thisOption->{'Key_function'} != 0) || ($thisOption->{'Additional_information'} =~ /LABEL_DYNAMIC_CBS_OPTION\s*=\s*TRUE/i))) {
    if ($$keyIDOutput eq '') {
      $$keyIDOutput .= "\n// KEY function ID\n";
    }
...
#
#########################################################
sub generateLabelCbsStr
{
  my $lableStartID = shift;
  my $labelCbsIDOutput = shift;
  my $thisOption = shift;

  if (($thisOption->{'Type'} =~ /^OPTION$/i) && ($thisOption->{'Additional_information'} =~ /LABEL_DYNAMIC_CBS_OPTION\s*=\s*TRUE/i)) {
    my $labelCbsToken = genLabelCbsToken ($thisOption->{'Variable_name'});
    $$labelCbsIDOutput .= sprintf "  #define %-70s  0x%X  //%s Start Label\n", $labelCbsToken."_START", $$lableStartID, $thisOption->{'Variable_name'};
    $$lableStartID++;
    $$labelCbsIDOutput .= sprintf "  #define %-70s  0x%X  //%s End Label\n", $labelCbsToken."_END", $$lableStartID, $thisOption->{'Variable_name'};
    $$lableStartID++;
  }
}
...
sub generateVfrFile
{
  ...
  if ($thisOption->{'Type'} =~ /^SECTION$/i) {
    # This is a section
    generateVfrSection ($vfrFile, $thisOption, $indentNum, $rootSection, $component, $Program);
  } elsif ($thisOption->{'Type'} =~ /^OPTION$/i) {
    # This is an option
    ...
    if ($multiInstance eq 'FALSE') {
      # common options
      generateVfrCommonOpt ($vfrFile, $thisOption, $indentNum, $rootSection, $keyIDExistHash, $newOptionNameToken, $Program);
    } else {
...
#########################################################
#
# Generate LABEL_CBS token
#   adding prefix = 'LABEL_CBS_'
#   adding suffix = ''
#   replace `~!@$%^&*()-=+[]{}\/,<>?| white space with _
#   replace . with DOT
#   replace # with POUND
# IN      : name string
#
# RETURN  : a lable used for 'dynamic function'
#
#########################################################
sub genLabelCbsToken
{
  my $lable = shift;
  $lable =~ s/([A-Z][a-z])/_$1/g;
  $lable =~ s/([a-z])([A-Z])/$1_$2/g;
  $lable =~ s/SMU/SMU_/ig;
  $lable = 'LABEL_CBS_'.uc($lable);
  $lable = updateStr ($lable);
  return $lable;
}
...
#  }
  ...
  if ((ref ($thisOption->{'Display_condition'}) ne 'HASH') || (ref ($thisOption->{'Active_condition'}) ne 'HASH')) {
    ...
    # 1.2 suppressif
    if (ref ($thisOption->{'Display_condition'}) ne 'HASH') {
      $displayCondition = findNegativeConditions ($thisOption->{'Display_condition'}, $rootSection, $Program);
      $space = '  ' x $indentNum.'      ';
    ...
    # 1.3 grayoutif
    if (ref ($thisOption->{'Active_condition'}) ne 'HASH') {
      $grayoutCondition = findNegativeConditions ($thisOption->{'Active_condition'}, $rootSection, $Program);
      $space = '  ' x $indentNum.'     ';
...
sub generateVfrCommonOpt
{
  ...
  my $requireLabelCbs = 0;
    
  # 0th, CBS label start
  if ($thisOption->{'Additional_information'} =~ /LABEL_DYNAMIC_CBS_OPTION\s*=\s*TRUE/i) {
    $requireLabelCbs = 1;
    printf $vfrFile '  ' x $indentNum."label " . genLabelCbsToken ($thisOption->{'Variable_name'}) . "_START;\n";
  } 
  ...
  if ($thisOption->{'Input_type'} =~ /^Fixed$/i) {
    ...
    #   questionid = KEY_XXXX,
    if (($thisOption->{'Key_function'} != 0) || $requireLabelCbs) {
    ...
    foreach $choice (@{$thisOption->{'OPTION_ITEM'}}) {
      ...
      if (($thisOption->{'Key_function'} == 0) and ($requireLabelCbs == 0)) {
        $keyFunction = '';
      } else {
  ...
  # 4th, CBS label end
  if ($requireLabelCbs) {
    printf $vfrFile '  ' x $indentNum."label " . genLabelCbsToken ($thisOption->{'Variable_name'}) . "_END;\n";
  } 
...
sub outputStr
{
  ...
  if (exists $uniExistHash->{"$str1"}) {
    ...
    $str2 =~ s/X570\/590\s*//ig;
    $str2 =~ s/300\/400\/500\s*//ig;
    $str2 =~ s/X370\/X399\/X470\/B550A\/PRO560\s*//ig;
    $str2 =~ s/A520\/B550\/PRO565\s*//ig;
    $str2 =~ s/B550\/PRO565\s*//ig;
    $str2 =~ s/A520 USB/USB/ig;
    $str2 =~ s/ \(  XHCI DID = .+ \)//ig;
    $str2 =~ s/B450 Chipset USB Port Configuration Options/Chipset USB Port Configuration Options/ig;
    $str2 =~ s/A320 Chipset USB Port Configuration Options/Chipset USB Port Configuration Options/ig;
    if (($isOption == 1) && ($ibvStr ne "")) {
        attachUniStr($uniFilestr, sprintf "#string %-70s  #language en-US    \"%s\"\n #language $ibvStr \"%s\" \n", $str1, $str2, $optionVariableName);
    } else {
        attachUniStr($uniFilestr, sprintf "#string %-70s  #language en-US    \"%s\"\n", $str1, $str2);
    }
  }
...
sub lookforDupOption
{
  ...
  #print "Eric debug -- NameToken = $NameToken\n";

  push (@TokenArray, $NameToken);
  $TempToken = $NameToken;

  if ($#TokenArray > 0) {

    do {
        if ($TokenArray[$index] eq $TempToken ) {
            #print "Eric Token debug -- Found duplicate for Token $TempToken \n";
            $TempToken = $NameToken."_$n";
            $n++;
            $index = 0;
        } else {
            $index++;
        }
    } while ( $index < $#TokenArray);


    if ($TempToken ne $NameToken) {
        $NameToken = $TempToken;
        push (@TokenArray, $NameToken);
        #print "Eric Token debug -- New Token = $NameToken \n";
    }
...
sub findNegativeConditions
{
  ...
  if ($parentConditionsString !~ /\|/) {
    ...
    @vfrConditions = &parseNegativeCondition (\@oredParentConditionStrings, $sectionName, $Program);
    $vfrCondition = join ("\n\tOR ", @vfrConditions);
  } elsif ($parentConditionsString !~ /,/) {
    my @andedParentConditionStrings = split (/\s*\|\s*/, $parentConditionsString);
    @vfrConditions = &parseNegativeCondition (\@andedParentConditionStrings, $sectionName, $Program);
    $vfrCondition = join ("\n\tAND ", @vfrConditions);
  } else {
    ...
    foreach my $oredParentConditionStrings (@oredParentConditionStrings) {
      ...
      if (scalar @andedParentConditionStrings == 2) {
        ...
        @vfrAndedConditions = (@vfrAndedConditions, &parseNegativeCondition (\@andedParentConditionStrings1, $sectionName, $Program));
    ...
    @vfrOredConditions = (&parseNegativeCondition (\@andedParentConditionStrings0, $sectionName, $Program));
...
sub parseNegativeCondition
{
  ...
  foreach my $condition (@$pConditions) {
    ...
    if ($condition =~ /\s*=\s*/) {
      ...
      #Override for CbsComboChipsetFlag
      if ($variableName =~ /CbsComboChipsetFlag/){
        $variableName = "Header.CbsComboChipsetFlag";
        $nvValue = $choiceName;
      }
      
      #Override for CbsChipsetVisibleFlag0
      if ($variableName =~ /CbsChipsetVisibleFlag0/){
        $variableName = "Header.CbsChipsetVisibleFlag0";
        $nvValue = $choiceName;
      }
      
      #Override for CbsChipsetVisibleFlag1
      if ($variableName =~ /CbsChipsetVisibleFlag1/){
        $variableName = "Header.CbsChipsetVisibleFlag1";
        $nvValue = $choiceName;
      }
