...
#define FIXED_TICKS_FREQUENCY_COUNT   3579

extern UINT16 mCpuFamily;

/**
 * @brief Calculate TSC frequency via Acpi Timer.
 *
 * @return The number of TSC counts per second.
**/
UINT64
CalculateTscFrequencyViaAcpiTimer (
 VOID
 )
{
  UINT64  TscFrequency;
  ...
  //
  // Set Ticks to 1ms in the future
  Ticks = MmioRead32 ((ACPI_MMIO_BASE + PMIO_BASE + FCH_PMIOA_REG508)) + (FIXED_TICKS_FREQUENCY_COUNT);
  ...
  )
{
  ...
  if (mCpuFamily >= FAMILY_1A) {
    TscFrequency = CalculateTscFrequencyViaAcpiTimer ();
  } else {
    TscFrequency = MultU64x32 (
                     AmdGetCoreCOF (),  // Number of TSC counts in 1us
                     1000000            // Number of us in a second
                     );
  }
