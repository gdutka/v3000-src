    ...
    if (DynamicStringPtr->KeyId == HotKey.KeyId) {
      ...
      for (StrCmpIndex = 0; StrCmpIndex < GET_ARRAY_COUNT (StringDefine); StrCmpIndex++) {
        ...
        if ((DynamicStringPtr->Flag.TextMode == mStringList[StrCmpIndex].Flag.TextMode) &&
          ...
          UniStrSize = AsciiStrnSizeS (&(DynamicStringPtr->String), MAX_STRING_LENGTH);
          ASSERT (UniStrSize != 0);
          UniStr = (CHAR16 *)AllocateZeroPool (UniStrSize * sizeof(CHAR16));
          AsciiStrToUnicodeStrS (&(DynamicStringPtr->String), UniStr, UniStrSize);
  ...
  )
{
  ...
  for (Index = 0; Index < McStringCount; Index++, McStringPtr = GET_NEXT_DYNAMIC_STRING_PTR (McStringPtr)) {
    if ((McStringPtr->KeyId == DSTRING_NO_HOTKEY) && (McStringPtr->SystemInfoId == DSTRING_NOT_SYSTEM_INFO)) {
        ...
        UniStrSize = AsciiStrnSizeS (&(McStringPtr->String), MAX_STRING_LENGTH);
        ASSERT (UniStrSize != 0);
        UniStr = (CHAR16 *)AllocateZeroPool (UniStrSize * sizeof(CHAR16));
        AsciiStrToUnicodeStrS (&(McStringPtr->String), UniStr, UniStrSize);
  ...
  )
{
  ...
  if ((*CoordinateX == STRING_LOCATION_MIN) && (*CoordinateY == STRING_LOCATION_MIN)) {
    ...
    *CoordinateY = IsGraphicMode ? (0 + LeftTopCount * EFI_GLYPH_HEIGHT) : (0 + LeftTopCount);
    ...
    // Center and Top
    //
    *CoordinateX = IsGraphicMode ? ((SizeOfX - StrnLenS (TmpStr, MAX_STRING_LENGTH) * EFI_GLYPH_WIDTH) / 2) : ((SizeOfX - StrnLenS (TmpStr, MAX_STRING_LENGTH)) / 2);
    *CoordinateY = IsGraphicMode ? (0 + CenterTopCount * EFI_GLYPH_HEIGHT) : (0 + CenterTopCount);
    ...
    // Right and Top
    //
    *CoordinateX = IsGraphicMode ? ((SizeOfX - (StrnLenS (TmpStr, MAX_STRING_LENGTH) + 1) * EFI_GLYPH_WIDTH)) : ((SizeOfX - (StrnLenS (TmpStr, MAX_STRING_LENGTH) + 1)));
  ...
  )
{
  ...
  *TotalSize   += *StringCount * (sizeof (DYNAMIC_STRING_EXTERNAL) - sizeof (CHAR8));
  for (StrIndex = 0; StrIndex < *StringCount; StrIndex++) {
    ...
    *TotalSize += (StrnLenS (StrPtr, MAX_STRING_LENGTH) + 1);
  }
  ...
  )
{
  ...
  TempAddress = AllocateReservedPool (NeedSize);
  if (TempAddress == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }
  ...
  )
{
  ...
  for (Index = 0; Index < GET_ARRAY_COUNT (StringDefine); Index++) {
    ...
    TempAsciiStringSize = StrnSizeS (TempUniString, MAX_STRING_LENGTH);
    ASSERT (TempAsciiStringSize != 0);
    TempAsciiString = AllocateZeroPool (TempAsciiStringSize);
    UnicodeStrToAsciiStrS (TempUniString, TempAsciiString, TempAsciiStringSize);
    CopyMem (&(DyStrPtr->String), TempAsciiString, AsciiStrnSizeS (TempAsciiString, MAX_STRING_LENGTH));
    DyStrPtr = (DYNAMIC_STRING_EXTERNAL *)((UINTN)DyStrPtr + sizeof (DYNAMIC_STRING_EXTERNAL) - sizeof (CHAR8) + AsciiStrnSizeS (TempAsciiString, MAX_STRING_LENGTH));
  ...
  for (Index = 0; Index < GET_ARRAY_COUNT (SystemInfoTable); Index++) {
    ...
    TempAsciiString = AllocateZeroPool (TempAsciiStringSize);
    UnicodeStrToAsciiStrS (TempUniString, TempAsciiString, TempAsciiStringSize);
    CopyMem (&(DySysInfoPtr->String), TempAsciiString, AsciiStrnSizeS (TempAsciiString, MAX_STRING_LENGTH));
    DySysInfoPtr = (DYNAMIC_SYSTEM_INFO_EXTERNAL *)((UINTN)DySysInfoPtr + sizeof (DYNAMIC_SYSTEM_INFO_EXTERNAL) - sizeof (CHAR8) + AsciiStrnSizeS (TempAsciiString, MAX_STRING_LENGTH));
    FreePool (TempAsciiString);
  }
  //
  ...
  for (Index = 0; Index < GET_ARRAY_COUNT (OperationTable); Index++) {
    ...
    TempAsciiString = AllocateZeroPool (TempAsciiStringSize);
    UnicodeStrToAsciiStrS (TempUniString, TempAsciiString, TempAsciiStringSize);
    CopyMem (&(DyOperationPtr->String), TempAsciiString, AsciiStrnSizeS (TempAsciiString, MAX_STRING_LENGTH));
    DyOperationPtr = (DYNAMIC_OPERATION_EXTERNAL *)((UINTN)DyOperationPtr + sizeof (DYNAMIC_OPERATION_EXTERNAL) - sizeof (CHAR8) + AsciiStrnSizeS (TempAsciiString, MAX_STRING_LENGTH));
    FreePool (TempAsciiString);
  }
  return EFI_SUCCESS;
}
