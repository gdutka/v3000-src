...
BOOLEAN  mOldInSmm     = FALSE;
BOOLEAN  mInSmmSaveRestoreProcess = FALSE;
//
  ...
  )
{
  ...
  if (!InLegacyBoot) {
    ...
    //
    if (!mInSmmSaveRestoreProcess) {
      mInSmmSaveRestoreProcess = TRUE;
      mOldInSmm = gSmmCorePrivate->InSmm;
    }
  ...
  if (!InLegacyBoot) {
    ...
    //
    gSmmCorePrivate->InSmm = mOldInSmm;
    mInSmmSaveRestoreProcess = FALSE;
    //
    ...
    if (!mOldInSmm) {
      ...
      //
      if (!gSmmCorePrivate->SmmEntryPointRegistered) {
