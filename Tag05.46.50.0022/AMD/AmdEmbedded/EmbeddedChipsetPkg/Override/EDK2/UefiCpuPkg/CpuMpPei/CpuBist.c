  ...
  )
{
  ...
  VOID                                  *GeneralSecInfoPpi;
  BOOLEAN                               IsPlatformInformation2;

  SecPlatformInformation2Ppi = NULL;
  SecPlatformInformationPpi = NULL;

  IsPlatformInformation2 = CompareGuid (Guid, &gEfiSecPlatformInformation2PpiGuid);
  Status = PeiServicesLocatePpi (
             Guid,                                // GUID
             0,                                   // INSTANCE
             PpiDescriptor,                       // EFI_PEI_PPI_DESCRIPTOR
             (VOID **)&GeneralSecInfoPpi          // PPI
             );
  ...
  if (Status == EFI_SUCCESS) {
    ...
    if (IsPlatformInformation2) {
      SecPlatformInformation2Ppi = (EFI_SEC_PLATFORM_INFORMATION2_PPI *) GeneralSecInfoPpi;
      ...
      SecPlatformInformationPpi = (EFI_SEC_PLATFORM_INFORMATION_PPI *) GeneralSecInfoPpi;
    ...
    if (Status == EFI_BUFFER_TOO_SMALL) {
      ...
      if (Status == EFI_SUCCESS) {
        if (IsPlatformInformation2) {
          ...
          if (Status == EFI_SUCCESS) {
            *BistInformationData = SecPlatformInformation;
            if (BistInformationSize != NULL) {
              *BistInformationSize = InformationSize;
            }
            return EFI_SUCCESS;
  ...
  )
{
  ...
  AsmCpuid (0x00000000, NULL, &RegEbx, &RegEcx, &RegEdx);

  if ((RegEbx == CPUID_SIGNATURE_AUTHENTIC_AMD_EBX) &&
      (RegEcx == CPUID_SIGNATURE_AUTHENTIC_AMD_ECX) &&
      (RegEdx == CPUID_SIGNATURE_AUTHENTIC_AMD_EDX)) {
    //
    // For AMD processor, only BSP is enabled in PEI phase.
    //
    IsAmdCpu = TRUE;
