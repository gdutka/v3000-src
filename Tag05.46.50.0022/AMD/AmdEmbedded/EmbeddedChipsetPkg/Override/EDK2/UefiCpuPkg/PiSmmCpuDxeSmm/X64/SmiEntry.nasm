...
%define MSR_AMD_CPUID_EXTFEATURE 0xc0011005
...
; MSR_IA32_MISC_ENABLE not supported
    ...
    ; AMD might need to restore the bit of NX in MSR_AMD_CPUID_EXTFEATURE before RSM (follow the design in mXdSupportedAbsAddr:)
    mov     ecx, MSR_AMD_CPUID_EXTFEATURE
    rdmsr
    sub     esp, 4
    push    rax                         ; save MSR_AMD_CPUID_EXTFEATURE[31-0]
    test    eax, BIT20                  ; MSR_AMD_CPUID_EXTFEATURE[20]
    jnz     EnableNxe
    or      eax, 0x00100000             ; set NX bit if it is not set
    wrmsr
...
mXdSupportedAbsAddr:
    ...
    cmp     al, 0
    jz      .1
    pop     rax                         ; get saved MSR_AMD_CPUID_EXTFEATURE[31-0]
    test    eax, BIT20
    jnz     .1
    mov     ecx, MSR_AMD_CPUID_EXTFEATURE
    rdmsr
    and     eax, 0xFFEFFFFF             ; clear NX bit if it was not set before entering into SMM
    wrmsr
