...
// SMM Supervisor protocol instance
SMM_SUPERVISOR_PROTOCOL *SmmSupervisor = NULL;
  ...
  )
{
  ...
  // UINTN                      ModelId;
  UINT32                     Cr3;
//[-start-230821-IB20840018-add]//
  UINT32                     SmmStackSize;
  UINT32                     SmmShadowStackSize;
//[-end-230821-IB20840018-add]//

  // SMM isolation
  #if SMM_ISOLATION_SUPPORT
    if (PcdGetBool (PcdSmmIsolationEnable)) {
      Status = gSmst->SmmLocateProtocol (&gSmmSupervisorProtocolGuid, NULL, (VOID **)&SmmSupervisor);
      if (EFI_ERROR (Status)) {
        ASSERT_EFI_ERROR (Status);
        DEBUG ((DEBUG_INFO, "%a Failed to locate Smm Supervisor protocol - %r\n", __FUNCTION__, Status));
        return Status;
      }
    }
  #endif
  ...
  //
//[-start-230821-IB20840018-modify]//
  SmmStackSize = EFI_PAGES_TO_SIZE (EFI_SIZE_TO_PAGES (PcdGet32 (PcdCpuSmmStackSize)));
  mSmmStackSize = (UINTN)SmmStackSize;
//[-end-230821-IB20840018-modify]//
...
//[-start-230821-IB20840018-modify]//
    ...
    mSmmShadowStackSize = (UINTN)SmmShadowStackSize;
//[-end-230821-IB20840018-modify]//
  ...
  )
{
  if (mSmmReadyToLock) {
    ...
    if (PcdGetBool (PcdSmmProtectionFeature)) {
      //
