...
#define SMM_SAVE_STATE_REGISTER_SMMIOTRAP_INDEX  2
#define SMM_SAVE_STATE_REGISTER_RESERVED_INDEX   3
...
CONST CPU_SMM_SAVE_STATE_LOOKUP_ENTRY  mSmmCpuWidthOffset[] = {
  ...
  { 0, 4, 0,                              SMM_CPU_OFFSET (x64.SMM_IO_TRAP),    0,                                   FALSE }, // SMM_SAVE_STATE_REGISTER_SMMIOTRAP_INDEX = 2
  { 0, 0, 0,                              0,                                   0,                                   FALSE }, // SMM_SAVE_STATE_REGISTER_RESERVED_INDEX  = 3
  ...
  { 0, 4, 0,                              SMM_CPU_OFFSET (x64.GdtrLimit),       0,                                    FALSE }, //  EFI_SMM_SAVE_STATE_REGISTER_GDTLIMIT = 7
  { 0, 4, 0,                              SMM_CPU_OFFSET (x64.IdtrLimit),       0,                                    FALSE }, //  EFI_SMM_SAVE_STATE_REGISTER_IDTLIMIT = 8
  { 0, 4, 0,                              SMM_CPU_OFFSET (x64.LdtrLimit),       0,                                    FALSE }, //  EFI_SMM_SAVE_STATE_REGISTER_LDTLIMIT = 9
  ...
  { 0, 2, 0,                              SMM_CPU_OFFSET (x64.LdtrSelector),   0,                                   FALSE }, //  EFI_SMM_SAVE_STATE_REGISTER_LDTR_SEL = 26
  { 4, 2, SMM_CPU_OFFSET (x86._TR),       SMM_CPU_OFFSET (x64.TrSelector),     0,                                   FALSE }, //  EFI_SMM_SAVE_STATE_REGISTER_TR_SEL   = 27
...
// };
///
/// Lookup table for the AMD I/O Instruction Restart Dword information (Lowest 4 bits)
///
CONST EFI_SMM_SAVE_STATE_IO_TYPE  mAmdSmmCpuIoType[] = {
  (EFI_SMM_SAVE_STATE_IO_TYPE)0,         // Undefined                  = 0
  (EFI_SMM_SAVE_STATE_IO_TYPE)0,         // Undefined                  = 1
  EFI_SMM_SAVE_STATE_IO_TYPE_OUTPUT,     // SMM_IO_TYPE_OUT & VAL      = 2
  EFI_SMM_SAVE_STATE_IO_TYPE_INPUT,      // SMM_IO_TYPE_IN & VAL       = 3
  (EFI_SMM_SAVE_STATE_IO_TYPE)0,         // Undefined                  = 4
  (EFI_SMM_SAVE_STATE_IO_TYPE)0,         // Undefined                  = 5
  EFI_SMM_SAVE_STATE_IO_TYPE_STRING,     // SMM_IO_TYPE_OUTS & VAL     = 6
  EFI_SMM_SAVE_STATE_IO_TYPE_STRING,     // SMM_IO_TYPE_INS & VAL      = 7
  (EFI_SMM_SAVE_STATE_IO_TYPE)0,         // Undefined                  = 8
  (EFI_SMM_SAVE_STATE_IO_TYPE)0,         // Undefined                  = 9
  (EFI_SMM_SAVE_STATE_IO_TYPE)0,         // Undefined                  = 10
  (EFI_SMM_SAVE_STATE_IO_TYPE)0,         // Undefined                  = 11
  (EFI_SMM_SAVE_STATE_IO_TYPE)0,         // Undefined                  = 12
  (EFI_SMM_SAVE_STATE_IO_TYPE)0,         // Undefined                  = 13
  EFI_SMM_SAVE_STATE_IO_TYPE_REP_PREFIX, // SMM_IO_TYPE_REP_OUTS & VAL = 14
  EFI_SMM_SAVE_STATE_IO_TYPE_REP_PREFIX, // SMM_IO_TYPE_REP_INS & VAL  = 15
};
  ...
  )
{
  ...
  if (mSmmSaveStateRegisterLma == EFI_SMM_SAVE_STATE_REGISTER_LMA_32BIT) {
    ...
    if (NeedSysCall () == TRUE) {
      // SMM isolation
      *(UINT32*)Buffer = (UINT32) SysCall (SMM_SC_SVST_READ, CpuIndex, (UINTN) ((UINT8 *)CpuSaveState + mSmmCpuWidthOffset[RegisterIndex].Offset64Lo), MIN(4, Width));
    } else {
    ...
    if (Width > 4) {
      ...
      if (NeedSysCall () == TRUE) {
        // SMM isolation
        *((UINT32*)Buffer + 1) = (UINT32) SysCall (SMM_SC_SVST_READ, CpuIndex, (UINTN) ((UINT8 *)CpuSaveState + mSmmCpuWidthOffset[RegisterIndex].Offset64Hi), Width - 4);
      } else {
  ...
  )
{
  ...
  // AMD_SMRAM_SAVE_STATE_SMMIOTRAP SmmIoTrap;
  EFI_SMM_SAVE_STATE_IO_INFO  *IoInfo;
  UINT32                       SMM_IO_TRAP = 0;
  ...
  if (Register == EFI_SMM_SAVE_STATE_REGISTER_IO) {
    ...
    // //
    // // See if the CPU supports AMD64 Architecture
    // //
    // if (SmmRevId < SMRAM_SAVE_STATE_MIN_REV_ID_AMD64) {
    //   return EFI_NOT_FOUND;
    // }

    // //
    // // Get the I/O Instruction Restart DWORD register value
    // //
    // ReadSaveStateRegisterByIndex (CpuIndex, SMM_SAVE_STATE_REGISTER_SMMIOTRAP_INDEX, sizeof (SmmIoTrap.Uint32), &SmmIoTrap.Uint32);

    // //
    // // Check for the SMI_FLAG in IOMisc
    // //
    // if (SmmIoTrap.Bits.Val == 0) {
    //   return EFI_NOT_FOUND;
    // }
    ...
    //
    if (NeedSysCall () == TRUE) {
      SMM_IO_TRAP = (UINT32) SysCall (SMM_SC_SVST_READ, CpuIndex, (UINTN) ((UINT8 *)CpuSaveState + SMM_CPU_OFFSET (x64.SMM_IO_TRAP)), 4);
    } else {
      SMM_IO_TRAP = CpuSaveState->x64.SMM_IO_TRAP;
    }
    IoInfo->IoPort  = (UINT16) (SMM_IO_TRAP >> 16);
    IoInfo->IoWidth = EFI_SMM_SAVE_STATE_IO_WIDTH_UINT8;
    IoInfo->IoType  = EFI_SMM_SAVE_STATE_IO_TYPE_INPUT;
    IoInfo->IoData  = (UINT64) IoRead8 (SMM_IO_TRAP >> 16);

    // //
    // // Use lookup tables to help fill in all the fields of the IoInfo structure
    // //
    // IoInfo->IoPort  = (UINT16)SmmIoTrap.Bits.Port;
    // IoInfo->IoWidth = mSmmCpuIoWidth[SmmIoTrap.Bits.Size].IoWidth;
    // IoInfo->IoType  = mAmdSmmCpuIoType[SmmIoTrap.Uint32 & 0xf];
    // ReadSaveStateRegister (CpuIndex, EFI_SMM_SAVE_STATE_REGISTER_RAX, mSmmCpuIoWidth[SmmIoTrap.Bits.Size].Width, &IoInfo->IoData);
  ...
  )
{
  ...
  *(UINTN *)(UINTN)CpuSmiStack = CpuIndex;
  ...
  // SMM isolation
  SmmEntryAddr = (VOID *) gcSmiHandlerTemplate;
  SmmEntrySize = gcSmiHandlerSize;
  if (SmmSupervisor != NULL) {
  ...
  if ((SmmEntryAddr != NULL) && (SmmSupervisor != NULL)) {
