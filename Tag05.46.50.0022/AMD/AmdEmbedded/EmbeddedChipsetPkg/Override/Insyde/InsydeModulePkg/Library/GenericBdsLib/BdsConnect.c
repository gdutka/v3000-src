  ...
  )
{
  ...
  if (FeaturePcdGet (PcdH2OBdsCpConnectAllBeforeSupported)) {
...
/**
 ...
 @param  DevicePathToConnect   If this path is a RAID path, then connect all RAID path.
                               If this path is NULL, then connect all RAID path.

 @retval EFI_SUCCESS  Connect RAID controller successfully.
 @retval Other        Connect RAID controller failed.

**/
EFI_STATUS
EFIAPI
PlatformBdsConnectRaidController (
  IN EFI_DEVICE_PATH_PROTOCOL  *DevicePathToConnect
  )
{
  EFI_STATUS                            Status;
  ...
  if (DevicePathToConnect != NULL) {
    ...
    if (VidDid[0] != 0x1022 || (VidDid[1] != 0xB000)) {
      return EFI_UNSUPPORTED;
    }
  }
  ...
  for (HandleIndex = 0; HandleIndex < HandleCount; HandleIndex++) {
    ...
    //
    // Test whether the controller belongs to NVMe
    //
    if (ClassCode[2] != PCI_CLASS_MASS_STORAGE &&
        (ClassCode[1] != PCI_CLASS_MASS_STORAGE_SOLID_STATE)
       ) {
  ...
  )
{
  ...
  PlatformBdsConnectRaidController (FullDevicePath);
