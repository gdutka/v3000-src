  ...
  )
{
  ...
  IoWrite8 (TIMER0_COUNT_PORT, (UINT8) (Count & 0xFF));        // LSB (Least Significant Byte)
  IoWrite8 (TIMER0_COUNT_PORT, (UINT8) ((Count >> 8) & 0xFF)); // MSB (Most Significant Byte)
}
  ...
  )
{
  ...
  Tick = IoRead32 (ACPI_TIME_COUNTER_ADDRESS);
  return Tick;
  ...
  )
{
  ...
  if (CurrentTick < mPreAcpiTick) {
    EndTick = CurrentTick + 0x100000000;
  }
  //
  ...
  )
{
  ...
  //
  // Save PCI register 0xCF8 to prevent incorrect access when 
  // callback function is triggered between assigning index value
  // and accessing data value. And we don't know if the callback
  // function modify the value or not.
  // #Tag:IB07990090
  //
  PciCf8Save = IoRead32 (0xCF8);;
  ...
  //
  // Restore PCI register 0xCF8
  //
  IoWrite32 (0xCF8, PciCf8Save);
}
  ...
  )
{
  ...
  POST_CODE (DXE_SMART_TIMER_INIT);
