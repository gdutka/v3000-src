...
{
  EFI_STATUS              Status;
  EFI_GUID                VbiosGuid = AMD_PEI_VBIOS_FFS_GUID;
  UINT8                   NextBridgeNum;
  VOID                    *VbiosImage = NULL;
  VOID                    *VbiosImageCopy = NULL;
  UINTN                   Instance;
  EFI_PEI_FV_HANDLE       VolumeHandle;
  EFI_PEI_FILE_HANDLE     FileHandle;
  UINT32                  VbiosSize;
  ...
  while (TRUE) {
    ...
    DEBUG ((EFI_D_ERROR, "Load RN VBIOS!\n"));
      Status = PeiServicesFfsFindFileByName (&VbiosGuid, VolumeHandle, &FileHandle);
    if (!EFI_ERROR (Status)) {
      ...
      VbiosSize = ((EFI_LEGACY_EXPANSION_ROM_HEADER *) VbiosImage)->Size512 * 512;
      DEBUG ((EFI_D_ERROR, "VBIOS Size = 0x%x!\n", VbiosSize));
      Status = PeiServicesAllocatePool(VbiosSize, (VOID **)&VbiosImageCopy);
      if (!EFI_ERROR(Status)) {
        DEBUG ((EFI_D_ERROR, "Copy new VBIOS image!\n"));
        CopyMem(VbiosImageCopy, VbiosImage, VbiosSize);
        VbiosImage = VbiosImageCopy;
      }
  ...
  AmdPeiGopPolicyPtr->LidStatus = 0;
  AmdPeiGopPolicyPtr->HorizontalResolution = 1024;
  AmdPeiGopPolicyPtr->VerticalResolution = 768;
  if (VbiosImage != NULL) {
    // Override Device ID for PCO
    Pcir = (PCI_3_0_DATA_STRUCTURE *)
           ((UINT8 *) VbiosImage + ((PCI_EXPANSION_ROM_HEADER *) VbiosImage)->PcirOffset);
    Pcir->DeviceId = PciRead16(PCI_LIB_ADDRESS (NextBridgeNum, 0, 0, 0x2));
  }
...
)
{
  ...
  VidDid = PciRead32(PCI_LIB_ADDRESS (4, 0, 0, 00));
  ...
  for (Index = 0; Index < AMD_GFX_NUM_OF_BAR; Index++) {
    PciWrite32 (PCI_LIB_ADDRESS (4, 0, 0, 0x10 + (Index * 4)), ALL_ONE_32_BITS);
    Data32 = PciRead32(PCI_LIB_ADDRESS (4, 0, 0, 0x10 + (Index * 4)));
    DEBUG((EFI_D_INFO, "-------------------------------------\n"));
  ...
  if ((MmioForEachDie.IO[0][0].Base <= RES_IO_BASE) && ((MmioForEachDie.IO[0][0].Base + MmioForEachDie.IO[0][0].Size) >= (RES_IO_BASE + TotalIOSize))) {
    AllocatedIoBase = RES_IO_BASE;
    AllocatedIoSize = TotalIOSize;
  ...
  for (Index = 0; Index < AMD_GFX_NUM_OF_BAR; Index++) {
    ...
    if( IsMemResource) {
      ...
      if(IsPrefetchableMem && (AllocatedPrefetchableBase != 0)) {
        ...
        PciWrite32 (PCI_LIB_ADDRESS (4, 0, 0, 0x10 + (Index * 4)), Data32);
        if (Is64BitBar) {
  ...
  PciOr8 (PCI_LIB_ADDRESS (4, 0, 0, 0x04), BIT0 + BIT1 + BIT2); //iGFX: Enable Bus Master, Enable memory/io decoding
...
)
{
  ...
  //
  // Clear CMOS flag now if in sync, while platform BIOS has read and aware of recovery mode in CheckCMOSRecoveryFlag
  // function.
  //
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "Get Boot Mode ERROR(%r)\n", Status));
    return Status;
  }
  ...
  if (PcdGetBool (PcdPeiGopSupported)) {

    FbTop = PcdGet32(PcdPeiGopVmFbLocationTop);
    DEBUG((EFI_D_INFO, "FbTop: 0x%x.\n", FbTop));
    if (FbTop >= 0x100) {
      DEBUG((EFI_D_INFO, "FbTop above 4G, set the recovery flag and restart\n"));
    }
    ...
    //
    // If mistmatch with boot mode, update CMOS flag and reset.
    //
    if (!(RecoveryFlag1 == RECOVERY_VALUE1 && RecoveryFlag2 == RECOVERY_VALUE2)) {
      DEBUG ((EFI_D_ERROR, "Sync recovery mode to CMOS flag and restart\n"));
