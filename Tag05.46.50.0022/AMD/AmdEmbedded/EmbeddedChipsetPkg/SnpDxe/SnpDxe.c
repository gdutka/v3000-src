  ...
  )
{
  ...
  Private->Mode.MaxPacketSize        = ETH_DATA_LEN;
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_NET, "\nSNPDXE_LOG: XgbeBaseAddr failed - %r\n", Status));
    return Status;
  }

  XgbePlatinit (Private);

  Status = xgbe_phy_init (Private);
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_NET, "\nSNPDXE_LOG: xgbe_phy_init failed - %r\n", Status));
    return Status;
  }
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_NET, "\nSNPDXE_LOG: EthSetMacAddrFromPcd failed - %r\n", Status));
    return EFI_NOT_FOUND;
  }
  SetMem (
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_NET, "\nSNPDXE_LOG: EnetDmaAllocBuffer desc_rx_dma - %r\n", Status));
    return Status;
  }
  ...
  for (Index = 0; Index < TX_DESC_CNT; Index++) {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((EFI_D_NET, "\nSNPDXE_LOG: EnetDmaAllocBuffer Index - %d pkt_tx_dma - %r\n", Index, Status));
      return Status;
    }
  ...
  for (Index = 0; Index < RX_DESC_CNT; Index++) {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((EFI_D_NET, "\nSNPDXE_LOG: EnetDmaAllocBuffer Index - %d pkt_rx_dma - %r\n", Index, Status));
      return Status;
    }
