...
VOID STATIC xgbe_an73_restart ( ENET_DXE_PRIVATE_DATA *pdata  );
VOID xgbe_an73_set (  ENET_DXE_PRIVATE_DATA *pdata,  BOOLEAN enable,  BOOLEAN restart  );
...
#define XGBE_PHY_PORT_SPEED_10       BIT0
#define XGBE_PHY_PORT_SPEED_100      BIT1
#define XGBE_PHY_PORT_SPEED_1000     BIT2
#define XGBE_PHY_PORT_SPEED_2500     BIT3
#define XGBE_PHY_PORT_SPEED_10000    BIT4
...
VOID xgbe_phy_reset_cdr_delay(ENET_DXE_PRIVATE_DATA *pdata)
{
...
VOID xgbe_phy_update_cdr_delay(ENET_DXE_PRIVATE_DATA *pdata)
{
  XGBE_PHY_DATA *phy_data = pdata->phy_data;
...
UINT32 xgbe_phy_kr_training_cdroff(ENET_DXE_PRIVATE_DATA *pdata)
{
  UINT32 ret;
  XGBE_PHY_DATA *phy_data = pdata->phy_data;
  ret = (UINT32)phy_data->phy_cdr_notrack;
                    ...
                    enum xgbe_mode mode)
{
    ...
    case XGBE_MODE_SGMII_10:
        return xgbe_phy_check_mode(pdata, mode, ADVERTISED_10baseT_Full);
    ...
    )
{
  ...
  /* Start KR training */
  if(!xgbe_phy_kr_training_cdroff(pdata) ) {
    DEBUG((EFI_D_ERROR,"setting phy_data->phy_cdr_notrack"));
  }
  ...
  /*  set RX_EQ_MGMT_MODE to disable RX Adapt Requests 0x8009 */
  XMDIO_WRITE_BITS(XpcsBase, MDIO_MMD_PMAPMD, 0x8009, (1<<15), (1<<15));

  XMDIO_WRITE_BITS(XpcsBase, MDIO_MMD_PMAPMD, MDIO_VEND2_PMA_CDR_CONTROL,
       XGBE_PMA_CDR_TRACK_EN_MASK,
       XGBE_PMA_CDR_TRACK_EN_OFF);

  XMDIO_WRITE_BITS(XpcsBase,MDIO_MMD_PMAPMD,0x8050,(1<<8),(0<<8));


  xgbe_phy_kr_training_post(pdata);

  XMDIO_WRITE_BITS(XpcsBase,MDIO_MMD_PMAPMD,0x8050,(1<<8),(1<<8));

  pdata->cdr_delay_required = 0;
  gBS->Stall(1);

  XMDIO_WRITE_BITS(XpcsBase, MDIO_MMD_PMAPMD, 0x8009,
         (1<<15),
         (0<<15));
...
static enum xgbe_mode xgbe_phy_switch_baset_mode( ENET_DXE_PRIVATE_DATA *pdata)
{
XGBE_PHY_DATA *phy_data = pdata->phy_data;
...
)
{
    ...
    /* Call PHY implementation support to complete rate change */
    xgbe_phy_set_mode(pdata, XGBE_MODE_SGMII_10);
  ...
  )
{
  ...
  ConnType = getBitfieldReg32 (XgmacBase + ENET_PORT_PROPERTY_0, 22, 5 ); // Check Speed Type
  if (ConnType == 0) {
    return FALSE;
  }
  return TRUE;
...
EFI_STATUS  xgbe_phy_an_advertising (ENET_DXE_PRIVATE_DATA *Private)
{
    ...
    case XGBE_PORT_MODE_10GBASE_T:
      if (phy_data->port_speeds & XGBE_PHY_PORT_SPEED_10000) {
          Private->phy.supported |= ADVERTISED_10000baseKR_Full;
      }else if(phy_data->port_speeds & XGBE_PHY_PORT_SPEED_2500){
          Private->phy.supported |= ADVERTISED_2500baseX_Full;
      }else if(phy_data->port_speeds & XGBE_PHY_PORT_SPEED_10){
          Private->phy.supported |= ADVERTISED_10baseT_Full;
      }else{
          Private->phy.supported |= ADVERTISED_1000baseKX_Full;
      }
      break;
  ...
  )
{
  ...
  phy_data->port_speeds = getBitfieldReg32 (XgmacBase + ENET_PORT_PROPERTY_0, 22, 5);        // Port Speed
  ...
  // 10GBase-T support
  case XGBE_PORT_MODE_10GBASE_T:
      Private->phy.speed = phy_data->port_speeds;
      Private->phy.supported |=   ADVERTISED_Pause | ADVERTISED_Asym_Pause | ADVERTISED_Autoneg | ADVERTISED_TP;
      if (phy_data->port_speeds & XGBE_PHY_PORT_SPEED_10) {
          //phy_data->start_mode = XGBE_MODE_SGMII_10;
          Private->phy.supported |= ADVERTISED_10baseT_Full;
    ...
    if (phy_data->port_speeds & XGBE_PHY_PORT_SPEED_10000) {
      //phy_data->start_mode = XGBE_MODE_KR;
      Private->phy.supported |= ADVERTISED_10000baseT_Full;
    }

    Private->mdio_an_mode = 1;
    Private->phy.advertising =  Private->phy.supported;
    phy_data->phydev_mode = XGBE_MDIO_MODE_CL45;
    break;
  ...
  )
{
  ...
  case XGBE_SFP_BASE_1000_CX:
    ...
    if (phy_data->sfp_base == XGBE_SFP_BASE_1000_T) {
        if (phy_data->port_speeds & XGBE_PHY_PORT_SPEED_10) {
            pdata->phy.supported |= ADVERTISED_10baseT_Full;
        }
        if (phy_data->port_speeds & XGBE_PHY_PORT_SPEED_100){
  ...
  )
{
    ...
    switch (phy_data->port_mode) {
        ...
        case XGBE_PORT_MODE_1000BASE_T:
            ...
            break;
        case XGBE_PORT_MODE_10GBASE_T:
             Status = Aqr113_phy_init_hw(pdata);
             if (EFI_ERROR (Status)) {
                DEBUG ((EFI_D_ERROR, "PHY INIT FW Failed \n"));
                return EFI_NOT_FOUND;
             }
    ...
    */
   if(XGBE_PORT_MODE_10GBASE_T == phy_data->port_mode)
       xgbe_set_mode(pdata ,xgbe_phy_mdio_an_outcome(pdata));
    else if (xgbe_phy_use_mode(pdata, XGBE_MODE_KR)) {
        xgbe_kr_mode(pdata);
        ...
        xgbe_sgmii_100_mode(pdata);
    } else if (xgbe_phy_use_mode(pdata, XGBE_MODE_SGMII_10)) {
        xgbe_sgmii_10_mode(pdata);
    }
  ...
  )
{
  ...
  if (mXgbeData.XgbeBusFunc == FUNC_2) {
  ...
  } else if (mXgbeData.XgbeBusFunc == FUNC_3) {
...
static void xgbe_phy_pll_ctrl(ENET_DXE_PRIVATE_DATA *pdata, BOOLEAN enable)
{
  ...
  if (pdata->phy.autoneg == AUTONEG_DISABLE ||
    ...
    XMDIO_WRITE_BITS(XpcsBase, MDIO_MMD_PMAPMD, MDIO_VEND2_PMA_MISC_CTRL0,
  ...
  )
{
  ...
  UINT32                    XgmacBaseI2c;
  Status = EFI_SUCCESS;
  SlaveAdd = phy_data->redrv_addr;  //0x40
...
)
{
    ...
    UINT32 XgmacBase;
    XgmacBase = pdata->xgmac_pdata.xgmac_regs;
    DEBUG((EFI_D_ERROR, "xgbe_phy_sgmii_10_mode\n"));
    xgbe_phy_set_redrv_mode(pdata);
    xgbe_phy_start_ratechange(pdata);
    ...
    DEBUG((EFI_D_ERROR, "10MbE SGMII mode set\n"));
}
  ...
  )
{
  ...
  reg = XPCS_IOREAD32 (XpcsBase, PCS_V2_RMB_WINDOW_DEF);
...
//offset 1 bit and reading 16 bits of data.
  ...
  XPCS_IOWRITE32 (XpcsBase, PCS_V2_RMB_WINDOW_SELECT, (UINT32)index);
  ...
  )
{
    ...
    case  XGBE_AN_MODE_MDIO:
     DEBUG((EFI_D_ERROR," xgbe_an_init for MDIO\n "));
    default:
        break;
    }
}
...
again:
  ...
  } else if ( Private->an_mode == XGBE_AN_MODE_MDIO){
    ...
    while(WaitCount < 1000) {
      ...
      if ((RegVal & PCS_STS1_RLU) != 0) {
        DEBUG ((EFI_D_INFO, "::LINK SPEED : %d  PCS LINK STS - ", mCurrLanSpeed));
        DEBUG ((EFI_D_INFO, "UP\n"));
        DEBUG ((EFI_D_INFO, "STS1 RegVal : %x ", RegVal));
        break;
      }
      gBS->Stall (100);
      WaitCount++;
    ...
    if(xgbe_phy_aneg_done(Private)) {
      xgbe_set_mode(Private ,xgbe_phy_mdio_an_outcome(Private));
      return EXIT_AN_RUN;
    }
    DEBUG ((EFI_D_INFO, "::LINK SPEED : %d  PCS LINK STS - ", mCurrLanSpeed));
    DEBUG ((EFI_D_INFO, "DOWN\n"));
    aqr_config_aneg(Private);
    gBS->Stall(1000);
    aqr107_read_status(Private);
  ...
  )
{
 ...
 enum xgbe_mode xgbe_phy_mdio_an_outcome(IN  ENET_DXE_PRIVATE_DATA *pdata)
{
  ...
  UINTN ad_reg, lp_reg;
   ...
   if (pdata->phy.pause_autoneg) {
    ...
    DEBUG ((EFI_D_ERROR, "xgbe_phy_mdio_an_outcome :pdata->phy.interface: %x \n",pdata->phy.interface));
    switch (pdata->phy.interface) {
      case PHY_INTERFACE_MODE_10GKR:
      if ((phy_data->port_speeds & XGBE_PHY_PORT_SPEED_10000) && (pdata->phy.speed == SPEED_10000))
        mode = XGBE_MODE_KR;
      if(pdata->phy.speed == SPEED_10) {
        if (phy_data->port_speeds & XGBE_PHY_PORT_SPEED_10)
 ...
 DEBUG ((EFI_D_ERROR, "xgbe_phy_mdio_an_outcome :pdata->phy.speed: %x  mode :%x\n",pdata->phy.speed,mode));
  return mode;
}
  ...
  )
{
  ...
  if (Private->phy.autoneg != AUTONEG_ENABLE /*&& phy_data->cur_mode == XGBE_MODE_SFI*/) {
    DEBUG ((EFI_D_ERROR, "SNPDXE_LOG: xgbe_an_state_machine Auto negotiation disabled\n"));
  ...
  DEBUG ((EFI_D_ERROR, "SNPDXE_LOG: xgbe_an_state_machine Auto negotiation enabledAN_MAX_RETR: %d\n",AN_MAX_RETR));
  ...
  if ((RegVal & PCS_STS1_RLU) != 0) {
    ...
    DEBUG ((EFI_D_ERROR, "STS1 RegVal : %x ", RegVal));
  if(Private->kr_done){
    xgbe_phy_reset_cdr_delay(Private);
  }
  	//StartTmr (Private);
	return  TRUE;
  } else {
  ...
  )
{
  ...
  if(pdata->an_mode != XGBE_AN_MODE_MDIO){
    if( (phy_data->cur_mode == XGBE_MODE_KR) && (enable|| restart)) {
      reg = XMDIO_READ(XpcsBase, MDIO_MMD_PMAPMD, MDIO_PMA_10GBR_PMD_CTRL);
      reg |= XGBE_KR_TRAINING_ENABLE;
      reg |= XGBE_KR_TRAINING_START;
      XMDIO_WRITE(XpcsBase, MDIO_MMD_PMAPMD, MDIO_PMA_10GBR_PMD_CTRL, reg);
      ...
      DEBUG((EFI_D_ERROR," %a XGBE_AN_MODE_MDIO  \n",__FUNCTION__));
...
{
  UINTN reg;
  DEBUG ((EFI_D_ERROR, "xgbe_an37_set - %s\n", enable ? "enable" : "disable" ));
  ...
  reg = XMDIO_READ(XpcsBase, MDIO_MMD_VEND2, MDIO_PCS_DIG_CTRL);
  reg |= XGBE_VEND2_MAC_AUTO_SW;
  XMDIO_WRITE(XpcsBase, MDIO_MMD_VEND2, MDIO_PCS_DIG_CTRL, reg);
}
  ...
  )
{
  ...
  case XGBE_AN_MODE_CL73:
  case XGBE_AN_MODE_CL73_REDRV:
	  xgbe_an73_set(pdata , enable , restart);
	  break;
  case XGBE_AN_MODE_CL37:
  case XGBE_AN_MODE_CL37_SGMII:
  ...
  )
{
  ...
  RegValue = 0;
  for (i = 0; i < q_count;) {
    RegValue |= (0x80 << ((i++ % MTL_RQDCM_Q_PER_REG) << 3));

    if ((i % MTL_RQDCM_Q_PER_REG) && (i != q_count)) {
      continue;
    }

    XGMAC_IOWRITE32 (pdata, reg, RegValue);
...
//  UINT32 z;
  ...
  reg = MAC_Q0TFCR;
  for (i = 0; i < q_count; i++) {
    RegValue = XGMAC_IOREAD32 (pdata, reg);
    XGMAC_SET_BITS (RegValue, MAC_Q0TFCR, TFE, 0);
    XGMAC_IOWRITE32 (pdata, reg, RegValue);
  ...
  )
{
  ...
  for (i = 0; i < q_count; i++) {
    ...
    XGMAC_SET_BITS (RegValue, MAC_Q0TFCR, TFE, 1);
    // Set pause time
    XGMAC_SET_BITS (RegValue, MAC_Q0TFCR, PT, 0xffff);
  ...
  )
{
  ...
  XGMAC_IOWRITE32 (pdata, MAC_RQC0R, RegValue);
  ...
  )
{
  ...
  if(Timeout==0) {
      DEBUG ((EFI_D_ERROR, "XGMAC : Reset Timed out. Returning Device error\n"));
      return EFI_DEVICE_ERROR;
  }
  ...
  )
{
  ...
  DEBUG ((EFI_D_ERROR, "CL73 AN enabled / restarted CL72 started\n"));
}
