...
EFI_STATUS xgbe_set_ext_mii_mode(ENET_DXE_PRIVATE_DATA *pdata, UINT32 port,
    ...
    Mdioreg |= (1 << PORT_DEF); // PORT_DEF, this is for getting the PHY_IDs, not to be modified w.r.t PortNum
 ...
 */
EFI_STATUS mv3310_amd_quirk(ENET_DXE_PRIVATE_DATA *phydev)
{
    int reg = 0, count = 0;
...
int phy_modify_mmd_changed(ENET_DXE_PRIVATE_DATA *pdata, int devad, UINT32 regnum,
                     UINT16 mask, UINT16 set)
{
  int Value;
  int new;
  EFI_STATUS Status = EFI_SUCCESS;

  Value = phy_read_mmd(pdata, devad, regnum);
  ...
  Status = phy_write_mmd(pdata, devad, regnum, new);
    if (Status)
      return -1;
    else
       return 1;
...
EFI_STATUS phy_modify_mmd(ENET_DXE_PRIVATE_DATA *pdata, int devad, UINT32 regnum,
     UINT16 mask, UINT16 set){
  int ret ;
  ret = phy_modify_mmd_changed(pdata, devad ,regnum , mask,set);
  if (ret < 0)
 ...
 * on MMD
 * @phydev: the XGBE private struct
 * @devad: the MMD containing register to modify
 * @regnum: register number to modify
 * @val: bits to clear
   @retval        EFI_STATUS
 */
EFI_STATUS  phy_set_bits_mmd(ENET_DXE_PRIVATE_DATA *pdata, int devad,
                       UINT32 regnum, UINT16 val)
{
  return phy_modify_mmd(pdata, devad, regnum, 0, val);
}

/**
 * phy_clear_bits_mmd - Convenience function for clearing bits in a register
 * on MMD
 * @phydev: the the XGBE private struct
 * @devad: the MMD containing register to modify
 * @regnum: register number to modify
 * @val: bits to clear
   @retval        EFI_STATUS
 */
EFI_STATUS  phy_clear_bits_mmd(ENET_DXE_PRIVATE_DATA *pdata, int devad,
            UINT32 regnum, UINT16 val)
{
   return phy_modify_mmd(pdata, devad, regnum, val, 0);
}

EFI_STATUS genphy_c45_an_disable_aneg(ENET_DXE_PRIVATE_DATA *pdata)
{

  return phy_clear_bits_mmd(pdata, MDIO_MMD_AN, MDIO_CTRL1,
                 MDIO_AN_CTRL1_ENABLE | MDIO_AN_CTRL1_RESTART);
}

EFI_STATUS genphy_c45_pma_setup_forced(ENET_DXE_PRIVATE_DATA *pdata)
{
  ...
  ctrl1 = phy_read_mmd(pdata, MDIO_MMD_PMAPMD, MDIO_CTRL1);
  if (ctrl1 < 0)
    return EFI_DEVICE_ERROR;

  ctrl2 = phy_read_mmd(pdata, MDIO_MMD_PMAPMD, MDIO_CTRL2);
  if (ctrl2 < 0)
    return EFI_DEVICE_ERROR;
...
ctrl2 &= ~(MDIO_PMA_CTRL2_TYPE | 0x30);
  ...
  Status = phy_write_mmd(pdata, MDIO_MMD_PMAPMD, MDIO_CTRL1, ctrl1);
  if(Status)
     return Status;

  Status = phy_write_mmd(pdata, MDIO_MMD_PMAPMD, MDIO_CTRL2, ctrl2);
  if(Status)
      return Status;
  return genphy_c45_an_disable_aneg(pdata);
}

UINT16 linkmode_adv_to_mii_adv_t(UINT64 advertising)
{
  UINT16 result = 0;


  if (ADVERTISED_10baseT_Full & (advertising) )
    result |= ADVERTISE_10FULL;

  if (ADVERTISED_100baseT_Full & (advertising))
    result |= ADVERTISE_100FULL;
  if (ADVERTISED_Pause & (advertising))
    result |= ADVERTISE_PAUSE_CAP;

   if (ADVERTISED_Asym_Pause & advertising)
     result |= ADVERTISE_PAUSE_ASYM;

  return result;
}
UINT16 linkmode_adv_to_mii_10gbt_adv_t(UINT64 advertising)
{
  UINT16 result = 0;

  if (ADVERTISED_2500baseT_Full & (advertising))
    result |= MDIO_AN_10GBT_CTRL_ADV2_5G;

  if (ADVERTISED_10000baseT_Full & (advertising))
    result |= MDIO_AN_10GBT_CTRL_ADV10G;

  return result;
}

int genphy_c45_an_config_aneg(ENET_DXE_PRIVATE_DATA *pdata)
{
  ...
  //changed = genphy_config_eee_advert(pdata);

  adv = linkmode_adv_to_mii_adv_t(pdata->phy.advertising);

  ret = phy_modify_mmd_changed(pdata, MDIO_MMD_AN, MDIO_AN_ADVERTISE,
  ...
  adv = linkmode_adv_to_mii_10gbt_adv_t(pdata->phy.advertising);

  ret = phy_modify_mmd_changed(pdata, MDIO_MMD_AN, MDIO_AN_10GBT_CTRL,
 ...
 */
EFI_STATUS genphy_c45_restart_aneg(ENET_DXE_PRIVATE_DATA *pdata)
{
  return phy_set_bits_mmd(pdata, MDIO_MMD_AN, MDIO_CTRL1,
                MDIO_AN_CTRL1_ENABLE | MDIO_AN_CTRL1_RESTART);
}
 ...
 */
EFI_STATUS genphy_c45_check_and_restart_aneg(ENET_DXE_PRIVATE_DATA *pdata, BOOLEAN restart)
{
  int ret;

  if (!restart) {
  /* Configure and restart aneg if it wasn't set before */
    ret = phy_read_mmd(pdata, MDIO_MMD_AN, MDIO_CTRL1);
    if (ret < 0)
      return EFI_DEVICE_ERROR;
  ...
  if (restart)
    return genphy_c45_restart_aneg(pdata);

  return EFI_SUCCESS;
}

EFI_STATUS aqr_config_aneg(ENET_DXE_PRIVATE_DATA *pdata)
{
  ...
  if (pdata->phy.autoneg == AUTONEG_DISABLE)
    return genphy_c45_pma_setup_forced(pdata);

  ret = genphy_c45_an_config_aneg(pdata);
  if (ret < 0)
    return EFI_DEVICE_ERROR;
  if (ret > 0)
    changed = TRUE;
 ...
 */
  reg = 0;
  if (ADVERTISED_1000baseT_Full & (pdata->phy.advertising))
             reg |= MDIO_AN_VEND_PROV_1000BASET_FULL;

  if (ADVERTISED_1000baseT_Half & (pdata->phy.advertising))
             reg |= MDIO_AN_VEND_PROV_1000BASET_HALF;

  ret = phy_modify_mmd_changed(pdata, MDIO_MMD_AN, MDIO_AN_VEND_PROV,
  ...
  return genphy_c45_check_and_restart_aneg(pdata, changed);
}
 ...
 *
 *
 * @param[in]     phydev    Pointer to Enet dxe private date
 *
 * @retval        UINT32    return the PHY vendor ID
 *
 */

UINT32 aqr107_wait_reset_complete(ENET_DXE_PRIVATE_DATA *phydev){
  UINT8 timeout = 0;
  UINT32 val;
  while(timeout < 100){
...
/*---------------------------------------------------------------------------------------------
/**
 * aqr107_chip_info - to get the Aqr113 chip info
 *
 * @param[in]     phydev    Pointer to Enet dxe private date
 *
 * @retval        EFI_SUCCESS     on success
 *               EFI_DEVICE_ERROR   on failure
 *
 */
EFI_STATUS aqr107_chip_info(ENET_DXE_PRIVATE_DATA *phydev)
{
  UINT32 val;
  val = phy_read_mmd(phydev, MDIO_MMD_VEND1, VEND1_GLOBAL_FW_ID);
  DEBUG((EFI_D_INFO, "SNPDXE_LOG: VEND1_GLOBAL_FW_ID :Value : %d\n", val));
  if (val < 0)
    return EFI_DEVICE_ERROR;

  val = phy_read_mmd(phydev, MDIO_MMD_VEND1, VEND1_GLOBAL_RSVD_STAT1);
  DEBUG((EFI_D_INFO, "SNPDXE_LOG: VEND1_GLOBAL_RSVD_STAT1 :Value : %d \n", val));
  if (val < 0)
    return EFI_DEVICE_ERROR;
...
/*---------------------------------------------------------------------------------------------
/**
 * aqr107_set_downshift - to set the Aqr113 downshift
 *
 * @param[in]     phydev    Pointer to Enet dxe private date
 * @param[in]   cnt
 *
...
EFI_STATUS aqr107_set_downshift( ENET_DXE_PRIVATE_DATA *phydev, UINT8 cnt)
{
  UINT32 val = 0;
  ...
  if (cnt != DOWNSHIFT_DEV_DISABLE) {
    ...
    DEBUG((EFI_D_ERROR, "SNPDXE_LOG: aqr107_set_downshift : vlaue: %d \n",val));

  return  phy_write_mmd(phydev, MDIO_MMD_AN, MDIO_AN_VEND_PROV, val);
}
/*---------------------------------------------------------------------------------------------
/**
 * aqr113_set_mode - to set the Aqr113 AN modes
 *
 * @param[in]     phydev    Pointer to Enet dxe private date
 *
 * @retval        EFI_SUCCESS     on success
...
EFI_STATUS aqr113_set_mode(ENET_DXE_PRIVATE_DATA *phydev)
{
  ...
  if ((fw == 0x506) && (build == 0x16)) {
    ...
    val = phy_read_mmd(phydev, MDIO_MMD_AN, 0xc400);

    if (ADVERTISED_1000baseT_Full & (phydev->phy.advertising))
      val =  val | (3 << 0xe);
    else
      val =  val & (~( 3<< 0xe));
    phy_write_mmd(phydev, MDIO_MMD_AN, 0xc400,val);

    /* set PHY in SGMI mode for 2500M with system side AN disabled */
    val = phy_read_mmd(phydev, MDIO_MMD_AN, 0xc400);
    if (ADVERTISED_2500baseT_Full & (phydev->phy.advertising))
      val =  val | (1 << 0xa);
    else
      val =  val & (~( 1<< 0xa));
    phy_write_mmd(phydev, MDIO_MMD_AN, 0xc400,val);
    ...
    /* set PHY in SGMI mode for 10G with system side AN disabled */
    val = phy_read_mmd(phydev, MDIO_MMD_AN, 0x20);
    if (ADVERTISED_10000baseT_Full & (phydev->phy.advertising))
      val =  val | (1 << 0xc);
    else
      val =  val & (~( 1<< 0xc));
    phy_write_mmd(phydev, MDIO_MMD_AN, 0x20,val);

  }
  return EFI_SUCCESS;
}

static int aqr107_read_rate(ENET_DXE_PRIVATE_DATA *pdata)
{
  int val, rate;

  val = phy_read_mmd(pdata, MDIO_MMD_AN, MDIO_AN_TX_VEND_STATUS1);
  if (val < 0)
    return val;
  rate = (val >> 1 & 7) ;
  ...
  )
{
  ...
  Linkstatus = ((Reg32 & PCS_STS1_RLU) == PCS_STS1_RLU)?TRUE:FALSE;

    return Linkstatus;
}


 BOOLEAN xgbe_phy_aneg_done(ENET_DXE_PRIVATE_DATA *pdata)
{

  if (pdata->an_mode == XGBE_AN_MODE_MDIO) {
    if(GetLinkstatus(pdata))
...
int aqr107_read_status(ENET_DXE_PRIVATE_DATA *pdata)
{
  int val, ret;

  if (!GetLinkstatus(pdata) || (pdata->phy.autoneg == AUTONEG_DISABLE) )
    return 0;
  val = phy_read_mmd(pdata, MDIO_MMD_PHYXS, MDIO_PHYXS_VEND_IF_STATUS);
  if (val < 0)
    return val;

  ret =  (val>>3) & 0x0001F ; // 3 to 7 indicates system interface

  switch (ret) {
    case MDIO_PHYXS_VEND_IF_STATUS_TYPE_KR:
          pdata->phy.interface = PHY_INTERFACE_MODE_10GKR;
          break;
    case MDIO_PHYXS_VEND_IF_STATUS_TYPE_XFI:
           pdata->phy.interface = PHY_INTERFACE_MODE_10GBASER;
           break;
    case MDIO_PHYXS_VEND_IF_STATUS_TYPE_USXGMII:
           pdata->phy.interface = PHY_INTERFACE_MODE_USXGMII;
           break;
    case MDIO_PHYXS_VEND_IF_STATUS_TYPE_SGMII:
           pdata->phy.interface= PHY_INTERFACE_MODE_SGMII;
           break;
    case MDIO_PHYXS_VEND_IF_STATUS_TYPE_OCSGMII:
          pdata->phy.interface = PHY_INTERFACE_MODE_2500BASEX;
          break;
    default:
          pdata->phy.interface = PHY_INTERFACE_MODE_NA;
         break;
    }

    /* Read possibly downshifted rate from vendor register */
  return aqr107_read_rate(pdata);
}


/*---------------------------------------------------------------------------------------------
/**
 * aqr113_config_init - to intialize  the Aqr113configuration
 *
 * @param[in]     phydev    Pointer to Enet dxe private date
 *
 * @retval        EFI_SUCCESS     on success
 ...
 *
 */
EFI_STATUS aqr113_config_init(ENET_DXE_PRIVATE_DATA *phydev)
{
  UINT32 Value;
  EFI_STATUS Status = EFI_SUCCESS;

  DEBUG((EFI_D_ERROR, "SNPDXE_LOG: aqr113_config_init start \n"));
  Value = aqr107_wait_reset_complete(phydev);
  if (!Value){
    Status = aqr107_chip_info(phydev);
      if (EFI_ERROR(Status)) {
         DEBUG((EFI_D_ERROR, "SNPDXE_LOG :aqr113 chip info get failed\n"));
     }
  }

  Status = aqr107_set_downshift(phydev, MDIO_AN_VEND_PROV_DOWNSHIFT_DFLT);
  if (EFI_ERROR(Status)) {
       DEBUG((EFI_D_ERROR, "SNPDXE_LOG : aqr107_set_downshift chip info get failed\n"));
   }
  Status  = aqr113_set_mode(phydev);
  DEBUG((EFI_D_ERROR, "\nSNPDXE_LOG : aqr113_set_mode status : %r\n", Status));

  Status = aqr_config_aneg(phydev);
  DEBUG((EFI_D_ERROR, "\nSNPDXE_LOG : aqr_config_aneg status : %r\n", Status));

  Value = aqr107_read_status(phydev);
  DEBUG((EFI_D_ERROR, "\nSNPDXE_LOG : aqr113_set_mode status : %r\n", Value));
 return Status;
}
...
/*---------------------------------------------------------------------------------------------
/**
 * Aqr113_phy_init_hw - to intialize  the Aqr113PHY
 *
 * @param[in]     phydev    Pointer to Enet dxe private date
 *
 * @retval        EFI_SUCCESS     on success
...
EFI_STATUS Aqr113_phy_init_hw(ENET_DXE_PRIVATE_DATA *pdata){

  EFI_STATUS Status;
  DEBUG((EFI_D_ERROR, "Aqr113_phy_init_hw\n"));
  Status =  aqr113_config_init(pdata);
  if (EFI_ERROR(Status)) {
    DEBUG((EFI_D_ERROR, "SNPDXE_LOG : Aqr113_phy_init_hw get failed\n"));
 }
return Status;
...
/*---------------------------------------------------------------------------------------------
/**
 * phy_init_hw - to intialize  the mv3310 PHY
 *
 * @param[in]     phydev    Pointer to Enet dxe private date
 *
 * @retval        EFI_SUCCESS     on success
...
EFI_STATUS phy_init_hw(ENET_DXE_PRIVATE_DATA *pdata) {
    ...
    Status = m88e1510_config_aneg(pdata);
    if (EFI_ERROR(Status)) {
      DEBUG((EFI_D_ERROR, "m88e1510_config_aneg failed\n"));
    }
  // Status = mv3310_amd_quirk(pdata);
  // if (EFI_ERROR(Status)) {
  //   DEBUG((EFI_D_ERROR, "SNPDXE_LOG: mv3310_amd_quirk failed - %r\n", Status));
  // }
