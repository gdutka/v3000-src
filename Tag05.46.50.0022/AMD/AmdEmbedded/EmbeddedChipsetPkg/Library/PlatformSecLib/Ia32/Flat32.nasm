...
extern   ASM_PFX(PcdGet32 (PcdPciExpressSizeMsrValue))
extern   ASM_PFX(PcdGet32 (PcdFlashFvRecoverySize))
extern   ASM_PFX(PcdGet32 (PcdFlashFvRecoveryBase))
extern   ASM_PFX(PcdGet32 (PcdH2OBootFirmwareVolumeBase))
extern   ASM_PFX(PcdGet32 (PcdFlashAreaBaseAddress))
extern   ASM_PFX(PcdGet32 (PcdBspStackSize))
extern   ASM_PFX(NonSmmS3ResumeInit)
extern   ASM_PFX(PcdGetBool (PcdAmdPspS3WakeFromSmm))

%define SMM_RESUME_SIGNATURE 0x55AABB66 ;When SMM rsm here on resume take alternate path
%define CAPSULE_SIGNATURE    0xCA9501EF
%define FLASH_REGION_START   0x1000000
%define BSP_STACK_BASE       0x0100000
...
SMMResumeInfo: ;; This offset can bie found as 0xFFFFFFF5 + word [ 0xFFFFFFF3] - 12
      ...
      DD ASM_PFX(S3ResumeEntryPoint)    ; Offset of our 32 bit code
...
;[-start-220126-IB14740206-modify];
        mov     ecx, [ASM_PFX(PcdGet32 (PcdPciExpressSizeMsrValue))]
        or      eax, ecx
...
SetPcieMmioMsr:
        ...
        xor     edx, edx
        mov     edx, [ASM_PFX(PcdGet32 (PcdFlashAreaBaseAddress))]
        or      edx, 0FF000000h ; cache 16M maximum
        ;AMD_ENABLE_UEFI_STACK2 STACK_AT_TOP, DWORD [ASM_PFX(PcdGet32 (PcdBspStackSize))], BSP_STACK_BASE
        AMD_ENABLE_UEFI_STACK2 STACK_AT_TOP, _PCD_VALUE_PcdBspStackSize, BSP_STACK_BASE

%define CAR_TEST_KEY       0x12345678
        xor     eax, eax
        push    CAR_TEST_KEY
        pop     eax
...
;global ASM_PFX(S3ResumeEntryPoint)
ASM_PFX(S3ResumeEntryPoint):
...
;[-end-220221-IB14740216-add];
  ...
  ; S3 resume path
  ;
  cmp     BYTE [ds:ASM_PFX(PcdGetBool (PcdAmdPspS3WakeFromSmm))], 1h
  jne     NonSmmS3Resume
...
NonSmmS3Resume:
  ...
  mov     esp, DWORD [esi + PSP_SMM_HDR_DATA.ApStackTop] ;Default AP. Will re-init for BSP.
  mov     ecx, 1Bh
  rdmsr
  bt      eax, 8
  jnc     .0

  mov     esp, DWORD [esi + PSP_SMM_HDR_DATA.PspSmmRsmMemInfo + PSP_SMM_RSM_MEM_INFO.StackPtr]
  add     esp, [esi + PSP_SMM_HDR_DATA.PspSmmRsmMemInfo + PSP_SMM_RSM_MEM_INFO.BspStackSize]

.0:
  ; Do CCX Init for non S3.
  push    esi  ; Pass in PSP_SMM_HDR_DATA to function.
  call    ASM_PFX(NonSmmS3ResumeInit)
  ...
  bt      eax, 8
  pop     eax
  jc      ASM_PFX(S3EntryCommon) ; If yes, jump to Common
  lock inc DWORD [esi + PSP_SMM_HDR_DATA.ApSyncFlag]
  jmp  ASM_PFX(S3EntryCommon)

global ASM_PFX(S3EntryCommon)
ASM_PFX(S3EntryCommon):
...
S3_resume:
  ...
  mov edi, [ASM_PFX(PcdGet32 (PcdH2OBootFirmwareVolumeBase))]
  push edi
  ;
...
ASM_PFX(TopOfCar):
                    DD      BSP_STACK_BASE + _PCD_VALUE_PcdBspStackSize -4
