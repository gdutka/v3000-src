  ...
  )
{
  ...
  String = HiiGetString (gPlatformBdsLibStrPackHandle, Id, NULL);
  if (String == NULL) {
    return NULL;
  }

  AsciiString = AllocateZeroPool(StrnLenS (String, MAX_STRING_LENGTH) + 1);
  if (AsciiString != NULL) {
    UnicodeStrToAsciiStrS (String, AsciiString, sizeof (AsciiString));
  }
