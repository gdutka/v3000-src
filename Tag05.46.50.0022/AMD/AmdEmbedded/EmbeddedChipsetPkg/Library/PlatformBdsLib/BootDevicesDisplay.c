...
/**
  ...
  @param  SetupNVRam         Pointer to SYSTEM_CONFIGURATION variable
...
ShowSataDevice (
  ...
  IN      UINT16                 HddCount,
  IN      KERNEL_CONFIGURATION   *SetupNVRam,
  ...
  )
{
  ...
  UINT8                                 Device;
  CHAR16                                *SataPortStr;
  CHAR16                                ModelNametring[MAX_NAME_LENGTH];
  CHAR16                                PortNoStr[10];
  UINTN                                 PortNum;
  UINTN                                 DeviceCnt;
  UINTN                                 BbsTableIndex;
  ...
  for (PortNum = 0; PortNum < 8; PortNum++) {
    ...
    //
    if ((SetupNVRam->SataCnfigure == 0) ||
       ((SetupNVRam->SataCnfigure == 4) || (SetupNVRam->SataCnfigure == 6))) {
      //
    ...
    if (HddCount > (Channel * 2 + Device)) {
      ...
      if (StrnLenS (ModelNametring, MAX_NAME_LENGTH) == 0) {
        continue;
      }
      UnicodeValueToStringS (PortNoStr, sizeof (PortNoStr), 0, PortNum, 0);
  ...
  )
{
  ...
  for (BbsTableIndex = 0; BbsTableIndex < BbsTotalCount; BbsTableIndex++) {
    ...
    if (IsUsbDevice (LocalBbsTable, BbsTableIndex)) {
      ...
      UnicodeValueToStringS (DeviceNoStr, sizeof (DeviceNoStr), 0, UsbDeviceNum, 0);
  ...
  for (BbsTableIndex = 0; BbsTableIndex < BbsTotalCount; BbsTableIndex++) {
    ...
    if (!IsUsbDevice (LocalBbsTable, BbsTableIndex)) {
      ...
      UnicodeValueToStringS (DeviceNoStr, sizeof (DeviceNoStr), 0, OtherDeviceNum, 0);
  ...
  )
{
  ...
  //
  // Use GetKernelConfiguation to get SetupNvData
  //
  ...
  // Show SATA devices
  //
  ShowSataDevice (LocalBbsTable, LocalHddInfo, HddCount, &KernelSetup, &ShowedDeviceNum);
