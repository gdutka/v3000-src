...
/*
    ...
    * OEM/IBV should check "_customize" keyword for TO-DO *
    ...
    - OEM has to create two EC IO address by "IOBS" & "IOB2" for the DYNAMIC BOOST 2.0 access
      Example One is for High core, another one is for Noraml core.
    - for Dynamic Boost, OEM has to update the objects which might be returned by _CPC method in ASL,
      including but not limited to CPC2/CPC3 and CPOC.
      Needs to separate the _CPC table for each core when support two ECIO. Please consult NVIDIA SW AE for more detail
...
Scope (\_SB)                            // DO NOT MODIFY THE PATH
{
    Device (NPCF)
    {
        ...
        Name(ACBT, 0x0050)              // AC:Baseline TGP (1/8-watt units)
                                        // +10W => 80
      If (LEqual (DGPU_SCOPE.NCTP, Zero))
      {
      ...
      Else                              
      {
        ...
        Name(IOBS, Zero)                // ** EC IO 1. Example port: 0x25F
        Name(IOB2, Zero)                // ** EC IO 2. Example port: 0x260

        Name(CMPL,0x33)                 // ** CPPC Maximum Performance Level
	                                //    High Core Maximum Performance, MSR value
        Name(NCMP, 0x1E)                // ** Normal Core Maximum Performance, MSR value
        ...
        // Get Minimum MaxPerfRatio
        // Arguments:
        //  Arg0: Driver Set the MaxPerfRatio 
        //  Arg1: Core MaxPerfRatio
        // Return: Minimum MaxPerfRatio           
                ...
                Case (NVPCF_FUNC_CPU_CONTROL)
                {
                    ...
                    Switch (ToInteger(INC6))
                    {
                        ...
                        Case(1)
                        {
                            ...
                            //
                            If(LAnd(LNotEqual(IOBS,0x0), LNotEqual(IOB2,0x0)))
                            {
                                OperationRegion(NVIO, SystemIO, IOBS, 0x1)    // IBV_customize/OEM_customize // Example port: 0x25F
                                Field(NVIO, ByteAcc, NoLock, Preserve)
                                {
                                    C0IO,  8
                                }

                                OperationRegion(NVI2, SystemIO, IOB2, 0x1)    // IBV_customize/OEM_customize // Example port: 0x25F
                                Field(NVI2, ByteAcc, NoLock, Preserve)
                                {
                                    C1IO,  8
                                }
                                // Params1 of CPU Control Entry, Input
                                //   [7:0] New CPPC Highest Performance value
                                Store(GMIN(NCHP, CMPL), C0IO) // Store HighestPerformance value to High core Embedded IO Space
                                Store(GMIN(NCHP, NCMP), C1IO) // Store HighestPerformance value to Normal core Embedded IO Space
                                Sleep (0x1)  // Wait ECIO stably
