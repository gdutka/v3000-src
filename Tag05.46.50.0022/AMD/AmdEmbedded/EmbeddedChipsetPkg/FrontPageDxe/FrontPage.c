  ...
  )
{
  ...
  if (Action == EFI_BROWSER_ACTION_CHANGED) {
    ...
    case FRONT_PAGE_KEY_LANGUAGE:
      ...
      //
      Lang = AllocatePool (AsciiStrnSizeS (LanguageString, PcdGet32 (PcdMaximumAsciiStringLength)));
      ASSERT (Lang != NULL);
      if (Lang == NULL) {
      ...
      if (PlatformSupportedLanguages == NULL) {
        PlatformSupportedLanguages = AllocateCopyPool (
                                       AsciiStrnSizeS ((CHAR8 *) PcdGetPtr (PcdUefiVariableDefaultPlatformLangCodes), MAX_STRING),
      ...
      if (BestLanguage != NULL) {
        Status = gRT->SetVariable (
                        ...
                        AsciiStrnSizeS (BestLanguage, MAX_STRING),
                        Lang
                        );
  ...
  )
{
  ...
  // Allocate working buffer for RFC 4646 language in supported LanguageString.
  //
  Lang = AllocatePool (AsciiStrnSizeS (LanguageString, MAX_STRING));
  ASSERT (Lang != NULL);
  if (Lang == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }
  ...
  )
{
  ...
  ASSERT (StringBuffer != NULL);
  UnicodeValueToStringS (StringBuffer, sizeof (CHAR16) * DestMax, LEFT_JUSTIFY, MemorySize, 6);
  StrCatS (StringBuffer, DestMax, L" MB RAM");
