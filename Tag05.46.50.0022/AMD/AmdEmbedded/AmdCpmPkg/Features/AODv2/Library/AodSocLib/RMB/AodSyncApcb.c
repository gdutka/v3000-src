  ...
  )
{
  ...
  UINT32                                ApcbTokens[AOD_APCB_NUMNER] = {
    ...
    APCB_TOKEN_UID_MEM_CA_DRV_DDR,
    APCB_TOKEN_UID_MEM_DQ_DRV_DDR,
  ...
  if ((AodConfig->AodMemTimingSettingDdr != 0xff)) {
    ApcbSetToken8 (ApcbProtocol, APCB_TYPE_PURPOSE_ADMIN, APCB_TOKEN_UID_MEM_ACTIVE_MEM_TIMING_SETTING_DDR, \
      AodConfig->AodMemTimingSettingDdr);  //User Input
  }

  //Check Display Condition AodMemTimingSettingDdr=Enabled,AodExpo!=Enabled
  if ((AodConfig->AodMemTimingSettingDdr == 0x1) &&
      (AodConfig->AodMemTargetSpeedDdr != 3200) &&
      (AodConfig->AodExpo != 1))  {
  ...
  //Check if select Auto
  if (AodConfig->AodMemDrvStrenCaDdr != 0xFF) {
    ApcbSetToken8 (ApcbProtocol, APCB_TYPE_PURPOSE_ADMIN, APCB_TOKEN_UID_MEM_CA_DRV_DDR, \
      AodConfig->AodMemDrvStrenCaDdr);
  }

  //Check if select Auto
  if (AodConfig->AodMemDrvStrenDqDdr != 0xFF) {
    ApcbSetToken8 (ApcbProtocol, APCB_TYPE_PURPOSE_ADMIN, APCB_TOKEN_UID_MEM_DQ_DRV_DDR, \
  ...
  //Check if select Auto
  if (AodConfig->AodMemDramOdtImpedRttNomRdDdr != 0xff) {
  ...
  //Check if select Auto
  if (AodConfig->AodGnbFclkOcFlag != 0xFF) {
    ApcbSetTokenBool (ApcbProtocol, APCB_TYPE_PURPOSE_ADMIN, APCB_TOKEN_UID_GNB_FCLK_OC_FLAG, \
      AodConfig->AodGnbFclkOcFlag);  //User Input
  }
  ...
  if (AodConfig->AodFclkFrequency  != 0xFFFF) {
    ApcbSetToken8 (ApcbProtocol, APCB_TYPE_PURPOSE_ADMIN, APCB_TOKEN_UID_CBS_CMN_GNB_SMU_FCLK_FREQUENCY, \
      (UINT8)GetRmbFclkFreqId (AodConfig->AodFclkFrequency));
  }
  ...
  if (((AodConfig->AodVddIoCtrl == 0x1)) ||((AodConfig->AodVddIoCtrl == 0x2))) {
    ApcbSetToken16 (ApcbProtocol, APCB_TYPE_PURPOSE_ADMIN, APCB_TOKEN_UID_MEM_OC_VDDQ , AodConfig->AodDimmVddq);  //User Input
    ApcbSetToken16 (ApcbProtocol, APCB_TYPE_PURPOSE_ADMIN, APCB_TOKEN_UID_MEM_OC_VDDIO, AodConfig->AodDimmVdd);  //User Input
  ...
  //Check if select Auto
  if (AodConfig->AodExpo != 0xff) {
    ApcbSetToken8 (ApcbProtocol, APCB_TYPE_PURPOSE_ADMIN, APCB_TOKEN_UID_MEM_RMP_ENABLE_DDR, AodConfig->AodExpo);
  }
  //Check if select Enable
  if (AodConfig->AodExpo == 1) {
    ApcbSetToken8 (ApcbProtocol, APCB_TYPE_PURPOSE_ADMIN, APCB_TOKEN_UID_MEM_RMP_PROFILE_INDEX_DDR, \
      AodConfig->AodExpoProfile);
  }
  if (AodConfig->AodVddMemCtrl == 0x1) {
    ApcbSetTokenBool (ApcbProtocol, APCB_TYPE_PURPOSE_ADMIN, APCB_TOKEN_UID_MEM_OC_VDD_CONTROL, 1);
  }
  AodBitMapDownCoreSyncApcb (AodConfig, ApcbProtocol);
