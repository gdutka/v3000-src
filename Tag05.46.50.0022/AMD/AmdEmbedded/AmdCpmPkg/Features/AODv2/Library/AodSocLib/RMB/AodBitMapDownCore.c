  ...
  )
{
  ...
  CurrentTpl = AodSetTpl (TPL_APPLICATION);
  do {
    do {
  ...
  AodSetTpl (CurrentTpl);     // Restore to original TPL.
  ...
  )
{
  ...
  //
  Status = AodReadCoreInfo (InputInfo, CoreMax);
  //
...
EFI_STATUS
EFIAPI
AodUpdateCoreManageText (
  IN  EFI_HII_HANDLE            HiiHandle,
  IN  AOD_CORE_INFO             *pInfo
  )
{
  EFI_STATUS              Status;
  UINTN                   Index, CoreIndex, LogicalCcdIndex, LogicalCoreIndex;
  ...
  for (Index = 0,LogicalCcdIndex = 0; Index < pInfo->AodPhysicalCcdCount; Index ++) {
    if (pInfo->AodCcdBitMap & (UINT32)(1 << Index)) {       // These ccd is not fused down
      ...
      for (CoreIndex = 0, LogicalCoreIndex = 0; CoreIndex < pInfo->AodPhysicalCoreCountPerCcd; CoreIndex ++) {
        ZeroMem (pTemp, 0x80);
        ...
        switch (pInfo->AodCoreStatus[Index][CoreIndex]) {
          case AOD_CORE_ACTIVE:
            ...
            if (EFI_ERROR (Status)) return Status;
            LogicalCoreIndex ++;
            if (pInfo->AodLogicalCoreCountPerCcd > LogicalCoreIndex) {
...
/**
  ...
  @param[in]  pInfo                - the CoreInfo Hob
  ...
  )
{
  ...
  AodConfig->AodCpuDownCoreMode = 0;
  for (CcdIndex = 0; CcdIndex < AodConfig->AodMaxCcdCount; CcdIndex ++) {
    ...
    for (CoreIndex = 0; CoreIndex < AodConfig->AodMaxCorePerCcd; CoreIndex ++) {
      ...
      if (pInfo->AodCoreStatus[CcdIndex][CoreIndex] == AOD_CORE_SOFT_OFF) {
        AodConfig->AodCpuDownCoreMode = 1;
        AodConfig->AodCoreBitMap[CcdIndex] |= (1 << LogicalCoreIndex);
        AOD_DEBUG_SOC ((EFI_D_ERROR, "Update AodCoreBitMap to %x\n", AodConfig->AodCoreBitMap[CcdIndex]));
      }
      LogicalCoreIndex ++;
    }
  }
  ...
  )
{
  ...
  //
  Status = AodSyncCoreInfoIntoVariable (&Info);
  if (EFI_ERROR (Status)) return Status;
  ...
  //
  AodUpdateCoreManageText (HiiHandle, &Info);
  return Status;
}
/**
...
EFI_STATUS
EFIAPI
AodDownCoreLoadDefaultCallback (
  ...
  )
{
  ...
  AodUpdateCoreManageText (Handle, pInfo);
  Status = AodSyncCoreInfoIntoVariable (pInfo);
  if (EFI_ERROR (Status)) return Status;
  return EFI_SUCCESS;
}
/**
  ...
  )
{
  ...
  for (Index = 0; Index < pCurrentInfo->AodPhysicalCoreCountPerCcd; Index ++) {
    ...
    switch (pCurrentInfo->AodCoreStatus[CcdIndex][Index]) {
      case AOD_CORE_ACTIVE:
        ...
        if (EFI_ERROR (Status)) goto Exit;
        CoreMax ++;
        if (CoreMax < pCurrentInfo->AodLogicalCoreCountPerCcd) {
  ...
  Status = AodDrawPopUp (StringBuffer1, StringBuffer2, pCoreString, CoreMax);
  if (!EFI_ERROR (Status)) {
    Index = 0;
    CoreMax = 0;
    ...
    AodUpdateCoreManageText (Handle, pCurrentInfo);
  }
  AOD_DEBUG_SOC ((EFI_D_INFO, "End\n"));
Exit:
