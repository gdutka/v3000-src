...
extern UINT8 AodSocLibRplStrings[];
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    Status = NbioSmuServices->AmdSmuServiceRequest (NbioSmuServices, 0, SMC_MSG_GetOverclockCap_RPL, SmuArg, SmuArg);
  ...
  )
{
  ...
  ZeroMem (AodConfig->AodCcdOptimizerFreq, AOD_MAX_CCD_COUNT * sizeof (UINT16));
  ZeroMem (AodConfig->AodPerCcdVddgVoltage, AOD_MAX_CCD_COUNT * sizeof (UINT16));
  ZeroMem (AodConfig->AodPerCcdVddgIODVoltage, AOD_MAX_CCD_COUNT * sizeof (UINT16));
  ...
  AodConfig->AodMemDataBusCfgCtlDdr = 0xff;                          // Data Bus Configuration User Controls
  AodConfig->AodMemDataBusCfgRttNomWrDdr = 0xFF;                     // RttNom_Wr
  AodConfig->AodMemDataBusCfgRttNomRdDdr = 0xff;                     // RttNom_Rd
  AodConfig->AodMemDataBusCfgRttWrDdr = 0xFF;                        // RttWr
  AodConfig->AodMemDataBusCfgRttParkDdr = 0xff;                      // RttPark
  AodConfig->AodMemDataBusCfgRttParkDqsDdr = 0xff;                   // RttPark_Dqs
  AodConfig->AodMemDataBusCfgProcOdtDdr = 0xFF;                      // ProcODT
  AodConfig->AodMemDataBusCfgProcDataDrvStrenDdr = 0xFF;             // Proc Data Drive Strength
  AodConfig->AodMemDataBusCfgDramDataDrvStrenDdr = 0xFF;             // DRAM Data Drive Strength
  ...
  AodConfig->AodCcdOptimizerFreqCtrl = 0;                            // CCD Frequency Optimizer
  AodConfig->AodVddgCtrl = 0;                                        // VDDG Voltage Control
  AodConfig->AodGlobalVddgCcdVoltage = 0;                            // Global VDDG CCD Voltage
  AodConfig->AodGlobalVddgIodVoltage = 0;                            // Global VDDG IOD Voltage
  AodConfig->AodGnbFclkOcFlag = 0;                                   // SoC/Uncore OC Mode
  AodConfig->AodSocOverclockingVoltage = 0;                          // SoC Voltage
  AodConfig->AodLN2Mode = 0xff;                                      // LN2 Mode
  AodConfig->AodEcoMode = 0;                                         // ECO Mode
  AodConfig->AodVddIoCtrl = 0;                                       // VDDIO Ctrl
  AodConfig->AodDimmVddIo = 1100;                                    // DIMM VDDIO Adjust
...
{
#if 0 //[Tank TBD] since AGESA not ready, just return 0
  UINT16  ElementSizeInByte;
  UINT64  ElementValue = 0;
  if (EFI_ERROR (ApobGetMemGenInfoElement(0, ElementId, &ElementSizeInByte, &ElementValue))) {
    return 0;
  }
  ...
  )
{
  ...
  for (ChannelIndex = 0; ChannelIndex <= 7; ChannelIndex ++) {
    AodSmnRead (DieBusNum, CH_SMN_BASE_CH0_MP0 + (ChannelIndex * 0x100000) + DRAM_CONFIGURATION, &Value);
  ...
  switch (CmdId) {
    case GET_INTERLEVAVINGCAP:
      return GetMemGenInfoRpl (InterLeaveCapability);
    case SET_INTERLEAVEMODE:
      return GetMemGenInfoRpl (InterLeaveCurrentMode);
    case SET_INTERLEAVESIZE:
      return GetMemGenInfoRpl (InterLeaveSize);
    ...
    case SET_TRFC1:
      AodSmnRead (DieBusNum, ChSmnChannelBase + DRAM_TRFC_TIMING_DIMM0 + 4*DimmPresent, &Value);
      return (UINT16)(Value & 0xffF);
    case SET_TRFC2:
      AodSmnRead (DieBusNum, ChSmnChannelBase + DRAM_TRFC_TIMING_DIMM0 + 4*DimmPresent, &Value);
      return (UINT16)((Value >> 16)& 0xFFF);
    case SET_TRFCSB:
      AodSmnRead (DieBusNum, ChSmnChannelBase + DRAM_REF_TIMING + 4*DimmPresent, &Value);
    ...
    case SET_CADBUSCLKDRVSTREN:
      return GetMemGenInfoRpl (MemCtrllerCadBusClkDrvStrenDddr4Current);
    case SET_CADBUSADDRCMDDRVSTREN:
      return GetMemGenInfoRpl (MemCtrllerCadBusAddrCmdDrvStrenDddr4Current);
    case SET_CADBUSCSODTCMDDRVSTREN:
      return GetMemGenInfoRpl (MemCtrllerCsOdtCmdDrvStrenDddr4Current);
    case SET_RTTNOMWR:
      return GetMemGenInfoRpl (MemBusCfgRttNomWr);
    case SET_RTTNOMRD:
      return GetMemGenInfoRpl (MemBusCfgRttNomRd);
    case SET_RTTWR:
      return GetMemGenInfoRpl (MemBusCfgRttWr);
    case SET_RTTPARK:
      return GetMemGenInfoRpl (MemBusCfgRttPark);
    case SET_RTTPARKDQS:
      return GetMemGenInfoRpl (MemBusCfgDqsRttPark);
    case SET_PROCODT:
      return GetMemGenInfoRpl (MemBusCfgPOdt);
    case SET_PROCDATADRIVESTRENGTH:
      return GetMemGenInfoRpl (MemBusCfgDqDrv);
    case SET_DRAMDATADRIVESTRENGTH:
      return GetMemGenInfoRpl (MemBusCfgDramDrv);
    case SET_APU_VDDIO:
    ...
    case SET_VDDIO:
      ...
      if (!EFI_ERROR (Status)) return DimmCurrent;
      return 0;
    default:
      break;
  }
  ...
  )
{
  ...
  CpuFreqBase = AodRplGetCpuBoostMaxFreqBase ();
  ...
  for (Index = 0; Index < ARRAY_SIZE (mWmiCmidList); Index ++) {
    ...
    if (!EFI_ERROR (Status) && (mWmiCmidList[Index].DataCount != 0)) {
      ...
      Status = pThis->UpdateCurrent (
                       pThis,
                       mWmiCmidList[Index].CmdId,
                       AodGetWmiCmdCurrentRpl (mWmiCmidList[Index].CmdId)
                       );
      AOD_DEBUG_SOC ((EFI_D_ERROR, "UpdateCurrent %a(%08X) Current = %08X, Status =%r\n", mWmiCmidList[Index].pName,\
        mWmiCmidList[Index].CmdId, AodGetWmiCmdCurrentRpl (mWmiCmidList[Index].CmdId), Status));
    }
  }
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  //
  AodGenerateCoreManageOptionsRpl (HiiHandle);
  AodGenerateApuVddIoOptionsRpl (HiiHandle);
  AodGeneratePerCoreCurveOptionsRpl (HiiHandle);
  AodGeneratePerCcdVddGVoltageOptionsRpl (HiiHandle);
  AodGenerateCcdFreqOptimizerOptionsRpl (HiiHandle);
  return EFI_SUCCESS;
}
/**
...
EFI_STATUS
EFIAPI
AodUpdateHiiCallbackRpl (
  VOID        *Context
  )
{
  EFI_STATUS         Status;
  Status = AodAddSetupCallback (Context, FALSE, AodDownCoreSaveCallbackRpl);
  AOD_DEBUG_SOC ((EFI_D_INFO, "  Add AodDownCoreSaveCallbackRpl Status %r\n",  Status));
  Status = AodAddSetupCallback (Context, FALSE, AodDownCoreBitControlCallbackRpl);
  AOD_DEBUG_SOC ((EFI_D_INFO, "  Add AodDownCoreBitControlCallbackRpl Status %r\n",  Status));
  Status = AodAddSetupCallback (Context, FALSE, AodDownCoreDiscardCallbackRpl);
  AOD_DEBUG_SOC ((EFI_D_INFO, "  Add AodDownCoreDiscardCallbackRpl Status %r\n",  Status));
  Status = AodAddSetupCallback (Context, FALSE, AodDownCoreLoadDefaultCallbackRpl);
  AOD_DEBUG_SOC ((EFI_D_INFO, "  Add AodDownCoreLoadDefaultCallbackRpl Status %r\n",  Status));
  Status = AodAddSetupCallback (Context, TRUE, AodUpdateStringCallBackRpl);
  AOD_DEBUG_SOC ((EFI_D_INFO, "  Add AodUpdateStringCallBackRpl Status %r\n",  Status));
  Status = AodAddSetupCallback (Context, FALSE, AodDimmVddIoCallBackRpl);
  AOD_DEBUG_SOC ((EFI_D_INFO, "  Add AodDimmVddIoCallBackRpl Status %r\n",  Status));
  Status = AodAddSetupCallback (Context, FALSE, AodApuVddIoCallBackRpl);
  AOD_DEBUG_SOC ((EFI_D_INFO, "  Add AodApuVddIoCallBackRpl Status %r\n",  Status));
  Status = AodAddSetupCallback (Context, FALSE, AodVddIoCtrlCallBackRpl);
  AOD_DEBUG_SOC ((EFI_D_INFO, "  Add AodVddIoCtrlCallBackRpl Status %r\n",  Status));
  Status = AodAddSetupCallback (Context, FALSE, AodLCLKCallBackRpl);
  AOD_DEBUG_SOC ((EFI_D_INFO, "  Add AodLCLKCallBackRpl Status %r\n",  Status));
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  AOD_DEBUG_SOC ((EFI_D_INFO, "Data Bus Configuration User Controls     = 0x%x \n", \
    AodConfig->AodMemDataBusCfgCtlDdr));
  AOD_DEBUG_SOC ((EFI_D_INFO, "RttNom_Wr                                = 0x%x \n", \
    AodConfig->AodMemDataBusCfgRttNomWrDdr));
  AOD_DEBUG_SOC ((EFI_D_INFO, "RttNom_Rd                                = 0x%x \n", \
    AodConfig->AodMemDataBusCfgRttNomRdDdr));
  AOD_DEBUG_SOC ((EFI_D_INFO, "RttWr                                    = 0x%x \n", \
    AodConfig->AodMemDataBusCfgRttWrDdr));
  AOD_DEBUG_SOC ((EFI_D_INFO, "RttPark                                  = 0x%x \n", \
    AodConfig->AodMemDataBusCfgRttParkDdr));
  AOD_DEBUG_SOC ((EFI_D_INFO, "RttPark_Dqs                              = 0x%x \n", \
    AodConfig->AodMemDataBusCfgRttParkDqsDdr));
  AOD_DEBUG_SOC ((EFI_D_INFO, "ProcODT                                  = 0x%x \n", \
    AodConfig->AodMemDataBusCfgProcOdtDdr));
  AOD_DEBUG_SOC ((EFI_D_INFO, "Proc Data Drive Strength                 = 0x%x \n", \
    AodConfig->AodMemDataBusCfgProcDataDrvStrenDdr));
  AOD_DEBUG_SOC ((EFI_D_INFO, "DRAM Data Drive Strength                 = 0x%x \n", \
  ...
  AOD_DEBUG_SOC ((EFI_D_INFO, "CCD Frequency Optimizer                  = 0x%x \n", \
    AodConfig->AodCcdOptimizerFreqCtrl));
  for (Index = 0; Index < AOD_MAX_CCD_COUNT; Index ++) {
    AOD_DEBUG_SOC ((EFI_D_INFO, "CCD%d Frequency Adjustment               = 0x%x \n", \
      Index, AodConfig->AodCcdOptimizerFreq[Index]));
  }
  ...
  AOD_DEBUG_SOC ((EFI_D_INFO, "ECO Mode                                 = 0x%x \n", \
    AodConfig->AodEcoMode));
  AOD_DEBUG_SOC ((EFI_D_INFO, "VDDIO Ctrl                               = 0x%x \n", \
    AodConfig->AodVddIoCtrl));
  AOD_DEBUG_SOC ((EFI_D_INFO, "DIMM VDDIO Adjust                        = 0x%x \n", \
    AodConfig->AodDimmVddIo));
  ...
  AOD_DEBUG_SOC ((EFI_D_INFO, "VDDG Voltage Control                     = 0x%x \n", \
    AodConfig->AodVddgCtrl));
  AOD_DEBUG_SOC ((EFI_D_INFO, "Global VDDG CCD Voltage                  = 0x%x \n", \
    AodConfig->AodGlobalVddgCcdVoltage));
  AOD_DEBUG_SOC ((EFI_D_INFO, "Global VDDG IOD Voltage                  = 0x%x \n", \
    AodConfig->AodGlobalVddgIodVoltage));
  for (Index = 0; Index < AOD_MAX_CCD_COUNT; Index ++) {
    AOD_DEBUG_SOC ((EFI_D_INFO, "CCD%d-CCD VDDG Voltage                 = 0x%x \n", \
      Index, AodConfig->AodPerCcdVddgVoltage[Index]));
    AOD_DEBUG_SOC ((EFI_D_INFO, "CCD%d-IOD VDDG Voltage                 = 0x%x \n", \
      Index, AodConfig->AodPerCcdVddgIODVoltage[Index]));
  }
...
///
/// Aod Rpl Dxe init table
///
AOD_INIT_TABLE mRplAodDxeInit [] = {
  { 0, AOD_CONFIG_LOAD_DEFAULT,   AodLoadDefalutValueRpl},
  { 0, AOD_UPDATE_VARIABLE,       AodUpdateVariableHeaderRpl},
  { 0, AOD_UPDATE_NVS,            AodUpdateNvsRpl},
  { 0, AOD_CONFIG_DEBUG,          AodConfigDebugRpl},
  { 0, AOD_BUILD_WMI_CMD,         AodBuildWmiCmdListRpl},
  { 0, AOD_UPDATE_HIIRESOURCE,    AodUpdateHiiPackageRpl},
  { 0, AOD_UPDATE_HII_CALLBACK,   AodUpdateHiiCallbackRpl},
  { 0, 0, NULL}
};
/**
  ...
  )
{
  ...
  if (SocHardwareIdentificationCheck (&SocId)) {
    mAodComboFlag = AOD_RPL_AM5_COMBO_FLAG;
    AOD_DEBUG_SOC ((DEBUG_INFO, "AodDispatchFlag = 0x%x\n", mAodComboFlag));
    return AodAddInitTable (mRplAodDxeInit);
  }
