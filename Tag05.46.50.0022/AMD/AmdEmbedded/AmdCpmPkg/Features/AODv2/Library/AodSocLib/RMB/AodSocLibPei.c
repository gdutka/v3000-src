...
AOD_SOC_NAME ("RMB")
/**
  This function set CoreCurveOptimizer AGESA Pcd

  @param[in]  pAodCoreInfo               Points to the Aod Core info hob
...
/**
  This function used to update Aod Core info
  ...
  )
{
  ...
  AodCoreCurveOptimizer (pAodCoreInfo);
  return EFI_SUCCESS;
}
/**
  ...
  )
{
  ...
  if (AodConfig->AodCpuSmtCtrl != 1) {
    PcdSet8S (PcdAmdSmtMode, AodConfig->AodCpuSmtCtrl);
  }
  ...
  //Check Display Condition AodNbioPBOCtl=Advanced
  if ((AodConfig->AodNbioPBOCtl == 0x2)) {
    PcdSet8S (PcdGpuBoostClockOverride, AodConfig->AodGpuBoostClock);
  }
    //Check Display Condition AodNbioPBOCtl=Advanced,AodGpuBoostClock=Enabled
  if ((AodConfig->AodGpuBoostClock == 1) && (AodConfig->AodNbioPBOCtl == 2)) {
    PcdSet32S (PcdGFXFMaxFrequency, AodConfig->AodGfxFMaxFreq);
  }
  ...
  // Display Condition: AodLclkCtrl=Manual
  ...
  // Display Condition: AodVddMiscCtrl=Manual
...
///
/// Aod Rmb Pei init table
///
AOD_INIT_TABLE mRmbAodPeiInit [] = {
  { 0, AOD_SYNC_VARIABLE_PCD, AodSyncVariableToPcds},
  { 0, AOD_UPDATE_CORE_INFO,  AodUpdateCoreInfo},
  { 0, 0, NULL}
};
/**
  The constructor function for RMB SOC Aod Init .
  ...
  )
{
  ...
  if (SocFamilyIdentificationCheck (F19_RMB_RAW_ID)) {
    return AodAddInitTable (mRmbAodPeiInit);
  }
