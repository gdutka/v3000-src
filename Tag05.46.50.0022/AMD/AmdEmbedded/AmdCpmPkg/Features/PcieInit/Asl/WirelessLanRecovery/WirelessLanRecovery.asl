  ...
  )
{
  ...
  External (\_SB.PCAA.GPBB.DECC, DeviceObj)
  External (\_SB.PCXX.GPXX.XHCX.HUBX.PTXX, DeviceObj)
  External (\_SB.PCXX.GPXX.XHCX.HUBX.GHBX.PTXX, DeviceObj)

  If (CondRefOf (\_SB.PCAA.GPBB.DECC)) {
    Scope (\_SB.PCAA.GPBB.DECC) {
      Name (XPRR, Package () {\_SB.PRWL})

      Method (XRMV, 0, NotSerialized) {
        Return (0)
      }
    } // End of Scope (\_SB.PCAA.GPBB.DECC)
  }

  If (CondRefOf (\_SB.PCXX.GPXX.XHCX.HUBX.PTXX)) {
    Scope (\_SB.PCXX.GPXX.XHCX.HUBX.PTXX) {
      Name (ZPRR, Package () {\_SB.PRWB})
    } // End of Scope (\_SB.PCXX.GPXX.XHCX.HUBX.PTXX)
  }

  // BT under external USB hub.
  If (CondRefOf (\_SB.PCXX.GPXX.XHCX.HUBX.GHBX.PTXX)) {
    Scope (\_SB.PCXX.GPXX.XHCX.HUBX.GHBX.PTXX) {
      Name (YPRR, Package () {\_SB.PRWB})
    } // End of Scope (\_SB.PCXX.GPXX.XHCX.HUBX.GHBX.PTXX)
  }
