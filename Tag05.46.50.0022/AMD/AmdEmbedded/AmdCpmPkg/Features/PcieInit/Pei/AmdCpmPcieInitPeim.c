  ...
  )
{
  ...
  AMD_CPM_DXIO_TOPOLOGY_TABLE         *BixbyDxioTopologyTablePtr;
  ...
  BixbyDxioTopologyTablePtr       = CpmTablePpiPtr->CommonFunction.GetTablePtr2 (CpmTablePpiPtr, CPM_SIGNATURE_BIXBY_DXIO_TOPOLOGY);
  ...
  if (PcieTopologyTablePtr && PcieTopologyOverrideTablePtr) {
    for (Index = 0; Index < AMD_PCIE_TOPOLOGY_OVERRIDE_SIZE; Index ++) {
      if (PcieTopologyOverrideTablePtr->Item[Index].Flag.Raw == CPM_END_OF_TABLE) {
        break;
      }
    ...
    for ( Index = 0; Index < AMD_PCIE_PORT_DESCRIPTOR_SIZE; Index ++ ) {
      if ((PcieTopologyTablePtr->Port[Index].EngineData.EngineType == PciePortEngine) && (PcieTopologyTablePtr->Port[Index].Port.PortPresent)) {
        if (!(PCIeStatus & (1 << Index))) {
          PcieTopologyTablePtr->Port[Index].Port.PortPresent = AmdPortDisabled;
        }
      }
  ...
  if (WirelessButtonTablePtr && PcieTopologyTablePtr && !IsHidBasedWirelessBotton) {
  ...
  if (WirelessButtonTablePtr && DxioTopologyTablePtr && !IsHidBasedWirelessBotton) {
    if (CpmTablePpiPtr->MainTablePtr->WirelessButtonEn == 2) {
      for ( Index = 0; Index < AMD_DXIO_PORT_DESCRIPTOR_SIZE; Index ++ ) {
        if ((DxioTopologyTablePtr->Port[Index].EngineData.EngineType == DxioPcieEngine) && (DxioTopologyTablePtr->Port[Index].Port.PortPresent)) {
          for (i = 0; i < 4; i++) {
      ...
      if ((DxioTopologyTablePtr->Port[Index].EngineData.EngineType == DxioPcieEngine) && (DxioTopologyTablePtr->Port[Index].Port.PortPresent)) {
        for (i = 0; i < AMD_PCIE_CLOCK_SIZE; i++) {
          ...
          if ((CpuRevisionId == CPM_CPU_REVISION_ID_RV) || (CpuRevisionId == CPM_CPU_REVISION_ID_RV2) || \
              (CpuRevisionId == CPM_CPU_REVISION_ID_RN) || (CpuRevisionId == CPM_CPU_REVISION_ID_VN) || \
              (CpuRevisionId == CPM_CPU_REVISION_ID_RMB)) {
            MaxLane = MinLane + 16;
          } else {
            MaxLane = MinLane + 32;
          }
  ...
  if (DxioTopologyTablePtr) {
    if (BixbyDxioTopologyTablePtr) {
      ...
      DxioComplexDescriptorPtr[0].SocketId      = DxioSocket0;
      DxioComplexDescriptorPtr[0].PciePortList  = &DxioTopologyTablePtr->Port[0];
      DxioComplexDescriptorPtr[0].DdiLinkList   = &DxioTopologyTablePtr->Ddi[0];
      DxioComplexDescriptorPtr[1].Flags         = DESCRIPTOR_TERMINATE_LIST;
      DxioComplexDescriptorPtr[1].SocketId      = DxioBixbySocket;
      DxioComplexDescriptorPtr[1].PciePortList  = &BixbyDxioTopologyTablePtr->Port[0];
      DxioComplexDescriptorPtr[1].DdiLinkList   = &BixbyDxioTopologyTablePtr->Ddi[0];
    }
    else {
      Status = (**PeiServices).AllocatePool (
                                              (CPM_PEI_SERVICES**)PeiServices,
