...
EFI_GUID SOC_WIRELESS_LAN_RECOVERY_ACPI_DEVICE_GUID = {
  0xAAD0FDDE, 0x2E43, 0x4483, 0xBD, 0x8E, 0x2E, 0x39, 0x90, 0x59, 0xC6, 0xB2
};

EFI_GUID PT_WIRELESS_LAN_RECOVERY_ACPI_DEVICE_GUID = {
  0xFC6A2190, 0x4ACD, 0x4C1A, 0xA2, 0x1F, 0xBA, 0x6B, 0x62, 0x7F, 0x93, 0x25
};

EFI_GUID BXB_WIRELESS_LAN_RECOVERY_ACPI_DEVICE_GUID = {
  0xF201DD7F, 0x4AEC, 0x48A9, 0xAE, 0x24, 0x23, 0x0F, 0xD5, 0xBD, 0x0F, 0xA7
};
  ...
  )
{
  ...
  BufferPtr = (UINT32*)Context;
  switch ( *((UINT32*)AmlObjPtr)) {
    case (CPM_SIGNATURE_32 ('P', 'C', 'A', 'A')):     // PCAA
      if (*(BufferPtr + 0)) {
        *(UINT32*) AmlObjPtr = *(BufferPtr + 0);
      }
      break;

    case (CPM_SIGNATURE_32 ('G', 'P', 'B', 'B')):     // GPBB
      if (*(BufferPtr + 1)) {
        *(UINT32*) AmlObjPtr = *(BufferPtr + 1);
      }
      break;

    case (CPM_SIGNATURE_32 ('D', 'E', 'C', 'C')):     // DECC
      if (*(BufferPtr + 2)) {
        *(UINT32*) AmlObjPtr = *(BufferPtr + 2);
      }
      break;

    case (CPM_SIGNATURE_32 ('X', 'P', 'R', 'R')):     // XPRR
      if (*(BufferPtr + 3)) {
        *(UINT8*) AmlObjPtr = '_';                    // _PRR
      }
      break;

    case (CPM_SIGNATURE_32 ('X', 'R', 'M', 'V')):     // XRMV
      if (*(BufferPtr + 3)) {
        *(UINT8*) AmlObjPtr = '_';                    // _RMV
      }
      break;

    case (CPM_SIGNATURE_32 ('P', 'C', 'X', 'X')):     // PCXX
      if (*(BufferPtr + 4)) {
        *(UINT32*) AmlObjPtr = *(BufferPtr + 4);
      }
      break;

    case (CPM_SIGNATURE_32 ('G', 'P', 'X', 'X')):     // GPXX
      if (*(BufferPtr + 5)) {
        *(UINT32*) AmlObjPtr = *(BufferPtr + 5);
      }
      break;

    case (CPM_SIGNATURE_32 ('X', 'H', 'C', 'X')):     // XHCX
      if (*(BufferPtr + 6)) {
        *(UINT32*) AmlObjPtr = *(BufferPtr + 6);
      }
      break;

    case (CPM_SIGNATURE_32 ('H', 'U', 'B', 'X')):     // HUBX
      if (*(BufferPtr + 7)) {
        *(UINT32*) AmlObjPtr = *(BufferPtr + 7);
      }
      break;

    case (CPM_SIGNATURE_32 ('G', 'H', 'B', 'X')):     // GHBX
      if (*(BufferPtr + 8)) {
        *(UINT32*) AmlObjPtr = *(BufferPtr + 8);
      }
      break;

    case (CPM_SIGNATURE_32 ('P', 'T', 'X', 'X')):     // PTXX
      if (*(BufferPtr + 9)) {
        *(UINT32*) AmlObjPtr = *(BufferPtr + 9);
      }
      break;

    case (CPM_SIGNATURE_32 ('Z', 'P', 'R', 'R')):     // ZPRR
      if (*(BufferPtr + 10) && (*(BufferPtr + 8) == 0)) {
        *(UINT8*) AmlObjPtr = '_';                    // _PRR
      }
      break;

    case (CPM_SIGNATURE_32 ('Y', 'P', 'R', 'R')):     // YPRR
      if (*(BufferPtr + 10) && *(BufferPtr + 8)) {
        *(UINT8*) AmlObjPtr = '_';                    // _PRR
      }
      break;
  }
...
/*----------------------------------------------------------------------------------------*/
/**
 * Callback function to update Wireless LAN Recovery PT SSDT table
 *
 * This function is used to update PCIE Bridge Name
 *
 * @param[in]     This           Pointer to Protocol
...
BOOLEAN
EFIAPI
AmdCpmWirelessLanRecoveryPtSsdtCallBack (
  IN      VOID                    *This,
  IN      VOID                    *AmlObjPtr,
  IN      VOID                    *Context
  )
{
  ...
  BufferPtr = (UINT32*)Context;
  switch ( *((UINT32*)AmlObjPtr)) {
    case (CPM_SIGNATURE_32 ('P', 'C', 'D', 'D')):     // PCDD
      if (*(BufferPtr + 0)) {
        *(UINT32*) AmlObjPtr = *(BufferPtr + 0);
      }
      break;

    case (CPM_SIGNATURE_32 ('G', 'P', 'E', 'E')):     // GPEE
      if (*(BufferPtr + 1)) {
        *(UINT32*) AmlObjPtr = *(BufferPtr + 1);
      }
      break;

    case (CPM_SIGNATURE_32 ('U', 'S', 'F', 'F')):     // USFF
      if (*(BufferPtr + 2)) {
        *(UINT32*) AmlObjPtr = *(BufferPtr + 2);
      }
      break;

    case (CPM_SIGNATURE_32 ('D', 'S', 'G', 'G')):     // DSGG
      if (*(BufferPtr + 3)) {
        *(UINT32*) AmlObjPtr = *(BufferPtr + 3);
      }
      break;

    case (CPM_SIGNATURE_32 ('D', 'E', 'H', 'H')):     // DEHH
      if (*(BufferPtr + 4)) {
        *(UINT32*) AmlObjPtr = *(BufferPtr + 4);
      }
      break;
    ...
    case (CPM_SIGNATURE_32 ('Y', 'R', 'M', 'V')):     // YRMV
      if (*(BufferPtr + 5)) {
        *(UINT8*) AmlObjPtr = '_';                    // _RMV
      }
      break;
...
/*----------------------------------------------------------------------------------------*/
/**
 * Callback function to update Wireless LAN Recovery BXB SSDT table
 *
 * This function is used to update PCIE Bridge Name
 *
 * @param[in]     This           Pointer to Protocol
...
BOOLEAN
EFIAPI
AmdCpmWirelessLanRecoveryBxbSsdtCallBack (
  IN      VOID                    *This,
  IN      VOID                    *AmlObjPtr,
  IN      VOID                    *Context
  )
{
  ...
  switch ( *((UINT32*)AmlObjPtr)) {
    ...
    case (CPM_SIGNATURE_32 ('U', 'S', 'X', 'X')):     // USXX
      if (*(BufferPtr + 8)) {
        *(UINT32*) AmlObjPtr = *(BufferPtr + 8);
      }
      break;

    case (CPM_SIGNATURE_32 ('D', 'S', 'X', 'X')):     // DSXX
      if (*(BufferPtr + 9)) {
        *(UINT32*) AmlObjPtr = *(BufferPtr + 9);
      }
      break;
  ...
  )
{
  ...
  if (!InitlateInvoked) {
    ...
    if (WirelessButtonTablePtr && (CpmTableProtocolPtr->MainTablePtr->WirelessButtonEn != 3)) {
      ZeroMem (Buffer, sizeof (Buffer));
    ...
    if (WirelessLanRecoveryTablePtr) {
      ZeroMem (Buffer, sizeof (Buffer));
      switch (WirelessLanRecoveryTablePtr->WlanBtLocation) {

        case 0:
          if (WirelessLanRecoveryTablePtr->WlanPldrSupport) {
            Buffer[0] = WirelessLanRecoveryTablePtr->PciBusAslName;
            ...
            Buffer[2] = WirelessLanRecoveryTablePtr->WlanDeviceAslName;
            Buffer[3] = WirelessLanRecoveryTablePtr->WlanPldrSupport;
          }
          if (WirelessLanRecoveryTablePtr->BtPldrSupport) {
            Buffer[4] = WirelessLanRecoveryTablePtr->PciBusAslName;
            Buffer[5] = WirelessLanRecoveryTablePtr->BtXhciUspAslName;
            Buffer[6] = WirelessLanRecoveryTablePtr->BtXhciAslName;
            Buffer[7] = WirelessLanRecoveryTablePtr->BtXhciRootHubAslName;
            Buffer[8] = WirelessLanRecoveryTablePtr->BtXhciExtHubAslName;
            Buffer[9] = WirelessLanRecoveryTablePtr->BtPortAslName;
            Buffer[10] = WirelessLanRecoveryTablePtr->BtPldrSupport;
          }
          CpmTableProtocolPtr->CommonFunction.AddSsdtTable ( CpmTableProtocolPtr,
                                                             &SOC_WIRELESS_LAN_RECOVERY_ACPI_DEVICE_GUID,
                                                             NULL,
                                                             AmdCpmWirelessLanRecoverySsdtCallBack,
                                                             &Buffer[0]);
          break;
        ...
        case 1:
          if (WirelessLanRecoveryTablePtr->WlanPldrSupport) {
            ...
            Buffer[1] = CpmTableProtocolPtr->CommonFunction.GetPcieAslName ( CpmTableProtocolPtr,
                                                                             WirelessLanRecoveryTablePtr->Device,
                                                                             WirelessLanRecoveryTablePtr->Function );
            Buffer[2] = WirelessLanRecoveryTablePtr->PtBxbUspAslName;
            Buffer[3] = WirelessLanRecoveryTablePtr->PtBxbDspAslName;
            Buffer[4] = WirelessLanRecoveryTablePtr->WlanDeviceAslName;
            Buffer[5] = WirelessLanRecoveryTablePtr->WlanPldrSupport;
          }
          if (WirelessLanRecoveryTablePtr->BtPldrSupport) {
            Buffer[6] = WirelessLanRecoveryTablePtr->PciBusAslName;
            Buffer[7] = WirelessLanRecoveryTablePtr->BtXhciUspAslName;
            Buffer[8] = WirelessLanRecoveryTablePtr->BtXhciAslName;
            Buffer[9] = WirelessLanRecoveryTablePtr->BtXhciRootHubAslName;
            Buffer[10] = WirelessLanRecoveryTablePtr->BtPortAslName;
            Buffer[11] = WirelessLanRecoveryTablePtr->BtPldrSupport;
          }
          CpmTableProtocolPtr->CommonFunction.AddSsdtTable ( CpmTableProtocolPtr,
                                                             &PT_WIRELESS_LAN_RECOVERY_ACPI_DEVICE_GUID,
                                                             NULL,
                                                             AmdCpmWirelessLanRecoveryPtSsdtCallBack,
                                                             &Buffer[0]);
          break;
        ...
        case 2:
          ...
          if (WirelessLanRecoveryTablePtr->BtPldrSupport) {
            Buffer[6] = WirelessLanRecoveryTablePtr->PciBusAslName;
            Buffer[7] = CpmTableProtocolPtr->CommonFunction.GetPcieAslName ( CpmTableProtocolPtr,
                                                                             WirelessLanRecoveryTablePtr->Device,
                                                                             WirelessLanRecoveryTablePtr->Function );
            Buffer[8] = WirelessLanRecoveryTablePtr->PtBxbUspAslName;
            Buffer[9] = WirelessLanRecoveryTablePtr->BtXhciUspAslName;
            Buffer[10] = WirelessLanRecoveryTablePtr->BtXhciAslName;
            Buffer[11] = WirelessLanRecoveryTablePtr->BtXhciRootHubAslName;
            Buffer[12] = WirelessLanRecoveryTablePtr->BtPortAslName;
            Buffer[13] = WirelessLanRecoveryTablePtr->BtPldrSupport;
          }
          CpmTableProtocolPtr->CommonFunction.AddSsdtTable ( CpmTableProtocolPtr,
                                                             &BXB_WIRELESS_LAN_RECOVERY_ACPI_DEVICE_GUID,
                                                             NULL,
                                                             AmdCpmWirelessLanRecoveryBxbSsdtCallBack,
                                                             &Buffer[0]);
          break;
