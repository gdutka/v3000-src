    ...
    )
{
    ...
    Scope(\_SB.PCI0.PB2) {
        ...
        Name (CpmDgpuD3ColdPcdSetS0i3PmeTurnOffDelay, 0x10000)                  // S0i3 PmeTurnOff Delay Time: Copy from gEfiAmdAgesaPkgTokenSpaceGuid.PcdSetS0i3PmeTurnOffDelay
        Name (CpmDgpuD3ColdDgpuSsidSvid, 0)                                     // Discrete GPU's SSID/SVID: Program Discrete GPU's SSID/SVID depends on HybridGraphics setting
        Name (CpmDgpuDsmFunBData, 10001)
        Name (CpmNonZ10CapablePcieZstateSupport, 0)                             // Non Z10 Capable PCIE z-state support
        ...
        //        0x11 - AMD Baffin
        //        0x12 - AMD Iceland(Weston), Tonga
        //        0x13 - AMD Lexa
        //        0x20 - AMD Greenland
        //        0x21 - AMD Vega12
        //        0x22 - AMD Navi10
        //        0x23 - AMD Navi14
        //        0x24 - AMD Navi21 / Navi22 / Navi23
        //        0x25 - AMD Navi24
        //        0x26 - AMD Navi31 / Navi32 / Navi33
        //        0x00 - AMD Old GPU
        //        0xC0 - NVIDIA GPU
        Method (CpmCheckDgpuVidDid, 0) {
            ...
            If (LEqual (And (CpmDevicePMemPostDeviceId, 0xFFDF), 0x1002)) {
                ...
                If (LAnd (LGreaterEqual (Local1, 0x67C0), LLessEqual (Local1, 0x67DF))) {
                    ...
                    // 0x23 - AMD Navi14
                    Store (0x23, Local0)
                } ElseIf (LAnd (LGreaterEqual (Local1, 0x73A0), LLessEqual (Local1, 0x73FF))) {
                    // 0x24 - AMD Navi21 / Navi22 / Navi23
                    Store (0x24, Local0)
                } ElseIf (LAnd (LGreaterEqual (Local1, 0x7420), LLessEqual (Local1, 0x743F))) {
                    // 0x25 - AMD Navi24
                    Store (0x25, Local0)
                } ElseIf (LAnd (LGreaterEqual (Local1, 0x7440), LLessEqual (Local1, 0x749F))) {
                    // 0x26 - AMD Navi31 / Navi32 / Navi33
                    Store (0x26, Local0)
                } Else {
                    // 0x00 - AMD Old GPU
        ...
        Method (CpmDisableDgpuAudio, 0, Serialized) {
            ...
            If (LAnd (LGreaterEqual (Local0, 0x00), LLessEqual (Local0, 0x0F))) {
                ...
                // BAR5 - AMD Ellesmere, AMD Baffin, AMD Iceland(Weston), Tonga, AMD Lexa
            ...
            } ElseIf (LAnd (LGreaterEqual (Local0, 0x20), LLessEqual (Local0, 0x23))) {
            ...
            } ElseIf (LAnd (LGreaterEqual (Local0, 0x24), LLessEqual (Local0, 0x25))) {
                // BAR5 - AMD Navi2x
                // Bit28 for STRAP_FUNC_EN_DEV0_F1 in RomStrapOffset 0x34E8
                Store (CpmReadMem32 (And (CpmDevicePMemPostBar5, 0xFFFFFFF0), 0x34E8, 0, 32), Local2)
                CpmWriteMem32 (And (CpmDevicePMemPostBar5, 0xFFFFFFF0), 0x34E8, 0, 32, And (Local2, 0xEFFFFFFF))
            } ElseIf (LAnd (LGreaterEqual (Local0, 0x26), LLessEqual (Local0, 0xBF))) {
                // BAR5 - AMD Navi3x
                // Bit28 for STRAP_FUNC_EN_DEV0_F1 in RomStrapOffset 0x3514
                Store (CpmReadMem32 (And (CpmDevicePMemPostBar5, 0xFFFFFFF0), 0x3514, 0, 32), Local2)
                CpmWriteMem32 (And (CpmDevicePMemPostBar5, 0xFFFFFFF0), 0x3514, 0, 32, And (Local2, 0xEFFFFFFF))
            }
        }

        // Disable AMD DGPU's USB Port
        Method (CpmDisableDgpuUsbPort, 0, Serialized) {
            CreateDwordField (CpmdGpuDevicePMemPostBuffer, 12, CpmDevicePMemPostBar5)

            Store (CpmCheckDgpuVidDid (), Local0)
            If (LAnd (LGreaterEqual (Local0, 0x22), LLessEqual (Local0, 0xBF))) {
                ...
                And (CpmDevicePMemPostBar5, 0xFFFFFFF0, Local0)
                Store (CpmReadMem32 (Local0, 0x5818C, 0, 32), Local1)
                Store (20202, Local2)
                While (LAnd (LGreater (Local2, 0), LNotEqual (And (Local1, 0x80000000), 0x80000000))) {
                ...
                CpmWriteMem32 (Local0, 0x5818C, 0, 32, 0x01000000)
                ...
                If (LAnd (LGreater (Local2, 0), LEqual (And (Local1, 0x0000FFFF), 0))) {
                    ...
                    If (LEqual (CpmReadMem32 (Local0, 0x58190, 0, 32), 1)) {
                        ...
                        // System BIOS checks status return code in bits [15:0] of the Command register. A
                        // value of 0 indicates successful response.
                    } // End of If (LEqual (CpmReadMem32 (Local0, 0x58190, 0, 32), 1))
                } // End of If (LAnd (LGreater (Local2, 0), LEqual (And (Local1, 0x0000FFFF), 0)))
            } // End of If (LAnd (LGreaterEqual (Local0, 0x22), LLessEqual (Local0, 0xBF)))
        } // End of Method (CpmDisableDgpuUsbPort, 0, Serialized)
        ...
        //  Arg1 - Device Num of Gfx Bridge
        //  Arg2 - Function Num of Gfx Bridge
        Method (CpmInitDgpu, 3) {
            Store (0x7FFFFFFF, Local6)
            Or (Local6, 0x80000000, Local6)
            Store (CpmReadPci32 (Arg0, Arg1, Arg2, 0x54), Local1)
            CpmWritePci32 (Arg0, Arg1, Arg2, 0x54, And (Local1, 0xFFFF7FFC)) // Set to D0 state
            Store (CpmReadPci (Arg0, Arg1, Arg2, 0x19, 0, 8), Local2)
            Store (1, Local4)
            ...
            While (LAnd (Local4, Local5)) {
                ...
                If (LNotEqual (Local0, Local6)) {
                    ...
                    If (LAnd (LGreaterEqual (Local0, 0x00), LLessEqual (Local0, 0xBF))) {
                        // All of AMD DGPU
                        CpmSaveGfxResource (Arg0, Arg1, Arg2)
                        CpmProgramGfxResource (Arg0, Arg1, Arg2)
                        If (LNotEqual (CpmdGpuAudioDisable, 0)) {
                            CpmDisableDgpuAudio ()
                        }
                        If (LNotEqual (CpmDgpuD3ColdDgpuSsidSvid, 0)) {
                            CpmProgramDgpuSsid ()
                        }
                        If (LEqual (And (ShiftRight (CpmReadTable (CpmMainTable, Add (CpmDisplayFeature, 3)), 1), 1), 1)) { // BIT25: DisableDgpuUsbPort
                            CpmDisableDgpuUsbPort ()
                        }
                        CpmRestoreGfxResource (Arg0, Arg1, Arg2)
                    }
                    Store (0, Local4)
                } Else {
            ...
            CpmWritePci32 (Arg0, Arg1, Arg2, 0x54, And (Local1, 0xFFFF7FFF)) // Restore to original state
        }
        ...
        Method (CpmDgpuSetPower, 1) {
            ...
            Name (CpmDgpuD3ColdGen1Support, 0)
            Name (CpmDgpuDummyD3ColdSupport, 0)
            ...
            And (ShiftRight (CpmReadTable (CpmMainTable, Add (CpmDisplayFeature, 3)), 6), 1, Local0)                        // BIT30: DgpuNonHotPlugMode DGPU Non-HotPlug Mode - 1: Non-Hotplug Mode
            Or (CpmDgpuD3ColdHotPlugMode, ShiftLeft (Local0, 1), CpmDgpuD3ColdHotPlugMode)
            And (ShiftRight (CpmReadTable (CpmMainTable, Add (CpmDisplayFeature, 2)), 6), 1, CpmNonEvalDgpuSupport)         // BIT22: NonEvalDgpuSupport
            And (ShiftRight (CpmReadTable (CpmMainTable, Add (CpmDisplayFeature, 3)), 2), 1, CpmDgpuD3ColdGen1Support)      // BIT26: D3ColdGen1Support
            And (ShiftRight (CpmReadTable (CpmMainTable, Add (CpmDisplayFeature, 3)), 3), 1, CpmDgpuDummyD3ColdSupport)     // BIT27: DummyD3ColdSupport

            If (LEqual (CpmDgpuD3ColdHotPlugMode, 0)) {
            ...
            Store (CpmReadTable (CpmDisplayFeatureTable, CpmDisplayFeatureDgpuSsidSvid), CpmDgpuD3ColdDgpuSsidSvid)
            Store (CpmReadTable32 (CpmDisplayFeatureTable, CpmDisplayFeaturePcdSetS0i3PmeTurnOffDelay), CpmDgpuD3ColdPcdSetS0i3PmeTurnOffDelay)
            If (LEqual (CpmDgpuD3ColdPcdSetS0i3PmeTurnOffDelay, 0x0)) {
                If (LLessEqual (\_SB.PCI0.PB2.CpmDgpuDsmFunBData, 10000)) {
                    Store (\_SB.PCI0.PB2.CpmDgpuDsmFunBData, CpmDgpuD3ColdPcdSetS0i3PmeTurnOffDelay)
                    Store (CpmDgpuD3ColdPcdSetS0i3PmeTurnOffDelay, CpmAlibControlValue)
                    Store (CpmAlibSubFunction, Local0)
                    Store (0x12, CpmAlibSubFunction)                    // BIOSSMC_MSG_SetS0i3PmeTurnOffDelay
                    \_SB.ALIB (0x0C, Local3)
                    Store (Local0, CpmAlibSubFunction)
                }
            }

            If (LEqual (CpmDgpuDummyD3ColdSupport, 1)) {
                CpmInitDgpu (0, CpmBridgeDevice, CpmBridgeFunction)
                Release (EEBC)
                Return ()
            }

            Store (CpmReadTable32 (CpmDisplayFeatureTable, CpmDisplayFeatureMacoGpio), CpmDgpuD3ColdMacoGpio)
            Store (CpmReadTable32 (CpmDisplayFeatureTable, CpmDisplayFeatureMxmPwrEnGpio), CpmDgpuD3ColdMxmPwrEnGpio)
            ...
            IF (LEqual (CpmCheckDeviceResetState (CpmDgpuPowerResourceDeviceId, Arg0), 0)) {
                IF (LAnd (Arg0, Local6)) {
                    ...
                    //
                    // Disable Debug Print in ASL to match timing request
                    //
                    Store (CpmDebugPrintSerialAddr, Local4)
                    Store (0, CpmDebugPrintSerialAddr)
                    ...
                    If (LGreaterEqual (CpmSbChipId, CPM_FCH_REVISION_ID_RMB)) {
                        If (LEqual(CpmNonZ10CapablePcieZstateSupport, 1)) {
                            // DXIO Exit D3
                            \_SB.ALIB (0x13, 0)
                        }
                    }

                    Store (CpmReadTable (CpmDisplayFeatureTable, CpmDisplayFeatureTimePeRstToWakeL23), Local1)
                    If (LEqual (Local1, 0)) {
                        Store (10, Local1)
                    }
                    Sleep (Local1)                                                  // Delay 1 ~ 30 ms, default is 10 ms.

                    If (LEqual (CpmDgpuPmeTurnOffSupport, 1)) {
                        CpmWakeLink (0, CpmBridgeDevice, CpmBridgeFunction, 1)
                    }

                    //
                    // Restore "Debug Print in ASL" settings
                    //
                    Store (Local4, CpmDebugPrintSerialAddr)
                    ...
                    If (LEqual (CpmDgpuD3ColdHotPlugMode, 1)) {
                        ...
                        If (LEqual (CpmDgpuPmeTurnOffSupport, 0)) {
                            ...
                            Store (CpmRetrainPcieLink (0, CpmBridgeDevice, CpmBridgeFunction, 0), Local1)
                        }
                        ...
                        // For HotPlugBasic and Non-Hotplug
                            ...
                            While (LLess (Local2, 15)) {
                                ...
                                If (LNot (Local4)) {                                // Timeout checking Training State?
                                    ...
                                    If (Local5) {
                                        ...
                                        If (LEqual (CpmRetrainPcieLink (0, CpmBridgeDevice, CpmBridgeFunction, 0), Ones)) {
                                            Store (1, Local0)
                                        }
                                        If (Local0) {
                                            Store (1, CpmDgpuD3ColdHotPlugOK)
                    ...
                    If (LEqual (CpmDgpuPmeTurnOffSupport, 1)) {
                        CpmWakeLink (0, CpmBridgeDevice, CpmBridgeFunction, 0)
                    }
                    CpmSetPostCode (CpmTpDgpuPowerOnAfterPcieTraining)
                    Store (CpmReadTable (CpmDisplayFeatureTable, CpmDisplayFeatureTimeDlActToCfgAcc), Local1)
                    If (LEqual (Local1, 0)) {
                        Store (100, Local1)
                    }
                    Sleep (Local1)                                                  // Delay 1 ~ 100 ms, default is 100 ms.
                    If (LEqual (CpmDgpuD3ColdHotPlugOK, 1)) {
                        CpmInitDgpu (0, CpmBridgeDevice, CpmBridgeFunction)
                    } // End of If (LEqual (CpmDgpuD3ColdHotPlugOK, 1))

                    If (LEqual (CpmDgpuStartStopSupport, 1)) {
                        Store (CpmCheckDgpuVidDid (), Local0)
                        If (LAnd (LGreaterEqual (Local0, 0x00), LLessEqual (Local0, 0xBF))) {
                            // All of AMD DGPU
                            // Only for RV
                    ...
                    If (LGreaterEqual (CpmDgpuD3ColdHotPlugMode, 1)) {
                        // For HotPlugEnhanced and Non-Hotplug
                    ...
                    } Else { // End of If (LGreaterEqual (CpmDgpuD3ColdHotPlugMode, 1))
                        // Only for HotPlugBasic
                        Store (CpmReadPci32 (0, CpmBridgeDevice, CpmBridgeFunction, 0x54), Local1)
                        CpmWritePci32 (0, CpmBridgeDevice, CpmBridgeFunction, 0x54, And (Local1, 0xFFFF7FFC)) // Set to D0 state
                    ...
                    If (LEqual (CpmDgpuD3ColdGen1Support, 1)) {
                        Store (CpmGetPcieLinkStatus (0, CpmBridgeDevice, CpmBridgeFunction), Local1)
                        And (Local1, 0xF, Local1)
                        If (LGreater (Local1, 1)) {
                            // If LinkSpeed > Gen1, retrain to Gen1
                            Store (CpmRetrainPcieLink (0, CpmBridgeDevice, CpmBridgeFunction, 1), Local4)
                        }
                    }

                    If (LEqual (CpmDgpuPmeTurnOffSupport, 1)) {
                        CpmSendPmeTurnOff (0, CpmBridgeDevice, CpmBridgeFunction)
                    }
                    ...
                    If (LGreater (CpmDgpuD3ColdPcdSetS0i3PmeTurnOffDelay, 0x0)) {
                        Divide (CpmDgpuD3ColdPcdSetS0i3PmeTurnOffDelay, 1000, Local5, Local4)
                        If (LGreaterEqual (Local4, 1)) {
                            // PERST# Assertion Delay > 1ms part
                            Sleep (Local4)
                        }
                        Divide (Local5, 99, Local5, Local4)
                        While (LGreaterEqual (Local4, 1)) {
                            // PERST# Assertion Delay >= 99us part
                            Decrement (Local4)
                            Stall (99)
                        }
                        If (LGreaterEqual (Local5, 1)) {
                            // PERST# Assertion Delay < 99us part
                            Stall (Local5)
                        }
                    }
                    ...
                    If (LGreaterEqual (CpmSbChipId, CPM_FCH_REVISION_ID_RMB)) {
                        If (LEqual(CpmNonZ10CapablePcieZstateSupport, 1)) {
                            // Clear PME_STATUS (Write-1-Clear Reg:0x54 BIT15)
                            Store (CpmReadPci8 (0, CpmBridgeDevice, CpmBridgeFunction, 0x55, 0, 8), Local1)
                            CpmWritePci8 (0, CpmBridgeDevice, CpmBridgeFunction, 0x55, 0, 8, Or (Local1, 0x80))

                            // Clear PME_STATUS (Write-1-Clear Reg:0x78 BIT16)
                            Store (CpmReadPci8 (0, CpmBridgeDevice, CpmBridgeFunction, 0x7A, 0, 8), Local1)
                            CpmWritePci8 (0, CpmBridgeDevice, CpmBridgeFunction, 0x7A, 0, 8, Or (Local1, 0x01))
                        }
                    }

                    If (LEqual (CpmDgpuD3ColdGen1Support, 1)) {
                        If (LGreater (Local1, 1)) {
                            CpmWritePci8 (0, CpmBridgeDevice, CpmBridgeFunction, 0x88, 0, 4, Local1)          // Bit[3:0] = TargetLinkSpeed
                        }
                    }
