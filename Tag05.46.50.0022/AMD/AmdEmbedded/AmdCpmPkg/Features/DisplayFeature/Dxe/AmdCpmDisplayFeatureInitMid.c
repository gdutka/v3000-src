...
{
  DEBUG((DEBUG_INFO, "FEA-DXE-%a-Start-MultiBridge=%d bus=%X [%X:%X] [%X:%X]\n", __FUNCTION__, MultiBridge, bus, GpuDevice->Bridge.Device, GpuDevice->Bridge.Function, GpuDevice->Device.Device, GpuDevice->Device.Function));
  ...
  )
{
  ...
  if (DisplayFeatureDataPtr->GfxDevicePfa[1].Raw) {
    ...
    } else if ((((DeviceIdReg >> 16) & 0xFFFF) >= 0x73A0) && (((DeviceIdReg >> 16) & 0xFFFF) <= 0x73FF)) {
      DgpuCase = 0x24;      // Navi21 / Navi22 / Navi23
    } else if ((((DeviceIdReg >> 16) & 0xFFFF) >= 0x7420) && (((DeviceIdReg >> 16) & 0xFFFF) <= 0x743F)) {
      DgpuCase = 0x25;      // Navi24
    } else if ((((DeviceIdReg >> 16) & 0xFFFF) >= 0x7440) && (((DeviceIdReg >> 16) & 0xFFFF) <= 0x749F)) {
  ...
  )
{
  ...
  if (DisplayFeatureDataPtr->GfxDevicePfa[1].Raw) {
    ...
    if (DgpuCase >= 0x22) {
      ...
      // BAR5
      Mmiobase = CpmTableProtocolPtr->CommonFunction.PciRead32 ( CpmTableProtocolPtr,
                        DisplayFeatureDataPtr->GfxDevicePfa[1].Pfa.Bus,
                        DisplayFeatureDataPtr->GfxDevicePfa[1].Pfa.Device,
                        DisplayFeatureDataPtr->GfxDevicePfa[1].Pfa.Function,
                        0x24 );
      ...
      if ((Mmiobase != 0xFFFFFFFF) && (Mmiobase != 0)) {
        ...
        Retry = 0;
        do {
          UsbCommandReg = CpmTableProtocolPtr->CommonFunction.MmioRead32 (Mmiobase + RomStrapOffset);
          Retry++;
          CpmTableProtocolPtr->CommonFunction.Stall (CpmTableProtocolPtr, 100000);
        ...
        CpmTableProtocolPtr->CommonFunction.MmioWrite32 (Mmiobase + RomStrapOffset, 0x01000000);
        ...
        if (((UsbCommandReg & 0x0000FFFF) == 0x00000000) && (Retry < RetryTimer)) {
          ...
          // port is present.
          UsbCommandReg = CpmTableProtocolPtr->CommonFunction.MmioRead32 (Mmiobase + RomStrapOffset + 4);
          if (UsbCommandReg == 1) {
            // System BIOS programs the Command register to a value of 0x00400000 to send a
            // command to security firmware to disable USB ports.
            CpmTableProtocolPtr->CommonFunction.MmioWrite32 (Mmiobase + RomStrapOffset, 0x00400000);
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a(%d)-Status = %r\n", __FUNCTION__, __LINE__, Status));
  ASSERT_EFI_ERROR (Status);
  ...
  )
{
  ...
  if (!IsInvoked) {
    ...
    CfgDisableDgpuUsbPort   = MainTablePtr->DisplayFeature.Config.DisableDgpuUsbPort;
    DisplayFeatureDataPtr   = &CpmDisplayFeatureProtocolPtr->DisplayFeatureData;
    DevicePathTablePtr      = DisplayFeatureDataPtr->DevicePathTablePtr;
    ...
    if (!(iGpuDevice && dGpuDevice)) {
      if (iGpuDevice) {
        ...
        for (Index = 0; Index < 2; Index ++) {
          ...
          if (i < AMD_DISPLAY_DEVICE_PATH_SIZE) {
      ...
      if (CfgDisableDgpuUsbPort) {
        AmdCpmDisableDgpuUsbPort (CpmDisplayFeatureProtocolPtr);
      }
  ...
  )
{
  ...
  if (!AmdCpmDisplayFeatureInitMidInvoked) {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG((DEBUG_INFO, "FEA-DXE-%a-End-2-EFI_ERROR\n", __FUNCTION__));
      return;
    }
    DEBUG((DEBUG_INFO, "FEA-DXE-%a-End-3-EFI_SUCCESS\n", __FUNCTION__));
  }
