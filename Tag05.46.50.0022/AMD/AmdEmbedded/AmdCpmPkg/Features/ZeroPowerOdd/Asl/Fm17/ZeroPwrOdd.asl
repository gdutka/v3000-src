    ...
    )
{
    ...
    External(\_SB.PCXX.GPXX.SAXX, DeviceObj)
    External(\_SB.PCXX.GPXX.SAXX._ADR, MethodObj)
            ...
            If (LEqual(XCMP(Arg0, Buffer(){0x9B, 0x14, 0xDB, 0xE4, 0xFE, 0xFC, 0x5B, 0x42, 0xA6, 0xD8, 0x92, 0x35, 0x7D, 0x78, 0xFC, 0x7F}), 1)) {
                ...
                //
                    ...
                    If (LOr(LEqual(Local0, Add(Multiply(Local4, 0x10000), 0x0000FFFF)), LEqual(Local0, Ones))) {
                        ...
                        If (LEqual(Local1, 0)) {
                            \_SB.PCXX.GPXX.SAXX.STPT(0)              // Disable PHY Port 1
                            Return(0)
                        } ElseIf (LEqual(Local1, 1)) {
                            If (CpmIsWin8()) {
                                \_SB.PCXX.GPXX.SAXX.ODD8._PS0()      // _PS0 implicitly enables PHY Port 1
                            } Else {
                                \_SB.PCXX.GPXX.SAXX.ODDZ._PS0()      // _PS0 implicitly enables PHY Port 1
                            }
                            Return(0)
                        }
                    }
...
//          Store(CpmReadPci32(CpmGetInternalBusNumber(1), STDV(), STFN(), 0x24), Local0)
            Add(CpmReadTable32(CpmZeroPowerOddTable, CpmZeroPowerOddSataPciCfgSmn), 0x24, Local3) // 0x10148024
            Store (CpmReadSmnRegister (0, 0, 0, Local3), Local0)
...
//          CpmWritePci(CpmGetInternalBusNumber(1), STDV(), STFN(), 0x42, Local4, 1, Xor(Arg0, 1))
            Store(CpmReadTable32(CpmZeroPowerOddTable, CpmZeroPowerOddSataAhciSmn), Local5)       // 0x03101000
            If (LEqual (Arg0, 0))
            {
              Store (CpmReadSmnRegister (0, 0, 0, Add (Multiply (Local4, 0x80), Add (Local5, 0x12C))), Local0)
              Add (And (Local0, 0xFFFFFFF0), 4, Local0)
              CpmWriteSmnRegister (0, 0, 0, Add (Multiply (Local4, 0x80), Add (Local5, 0x12C)), Local0)

              Store (1, Local1)
              Store (20, Local2)
              while (LAnd (Local1, Local2)) {
                Store (CpmReadSmnRegister (0, 0, 0, Add (Multiply (Local4, 0x80), Add (Local5, 0x128))), Local0)
            ...
            Store (CpmReadSmnRegister (0, 0, 0, Add (Local5, 0x800)), Local0)
            And (Local0, Xor (0xFFFFFFFF, ShiftLeft (1, Add (Local4, 16))), Local0)
            ShiftLeft (Xor (Arg0, 1), Add (Local4, 16), Local1)
            Or (Local0, Local1, Local0)
            CpmWriteSmnRegister (0, 0, 0, Add (Local5, 0x800), Local0)
        ...
        Method(STPP, 1, Serialized) {
            ...
            // Enable PHY Port n
//          CpmWritePci(CpmGetInternalBusNumber(1), STDV(), STFN(), 0x42, Local4, 1, Xor(Arg0, 1))
            \_SB.PCXX.GPXX.SAXX.STPT(Arg0)                // Disable PHY Port 1

//          If (LEqual(CpmReadPci(CpmGetInternalBusNumber(1), STDV(), STFN(), 0x0A, 0, 16), 0x106)) {
            Add(CpmReadTable32(CpmZeroPowerOddTable, CpmZeroPowerOddSataPciCfgSmn), 0x8, Local3)  // 0x10148008
            If (LEqual (ShiftRight (CpmReadSmnRegister (0, 0, 0, Local3), 16), 0x106)) {
                If (LNotEqual(PBAA(), 0)) {
                    // Write Enable
...
//                  CpmWriteMem32(PBAA(), 0x0C, Local4, 1, Arg0)
                    Store (CpmReadSmnRegister (0, 0, 0, Add (Local5, 0x00C)), Local0)
                    ...
                    CpmWriteSmnRegister (0, 0, 0, Add (Local5, 0x00C), Local0)
//                  // Port Status
...
//                  CpmWriteMem32(PBAA(), 0, 0, 5, Decrement(Local2))
                    Store (CpmReadSmnRegister (0, 0, 0, Local5), Local0)
                    And (Local0, 0xFFFFFFE0, Local0)
                    Or (Local0, Decrement(Local2), Local0)
                    CpmWriteSmnRegister (0, 0, 0, Local5, Local0)
                    // Write Disable
...
//              Store(CpmReadMem32(PBAA(), Local0, 0, 1), Local0)
                Store(CpmReadTable32(CpmZeroPowerOddTable, CpmZeroPowerOddSataAhciSmn), Local5)   // 0x03101000
                Store (CpmReadSmnRegister (0, 0, 0, Add (Multiply (Local4, 0x80), Add (Local5, 0x118))), Local0)
                And (Local0, 1, Local0)
            }
            Return(Local1)
        }
...
//                  CpmWriteMem32(PBAA(), Local0, 0, 1, 0)
                    ...
                    Store (CpmReadSmnRegister (0, 0, 0, Add (Multiply (Local4, 0x80), Add (Local5, 0x118))), Local0)
                    And (Local0, 0xFFFFFFFE, Local0)
                    CpmWriteSmnRegister (0, 0, 0, Add (Multiply (Local4, 0x80), Add (Local5, 0x118)), Local0)
                    // wait for 1ms
                    Sleep(1)
...
//                  CpmWriteMem32(PBAA(), Local0, 0, 4, 0)
            ...
            If (LEqual(And(Local0, 3), 3)) {
                ...
                If (LEqual(Local6, 0)) {
                    If (CpmReadGevent(Local3)) {
                        Store(0x1, \_SB.PCXX.GPXX.SAXX.SAPO)
                    } Else {
                        Store(0x0, \_SB.PCXX.GPXX.SAXX.SAPO)
                    }
        ...
        //
        Method (TWAK, 1) {
            Store(Arg0, \_SB.PCXX.GPXX.SAXX.SASS)
            Store(CpmReadTable(CpmMainTable, CpmZeroPowerOddEn), Local0)
            Store(CpmReadTable(CpmZeroPowerOddTable, CpmZeroPowerOddEventPin2), Local3)
            Store(CpmReadTable(CpmZeroPowerOddTable, CpmZeroPowerOddEventSource2), Local6)
            // ODD Zero Power is enabled, and ODD Hotplug is enabled.
            If (LEqual(And(Local0, 3), 3)) {
                ...
                If (LEqual(Local6, 0)) {
                    If (CpmReadGevent(Local3)) {
                        If (LAnd(LEqual(Arg0, 4), LEqual(\_SB.PCXX.GPXX.SAXX.SAPO, 0x0))) {
                            If (LNot(CpmIsWin8())) {
                                Notify(\_SB.PCXX.GPXX.SAXX.ODDZ, 0x1)
                                Notify(\_SB.PCXX.GPXX.SAXX.ODDZ, 0x81)
                            }
                        }
                    }
                // Source of ODD_PLUGIN# - 1: KBC GPIO/QEVENT Pin, 2: ECRAM GPIO/QEVENT Pin
                    ...
                    If (LEqual(\_SB.PCI0.LPC0.EC0.OKEC, 0x01)) {
                        If (CpmReadGpio(Add(Local3, ShiftLeft(Local6, 8)))) {
                            If (LAnd(LEqual(Arg0, 3), LEqual(\_SB.PCXX.GPXX.SAXX.SAPO, 0x0))) {
                                If (LNot(CpmIsWin8())) {
                                    Notify(\_SB.PCXX.GPXX.SAXX.ODDZ, 0x1)
                                    Notify(\_SB.PCXX.GPXX.SAXX.ODDZ, 0x81)
                                }
                            } ElseIf (LAnd(LEqual(Arg0, 4), LEqual(\_SB.PCXX.GPXX.SAXX.SAPO, 0x0))) {
                                If (LNot(CpmIsWin8())) {
                                    Notify(\_SB.PCXX.GPXX.SAXX.ODDZ, 0x1)
                                    Notify(\_SB.PCXX.GPXX.SAXX.ODDZ, 0x81)
                                }
    ...
    } // End of Scope(\_SB.PCXX.GPXX.SAXX)
            ...
            If (LEqual(Local7, 0x0)) {
                ...
                If (LEqual(CpmReadGevent(Local2), 0x0)) {
                    ...
                    If (And(Local0, 0x10)) {
                        ...
                        \_SB.PCXX.GPXX.SAXX.STPR()
                    }
                    // Notify for different OS mode
                    If (CpmIsWin8()) {
                        Notify(\_SB.PCXX.GPXX.SAXX.ODD8, 0x02)
                    } Else {
                        Notify(\_SB.PCXX.GPXX.SAXX.ODDZ, 0x80)
                    }
                }
            }
        ...
        Method(XL10) {
            ...
            If (CpmReadGeventTriggerType(Local3)) {
                ...
                If (CpmReadGevent(Local3)) {
                    ...
                    If (CpmIsWin8()) {
                        Store(0, \_SB.PCXX.GPXX.SAXX.ODD8.CpmOddStatus)
                    } Else {
                        Store(0, \_SB.PCXX.GPXX.SAXX.ODDZ.CpmOddStatus)
                    }
                    Sleep(40)
                    // Disable Port
                    \_SB.PCXX.GPXX.SAXX.STPP(0)
                    If (CpmIsWin8()) {
                        Store(0x3, \_SB.PCXX.GPXX.SAXX.ODD8.ODPS)
                    } Else {
                        Store(0x3, \_SB.PCXX.GPXX.SAXX.ODDZ.ODPS)
                    }
                ...
                If (LNot(CpmReadGevent(Local3))) {
                    ...
                    // Enable Port
                    \_SB.PCXX.GPXX.SAXX.STPP(1)
                    Sleep(40)
                    // Power On ODD
                    CpmSetDevicePower(Local1, 1)
                    Sleep(40)
                    If (CpmIsWin8()) {
                        Store(1, \_SB.PCXX.GPXX.SAXX.ODD8.CpmOddStatus)
                    } Else {
                        Store(1, \_SB.PCXX.GPXX.SAXX.ODDZ.CpmOddStatus)
                    }
                    // Apply a port reset in AHCI mode
//                  \_SB.PCXX.GPXX.SAXX.STPR()
                    If (CpmIsWin8()) {
                        Store(0x0, \_SB.PCXX.GPXX.SAXX.ODD8.ODPS)
                    } Else {
                        Store(0x0, \_SB.PCXX.GPXX.SAXX.ODDZ.ODPS)
                    }
                    // Switch SB detection logic to raising edge
                    CpmWriteGeventTriggerType(Local3, Xor(CpmReadGevent(Local3), 1))
                }
            ...
            If (And(Local0, 1)) {
                If (CpmIsWin8()) {
                    Notify(\_SB.PCXX.GPXX.SAXX.ODD8, 0x1)
                } Else {
                    Notify(\_SB.PCXX.GPXX.SAXX.ODDZ, 0x1)
                    Notify(\_SB.PCXX.GPXX.SAXX.ODDZ, 0x81)
                }
            } Else {
                Notify(\_SB.PCXX.GPXX.SAXX.PRT1, 0x1)
                // for AMDAHCI's prior to Win8 driver, extra requirement for plug in
                If (LNot(CpmIsWin8())) {
                    Notify(\_SB.PCXX.GPXX.SAXX.PRT1, 0x81)
                }
            }
        } // End of Method(XL10)
    } // End of Scope(\_GPE)
    ...
    Scope(\_SB.PCI0.LPC0.EC0) {
        ...
        Method(XQ43, 0x0, Serialized) {
            ...
            If (LAnd(Local6, And(Not(Local7), 1))) {
                ...
                If (CpmIsWin8()) {
                    If (LEqual(\_SB.PCXX.GPXX.SAXX.ODD8.CpmOddStatus, 0)) {
                        Notify(\_SB.PCXX.GPXX.SAXX.ODD8, 0x02)
                        CpmSetPostCode(CpmTpZeroPowerOddRemoveQEvent_ODD_DA)
                    }
                } Else {
                    If (LEqual(\_SB.PCXX.GPXX.SAXX.ODDZ.CpmOddStatus, 0)) {
                        Notify(\_SB.PCXX.GPXX.SAXX.ODDZ, 0x80)
                        CpmSetPostCode(CpmTpZeroPowerOddRemoveQEvent_ODD_DA)
                    }
                }
