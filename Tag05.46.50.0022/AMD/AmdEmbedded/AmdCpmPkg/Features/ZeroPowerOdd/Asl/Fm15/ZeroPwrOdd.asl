    ...
    )
{
    ...
    External(\_SB.PCXX.SAXX, DeviceObj)
    External(\_SB.PCXX.SAXX._ADR, MethodObj)
    ...
    Scope(\_SB.PCXX.SAXX) {
            ...
            If (LEqual(XCMP(Arg0, Buffer(){0x9B, 0x14, 0xDB, 0xE4, 0xFE, 0xFC, 0x5B, 0x42, 0xA6, 0xD8, 0x92, 0x35, 0x7D, 0x78, 0xFC, 0x7F}), 1)) {
                ...
                //
                    ...
                    If (LOr(LEqual(Local0, Add(Multiply(Local4, 0x10000), 0x0000FFFF)), LEqual(Local0, Ones))) {
                        ...
                        If (LEqual(Local1, 0)) {
                            \_SB.PCXX.SAXX.STPT(0)              // Disable PHY Port 1
                            Return(0)
                        } ElseIf (LEqual(Local1, 1)) {
                            If (CpmIsWin8()) {
                                \_SB.PCXX.SAXX.ODD8._PS0()      // _PS0 implicitly enables PHY Port 1
                            } Else {
                                \_SB.PCXX.SAXX.ODDZ._PS0()      // _PS0 implicitly enables PHY Port 1
                            }
                            Return(0)
                        }
                    }
        ...
        Method(STPT, 1, Serialized) {
            Store(CpmReadTable(CpmZeroPowerOddTable, CpmZeroPowerOddSataPortId), Local4)
            // Enable PHY Port n
            CpmWritePci(0, STDV(), STFN(), 0x42, Local4, 1, Xor(Arg0, 1))
        }
        ...
        Method(STPP, 1, Serialized) {
            ...
            If (LEqual(CpmReadPci(0, STDV(), STFN(), 0x0A, 0, 16), 0x106)) {
                If (LNotEqual(PBAA(), 0)) {
                    // Write Enable
                    CpmWritePci(0, STDV(), STFN(), 0x40, 0, 1, 1)
                    // Port n implemented
                    CpmWriteMem32(PBAA(), 0x0C, Local4, 1, Arg0)
                    // Port Status
                    Store(CpmReadPci(0, STDV(), STFN(), 0x42, 0, 6), Local0)
                    ...
                    // Write Disable
                    CpmWritePci(0, STDV(), STFN(), 0x40, 0, 1, 0)
                }
            }
            ...
            If (LEqual(And(Local0, 3), 3)) {
                ...
                If (LEqual(Local6, 0)) {
                    If (CpmReadGevent(Local3)) {
                        Store(0x1, \_SB.PCXX.SAXX.SAPO)
                    } Else {
                        Store(0x0, \_SB.PCXX.SAXX.SAPO)
                    }
        ...
        Method (TWAK, 1) {
            ...
            If (LEqual(And(Local0, 3), 3)) {
                ...
                If (LEqual(Local6, 0)) {
                    If (CpmReadGevent(Local3)) {
                        If (LAnd(LEqual(Arg0, 4), LEqual(\_SB.PCXX.SAXX.SAPO, 0x0))) {
                            If (LNot(CpmIsWin8())) {
                                Notify(\_SB.PCXX.SAXX.ODDZ, 0x1)
                                Notify(\_SB.PCXX.SAXX.ODDZ, 0x81)
                            }
                        }
                    }
                // Source of ODD_PLUGIN# - 1: KBC GPIO/QEVENT Pin, 2: ECRAM GPIO/QEVENT Pin
                    ...
                    If (LEqual(\_SB.PCI0.LPC0.EC0.OKEC, 0x01)) {
                        If (CpmReadGpio(Add(Local3, ShiftLeft(Local6, 8)))) {
                            If (LAnd(LEqual(Arg0, 3), LEqual(\_SB.PCXX.SAXX.SAPO, 0x0))) {
                                If (LNot(CpmIsWin8())) {
                                    Notify(\_SB.PCXX.SAXX.ODDZ, 0x1)
                                    Notify(\_SB.PCXX.SAXX.ODDZ, 0x81)
                                }
                            } ElseIf (LAnd(LEqual(Arg0, 4), LEqual(\_SB.PCXX.SAXX.SAPO, 0x0))) {
                                If (LNot(CpmIsWin8())) {
                                    Notify(\_SB.PCXX.SAXX.ODDZ, 0x1)
                                    Notify(\_SB.PCXX.SAXX.ODDZ, 0x81)
                                }
        ...
        Method(XL06, 0x0, Notserialized) {
            ...
            If (LEqual(Local7, 0x0)) {
                ...
                If (LEqual(CpmReadGevent(Local2), 0x0)) {
                    ...
                    If (CpmIsWin8()) {
                        Notify(\_SB.PCXX.SAXX.ODD8, 0x02)
                    } Else {
                        Notify(\_SB.PCXX.SAXX.ODDZ, 0x80)
                    }
                }
            }
        ...
        Method(XL10) {
            ...
            If (CpmReadGeventTriggerType(Local3)) {
                ...
                If (CpmReadGevent(Local3)) {
                    ...
                    If (CpmIsWin8()) {
                        Store(0, \_SB.PCXX.SAXX.ODD8.CpmOddStatus)
                    } Else {
                        Store(0, \_SB.PCXX.SAXX.ODDZ.CpmOddStatus)
                    }
                    Sleep(40)
                    // Disable Port
                    \_SB.PCXX.SAXX.STPP(0)
                    If (CpmIsWin8()) {
                        Store(0x3, \_SB.PCXX.SAXX.ODD8.ODPS)
                    } Else {
                        Store(0x3, \_SB.PCXX.SAXX.ODDZ.ODPS)
                    }
                ...
                If (LNot(CpmReadGevent(Local3))) {
                    ...
                    // Enable Port
                    \_SB.PCXX.SAXX.STPP(1)
                    Sleep(40)
                    // Power On ODD
                    CpmSetDevicePower(Local1, 1)
                    Sleep(40)
                    If (CpmIsWin8()) {
                        Store(1, \_SB.PCXX.SAXX.ODD8.CpmOddStatus)
                    } Else {
                        Store(1, \_SB.PCXX.SAXX.ODDZ.CpmOddStatus)
                    }
                    // Apply a port reset in AHCI mode
//                  \_SB.PCXX.SAXX.STPR()
                    If (CpmIsWin8()) {
                        Store(0x0, \_SB.PCXX.SAXX.ODD8.ODPS)
                    } Else {
                        Store(0x0, \_SB.PCXX.SAXX.ODDZ.ODPS)
                    }
                    // Switch SB detection logic to raising edge
                    CpmWriteGeventTriggerType(Local3, Xor(CpmReadGevent(Local3), 1))
                }
            ...
            If (And(Local0, 1)) {
                If (CpmIsWin8()) {
                    Notify(\_SB.PCXX.SAXX.ODD8, 0x1)
                } Else {
                    Notify(\_SB.PCXX.SAXX.ODDZ, 0x1)
                    Notify(\_SB.PCXX.SAXX.ODDZ, 0x81)
                }
            } Else {
                Notify(\_SB.PCXX.SAXX.PRT1, 0x1)
                // for AMDAHCI's prior to Win8 driver, extra requirement for plug in
                If (LNot(CpmIsWin8())) {
                    Notify(\_SB.PCXX.SAXX.PRT1, 0x81)
                }
            }
        } // End of Method(XL10)
    } // End of Scope(\_GPE)
    ...
    Scope(\_SB.PCI0.LPC0.EC0) {
        ...
        Method(XQ43, 0x0, Serialized) {
            ...
            If (LAnd(Local6, And(Not(Local7), 1))) {
                ...
                If (CpmIsWin8()) {
                    If (LEqual(\_SB.PCXX.SAXX.ODD8.CpmOddStatus, 0)) {
                        Notify(\_SB.PCXX.SAXX.ODD8, 0x02)
                        CpmSetPostCode(CpmTpZeroPowerOddRemoveQEvent_ODD_DA)
                    }
                } Else {
                    If (LEqual(\_SB.PCXX.SAXX.ODDZ.CpmOddStatus, 0)) {
                        Notify(\_SB.PCXX.SAXX.ODDZ, 0x80)
                        CpmSetPostCode(CpmTpZeroPowerOddRemoveQEvent_ODD_DA)
                    }
                }
