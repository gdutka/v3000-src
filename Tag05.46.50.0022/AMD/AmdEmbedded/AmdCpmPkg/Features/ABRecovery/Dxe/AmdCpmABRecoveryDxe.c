  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_INFO, "FEA-DXE-%a-End-6-Allocate ISHActiveBuffer fail\n", __FUNCTION__));
    retStatus = FALSE;
  }
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_INFO, "FEA-DXE-%a-End-6-Allocate ISHAlternativeBuffer fail\n", __FUNCTION__));
    retStatus = FALSE;
  }

  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-ISHActiveAddress=%x\n", __FUNCTION__, ISHActiveAddress));
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-ISHAlternativeAddress=%x\n", __FUNCTION__, ISHAlternativeAddress));
  ...
  if ( ISHActiveHeader->PspId != ISHAlternativeHeader->PspId ) {
    DEBUG ((DEBUG_INFO, "FEA-DXE-%a-SLOTA and SLOTB are not Identical \n", __FUNCTION__));
    retStatus = FALSE;
  }
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_INFO, "FEA-DXE-%a-End-6-Free Buffer fail\n", __FUNCTION__));
  }
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_INFO, "FEA-DXE-%a-End-6-Allocate temp Buffer fail\n", __FUNCTION__));
    return FALSE;
  }

  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-Size=%x BiosBaseAddr = %lx \n", __FUNCTION__, Size,BiosBaseAddr));

  CopyMem ((UINT8*)(UINTN)PspDirBuffer, (VOID *)(UINTN)(BiosBaseAddr), MaxPspDirSize);

  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-PspDirBuffer=%lx \n", __FUNCTION__, PspDirBuffer));
...
//  *Size = (UINT32)((PspDir->Header.Reserved & 0x3FF) << 12 );
  *Size = PspDir->Header.AdditionalInfo.Field.MaxSize * SIZE_4KB;
  ...
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-Size=%x \n", __FUNCTION__, *Size));
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-BiosDirBuffer=%lx \n", __FUNCTION__, BiosDirBuffer));
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-Enter \n", __FUNCTION__));
  ...
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-Status1 %x \n", __FUNCTION__, Status));

  if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_INFO, "FEA-DXE-%a-End-6-Allocate global buffer fail \n", __FUNCTION__));
      return FALSE;
  }

  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-GlobalPSPL2DirBufferAddr %lx \n", __FUNCTION__, GlobalDirBufferAddr));
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-BiosBaseAddr %lx \n", __FUNCTION__, BiosBaseAddr));
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-GlobalDirBufferAddr %lx \n", __FUNCTION__, GlobalDirBufferAddr));
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-BiosBaseAddr %lx \n", __FUNCTION__, BiosBaseAddr));
  ...
  )
{
  ...
  DEBUG((DEBUG_INFO, "FEA-DXE-%a-BiosImageMemoryAddr %lx \n", __FUNCTION__, (*BiosImageMemoryAddr)));
  ...
  if (FetchPspDirBaseV2 (&PspDir, DirectoryMemoryAddr) != TRUE) {
    DEBUG((DEBUG_INFO, "FEA-DXE-%a-End-FetchPspDirBaseV2 Fail\n", __FUNCTION__));
    return FALSE;
  }

  //Check if we have Both Slots
  if (VerifyTwoSlotAbRecovery (PspDir) != TRUE )  {
     DEBUG((DEBUG_INFO, "FEA-DXE-%a-End-VerifyTwoSlotAbRecovery Fail\n", __FUNCTION__));
     return FALSE;
  }

  // Fetch the ISH info for both Slots.
  if (FetchImageSlotInfo (PspDir, &Slot, DirectoryMemoryAddr) != TRUE) {
     DEBUG((DEBUG_INFO, "FEA-DXE-%a-End-FetchImageSlotInfo Fail\n", __FUNCTION__));
     return FALSE;
  }
  ...
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-ActivePSPLevel2BaseAddress=%x\n", __FUNCTION__, ActivePSPLevel2BaseAddress));
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-AlternativePSPLevel2BaseAddress=%x\n", __FUNCTION__, AlternativePSPLevel2BaseAddress));
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-ISH-ActiveSlotIndex=%x\n", __FUNCTION__, ActiveSlotIndex));
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-ISH-AlternativeSlotIndex=%x\n", __FUNCTION__, AlternativeSlotIndex));
  ...
  if (CheckSLOTSIDENTICAL ((UINT32)SourceEntryAddress,(UINT32)DestinationEntryAddress, DirectoryMemoryAddr) == TRUE) {
     DEBUG ((DEBUG_INFO, "FEA-DXE-% a SLOT A and SLOT B are identical \n", __FUNCTION__));
     return TRUE;
  }
  ...
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-BIOSL2SourceMemoryAddr=%lx\n", __FUNCTION__, BIOSL2SourceMemoryAddr));

  SetupBIOSL2DirBuffer (&BIOSL2DestMemoryAddr, ((*BiosImageMemoryAddr) + OrgAlternativeBIOSLevel2BaseAddress));
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-OrgAlternativeBIOSLevel2BaseAddress=%lx\n", __FUNCTION__, OrgAlternativeBIOSLevel2BaseAddress));
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-BIOSL2DestMemoryAddr=%lx\n", __FUNCTION__, BIOSL2DestMemoryAddr));

  if (GetBIOSDirectorySize (BIOSL2SourceMemoryAddr, &BIOSSourceDirectorySize) != TRUE) {
    DEBUG ((DEBUG_INFO, "FEA-DXE-% a Get Source BIOS Directory Size failed \n", __FUNCTION__));
    return FALSE;
  }

  if (GetBIOSDirectorySize (BIOSL2DestMemoryAddr, &BIOSDestDirectorySize) != TRUE) {
    DEBUG ((DEBUG_INFO, "FEA-DXE-% a Get Dest BIOS Directory Size failed \n", __FUNCTION__));
    return FALSE;
  }

  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-PSPL2-DirectorySize %lx \n", __FUNCTION__, PSPSourceDirectorySize));
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-BIOSL2-DirectorySize=%x\n", __FUNCTION__, BIOSSourceDirectorySize));
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-PSPL2SourceAddr=%lx\n", __FUNCTION__, (*BiosImageMemoryAddr) + ActivePSPLevel2BaseAddress));
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-PSPL2-ActivePSPLevel2BaseAddress=%x\n", __FUNCTION__, ActivePSPLevel2BaseAddress));
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-PSPL2SourceAddr=%lx\n", __FUNCTION__, ((*BiosImageMemoryAddr) + ActivePSPLevel2BaseAddress)));
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-PSPL2-AlternativeBIOSLevel2BaseAddress=%x\n", __FUNCTION__, AlternativePSPLevel2BaseAddress));
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-PSPL2DestAddr=%lx\n", __FUNCTION__, ((*BiosImageMemoryAddr) + AlternativePSPLevel2BaseAddress)));

  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-BIOSL2-ActiveBiosLevel2BaseAddress=%x\n", __FUNCTION__, ActiveBiosLevel2BaseAddress));
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-BIOSL2SourceAddr=%lx\n", __FUNCTION__, ((*BiosImageMemoryAddr) + ActiveBiosLevel2BaseAddress)));
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-BIOSL2-AlternativeBIOSLevel2BaseAddress=%x\n", __FUNCTION__, AlternativeBIOSLevel2BaseAddress));
  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-BIOSL2DestAddr=%lx\n", __FUNCTION__, ((*BiosImageMemoryAddr) + AlternativeBIOSLevel2BaseAddress)));
  ...
  if (RestoreISHBHeader ((UINT32)SourceEntryAddress,(UINT32)DestinationEntryAddress, DirectoryMemoryAddr) != TRUE) {
    DEBUG ((DEBUG_INFO, "FEA-DXE-% a Restore SLOT B failed \n", __FUNCTION__));
    return FALSE;
  }

  DEBUG ((DEBUG_INFO, "FEA-DXE-%a-BIOS L2B Restore Completed \n", __FUNCTION__));
  ...
  )
{
  ...
  DEBUG((DEBUG_INFO, "FEA-DXE-%a-Status-%x BiosImageMemoryAddr %lx \n", __FUNCTION__, Status, (*BiosImageMemoryAddr)));
...
/*********************************************************************************
 * Name: AmdCpmABRecoveryDxeInit
 *
 * Description
 *   Entry point of the AMD AB Recovery DXE driver
 *
 * Input
