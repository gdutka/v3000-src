  ...
  )
{
  ...
  SubordinateBus = PciRead8 (Bridge.AddressValue | PCI_SUBORDINATE_BUS_REG);
  DEBUG ((EFI_D_ERROR, "  Secondry Bus Number: 0x%x\n", SecondaryBus));
  DEBUG ((EFI_D_ERROR, "  Subordinate Bus Number: 0x%X\n", SubordinateBus));

  if (SecondaryBus != 0) {
    StartRange.AddressValue = MAKE_SBDFO (0, SecondaryBus, 0, 0, 0);
    EndRange.AddressValue = MAKE_SBDFO (0, SubordinateBus, 0x1f, 0x7, 0);
...
RasPciErrCheckGn(
  ...
  OUT      BOOLEAN  *DpcStatusSet,
  OUT      UINT16   *DpcStatus,
...
)
{
  ...
  Data32 = PciRead32(PciCfgAddr.AddressValue);
  ...
  DEBUG ((EFI_D_ERROR, "  Check PCIE Error Status @ Bus: 0x%x, Dev: 0x%x, Func: 0x%x. %a\n",
    PciCfgAddr.Address.Bus, PciCfgAddr.Address.Device, PciCfgAddr.Address.Function, (OsEdrEnabledFlag)? "(EDR recovery flow)":" "));

  *DpcStatusSet = FALSE;
  *DpcStatus = 0;
  if (OsEdrEnabledFlag && !(*RootStatusSet) && (DpcCapPtr != 0)) {
    DpcCntlReg.Value = PciRead16(PciCfgAddr.AddressValue + DpcCapPtr + DPC_CONTROL_REG);
    DEBUG ((EFI_D_ERROR, "  DpcCntlReg.Value: 0x%08x\n", DpcCntlReg.Value));
    if ((DpcCntlReg.Field.DpcTriggerEnable == 0x01) || (DpcCntlReg.Field.DpcTriggerEnable == 0x02)) {
      //DPC is enabled => Continue checking DPC status
      DpcStsReg.Value = PciRead16(PciCfgAddr.AddressValue + DpcCapPtr + DPC_STATUS_REG);
      DEBUG ((EFI_D_ERROR, "  DpcStsReg.Value: 0x%08x\n", DpcStsReg.Value));
      if (DpcStsReg.Field.DpcTriggerStatus == 1) {
        *DpcStatusSet = TRUE;
        *DpcStatus = DpcStsReg.Value;
        DEBUG ((EFI_D_ERROR, "  EDR Enabled and DPC Error Found - DPC_STATUS_REG = 0x%08x\n", *DpcStatus));

        //check root port AER register.
        if (AerCapPtr != 0) {
          DEBUG ((EFI_D_ERROR, "  AER Error Check: 0x%0x\n", AerErrCheck(PciCfgAddr, AerCapPtr)));
        }
      } else {
          DEBUG ((EFI_D_ERROR, "  EDR Enabled but no DPC Error found\n"));
      }
      return *DpcStatusSet;
    }
  }
...
)
{
  ...
  *PcieErrorEntryCnt = 0;
  ...
  DEBUG ((EFI_D_ERROR, "  RootPort Address: 0x%08x. %a\n", PciCfgAddr.AddressValue, (OsEdrEnabledFlag)? "(EDR recovery flow)":" "));

  IsRootPortError = FALSE;
  if (RasPciErrCheckGn(PciCfgAddr, &RootStatusSet, &RootErrStatus, &DpcStatusSet, &DpcErrStatus, OsEdrEnabledFlag)) {
    LocalErrEntry->DevAddr = PciCfgAddr.AddressValue;
    LocalErrEntry->DevType = PcieDeviceRootComplex;

    //The value of DpcStatusSet will only be set under OsEdrEnabledFlag = TRUE
    //RootErrSts [31:16] == 0xFFFF => DpcStatusSet is TRUE, RootErrSts [15:00] = DpcErrStatus
    LocalErrEntry->RootErrSts = (DpcStatusSet) ? (DpcErrStatus | (0xFFFF << 16)) : RootErrStatus;
  ...
  )
{
  ...
  if (MAX_NBIO_PER_DIE > NbioNumber)  {
    ...
    *NbioSecBusNum = (UINT8)RasGetDfReg(RMB_IOMS0_INSTANCE_ID, DfNode, DF_CFGADDRESSCNTL);
...
)
{
  ...
  if (RasOemDimmMap == NULL) {
    ...
    if (GENERIC_ERROR_DATA_ENTRY_FRUTEXT_SIZE < MemErrFruDfltStrSize) {
      AsciiStrCpyS(FruTextDest, FRU_TEXT_MAX_LENGTH, "DIMM# Sourced");       //Use hardcode string
      return EFI_SUCCESS;
    }
    // Use Pcd string
    AsciiStrCpyS(FruTextDest, FRU_TEXT_MAX_LENGTH, MemErrFruDfltStr);
    return EFI_SUCCESS;
  }
  ...
  AsciiStrCpyS(FruTextDest, FRU_TEXT_MAX_LENGTH, MemErrFruTextOemPrefixStr);
  ...
  AsciiStrCpyS(&FruTextDest[MemErrFruTextOemPrefixStrSize-1], FRU_TEXT_MAX_LENGTH, "None");
