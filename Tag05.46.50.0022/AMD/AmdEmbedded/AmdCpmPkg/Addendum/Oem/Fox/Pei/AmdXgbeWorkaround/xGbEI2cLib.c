  ...
  )
{
   ...
   Status = I2cAccess(This, SlaveAddress, RequestPacket);
   ...
   )
{
  ...
  Status= XgbeSetBusFrequency(Private, &BusClockHertz);
  return Status;
}
...
)
{
   ...
   if (MmioRead32(Base + DW_IC_TAR) != Chip) {
       ...
       DEBUG((EFI_D_ERROR, "I2C Target Set - Chip Address:%x BaseAddress:%x\n", Chip,Base));
       I2cStatusCnt = DW_STATUS_WAIT_RETRY;
