 ...
 */
 DefinitionBlock (
    "PlatformRasGn.aml",            // Output file
    "SSDT",                         // Signature
    0x99,                           // SSDT Revision
    "AMD",                          // OEM ID
    "NVDIMM-N",                       // OEM Table ID
    0x77                             // OEM Revision
    )
{
Scope (_SB)
{
  ...
  // _DSM helper function for Root NVDIMM device
  // Arg0: _DSM Arg0 - UUID (see Section 5.2.4) (set to 2f10e7a4-9e91-11e4-89d3-123b93f75cba)
  // Arg1: _DSM Arg2 - Function Index
  // Arg2: _DSM Arg3 - a package containing parameters for the function specified by the UUID, Revision ID and Function Index.
  Method (RDSM, 3, Serialized)
  {
    If (LEqual (Arg0, ToUUID("2F10E7A4-9E91-11E4-89D3-123B93F75CBA")))
    {
      If (LEqual (RSMI, 0))
      {
        // If SW SMI number is 0, return Not Supported
        Return (Buffer () {1})
      }

      Switch (ToInteger(Arg1))        // Arg1: Function Index
      {
        // Query command implemented (Function Index 0)
        Case (0)
        {
          // !!!! Todo !!!! - ARS is not fully supported
          // Return (Buffer () {0x1FF})  //Support function 0 - 9
          Return (Buffer () {0x1F}) //Support function 0 - 4
        }
        ...
        Default
        {
          // If not function 0 - 9, return Not Supported
          Return (Buffer () {1})
        }
      }

      Store (ToInteger (Arg1), RFUN)  // Pass Function Index to SMM

      // If input data then extract it to ARS_ACPI_SMM_DATA input data area
      If (LGreater(Local0, 0))
      {
        Store (Local0, RILN)          // Size of input data
        CreateField (DeRefOf (Index (Arg2, 0)), 0, Multiply (RILN, 8), IBUF)
        Mid (IBUF, 0, RILN, RIDT)
      }

      // Trigger DSM SMI
      Store (RSMI, SMIW)

      Name (RBUF, Buffer (ROLN) {})   // Return buffer

      // If output data then extract it from ARS_ACPI_SMM_DATA output data area to return buffer
      If (LGreater(ROLN, 0))
      {
        CreateField (RBUF, 0, Multiply (ROLN, 8), OBUF)
        Mid (RODT, 0, ROLN, OBUF)
      }
...
*/
  ...
  OperationRegion(DRSD, SystemMemory, 0x44535241, 0x217)   // 0x44535241 = ('A', 'R', 'S', 'D')
  Field(DRSD, ByteAcc, NoLock, Preserve)
  {
    RFUN, 32,     // Function Index 0 - 9
    RILN, 32,     // Size of input data
    RIDT, 2048,   // Input data (bits)
    ROLN, 32,     // Size of output data
    RODT, 2048,   // Output data (bits)
    RDEV, 16,     // NFIT Device Handle (_ADR)
    RBMP, 32,     // NVDIMM Bitmap
    RSMI, 8,      // ARS_SW_SMI_CMD
    RERR, 32
  }
}
