...
#define FILECODE JEDECNVDIMM_SMM_JEDECNVDIMMACPI_FILECODE
    ...
    )
{
    ...
    *TotalNvdimm = 0;
    ...
    DEBUG ((EFI_D_ERROR, "Nvdimm Map (32 DIMM slots) = 0x%08X\n", gNvdimmBitMap));
    ...
    for (i = 0; i < *TotalNvdimm; i++) {
        ...
        SpaRangeStructPtr->ProximityDomain = GetProximityDomain (LShiftU64 ((UINT64) NvdimmInfo->NvdimmAddrSize[i][0], 16), &StdHeader);
        SpaRangeStructPtr->AddrRangeTypeGUID = EFI_BYTE_ADDRESSABLE_PERSISTENT_MEMORY_REGION;
        SpaRangeStructPtr->SpaRangeBase = LShiftU64 ((UINT64) NvdimmInfo->NvdimmAddrSize[i][0], 16);     // Convert address to byte
        SpaRangeStructPtr->SpaRangeLength = LShiftU64 ((UINT64) NvdimmInfo->NvdimmAddrSize[i][1], 16);   // Convert size to byte
        SpaRangeStructPtr->AddrRangeMemMappingAttr = EFI_MEMORY_UC;

        NfitTypePtr += (sizeof (SPA_RANGE_STRUCT));

#if PRINT_TABLES
        DEBUG ((EFI_D_ERROR, "\n\rSystem Physical Address (SPA) Range Structure\n"));
    ...
    for (i = 0; i < *TotalNvdimm; i++) {
        ...
        for (j = 204; j <= 218; j += 2) {
            ...
            FunctionInterfaceDescriptor |= ((UINT16) Data << 8);
...
//      NvdimmSpdInfo++;    // Point to next NVDIMM SPD data
...
#if PRINT_TABLES
        DEBUG ((EFI_D_ERROR, "\n\rNVDIMM Control Region Structure\n"));
    ...
    for (i = 0; i < *TotalNvdimm; i++) {
        ...
        RegionMappingStructPtr->NvdimmRegionSize = LShiftU64 ((UINT64) NvdimmInfo->NvdimmAddrSize[i][1], 16);                // Convert size to byte
...
#if PRINT_TABLES
        DEBUG ((EFI_D_ERROR, "\n\rNVDIMM Region Mapping Structure\n"));
...
#if PRINT_TABLES
    DEBUG ((EFI_D_ERROR, "\n\rSMBIOS Management Information Structure\n"));
    DEBUG ((EFI_D_ERROR, "Type      %02X\n", SmbiosMgmtInfoStructPtr->Type));
#endif
    LibAmdMemCopy ((VOID*)SmbiosMgmtInfoStructHandlePtr,
                    (VOID *) gSmbiosHandles, (UINTN)(sizeof(UINT16) * gTotalSmbiosHandles), &StdHeader);
#if PRINT_TABLES
    ...
    )
{
    ...
    // - The first entry: base=0, length=?
    // - The 2nd entry: base=0x1_0000_0000, length=?
    // - Search SRAT entry from 2nd entry
    ...
    while (TRUE) {
        ...
        if ((DimmBase >= MemoryBase) && (DimmBase < (MemoryBase + MemorySize))) {
                ...
                if (Smbios->GetNext(Smbios, &SmbiosHandle, &Type, &Record, NULL) == EFI_SUCCESS) {
                    ...
                    if (T17->MemoryType == Ddr4MemType) {   // Ignore the empty entry
                        ...
                        if (SerialNumber == SpdSerialNumber) {
                          DEBUG ((EFI_D_ERROR, "Type 17 handle = %x, Record = %x, SerialNumber = %08x\n", SmbiosHandle, Record, SerialNumber));
    ...
    )
{
    ...
    Type17Ptr += Record->Length;  // Move pointer to the first string
