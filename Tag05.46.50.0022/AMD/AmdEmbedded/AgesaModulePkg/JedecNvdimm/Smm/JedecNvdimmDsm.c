 ...
 *----------------------------------------------------------------------------------------
 */
#define FILECODE JEDECNVDIMM_SMM_JEDECNVDIMMDSM_FILECODE
    ...
    )
{
    ...
    DSM_DEBUG ((EFI_D_ERROR, "Socket %d Channel %d Dimm %d, Arm Timeout = %d\n", Socket, Channel, Dimm, TimeoutUs));
    StartTimeout(&SmmTimeout, TimeoutUs);
    ...
    ReadNvdimm(Socket, Channel, Dimm, 0, PAGE0_NVDIMM_CMD_STATUS0, &Data, FALSE);
    if (!(Data & BIT6)) {
        ReadNvdimm(Socket, Channel, Dimm, 0, PAGE0_ARM_STATUS0, &Data, FALSE);
        if (!(Data & BIT0)) {  // ARM_SUCCESS still 0
    ...
    )
{
    ...
    NvdimmBitMap = pDsmAcpiSmmData->NvdimmBitMap;
    if (0 == (NvdimmBitMap & (((1 << Dimm) << Channel * 2) << Socket * 16))) {
      DEBUG ((EFI_D_ERROR, "Return EFI_INVALID_PARAMETER due to No NVDIMM at this Socket/Channel/Dimmn\n"));
    ...
    switch (pDsmAcpiSmmData->FunctionIndex) {
        ...
        case 22:
            ...
            ((UINT32 *)pDsmAcpiSmmData->OutputData)[0] = DsmStatus;
            pDsmAcpiSmmData->OutputDataSize = sizeof(UINT32);
            DSM_DEBUG ((EFI_D_ERROR, "DSM: Status = 0x%x\n", DsmStatus));
            break;
        ...
        case 27:
            ...
            if (Page > 3) {
                ...
                Status = ReadNvdimm(Socket, Channel, Dimm, Page, Offset, &Data, FALSE);
                if (EFI_ERROR(Status)) DSM_DEBUG ((EFI_D_ERROR, "DSM: ReadNvdimm Status = %r\n", Status));
        ...
        case 28:
            ...
            if (Page > 3) {
                ...
                Status = WriteNvdimm(Socket, Channel, Dimm, Page, Offset, &Data, FALSE);
                if (EFI_ERROR(Status)) DSM_DEBUG ((EFI_D_ERROR, "DSM: WriteNvdimm Status = %r\n", Status));
            }
            ((UINT32 *)pDsmAcpiSmmData->OutputData)[0] = DsmStatus;
            pDsmAcpiSmmData->OutputDataSize = sizeof(UINT32);
            DSM_DEBUG ((EFI_D_ERROR, "DSM: Status = 0x%d\n", DsmStatus));
    ...
    )
{
    ...
    SwContext.SwSmiInputValue = JEDEC_NVDIMM_SMI_CMD;
