 ...
 *----------------------------------------------------------------------------------------
 */
#define FILECODE LIBRARY_FCHSPIACCESSLIB_FCHSPIACCESSSMNV2LIB_FILECODE
...
{
  UINT32    SmnBank[FCH_ROM_SMN_MAX_BANK];
  UINT32    SmnPage[FCH_ROM_SMN_MAX_BANK];
  UINT32    BankCount = 0;
  ...
  BankCount = FCH_ROM_SMN_MAX_BANK;

  while (BankCount > 0) {
    BankCount--;
    SmnBank[BankCount] = FCH_ROM_SMN_START_ADDRESS + FCH_ROM_SIZE_16M * BankCount;
    SmnPage[BankCount] = SearchSmnPage(SmnBank[BankCount]);
    if ((SmnPage[BankCount] & (BIT24 | BIT25)) == (Offset & (BIT24 | BIT25))){
      *Bank = SmnBank[BankCount];
      return TRUE;
    }
  }
  return FALSE;
...
* @param[in] Location - the address of host SMN address window
...
)
{
  ...
  StartAddr = Location;
  EndAddr = Location + Length - 1;

  if (StartAddr % BIT2){
    StartAlign = StartAddr & ~(BIT1 | BIT0);
  ...
  DataCount = 0;
  for (Address = StartAlign; Address <= EndAlign; Address += sizeof(UINT32)){
  ...
  )
{
  ...
  if (Rom32MbSupported()) {
    if (CheckSmnBank((UINT32)Offset, &SmnBank)) {
      if (CheckSmnOffset((UINT32)(Offset & FCH_ROM_RANGE_MASK), Length)) {
        SmnBank += (UINT32)(Offset & FCH_ROM_RANGE_MASK);
        Status = FchSpiRomSmnRead(0x00, SmnBank, Data, Length);
      } else {
        Status = FALSE;
      }
    }
  ...
  )
{
  ...
  if (Rom32MbSupported()) {
    ...
    if ((sizeof(UINTN) == sizeof(UINT64)) && (CheckRom3Window((UINT64)Location, Length))) {
      ...
      CopyMem(Data, (VOID *)Location, Length);
      Status = TRUE;
    } else if ((Location < FCH_ROM_SIZE_64M) && (CheckSmnBank((UINT32)Location, &SmnBank))) {
      if (CheckSmnOffset((UINT32)(Location & FCH_ROM_RANGE_MASK), Length)) {
        SmnBank += (UINT32)(Location & FCH_ROM_RANGE_MASK);
        Status = FchSpiRomSmnRead(0x00, SmnBank, Data, Length);
      }
    }else{
      Status = FALSE;
