 ...
 *----------------------------------------------------------------------------------------
 */
#define FILECODE LIBRARY_FCHSPIACCESSLIB_FCHSPIACCESSROM3V2LIB_FILECODE
  ...
  )
{
  ...
  if (Rom32MbSupported()) {
    if (CheckRom3Bank((UINT32)Offset, &Rom3Bank)) {
      if (CheckRom3Offset ((UINT32)(Offset & FCH_ROM_RANGE_MASK), Length)) {
        Rom3Bank += (UINT32)(Offset & FCH_ROM_RANGE_MASK);
        CopyMem(Data, (VOID *)Rom3Bank, Length);
        Status = TRUE;
      } else {
        Status = FALSE;
      }
  ...
  )
{
  ...
  if (Rom32MbSupported()) {
    if (CheckRom3Window((UINT64)Location, Length)) {
      ...
      if (CheckRom3Offset ((UINT32)(Location & FCH_ROM_RANGE_MASK), Length)) {
        Rom3Bank += (UINT32)(Location & FCH_ROM_RANGE_MASK);
        CopyMem(Data, (VOID *)(UINTN) Rom3Bank, Length);
        Status = TRUE;
      } else {
        Status = FALSE;
      }
