...
#define FILECODE LIBRARY_FABRICRESOURCEMANAGERDF3RMBLIB_FABRICRESOURCEINIT3_FILECODE
  ...
  )
{
  ...
  // get current total number of rootbridges & compare to saved number
  ...
  //if ((SetResourceBaseOnNv) && (!PcdGetBool (PcdAmdMCTPEnable))) {
  ...
  )
{
  ...
  //    |                                 |          If PcdAmdBottomMmioReservedForPrimaryRb == BottomOfCompat, not preserve the region by PCD first
  //    +---------------------------------+ +--+ 0xFEC0_0000 (PcdAmdBottomMmioReservedForPrimaryRb), from TOM to PcdAmdBottomMmioReservedForPrimaryRb is the region that AllocateMmio function could use
  //    |  MMIO Above PCIe Cfg            |
  //    |                                 |
  //    |                                 |
  //    +---------------------------------+ +--+ PcdPciExpressBaseAddress + Size (defined by MSRC001_0058[5:2][BusRange])
  //    |  PCIe Configuration Space       |
  //    |                                 |
  //    +---------------------------------+ +--+ PcdPciExpressBaseAddress (defined by MSRC001_0058[47:20][MmioCfgBaseAddr]) (recommend to set PcdPciExpressBaseAddress equal to TOM)
  //    |                                 |
  //    |  MMIO Below PCIe Cfg            |
  //    |                                 |
  //    |                                 |
  ...
  FabricMmioManager->MmioRegionBelow4G[i][j].BaseNonPci = (MmioBaseAddr + MmioSizeRemained - FabricMmioManager->MmioRegionBelow4G[i][j].SizeNonPci) & (~NON_PCI_MMIO_ALIGN_MASK);
  ...
  if (SystemRbNumber < (RMB_MAX_SYSTEM_RB_COUNT + PRIMARY_RB_HAS_2ND_MMIO)) {
...
//[-start-220920-IB14740263-modify]//
            FabricMmioManager->MmioRegionBelow4G[i][j].SizePrefetch = 0;
            FabricMmioManager->MmioRegionBelow4G[i][j].SizeNonPrefetch = MmioSize;
            FabricMmioManager->MmioRegionBelow4G[i][j].BasePrefetch = 0;
            FabricMmioManager->MmioRegionBelow4G[i][j].BaseNonPrefetch = MmioBaseAddr;
//[-end-220920-IB14740263-modify]//
    ...
    for (i = 0; i < SocketNumber; i++) {
      for (j = 0; j < RbsPerSocket; j++) {
        ...
        if ((i == (SocketNumber - 1)) && (j == (RbsPerSocket - 1))) {
          ...
          FabricSetMmioReg3 (SocketNumber, ((i * RbsPerSocket + j) * 2 + 1), i, j, MmioBaseAddr, (MmioLimitAbove4G - MmioBaseAddr));
        } else {
        ...
        // Prefetchable/Non Prefetchable 80/20 Ratio
        FabricMmioManager->MmioRegionAbove4G[i][j].BaseNonPci = (MmioBaseAddr + NON_PCI_MMIO_ALIGN_MASK_ABOVE4G) & (~NON_PCI_MMIO_ALIGN_MASK_ABOVE4G);
