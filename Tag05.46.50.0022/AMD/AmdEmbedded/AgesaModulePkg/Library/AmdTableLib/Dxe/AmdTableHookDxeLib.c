...
#define FILECODE LIBRARY_AMDTABLELIB_DXE_AMDTABLEHOOKDXELIB_FILECODE
  ...
  )
{
  if (!mLogicalCpuidInited) {
    if (SocLogicalIdProtocol == NULL) {
      gBS->LocateProtocol (&gAmdSocLogicalIdProtocolGuid, NULL, &SocLogicalIdProtocol);
    }
  ...
  )
{
  ...
  if (!mPlatformFeatsInited) {
    ...
    GetBasePlatformFeatures (StdHeader, &SocLogicalId, &mPlatformFeats);
    mApicMode = PcdGet8 (PcdAmdApicMode);
  ...
  if (mApicMode == ApicCompatibilityMode) {
    // get Local APIC ID
    AsmCpuid (
        0x00000001,
        &(CpuId.EAX_Reg),
