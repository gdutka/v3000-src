...
#define FILECODE LIBRARY_AMDUSB4LIB_COMMON_FILECODE

#define USBCRCSWAP(x) ( x + (sizeof(UINT32)-1) - ((x % 4) * 2) )

GLOBAL_REMOVE_IF_UNREFERENCED CONST UINT32 mCRC32_table[256] = {
  0x00000000,
  0xF26B8303,
  0xE13B70F7,
  0x1350F3F4,
  ...
  )
{
  ...
  for ( BufferIndex = 0; BufferIndex != len; ++BufferIndex ) {
    Index = (CRC32 ^ buffer[USBCRCSWAP (BufferIndex)]) & 0xFF;
    CRC32 = ((CRC32 >> 8) & 0xFFFFFF) ^ mCRC32_table[Index];
  }
  ...
  )
{
  ...
  switch (CrcLength) {
    case 32:
      Crc32 = CalculateCrc32 ((UINT8 *)pBuffer, DataSize);
      *CrcOut = Crc32;
      if (BitSwap) {
        NewCrc = ((Crc32 & 0x000000ff) << 24) |
                 ((Crc32 & 0x0000ff00) << 8) |
                 ((Crc32 & 0x00ff0000) >> 8) |
                 ((Crc32 & 0xff000000) >> 24);
        *CrcOut = NewCrc;
      ...
      return EFI_SUCCESS;
    case 16:
      *CrcOut = (UINT32)CalculateCheckSum16 ((UINT16*)pBuffer, DataSize);
      return EFI_SUCCESS;
  }
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (MAIN_FLOW, "     00  01  02  03  04  05  06  07  08  09  0A  0B  0C  0D  0E  0F\n");
  IDS_HDT_CONSOLE (MAIN_FLOW, "--------------------------------------------------------------------");
  ...
  for (i=0; i<iSize; i++) {
      ...
      if ( i != 0 ) {
        for (k=0; k<16; k++) {
          if ( arr[k] >= 0x21 && arr[k] <= 0x7e ) {
            IDS_HDT_CONSOLE (MAIN_FLOW, "%c", arr[k]);
          } else {
            IDS_HDT_CONSOLE (MAIN_FLOW, " ");
          }
        }
    ...
    arr[j] = ptr[i];
    j++;
    IDS_HDT_CONSOLE (MAIN_FLOW, "%02X  ", ptr[i]);
  }
  IDS_HDT_CONSOLE (MAIN_FLOW, "\n====================================================================\n");
}
...
GetACPITimer ()
{
  ...
  if ( timerAddr == 0 ) {
    ...
    LibAmdIoRead (AccessWidth32, timerAddr, &tick, NULL);
    return tick;
  }
}
...
{
  UINT64  TicksNeeded = 0;
  UINT64  EndValue    = 0;
  UINT16  timerAddr = 0;

#if AMD_USB4_DEBUG_WAIT_OV_INFINITE_TIMEOUT
  Timeout->IsInfinite = ( Time == (UINTN)(-1) ) ? TRUE : FALSE;
#endif

  LibAmdMemRead (AccessWidth16, (UINT64) (ACPI_MMIO_BASE + PMIO_BASE + FCH_PMIOA_REG64), &timerAddr, NULL);
  if ( timerAddr == 0 ) {
    // We suppose this never happen
    IDS_HDT_CONSOLE_USB4_WARNING ("ACPI timer is not configured.\n");
    return EFI_SUCCESS;
  }
  ...
  // Read ACPI Timer
  LibAmdIoRead (AccessWidth32, timerAddr, &(Timeout->OldTimerValue), NULL);
  ...
  )
{
  ...
  // If Timout is not set by StartTimout, just return expired.
  if ( Timeout->OldTimerValue == 0 || Timeout->EndValue == 0 ) {
    return EFI_TIMEOUT;
  }
  ...
  // Read ACPI Timer
  LibAmdIoRead (AccessWidth32, timerAddr, &TimerValue, NULL);
  ...
  if (TimerValue >= Timeout->EndValue) {
    return EFI_TIMEOUT;
  }
