...
#define FILECODE LIBRARY_MEM_PSLIB_MP_FILECODE
  ...
  )
{
  ...
  if (BypassedDimmSlotsPtr != NULL) {
    BypassedDimmSlots = *BypassedDimmSlotsPtr;
  }
  ...
  NBPtr->PsPtr->NumOfDimmSlots -= BypassedDimmSlots;
  NBPtr->PsPtr->DimmType = (UINT8) DimmType;
  NBPtr->PsPtr->RankType = MemPGetPsRankType (CurrentChannel);
  NBPtr->PsPtr->RankType = (NBPtr->PsPtr->RankType >> (BypassedDimmSlots << 2));
    ...
    if (((ListOfTables[i])->Header.DimmType & NBPtr->PsPtr->DimmType) != 0) {
      if (((ListOfTables[i])->Header.NumOfDimm & NBPtr->PsPtr->NumOfDimmSlots) != 0) {
        if ((ListOfTables[i])->Header.TechType == NBPtr->ChannelPtr->TechType) {
          if (!NBPtr->IsSupported[DqsRoutingTypeDependence] || ((ListOfTables[i])->Header.DqsRoutingType & NBPtr->DqsRoutingType) != 0) {
            //
            // Determine if this is the expected NB Type
            //
