...
#define FILECODE LIBRARY_MEM_FEATDMICOLLECTLIB_AMDMEMDMILIB_FILECODE
/*----------------------------------------------------------------------------
  ...
  for (Socket = 0; Socket < MaxSockets; Socket ++) {
    ...
    for (Channel = 0; Channel < MaxChannelsPerSocket; Channel ++) {
      ...
      for (Dimm = 0; Dimm < MaxDimmsPerChannel; Dimm ++, SpdDataStructure ++, LocalHandle++) {
        ...
        DmiPhysicalDimmInfoTable->MinimumVoltage = 0;
        DmiPhysicalDimmInfoTable->MaximumVoltage = 0;
        DmiPhysicalDimmInfoTable->ConfiguredVoltage = 0;
        ...
        if (SpdDataStructure->DimmPresent) {
          ...
          temp = (UINT8) SpdDataStructure->Data[4] & 0xF;
          ASSERT (temp <= 6);
          Capacity = 0x100 << temp;

          temp = (UINT8) SpdDataStructure->Data[8] & 0x7;
          ASSERT (temp <= 3);
          BusWidth = 8 << temp;

          temp = (UINT8) SpdDataStructure->Data[7] & 0x7;
          ASSERT (temp <= 3);
          Width = 4 << temp;

          temp = (UINT8) (SpdDataStructure->Data[7] >> 3) & 0x7;
          ASSERT (temp <= 4);
          Rank = DmiPhysicalDimmInfoTable->Attributes = (temp < 4) ? (temp + 1) : 8;
          ...
          DmiPhysicalDimmInfoTable->MinimumVoltage = CONVERT_ENCODED_TO_VDDIO_MILLIVOLTS (HighBitSet32 (VoltageMap), DDR3_TECHNOLOGY);
          DmiPhysicalDimmInfoTable->MaximumVoltage = CONVERT_ENCODED_TO_VDDIO_MILLIVOLTS (LowBitSet32 (VoltageMap), DDR3_TECHNOLOGY);
          switch (NBPtr->RefPtr->DDRVoltage) {
          case VOLT1_5:
            DmiPhysicalDimmInfoTable->ConfiguredVoltage = 1500;
            break;
          case VOLT1_35:
            DmiPhysicalDimmInfoTable->ConfiguredVoltage = 1350;
            break;
          case VOLT1_25:
            DmiPhysicalDimmInfoTable->ConfiguredVoltage = 1250;
            break;
          default:
            DmiPhysicalDimmInfoTable->ConfiguredVoltage = 1500;
            break;
  ...
  )
{
  ...
  for (Node = 0; Node < NodeLimit; Node++) {
    NodeMemBase += LShiftU64 (NBPtr[Node].MCTPtr->NodeMemSize, 6);
  }
