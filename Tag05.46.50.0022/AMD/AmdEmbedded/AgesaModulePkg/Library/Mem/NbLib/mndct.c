...
#define FILECODE LIBRARY_MEM_NBLIB_MNDCT_FILECODE
/*----------------------------------------------------------------------------
  ...
  )
{
  ...
  if (!MemoryAllClocks) {
    ...
    if (!NBPtr->MCTPtr->Status[SbDiagClks]) {
      MemClkDisMap = FindPSOverrideEntry (RefPtr->PlatformMemoryConfiguration, PSO_MEMCLK_DIS, NBPtr->MCTPtr->SocketId, NBPtr->Dct, 0,
  ...
  )
{
  ...
  if (Limit < (1 << (34 - 27))) {
    ...
    Size = Limit - Base;
    SizeOfAlign = (UINT32) 1 << LowBitSet32 (Base);
  ...
  )
{
  ...
  OrgMMIOCfgBase = AsmReadMsr64 (MSR_MMIO_Cfg_Base);
  NewMMIOCfgBase = OrgMMIOCfgBase & (~(BIT0));
  AsmWriteMsr64 (MSR_MMIO_Cfg_Base, NewMMIOCfgBase);
  ...
  AsmWriteMsr64 (MSR_MMIO_Cfg_Base, OrgMMIOCfgBase);
  ...
  )
{
  ...
  TargetNbVid = (UINT8)MemNGetBitFieldNb (NBPtr, BFNbVid0 + (NBPstate << 1));
  TargetNbFid = (UINT8)MemNGetBitFieldNb (NBPtr, BFNbFid0 + NBPstate);
  TargetNbDid = (UINT8)MemNGetBitFieldNb (NBPtr, BFNbDid0 + NBPstate);

  MemNPollBitFieldNb (NBPtr, BFCurNbPstate, NBPstate, PCI_ACCESS_TIMEOUT, FALSE);
  MemNPollBitFieldNb (NBPtr, BFCurNbFid, TargetNbFid, PCI_ACCESS_TIMEOUT, FALSE);
  MemNPollBitFieldNb (NBPtr, BFCurNbDid, TargetNbDid, PCI_ACCESS_TIMEOUT, FALSE);
}
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (MEM_FLOW, "\nLast NBPState-->NBP%d  NBCLK: %dMHz", FabricPstatesGetCurrentPstateOnDie (NBPtr->MCTPtr->SocketId, NBPtr->MCTPtr->DieId), NBPtr->NBClkFreq);
  NBPtr->GetMemClkFreqInCurrentContext (NBPtr);

  // Make sure target NB Pstate is enabled, else find next enabled NB Pstate
  NbPstateMaxVal = (UINT8)FabricPstatesGetNumberOfPstatesOnDie (NBPtr->MCTPtr->SocketId, NBPtr->MCTPtr->DieId) - 1;
  if (NBPtr->NbFreqChgState == 0) {
    ...
    NBPtr->NBClkFreq = (UINT32)NbPstateInfo.Frequency;
    return TRUE;
  } else if (NBPtr->NbFreqChgState <= NbPstateMaxVal) {
    // While in other states, go to the next lower NB Pstate
    TargetNbPs = (UINT8) FabricPstatesGetCurrentPstateOnDie (NBPtr->MCTPtr->SocketId, NBPtr->MCTPtr->DieId) + 1;
  } else {
    // When done with training, release NB Pstate force by restoring NbPsCtl register
    FabricPstatesReleaseForceOnDie (NBPtr->MCTPtr->SocketId, NBPtr->MCTPtr->DieId);
    IDS_HDT_CONSOLE (MEM_FLOW, "\tRelease NB Pstate force\n");
    return TRUE;
  }

  if (TargetNbPs <= NbPstateMaxVal) {
    FabricPstatesGetPstateInfo (NBPtr->MCTPtr->SocketId, NBPtr->MCTPtr->DieId, TargetNbPs, &NbPstateInfo);
    NBPtr->NBClkFreq = (UINT32)NbPstateInfo.Frequency;

    IDS_HDT_CONSOLE (MEM_FLOW, "\nChange NBPState-->NBP%d  NBCLK: %dMHz", TargetNbPs, NBPtr->NBClkFreq);
    ...
    if (FabricPstatesIsSwitchingEnabledOnDie (NBPtr->MCTPtr->SocketId, NBPtr->MCTPtr->DieId)) {
      ...
      FabricPstatesForcePstateOnDie (NBPtr->MCTPtr->SocketId, NBPtr->MCTPtr->DieId, TargetNbPs);
    }

    // When NB frequency change succeeds, TSC rate may have changed.
    // We need to update TSC rate
    CcxGetPstateInfo (0, SwPstate0, (UINTN *)&NBPtr->MemPtr->TscRate, &VoltageInuV, &PowerInmW, &NBPtr->MemPtr->StdHeader);
    // Switch MemPstate context if the current MemPstate does not sync with MemPstate context
    if ((UINT8)NbPstateInfo.AssociatedMstate != NBPtr->MemPstate) {
      MemNChangeMemPStateContextNb (NBPtr, (UINT8)NbPstateInfo.AssociatedMstate);
    }
  } else {
