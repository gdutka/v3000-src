...
#define FILECODE LIBRARY_MEM_TECHLIB_MTTDIMBT_FILECODE
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (MEM_FLOW, "\n\t\tCalculating worst-case round trip delay on channel %d\n", NBPtr->Dct);
  IDS_HDT_CONSOLE (MEM_FLOW, "\t\t Max Delay = DQS + MAX(DQ_L, DQ_H). *indicates new max value\n");
  IDS_HDT_CONSOLE (MEM_FLOW, "\t\t                      ------- RxDly -------\n");
  IDS_HDT_CONSOLE (MEM_FLOW, "\t\tDimm     ByteLane     DQS     DQ_L     DQ_H     Max Delay\n");
  for (ChipSelect = 0; ChipSelect < NBPtr->CsPerChannel; ChipSelect = ChipSelect + NBPtr->CsPerDelay) {
  ...
  IDS_HDT_CONSOLE (MEM_FLOW, "\t\tMax Delay Value is %02x on Chipselect %d.\n\n", MaxDly, MaxDlyCs);
  TechPtr->MaxDlyForMaxRdLat = MaxDly;
  *ChipSel = MaxDlyCs;
  return RetVal;
}
