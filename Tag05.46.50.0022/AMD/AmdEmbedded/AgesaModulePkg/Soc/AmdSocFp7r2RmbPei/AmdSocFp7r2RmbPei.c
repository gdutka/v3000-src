...
EFI_STATUS
EFIAPI
FabricResourceInitNotifyCallback (
...
//[-start-240806-IB20840078-add]//
STATIC EFI_PEI_NOTIFY_DESCRIPTOR mFabricResourceInitNotify = {
  ...
  )
{
  ...
  DEBUG ((EFI_D_ERROR, "*****************************PEI SOC FP7r2 RMB Driver Entry*********************\n"));
  ...
  if (SocHardwareIdentificationCheck (&SocId) || SocHardwareIdentificationCheck (&SocId2)) {
    ...
    // FabricResourceInit ();
//[-start-240806-IB20840078-add]//
    DEBUG((DEBUG_INFO, "FEA-PEI-%a-NotifyPpi-gPlatformCustomizePeiInitReadyPpiGuid-For-FabricResourceInitNotifyCallback\n", __FUNCTION__));
    Status = (**PeiServices).NotifyPpi (PeiServices, &mFabricResourceInitNotify);
//[-end-240806-IB20840078-add]//
    //Init APOB service
...
///@todo    FabricSocSpecificServicesPpiInstall (PeiServices);
  ...
  DEBUG ((EFI_D_ERROR, "*****************************PEI SOC FP7r2 RMB Driver Exit*********************\n"));
  return (Status);
}
