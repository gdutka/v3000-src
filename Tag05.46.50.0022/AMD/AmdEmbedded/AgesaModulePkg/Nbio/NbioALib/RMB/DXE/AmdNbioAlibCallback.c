...
#define FILECODE NBIO_NBIOALIB_RMB_DXE_AMDNBIOALIBCALLBACK_FILECODE
/*----------------------------------------------------------------------------------------
  ...
  )
{
  ...
  if (AmlObjPtr != NULL) {
    ...
    IDS_HDT_CONSOLE (GNB_TRACE, "Sizeof(ALIB_DATA)=%d, PeApmEnable = %x\n", sizeof (ALIB_DATA),\
                                ((ALIB_DATA *) AmlObjPtr)->Data.Data.PeApmEnable);
  ...
  Status = AcpiTable->InstallAcpiTable (AcpiTable, AlibSsdtBuffer,((ACPI_TABLE_HEADER*) AlibSsdtBuffer)->TableLength,
                                                                                                     &TableHandle);
  ...
  )
{
  ...
  BridgeNumber = (Engine->Type.Port.CoreId * 5) + Engine->Type.Port.PortId;
  PortData = &((ALIB_DATA *) Buffer)->PortData[BridgeNumber].PortData;

  if (PcieConfigIsEngineAllocated (Engine) && (Engine->Type.Port.PortData.LinkHotplug != HotplugDisabled ||
