...
#define FILECODE MPM_MPMKVM_MPMKVMKBDTEXTIN_FILECODE
UINT32 gMpmKbWritePointer = 0;
UINT32 gMpmKbReadPointer  = 0;
...
{
  //WritePointer is maintianed by MPM, ReadPointer is maintianed by BIOS
  //BIOS not trust KbBuffer as malicious MPM may tamper ringbuffers
  gMpmKbWritePointer = Queue->KbBuffer->WritePointer;
  return (BOOLEAN) (gMpmKbWritePointer == gMpmKbReadPointer);
}
  ...
  )
{
  ...
  if (KeyData != NULL) {
    KvmSyncKeyboardState((Queue->KbBuffer->Data[gMpmKbReadPointer].KeySym & 0xFFFF), &(KeyData->KeyState));
    KvmScanCodeTransfer((UINT16)(Queue->KbBuffer->Data[gMpmKbReadPointer].KeySym & 0xFFFF), &(KeyData->Key));
    KvmMagicKey((UINT16)(Queue->KbBuffer->Data[gMpmKbReadPointer].KeySym & 0xFFFF), ConsoleInDev);
  }
  gMpmKbReadPointer = (gMpmKbReadPointer + 1 == KVM_KEYBOARD_KEY_MAX_COUNT) ? 0 : gMpmKbReadPointer + 1;
  Queue->KbBuffer->ReadPointer = gMpmKbReadPointer;
  return EFI_SUCCESS;
}
...
EFI_STATUS
KvmKeyboardComboKeyCallback (
  OUT EFI_KEY_DATA                      *KeyData
  )
{
  BOOLEAN KeyControlPressed = FALSE;
  BOOLEAN KeyAltPressed = FALSE;
  BOOLEAN KeyShiftPressed = FALSE;
  UINT32 KeyShiftState = KeyData->KeyState.KeyShiftState;
  if ((KeyShiftState & (UINT32)(EFI_LEFT_ALT_PRESSED + EFI_RIGHT_ALT_PRESSED)) != 0) {
    KeyAltPressed = TRUE;
  }
  if ((KeyShiftState & (UINT32)(EFI_LEFT_CONTROL_PRESSED + EFI_RIGHT_CONTROL_PRESSED)) != 0) {
    KeyControlPressed = TRUE;
  }
  if ((KeyShiftState & (UINT32)(EFI_LEFT_SHIFT_PRESSED + EFI_RIGHT_SHIFT_PRESSED)) != 0) {
    KeyShiftPressed = TRUE;
  }
  if (KeyData->Key.ScanCode == SCAN_DELETE &&
...

{
  ...
  KvmKeyboardComboKeyCallback (KeyData);
  ...
  )
{
  ...
  if (!ConsoleIn->KeyboardErr) {
    while (!IsKvmkeyBufEmpty (&ConsoleIn->KvmKeyQueue)) {
      ScanCode = (UINT16)(KbBuffer->Data[gMpmKbReadPointer].KeySym & 0xFFFF);
