...
#define FILECODE MPM_MPMPLDM_MPMPLDMBCC_BIOSATTRIBUTETABLE_FILECODE
/**
...
EFI_STATUS
EFIAPI
SetupGenericBiosAttributeTable (
  IN OUT BIOS_ATTRIBUTE_TABLE_BUFFER *BiosAttributeTableBuffer
  )
{
  EFI_STATUS Status;
  // TODO: Add customized OEM BIOS Attribute Table entries here.
  // BIOS Amdin Password
  Status = AppendBiosAttributeTable (
             BiosAttributeTableBuffer,
             PLDM_BIOS_STRING_HANDLE_BIOS_ADMIN_PASSWORD,
             PLDM_ATTRIBUTE_TYPE_BIOSPassword,
             PLDM_BIOS_STRING_HANDLE_BIOS_ADMIN_PASSWORD,
             BIOS_ATTRIBUTE_BOOT_CONFIG_TYPE_Default,
             BIOS_ATTRIBUTE_ORDERANDFAILTHROUGHMODE_OrderedAndFailThrough,
             PASSWORD_ENCODING_TYPE_ASCII,          //Password Encoding Type
             MIN_BIOS_PASSWORD_LENGTH,              //Minimum Password Length
             MAX_BIOS_PASSWORD_LENGTH,              //Maximum Password Length
             DEFAULT_BIOS_PASSWORD_LENGTH,          //Default Password Length
  ...
  )
{
  ...
  Status = SetupGenericBiosAttributeTable (BiosAttributeTableBuffer);
  if (EFI_ERROR (Status)) {
    IDS_HDT_CONSOLE_PSP_TRACE ("SetupGenericBiosAttributeTable failed\n");
    return Status;
  }
