...
SET SHATYPE=sha256
SET RSA_PSS_SALTLEN=-2
if /I "%6" == "sha384" (
  SET SHATYPE=sha384
  SET RSA_PSS_SALTLEN=-1
)
if /I "%5" == "TK" (
  IF "%OPENSSL_PATH%" == "" (
...
IF EXIST %BIOS_L2_DIR_B_FILE% del %BIOS_L2_DIR_B_FILE% /F /Q
...
::AB recovery BIOS does not have BIOS_L1_DIR_FILE
IF EXIST %BIOS_L1_DIR_FILE% (
...
IF EXIST %BIOS_L2_DIR_FILE% (
  IF EXIST %BIOS_L1_DIR_FILE% (
    ...
    @echo copy /b %RTM_FILE%+%BIOS_L2_DIR_FILE% %RTM_BIOSDIRL1_L2_COMBINE_FILE%
    copy /b %RTM_FILE%+%BIOS_L2_DIR_FILE% %RTM_BIOSDIRL1_L2_COMBINE_FILE%
  )
)
::Check if LV2 B DIR Binary exist, which imply the AB recovery support is enabled
IF EXIST %BIOS_L2_DIR_B_FILE% (
  IF EXIST %BIOS_L1_DIR_FILE% (
    @echo copy /b %RTM_FILE%+%BIOS_L1_DIR_FILE%+%BIOS_L2_DIR_B_FILE% %RTM_BIOSDIRL1_L2_B_COMBINE_FILE%
    copy /b %RTM_FILE%+%BIOS_L1_DIR_FILE%+%BIOS_L2_DIR_B_FILE% %RTM_BIOSDIRL1_L2_B_COMBINE_FILE%
  ) ELSE (
    @echo copy /b %RTM_FILE%+%BIOS_L2_DIR_B_FILE% %RTM_BIOSDIRL1_L2_B_COMBINE_FILE%
    copy /b %RTM_FILE%+%BIOS_L2_DIR_B_FILE% %RTM_BIOSDIRL1_L2_B_COMBINE_FILE%
  )
)
...
if /I "%5" == "TK" (
  ...
  IF EXIST %RTM_BIOSDIRL1_COMBINE_FILE% (
    @echo %OPENSSL% dgst -binary -sign %RTM_PRIVATE_KEY% -%SHATYPE% -sigopt rsa_padding_mode:pss -sigopt rsa_pss_saltlen:%RSA_PSS_SALTLEN% -out %RTM_FILE_L1_SIGNATURE% %RTM_BIOSDIRL1_COMBINE_FILE%
    %OPENSSL% dgst -binary -sign %RTM_PRIVATE_KEY% -%SHATYPE% -sigopt rsa_padding_mode:pss -sigopt rsa_pss_saltlen:%RSA_PSS_SALTLEN% -out %RTM_FILE_L1_SIGNATURE% %RTM_BIOSDIRL1_COMBINE_FILE%
    @echo reversestr.exe %RTM_FILE_L1_SIGNATURE%
    reversestr.exe %RTM_FILE_L1_SIGNATURE%
  )

  IF EXIST %RTM_BIOSDIRL1_L2_COMBINE_FILE% (
    @echo %OPENSSL% dgst -binary -sign %RTM_PRIVATE_KEY% -%SHATYPE% -sigopt rsa_padding_mode:pss -sigopt rsa_pss_saltlen:%RSA_PSS_SALTLEN% -out %RTM_FILE_L1_L2_SIGNATURE% %RTM_BIOSDIRL1_L2_COMBINE_FILE%
    %OPENSSL% dgst -binary -sign %RTM_PRIVATE_KEY% -%SHATYPE% -sigopt rsa_padding_mode:pss -sigopt rsa_pss_saltlen:%RSA_PSS_SALTLEN% -out %RTM_FILE_L1_L2_SIGNATURE% %RTM_BIOSDIRL1_L2_COMBINE_FILE%
    @echo reversestr.exe %RTM_FILE_L1_L2_SIGNATURE%
    reversestr.exe %RTM_FILE_L1_L2_SIGNATURE%
  )

  IF EXIST %RTM_BIOSDIRL1_L2_B_COMBINE_FILE% (
    @echo %OPENSSL% dgst -binary -sign %RTM_PRIVATE_KEY% -%SHATYPE% -sigopt rsa_padding_mode:pss -sigopt rsa_pss_saltlen:%RSA_PSS_SALTLEN% -out %RTM_FILE_L1_L2_B_SIGNATURE% %RTM_BIOSDIRL1_L2_B_COMBINE_FILE%
    %OPENSSL% dgst -binary -sign %RTM_PRIVATE_KEY% -%SHATYPE% -sigopt rsa_padding_mode:pss -sigopt rsa_pss_saltlen:%RSA_PSS_SALTLEN% -out %RTM_FILE_L1_L2_B_SIGNATURE% %RTM_BIOSDIRL1_L2_B_COMBINE_FILE%
    @echo reversestr.exe %RTM_FILE_L1_L2_B_SIGNATURE%
    reversestr.exe %RTM_FILE_L1_L2_B_SIGNATURE%
  )
  goto BLDBIOS
)
...
if /I "%5" == "PK" (
  ...
  @echo del %RTM_FILE_L1_L2_B_SIGNATURE% /F /Q
  IF EXIST %RTM_FILE_L1_L2_B_SIGNATURE% del %RTM_FILE_L1_L2_B_SIGNATURE% /F /Q
  IF EXIST %cd%\%RTM_BIOSDIRL1_COMBINE_FILE% (
  ...
  ::Generate Signature for RTM+L1+L2B
  IF EXIST %cd%\%RTM_BIOSDIRL1_L2_B_COMBINE_FILE% (
    @echo  SPClient.bat -in %cd%\%RTM_BIOSDIRL1_L2_B_COMBINE_FILE% -out %RTM_FILE_L1_L2_B_SIGNATURE% -sp_function "%SP_FUNCTION%"
    call SPClient.bat -in %cd%\%RTM_BIOSDIRL1_L2_B_COMBINE_FILE% -out %RTM_FILE_L1_L2_B_SIGNATURE% -sp_function "%SP_FUNCTION%"
  )

  @echo Copy back to Working path, and delete file in SPClient path
  IF EXIST %RTM_FILE_L1_SIGNATURE% (
    @echo copy /Y /B %RTM_FILE_L1_SIGNATURE% %TEMP_PATH%
    copy /Y /B %RTM_FILE_L1_SIGNATURE% %TEMP_PATH%
    @echo del %RTM_FILE_L1_SIGNATURE% /F /Q
    del %RTM_FILE_L1_SIGNATURE% /F /Q
  ) ELSE (
    IF NOT EXIST %RTM_FILE_L1_L2_SIGNATURE% (
      @echo sign failed
	  goto ERROR
	)
  )
  ...
  IF EXIST %RTM_FILE_L1_L2_B_SIGNATURE% (
    @echo copy /Y /B %RTM_FILE_L1_L2_B_SIGNATURE% %TEMP_PATH%
    copy /Y /B %RTM_FILE_L1_L2_B_SIGNATURE% %TEMP_PATH%
    @echo del %RTM_FILE_L1_L2_B_SIGNATURE% /F /Q
    IF EXIST %RTM_FILE_L1_L2_B_SIGNATURE% del %RTM_FILE_L1_L2_B_SIGNATURE% /F /Q
  )
