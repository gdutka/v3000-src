...
def ValidateCmnEntry (CmnEntry):
  ...
  if (IsImageEntry (CmnEntry)):
    ...
    if ((CmnEntry['Address'] == None) or (CmnEntry['Size'] == None)):
      log.info ("ERROR: Address or Size haven't been explicit defined for IMAGE_ENTRY")
      return False
...
def ParseCfgFile (CfgFile):
  ...
  return (PspDicts, BiosDicts, ComboDict, CmnDict)
...
def ParsePspDirBin (FileHandle, BinarySize, PspDirOffSetInBin, Level, PspDirectoryInfos):
  ...
  if (PspLevel2Addr > 0):
    ParsePspDirBin (FileHandle, BinarySize, PspLevel2Addr, 2, PspDirectoryInfos)
...
def main ():
  ...
  if (Args.subcommand == 'bd' or Args.subcommand == 'bb'):
    ...
    (PspDicts, BiosDicts, ComboDict, CmnDict) = ParseCfgFile (Args.CfgFile)
    if (len(PspDicts) != 0):
        ...
        for Entry in CmnDict['Entries']:
          if (IsImageEntry(Entry)):
            ...
            if(len (PspBinary) > Entry['Size']):
              log.info ("%s size is larger than defined entry size %x\n", Entry['File'], Entry['Size'])
              FatalErrorExit()
            # And patch the output image
