...
#define FILECODE PSP_AMDPSPROMARMOR2SMM_AMDPSPROMARMOR2SMM_FILECODE
  ...
  )
{
  ...
  for (ByteCounter = 0; ByteCounter < LengthInBytes;) {
    ...
    if (EFI_ERROR (Status)) {
      IDS_HDT_CONSOLE_PSP_TRACE ("SPI ReadData Error:%r\n", Status);
      ASSERT (FALSE);
      break;
    }
  ...
  )
{
  ...
  for (ByteCounter = 0; ByteCounter < LengthInBytes;) {
    ...
    Status = PspMboxBiosCmdArmorSpiTransction (&mMboxRomArmorCommandBuffer, mArmorLibPspMboxSmmFlagAddr);
    if (EFI_ERROR (Status)) {
      IDS_HDT_CONSOLE_PSP_TRACE ("SPI WriteData Error:%r\n", Status);
      ASSERT (FALSE);
      break;
    }
    ByteCounter += Length;
...
EFI_STATUS
RomArmor2SmmCommunicateHandler (
  ...
  )
{
  ...
  if (TempCommBufferSize < sizeof (PSP_SMM_COMMUNICATION_CMN) || TempCommBufferSize > AMD_PSP_SMM_TMP_BUFFER_SIZE) {
    IDS_HDT_CONSOLE_PSP_TRACE ("RomArmor2SmmCommunicateHandler Command Buffer Size invalid!\n");
    return EFI_INVALID_PARAMETER;
  }
  ...
  IDS_HDT_CONSOLE_PSP_TRACE ("RomArmor2SmmCommunicateHandler ID %x\n", Buffer->id);
...
/**
 * @brief Set the Rom Armor 2 Flag In Hsti State Heap so that we can get the update to date HstiState in SMM
 *
 * @return EFI_STATUS EFI_SUCCESS: execution success, EFI_DEVICE_ERROR: execution failed
 */
EFI_STATUS
EFIAPI
SetRomArmor2FlagInHstiStateHeap ()
{
  ...
  )
{
  ...
  mBIOSNvStore.LABase = TransRom2Addr (PcdGet32 (PcdFlashNvStorageVariableBase));
  ...
  if (EFI_ERROR (Status)) {
    gBS->FreePages(ShadowBase, EFI_SIZE_TO_PAGES (mBIOSNvStore.Size));
    return EFI_DEVICE_ERROR;
  }
  SetRomArmor2FlagInHstiStateHeap ();
  ...
  )
{
  ...
  Status = AmdRomArmor2Init (mProtocol);
  if (EFI_ERROR (Status)) {
  ...
  if (!EFI_ERROR (Status)) {
    // Install S3 resume handler to enable RomArmor2 in S3 resume
    SwContext.AmdSwValue  = PcdGet8 (PcdFchOemBeforePciRestoreSwSmi);
    SwContext.Order       = 0x80;
    Status = SwDispatch->Register (
                            SwDispatch,
    ...
    if (EFI_ERROR (Status)) {
      IDS_HDT_CONSOLE_PSP_TRACE ("\tFailed to register callback function AmdRomArmor2S3InitCallback: %r\n", Status);
      ASSERT (Status == EFI_SUCCESS);
    }
  }
  ...
  Status = gSmst->SmiHandlerRegister (RomArmor2SmmCommunicateHandler, &gPspSmmCommHandleGuid, &mPspSmmCommHandle);
