...
// TargetID,           Directory,   DirEntryID, Detect, NvRegionBase, NvRegionSize
  ...
  {SMI_TARGET_MPM_PERSISTENT_STORAGE, DIR_TYPE_PSP, MPM_PERSISTENT_STORAGE,  FALSE, 0, 0},
  ...
  )
{
  ...
  for (PspNvDescriptor = &mPspNvDb[0]; PspNvDescriptor->TargetID != SMI_TARGET_END; PspNvDescriptor++) {
    ...
    IDS_HDT_CONSOLE_PSP_TRACE ("PSP NV ID[%x] Detected:%x Base:%lx Size:%x\n",
                                PspNvDescriptor->DirEntryID,
  ...
  )
{
  ...
  if (RomArmorSelection == 2 || RomArmorSelection == 3) {
    //RomArmor2/3 Enabled, PSP will access SPI ROM directly, will not use fake SMI for SPI access.
