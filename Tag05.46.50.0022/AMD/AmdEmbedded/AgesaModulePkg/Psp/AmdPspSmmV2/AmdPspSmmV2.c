...
{
  SX_REQ_BUFFER  SleepRequest;
  if (!mCapsuleUpdateFlag) {
    SleepRequest.SleepType = DispatchContext->Type;
    SleepRequest.Flag = 0;
    IDS_HDT_CONSOLE_PSP_TRACE ("PSP.PspSmm.SleepCallBack\n");
    if (PspMboxBiosCmdSxInfo (&SleepRequest, mPspMboxSmmBuffer, mPspMboxSmmFlagAddr)) {
...
AmdCapsuleSmmHook (
  ...
  SX_REQ_BUFFER  SleepRequest;
  mCapsuleUpdateFlag = TRUE;
  if (!mPspS3WakeFromSmm) {
    mPspSmmHdrData->CapsuleUpdateSignature = CAPSULE_UPDATE_SIGNATURE;
  }
  if ((AmdCapsuleHookFlag == PSP_CAPSULE_HOOK_FLAG_SXINFO_NOCLRSMMLCK) ||
    ...
    SleepRequest.SleepType = 0x3;
    SleepRequest.Flag = SX_REQ_FLAG_FAKE_S3;
    PspMboxBiosCmdSxInfo (&SleepRequest, mPspMboxSmmBuffer, mPspMboxSmmFlagAddr);
  }
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  if (!mPspS3WakeFromSmm) {
    ...
    if (GuidHob != NULL) {
      ...
      mPspSmmHdrData = (PSP_SMM_HDR_DATA *)(UINTN)SmramDescriptor->CpuStart;  //Note: SMM_S3_RESUME_STATE is the first member of this structure.
    }
  }
  ...
  SxRegisterContext.Order = 0xC0;  // set the order to make sure PspSxCallback is called after AmdCapsuleSmmHook
  ...
  mPspS3WakeFromSmm = PcdGetBool (PcdAmdPspS3WakeFromSmm);
  if (mPspS3WakeFromSmm) {
  ...
  if (mPspS3WakeFromSmm) {
    ...
    mPspSmmRsmMemInfoPtr = &mPspSmmHdrData->PspSmmRsmMemInfo;
    mPspSmmHdrData->CcxEarlyInit = 0;
    mPspSmmHdrData->CapsuleUpdateSignature = 0;
  }
  ...
  if (mPspS3WakeFromSmm) {
    ...
    if (!EFI_ERROR (Status)) {
      Status = VariableLock->RequestToLock (VariableLock, L"ApSyncFlagNv", &gApSyncFlagNvVariableGuid);
      if (Status != EFI_SUCCESS) {
        IDS_HDT_CONSOLE_PSP_TRACE ("Error: Unable to Request lock.\n");
        return Status;
      }
    }
  }
  ...
  AlignStackPtr = ((TempStackPtr + mPspPlatformProtocolCopy.BspStackSize) & (~((UINTN) mPspPlatformProtocolCopy.BspStackSize - 1)));
