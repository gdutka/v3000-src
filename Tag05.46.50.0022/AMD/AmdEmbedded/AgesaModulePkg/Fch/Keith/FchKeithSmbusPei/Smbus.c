...
// Produced PPI
#include "Smbus.h"
#define FILECODE FCH_KEITH_FCHKEITHSMBUSPEI_SMBUS_FILECODE
  ...
  )
{
  ...
  for (retryCount = 0; retryCount < TRY_TIMES; retryCount++) {
    ...
    do {
      DwLoopCount = 1000; //Increase counter 1000 from 100
      do {
        bValue = CpuIo->IoRead8 (
                          (CONST EFI_PEI_SERVICES **) PeiServices,
                          CpuIo,
  ...
  )
{
  ...
  FchRwPmioV2 (FCH_PMIOA_REG04, AccessWidth8, 0xFD, BIT1);
  FchReadPmioV2 (FCH_PMIOA_REG00, AccessWidth16, &Value16);
  ASSERT (Value16);
  FchRwPmioV2 (FCH_PMIOA_REG00, AccessWidth8, 0xEF, BIT4);
  Value16 &= 0xFF00;
