...
#define FILECODE FCH_KEITH_FCHKEITHSMMDISPATCHER_FCHSMMDIAGDISPATCHER_FILECODE
  ...
  )
{
  ...
  mSwSmiCmdAddress = ACPIMMIO16 (ACPI_MMIO_BASE + PMIO_BASE + FCH_PMIOA_REG6A);
  return Status;
}
  ...
  )
{
  ...
  //LibFchSmmPmioRead16 (&gSmst->SmmIo, FCH_PMIOA_REG6A, &SwSmiCmdAddress);
  FchSmmDiagIoWrite (&gSmst->SmmIo, SMM_IO_UINT16, 0x80, &mSwSmiCmdAddress);
  FchSmmDiagIoRead (&gSmst->SmmIo, SMM_IO_UINT8, mSwSmiCmdAddress, &SwSmiValue);
  if (SwSmiValue == DIAG_ENABLE) {
    ...
    ACPIMMIO8 (ACPI_MMIO_BASE + PMIO_BASE + FCH_PMIOA_REG04) |= BIT0;
