  ...
  )
{
  ...
  *TableSize = sizeof (PRESERVED_VARIABLE_TABLE);
  for (Index = 0; (Index < (sizeof (mReclaimFailPreservedVariableTable) / sizeof (PRESERVED_VARIABLE_TABLE))) && (mReclaimFailPreservedVariableTable[Index].VariableName != NULL); Index++) {
    *TableSize += sizeof (PRESERVED_VARIABLE_TABLE);
  }
