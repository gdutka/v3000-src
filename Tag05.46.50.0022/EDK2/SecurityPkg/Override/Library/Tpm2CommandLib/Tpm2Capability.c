      ...
      for (Index = 0; Index < AlgList.count; Index++) {
        switch (AlgList.algProperties[Index].alg) {
          case TPM_ALG_SHA1:
            ...
            break;
          case TPM_ALG_SHA256:
            Algs |= HASH_ALG_SHA256;
            break;
          case TPM_ALG_SHA384:
            Algs |= HASH_ALG_SHA384;
            break;
          case TPM_ALG_SHA512:
            Algs |= HASH_ALG_SHA512;
            break;
          case TPM_ALG_SM3_256:
            Algs |= HASH_ALG_SM3_256;
            break;
        }
      }

      if (BitFieldCountOnes32 (Algs, 0, 31) >= BitFieldCountOnes32 (*TpmHashAlgorithmBitmap, 0, 31)) {
        *TpmHashAlgorithmBitmap = Algs;
      }
    }
  }
