...
;******************************************************************************
;* Copyright (c) 2015 - 2022, Insyde Software Corporation. All Rights Reserved.
;*
...
#include <Board/Include/BoardHook.h>
#include <PlatformBoardConfig.h>
//[-start-170818-IB04840326-remove]//
//#include <SystemConfigVariable.h>
//[-end-170818-IB04840326-remove]//
...
//[-start-171108-IB14630132-remove]//
//GLOBAL_REMOVE_IF_UNREFERENCED EFI_PCI_ROOT_BRIDGE_IO_PROTOCOL *mPciRootBridgeIo;
//[-end-171108-IB14630132-remove]//

//[-start-170818-IB04840326-add]//
GLOBAL_REMOVE_IF_UNREFERENCED UINT64                          mTotalMemorySize;
//[-end-170818-IB04840326-add]//
...
//  return ((UINT8)(Cpuid.RegEbx)/Threads);
  //
  // Not check from Cpuid to prevent from triggering excpetion if system is not INTEL CPU.
  //
  return 8;
}
...
/**
  Returns the Number of overclocking bins, which expose the level of overclocking supported on the CPU.

  @retval Number of OC Bins (0-7).
          0:   no overclocking
          1-6: partially unlocked
          7:   fully unlocked
...
{
  return (UINT8)(RShiftU64(AsmReadMsr64(MSR_FLEX_RATIO), N_OVERCLOCKING_BINS) & 0x07);
}
...
//[-start-170818-IB04840326-modify]//
  StrnCpyS (String, sizeof (String) / sizeof (CHAR16), L"CORE = 0x", sizeof (String) / sizeof (CHAR16) - 1);
  HexToString(StrBuffer, Core, 2);
  StrnCpyS (String + StrLen(String), sizeof (String) / sizeof (CHAR16) - StrLen (String), StrBuffer, sizeof (String) / sizeof (CHAR16) - StrLen (String) - 1);
  gST->ConOut->OutputString (gST->ConOut,String);

  //
  // Display  Thread detail
  //
  StrnCpyS (String, sizeof (String) / sizeof (CHAR16), L" Thread = 0x", sizeof (String) / sizeof (CHAR16) - 1);
  ...
  // Display  Eax detail
  //
  StrnCpyS (String, sizeof (String) / sizeof (CHAR16), L" EAX = 0x", sizeof (String) / sizeof (CHAR16) - 1);
...
//[-start-171120-IB10870414-modify]//
    if ((MpContext.StatusFlag & PROCESSOR_HEALTH_STATUS_BIT) == 0) {
//[-end-171120-IB10870414-modify]//
      BistFailed = TRUE;
...
//}
  ...
  SetupCpuFeatures.NumOcBins = GetNumberOfOcBins();
  ...
  //
  // Check CPU SKU to determine FIVR and McIVR support
  //
  CpuSku = GetCpuSku();
  ...
  if ((CpuGeneration == EnumCnlCpu) && (CpuSku == EnumCpuUlx)) {
    SetupCpuFeatures.McivrSupported = 1;
  } else if ((CpuGeneration == EnumCnlCpu) && ((CpuSku == EnumCpuTrad) || (CpuSku == EnumCpuHalo))) {
    SetupCpuFeatures.FivrSupported = 1;
  }

  if ((CpuGeneration == EnumCnlCpu) && (CpuSku == EnumCpuUlt)) {
    SetupCpuFeatures.PkgCStateSupported = 0;
  } else {
    SetupCpuFeatures.PkgCStateSupported = 1;
  }
...
//CpuConfigData     = (CPU_CONFIG_DATA *)(UINTN)CpuInitDataHob->CpuConfigData;
  ...
  /// set C6DRAM support
  ///
//SetupCpuFeatures.C6DramSupported = 0;
//if (IsC6dramSupported (CpuConfigData->EnableC6Dram)) {
//  SetupCpuFeatures.C6DramSupported = TRUE;
//}
...
//[-end-170818-IB04840326-add]//
  mSetupVolatileData.PlatformType   = PcdGet8 (PcdPlatformType);
  mSetupVolatileData.PlatformFlavor = PcdGet8 (PcdPlatformFlavor);
//[-start-171006-IB16560005-modify]//
//[-start-170818-IB04840326-add]//
  mSetupVolatileData.PlatId = (UINT8)PcdGet64 (PcdH2OBoardId);
//[-end-170818-IB04840326-add]//
//[-end-171006-IB16560005-modify]//
  mSetupVolatileData.FabId          = PcdGet8 (PcdBoardRev);
  mSetupVolatileData.BoardBomId     = PcdGet8 (PcdBoardBomId);
  mSetupVolatileData.PchGeneration  = PchGeneration ();
  mSetupVolatileData.DockAttached   = PcdGetBool (PcdDockAttached);
