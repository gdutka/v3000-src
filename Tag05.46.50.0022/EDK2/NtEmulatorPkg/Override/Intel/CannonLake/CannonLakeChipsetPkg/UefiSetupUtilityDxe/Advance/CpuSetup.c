...
GLOBAL_REMOVE_IF_UNREFERENCED CPU_REV  ProcessorRevisionTable[] = {
  {EnumCpuCflUltUlx    + EnumKblH0,    "H0/J0"},
  {EnumCpuCflUltUlx    + EnumCflD0,    "D0"},
  {EnumCpuCflUltUlx    + EnumCflW0,    "W0"},
  {EnumCpuCflDtHalo    + EnumCflB0,    "B0"},
  {EnumCpuCflDtHalo    + EnumCflU0,    "U0"},
  {EnumCpuCflDtHalo    + EnumCflP0,    "P0"},
  {EnumCpuCnlUltUlx    + EnumCnlA0,    "A0"},
  {EnumCpuCnlUltUlx    + EnumCnlB0,    "B0"},
  {EnumCpuCnlUltUlx    + EnumCnlC0,    "C0"},
  {EnumCpuCnlUltUlx    + EnumCnlD0,    "D0"},
  {EnumCpuCnlUltUlx    + EnumCnlT0,    "T0"},
  {EnumCpuCnlUltUlx    + EnumCnlU0,    "U0"},
  {EnumCpuCnlUltUlx    + EnumCnlV0,    "V0"},
  {EnumCpuCnlUltUlx    + EnumCnlW0,    "W0"},
  {EnumCpuCnlUltUlx    + EnumCnlJ0,    "J0"},
  {EnumCpuCnlUltUlx    + EnumCnlK0,    "K0"},
  {EnumCpuCnlDtHalo    + EnumCnlP0,    "P0"},
  {EnumCpuCnlDtHalo    + EnumCnlG0,    "G0"},
  {EnumCpuCnlDtHalo    + EnumCnlM0,    "M0"},
  {EnumCpuCnlDtHalo    + EnumCnlN0,    "N0"},
  {EnumCpuCnlDtHalo    + EnumCnlS0,    "S0"}
};
//[-start-170818-IB04840326-modify]//
...
//  }
  ...
  if ((MmioRead32 (PchSpiBar0 + R_SPI_MEM_HSFSC) & B_SPI_MEM_HSFSC_SAF_MODE_ACTIVE)) {
...
//UINT64                      TempDcLoadline;
  ...
  SetupVariableSize = sizeof (SETUP_CPU_FEATURES);
  Status = gRT->GetVariable (
                  L"SetupCpuFeatures",
                  &gSetupVariableGuid,
                  NULL,
  ...
  /// Get CPU VR topology
  ///
  ZeroMem (&VrTopology, sizeof (VrTopology));
...
//  ///
//  /// If VR topology fails, it is pointless to continue. Return.
//  ///
//  return;
//}
...
#ifdef CPU_CFL
  ///
  /// No FIVR architecture on CFL CPU
  ///
  MaxVrIndex = CPU_VR_DOMAIN_GT;
#else
  if ((CpuFamily == EnumCpuCnlUltUlx) && (CpuSku == EnumCpuUlx)) {
    ///
    /// CNL-Y uses McIVR VR architecture which is the only sku with VccIn overrides
    ///
    MaxVrIndex = CPU_VR_DOMAIN_FIVR;
  } else {
    ///
    /// This will bypass the VccIn domain from being programmed
    ///
    MaxVrIndex = CPU_VR_DOMAIN_GT;
  }
#endif
  ...
  for (VrIndex = CPU_VR_DOMAIN_SA; VrIndex <= MaxVrIndex; VrIndex++) {
    if (mCpuSetup.TdcEnable[VrIndex] == 2) {
      // TDC should not be enabled if the Number of OC Bins is 0x7
      if (SetupCpuFeatures.NumOcBins == 0x7) {
        mCpuSetup.TdcEnable[VrIndex] = 0;
      // All external VRs should be TDC disabled if McIVR or FIVR is supported
      } else if (SetupCpuFeatures.McivrSupported || SetupCpuFeatures.FivrSupported) {
        mCpuSetup.TdcEnable[VrIndex] = 0;
        if (VrIndex == CPU_VR_DOMAIN_FIVR) {
          mCpuSetup.TdcEnable[VrIndex] = 1;
        }
      // If McIVR or FIVR is not supported, external VRs should be TDC enabled
      } else {
        mCpuSetup.TdcEnable[VrIndex] = 1;
        mCpuSetup.TdcEnable[CPU_VR_DOMAIN_FIVR] = 0;
      }
    }
    ...
    if (mCpuSetup.VrConfigEnable[VrIndex] == 1 && SvidEnabled == 1) {
      ...
      /// -Policy Imon slope1 is defined in 1/100 increments
      ...
      if (MailboxStatus == PCODE_MAILBOX_CC_SUCCESS) {
        ...
        mCpuSetup.ImonSlope1[VrIndex] = (UINT16) ((((MailboxData & VR_IMON_SLOPE_MASK) >> VR_IMON_SLOPE_OFFSET) * 100 + (1<< 14)) >> 15);
      } else {
...
//   CPU_SKU                 VrSku;
  ...
  CHAR8                   *SkuString;
  CHAR8                   *ProcessorUpgradeString;
//[-start-171009-IB10560317-remove]//
//  CHAR8                   *RfiFreqAdjString;
//[-end-171009-IB10560317-remove]//
//[-end-170818-IB04840326-add]//
...
//[-start-171121-IB10560319-remove]//
//  RegEcx                                 = 0;
//  RfiFreqAdjString                       = NULL;
//[-end-171121-IB10560319-remove]//
  ...
  switch (CpuFamilyId) {
    case EnumCpuCflUltUlx:
      if (CpuSteppingId == EnumCflW0) {
        GenerationString = "WhiskeyLake";
      } else {
        GenerationString = "CoffeeLake";
      }
      break;
  ...
  switch (CpuSku) {
    case EnumCpuUlt:
      SkuString = "ULT";
      if ((GenerationString != NULL) && ((AsciiStrCmp (GenerationString, "CoffeeLake") == 0) || (AsciiStrCmp (GenerationString, "WhiskeyLake") == 0))) {
        ProcessorUpgradeString = "BGA1528";
      } else {
        ProcessorUpgradeString = "BGA1510";
      }
      break;

    case EnumCpuUlx:
      SkuString = "ULX";
      ProcessorUpgradeString = "BGA1392";
      break;

    case EnumCpuTrad:
      SkuString = "DT";
      ProcessorUpgradeString = "LGA1151";
      break;

    case EnumCpuHalo:
      SkuString = "Halo";
      if ((GenerationString != NULL) && (AsciiStrCmp (GenerationString, "CoffeeLake") == 0)) {
        ProcessorUpgradeString = "BGA1440";
      } else {
        ProcessorUpgradeString = "BGA1287";
      }
      break;

    default:
      SkuString = NULL;
      ProcessorUpgradeString = "Unknown";
      break;
  }
  ...
  if (CtdpLevels != 0) {
    ...
    //
//[-start-170818-IB04840326-modify]//
    CtdpAddress = (UINT32)((UINTN) PcdGet64 (PcdSaMchBaseAddress) + 0x59A0);
//[-end-170818-IB04840326-modify]//
    ...
    //
//[-start-170818-IB04840326-modify]//
    CtdpAddress = (UINT32)((UINTN) PcdGet64 (PcdSaMchBaseAddress) + 0x59A4);
//[-end-170818-IB04840326-modify]//
    CtdpTdp = (UINT16)((*((UINT32*) ((UINTN) CtdpAddress))) & PACKAGE_TDP_POWER_MASK);
...
//[-start-170818-IB04840326-modify]//
...
#ifdef CPU_CFL
    CpuSetupVolData.FivrVrLocked  = 1;
#else
  ...
  RfiNomFreq = (UINT16) (((MmioRead64 (MchBar + R_RFI_STATUS_0_0_0_MCHBAR_PCU))& B_RFI_NOM_FREQ_MASK) >> 1);
...
//[-start-170818-IB04840326-modify]//
//  WriteCmos (CMOS_CPU_RATIO_OFFSET, (UINT8*)&mCpuSetup.CpuRatio);
  ...
  ///
//  WriteCmos (CMOS_TXT_REG, (UINT8*)&mCpuSetup.AcheckRequest);
...
//  EFI_STRING              RequestString = NULL;
  ...
  // Init local variable "CpuSetup"
  MyCpuIfrNVData = (CPU_SETUP *)gRcSUBrowser->CpuSUBrowserData;
  CopyMem (CurrentUserSetupData, MyCpuIfrNVData, VarSize);
  ///
  /// GetBrowserData by VarStore Name (Setup)
  ///
  ActionStatus = HiiGetBrowserData (&gCpuSetupVariableGuid, L"CpuSetup", VarSize, (UINT8 *) CurrentUserSetupData);
...
#else
      CopyMem (MyCpuIfrNVData, CurrentUserSetupData, VarSize);
#endif
      Status = EFI_SUCCESS;
      break;
    default:
  ...
  )
{
  ...
  switch (KeyValue) {
    case KEY_EpochChange:
      ...
      ///
      Status = HiiGetBrowserData (&gCpuSetupVariableGuid, L"CpuSetupSgxEpochData", VarSize, (UINT8 *) CurrentUserSgxEpochSetupData);
      if (Status != TRUE) {
        return EFI_NOT_FOUND;
      }
...
//      StrCpyS (StringBuffer3, StringBuffSize, L"Press 'Y' if you wish to continue or press any other key otherwise");
      StrCpyS (StringBuffer3, StringBuffSize, L"Select \"Yes\" if you wish to continue.");

//      CreatePopUp (EFI_LIGHTGRAY | EFI_BACKGROUND_RED,&Key, StringBuffer1,StringBuffer2,StringBuffer3, NULL);

      StrCatS (StringBuffer1, StringBuffSize, StringBuffer2);
      StrCatS (StringBuffer1, StringBuffSize, StringBuffer3);
      gSUBrowser->H2ODialog->ConfirmDialog (
                               DlgYesNo,
                               FALSE,
                               (UINT32)StrnSizeS (StringBuffer1, StringBuffSize),
                               NULL,
                               &Key,
                               StringBuffer1
                               );
...
//      if (Key.UnicodeChar != L'Y' && Key.UnicodeChar != L'y') {
      if (Key.UnicodeChar != CHAR_CARRIAGE_RETURN) {
        ...
        if (CurrentUserSetupData->EpochUpdate != mCpuSetup.EpochUpdate) {
          ...
          Status = HiiSetBrowserData (&gCpuSetupVariableGuid, L"CpuSetup", VarSize, (UINT8 *) CurrentUserSetupData, RequestString);
          CopyMem (MyCpuIfrNVData, CurrentUserSetupData, VarSize);
//[-start-180426-IB16270040-add]//
          FreePool (RequestString);
          RequestString = NULL;
//[-end-180426-IB16270040-add]//
//[-start-171120-IB10870414-remove]//
//          Status = TRUE;
//          if (Status != TRUE) {
//            FreePool (CurrentUserSetupData);
//            return EFI_NOT_FOUND;
//          }
//[-end-171120-IB10870414-remove]//
        }
        FreePool (CurrentUserSetupData);
        FreePool (CurrentUserSgxEpochSetupData);
        return EFI_UNSUPPORTED;
...
//[-start-180426-IB16270040-add]//
        FreePool (RequestString);
        RequestString = NULL;
        MyCpuIfrNVData->EpochUpdate = SGX_NO_CHANGE_IN_EPOCH;
//[-end-180426-IB16270040-add]//
      }
      ...
      if (RequestString != NULL) {
        VarSize = sizeof(CPU_SETUP_SGX_EPOCH_DATA);
        Status = HiiSetBrowserData (&gCpuSetupVariableGuid, L"CpuSetupSgxEpochData", VarSize, (UINT8 *) CurrentUserSgxEpochSetupData, RequestString);
        if (Status != TRUE) {
          FreePool (RequestString);
          FreePool (CurrentUserSetupData);
          FreePool (CurrentUserSgxEpochSetupData);
...
//[-start-180426-IB16270040-add]//
        FreePool (RequestString);
        RequestString = NULL;
        MyCpuIfrNVData->SgxEpoch0 = CurrentUserSgxEpochSetupData->SgxEpoch0;
        MyCpuIfrNVData->SgxEpoch1 = CurrentUserSgxEpochSetupData->SgxEpoch1;
//[-end-180426-IB16270040-add]//
      }
...
//[-start-180426-IB16270040-add]//
        FreePool (RequestString);
        RequestString = NULL;
        MyCpuIfrNVData->ShowEpoch = CurrentUserSetupData->ShowEpoch;
//[-end-180426-IB16270040-add]//
  ...
  )
{
  ...
  //
//[-start-170818-IB04840326-modify]//
  PackageHdr = (EFI_HII_PACKAGE_HEADER *)(AdvanceVfrBin + sizeof(UINT32));
//[-end-170818-IB04840326-modify]//
