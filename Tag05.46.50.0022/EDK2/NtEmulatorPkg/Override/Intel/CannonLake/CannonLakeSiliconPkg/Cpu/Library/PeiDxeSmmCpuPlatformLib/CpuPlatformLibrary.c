...
#ifdef CPU_CFL
        if ((PackageTdp == CPU_TDP_28_WATTS) || (HasEdram)) {
          ///
          ///  CFL-U 4+3e
          ///
          ...
          if ((CpuDid == V_SA_DEVICE_ID_CFL_ULT_1) || (CpuDid == V_SA_DEVICE_ID_CFL_ULT_3)) {
            ...
            ///  CFL-U 4+2f
            ///
            CpuIdentifier = EnumCflU15Watt42fCpuId;
          }
        }
#endif
...
#ifdef CPU_CFL
  ///
  ///@todo: Pending Host Device ID (DID0) for CFL H 8+2
  ///
  ...
  )
{
  ...
  ///
  AsmCpuid (CPUID_VERSION_INFO, &Cpuid.RegEax, &Cpuid.RegEbx, &Cpuid.RegEcx, &Cpuid.RegEdx);
  CpuFamilyModel = EnumCpuCflUltUlx;
