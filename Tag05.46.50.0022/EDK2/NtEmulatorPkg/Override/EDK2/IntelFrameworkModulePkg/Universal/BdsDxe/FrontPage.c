...
#include <Guid/DebugMask.h> // for gEfiGenericVariableGuid
  ...
  )
{
  ...
  H2O_BDS_CP_INIT_DATA                            *BdsCpInitData;
  EFI_HANDLE                                      Handle;

  Status = gBS->LocateProtocol (&gH2OBdsCpInitGuid, NULL, (VOID **) &BdsCpInitData);
  if (EFI_ERROR (Status)) {
    BdsCpInitData = AllocateZeroPool (sizeof (H2O_BDS_CP_INIT_DATA));
    if (BdsCpInitData == NULL) {
      return;
    }
    BdsCpInitData->Size = sizeof (H2O_BDS_CP_INIT_DATA);
    ...
    DEBUG((DEBUG_INFO, "Before trigger\n"));
    H2OCpTrigger (&gH2OBdsCpInitGuid, BdsCpInitData);
    DEBUG((DEBUG_INFO, "After trigger\n"));
  }

  gBS->LoadImage (
         TRUE,
         gImageHandle,
         (EFI_DEVICE_PATH_PROTOCOL *) &gH2OSecureBootMgrDevicePath,
         NULL,
         0,
         &ImageHandle
         );
...
UINTN           mOriginalSendForm;
extern UINT8    BdsDxeImages[];
  ...
  )
{
  ...
  StartLabel->Number       = LABEL_SECURE_BOOT_OPTION_START;
  ...
  EndLabel->Number       = LABEL_SECURE_BOOT_OPTION_END;
  ...
  )
{
  ...
  StartLabel = (EFI_IFR_GUID_LABEL *) HiiCreateGuidOpCode (StartOpCodeHandle, &gEfiIfrTianoGuid, NULL, sizeof (EFI_IFR_GUID_LABEL));
  StartLabel->ExtendOpCode = EFI_IFR_EXTEND_OP_LABEL;
  StartLabel->Number       = LABEL_SCU_START;
  ...
  EndLabel = (EFI_IFR_GUID_LABEL *) HiiCreateGuidOpCode (EndOpCodeHandle, &gEfiIfrTianoGuid, NULL, sizeof (EFI_IFR_GUID_LABEL));
  EndLabel->ExtendOpCode = EFI_IFR_EXTEND_OP_LABEL;
  EndLabel->Number       = LABEL_SCU_END;

  RefOpCode = (EFI_IFR_REF *) HiiCreateGotoOpCode (
                                StartOpCodeHandle,
                                FRONT_PAGE_ITEM_SEVEN,
                                STRING_TOKEN(STR_SETUP_UTILITY),
                                STRING_TOKEN(STR_SETUP_UTILITY_HELP),
                                EFI_IFR_FLAG_CALLBACK,
                                FRONT_PAGE_KEY_SETUP_UTILITY
                                );
  ...
  ImageOp.Header.Length = sizeof (EFI_IFR_IMAGE);
  ImageOp.Header.Scope  = 0;
  ImageOp.Id            = IMAGE_TOKEN (IMAGE_SCU);
  HiiCreateRawOpCodes (StartOpCodeHandle, (UINT8 *) &ImageOp, sizeof(ImageOp));
  HiiCreateEndOpCode (StartOpCodeHandle);
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    Token   = ((EFI_IFR_FORM_SET *) TempPtr)->FormSetTitle;
  }
...
{
  EFI_STATUS                           Status;
  EFI_SETUP_UTILITY_BROWSER_PROTOCOL   *SetupUtilityBrowser;
  UINT32                               Index;
  EFI_FORM_BROWSER2_PROTOCOL           *FormBrowser2;

  Status = gBS->LocateProtocol (&gEfiSetupUtilityBrowserProtocolGuid, NULL, &SetupUtilityBrowser);
  if (EFI_ERROR (Status)) {
    return Status;
  }

  for (Index = 0; Index < HandleCount; Index++) {
    SetupUtilityBrowser->MenuList[Index].MenuTitle = GetTitleToken (Handles[Index]);
    SetupUtilityBrowser->MenuList[Index].Page      = Handles[Index];
    SetupUtilityBrowser->MenuList[Index].String    = NULL;
  }

  SetupUtilityBrowser->MenuItemCount = (UINT8) HandleCount;
  SetupUtilityBrowser->CurRoot       = (UINT8) 0;
  SetupUtilityBrowser->AtRoot        = TRUE;
  ...
  FormBrowser2->SendForm = (EFI_SEND_FORM2) mOriginalSendForm;
  mOriginalSendForm = 0;
  ...
  )
{
  ...
  mOriginalSendForm      = (UINTN) FormBrowser2->SendForm;
  FormBrowser2->SendForm = ScuSendForm;
...
//UINT8                       Data;
  ...
  if (InitializeHiiData) {
    ...
    //
    // Initialize system security status
    //
//  Data = 0;
