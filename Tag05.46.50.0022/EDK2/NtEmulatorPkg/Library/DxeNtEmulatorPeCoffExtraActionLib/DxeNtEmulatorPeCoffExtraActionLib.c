...
Abstract:
  ...
  PE/Coff image for NT environment specific purpose such as souce level debug.
...
EFI_STATUS
EFIAPI
NtEmulatorPeCoffGetWinNtThunkStucture (
  VOID
  )
{
...

{
  ...
  for (Index = 0; Index < mPdbNameModHandleArraySize; Index++, Array++) {
    if (Array->PdbPointer == NULL) {
      ...
      AsciiStrCpyS (Array->PdbPointer, AsciiStrLen (ImageContext->PdbPointer) + 1, ImageContext->PdbPointer);
...
/**
  ...
  For emulator, this function load symbols to support source level debugging.
...
/**
  ...
  For emulator, this function unloads symbols for source level debugging.
