...
#define USB_HDD_INSTANCE_INDEX    3
  ...
  )
{
  ...
  HardDriveNode.Header.Type    = MEDIA_DEVICE_PATH;
  HardDriveNode.Header.SubType = MEDIA_HARDDRIVE_DP;
  SetDevicePathNodeLength (&HardDriveNode.Header, sizeof (HARDDRIVE_DEVICE_PATH));

  HardDriveNode.PartitionNumber = InstanceNumber;
  HardDriveNode.MBRType         = MBR_TYPE_EFI_PARTITION_TABLE_HEADER;
  HardDriveNode.SignatureType   = SIGNATURE_TYPE_GUID;
  HardDriveNode.PartitionStart  = 10;
  HardDriveNode.PartitionSize   = HardDriveNode.PartitionStart + 1;
  CopyMem (HardDriveNode.Signature, &gEfiWinNtFileSystemGuid, sizeof (EFI_GUID));
  HardDriveNode.Signature[0]   += (UINT8) (UINTN) InstanceNumber;

  return AppendDevicePathNode (
           RootDevicePath,
           (EFI_DEVICE_PATH_PROTOCOL *) &HardDriveNode
           );
}

EFI_DEVICE_PATH_PROTOCOL *
AppendUsbHddDevicePath (
  IN EFI_DEVICE_PATH_PROTOCOL     *RootDevicePath,
  IN UINT16                       InstanceNumber
  )
{
  USB_DEVICE_PATH                 UsbDeviceNode;
  EFI_DEVICE_PATH_PROTOCOL        *UsbDevicePath;
  EFI_DEVICE_PATH_PROTOCOL        *UsbHddDevicePath;

  UsbDeviceNode.Header.Type    = MESSAGING_DEVICE_PATH;
  UsbDeviceNode.Header.SubType = MSG_USB_DP;
  SetDevicePathNodeLength (&UsbDeviceNode.Header, sizeof (USB_DEVICE_PATH));
  UsbDeviceNode.ParentPortNumber = 0;
  UsbDeviceNode.InterfaceNumber  = (UINT8) InstanceNumber;

  UsbDevicePath = AppendDevicePathNode (
                    RootDevicePath,
                    (EFI_DEVICE_PATH_PROTOCOL *) &UsbDeviceNode
                    );
  if (UsbDevicePath == NULL) {
    return NULL;
  }

  UsbHddDevicePath = AppendHddDevicePath (
                       UsbDevicePath,
                       InstanceNumber
                       );
  FreePool (UsbDevicePath);
  return UsbHddDevicePath;
}

EFI_BLOCK_IO_PROTOCOL *
GetNtVirtualDiskBlockIo (
  IN UINT16                         InstanceNumber
  )
{
  EFI_STATUS                        Status;
  ...
  for (Index = 0; Index < NumberOfHandles; Index++) {
    Status = gBS->HandleProtocol (HandleBuffer[Index], &gEfiWinNtIoProtocolGuid, (VOID **) &WinNtIo);
    if (EFI_ERROR (Status) || !CompareGuid (WinNtIo->TypeGuid, &gEfiWinNtVirtualDisksGuid)) {
      continue;
    }
    ...
    if (WinNtIo->InstanceNumber != InstanceNumber) {
      continue;
    }
...
--*/
{
  ...
  StrCpyS (Private->VolumeLabel, StrSize (L"EFI_EMULATED") / sizeof (CHAR16), L"EFI_EMULATED");
  ...
  Handle = NULL;
  Private->BlockIo = GetNtVirtualDiskBlockIo (WinNtIo->InstanceNumber);
  if (Private->BlockIo != NULL) {
    gBS->InstallMultipleProtocolInterfaces (&Handle, &gEfiBlockIoProtocolGuid, Private->BlockIo, NULL);
  }

  Private->DevicePath = NULL;
  Status = gBS->HandleProtocol (ControllerHandle, &gEfiDevicePathProtocolGuid, (VOID **) &DevicePath);
  if (!EFI_ERROR (Status)) {
    Private->DevicePath = (WinNtIo->InstanceNumber == USB_HDD_INSTANCE_INDEX) ? AppendUsbHddDevicePath (DevicePath, WinNtIo->InstanceNumber) :
                                                                                AppendHddDevicePath (DevicePath, WinNtIo->InstanceNumber);
    if (Private->DevicePath != NULL) {
      gBS->InstallMultipleProtocolInterfaces (&Handle, &gEfiDevicePathProtocolGuid, Private->DevicePath, NULL);
    }
  }
...
// TODO:    EFI_UNSUPPORTED - add return value to function comment
{
  ...
  if (NumberOfChildren == 0) {
    //
    // In the case of re-entry of the WinNtSimpleFileSystemDriverBindingStop, the
    ...
    if (HasChildren (ControllerHandle)) {
      DEBUG((EFI_D_ERROR, "WinNtSimpleFileSystemDriverBindingStop: Still has child.\n"));
  ...
  if (Private->BlockIo    != NULL) {
    gBS->UninstallMultipleProtocolInterfaces (ChildHandleBuffer[0], &gEfiBlockIoProtocolGuid, &Private->BlockIo, NULL);
  }
  if (Private->DevicePath != NULL) {
    gBS->UninstallMultipleProtocolInterfaces (ChildHandleBuffer[0], &gEfiDevicePathProtocolGuid, &Private->DevicePath, NULL);
  }
...
// TODO:    EFI_INVALID_PARAMETER - add return value to function comment
{
  ...
  StrCpyS (PrivateFile->FilePath, StrSize (Private->FilePath) / sizeof (CHAR16), Private->FilePath);
  StrCpyS (PrivateFile->FileName, StrSize (Private->FilePath) / sizeof (CHAR16), PrivateFile->FilePath);
...
// TODO:    EFI_INVALID_PARAMETER - add return value to function comment
{
  ...
  if (PrivateFile->IsDirectoryPath) {
    StrCpyS (NewPrivateFile->FilePath, StrSize (PrivateFile->FileName) / sizeof (CHAR16), PrivateFile->FileName);
  } else {
  ...
  StrCpyS (NewPrivateFile->FilePath, StrSize (NewPrivateFile->FileName) / sizeof (CHAR16), NewPrivateFile->FileName);
  if (TempChar != 0) {
...
// TODO:    EFI_INVALID_PARAMETER - add return value to function comment
{
  ...
  if (CompareGuid (InformationType, &gEfiFileSystemInfoGuid)) {
    ...
    StrCpyS (DriveName, (StrSize (PrivateFile->FilePath) + 1) / sizeof (CHAR16), PrivateFile->FilePath);
    ...
    StrCpyS ((CHAR16 *) FileSystemInfoBuffer->VolumeLabel, StrSize (PrivateRoot->VolumeLabel) / sizeof (CHAR16), PrivateRoot->VolumeLabel);
    *BufferSize = SIZE_OF_EFI_FILE_SYSTEM_INFO + StrSize (PrivateRoot->VolumeLabel);
    Status      = EFI_SUCCESS;
  }
  ...
  if (CompareGuid (InformationType, &gEfiFileSystemVolumeLabelInfoIdGuid)) {
    ...
    *BufferSize = StrSize (PrivateRoot->VolumeLabel);
    StrCpyS ((CHAR16 *) Buffer, *BufferSize / sizeof (CHAR16), PrivateRoot->VolumeLabel);
    Status      = EFI_SUCCESS;
  }
...
// TODO:    EFI_INVALID_PARAMETER - add return value to function comment
{
  ...
  if (CompareGuid (InformationType, &gEfiFileSystemInfoGuid)) {
    ...
    StrCpyS (PrivateRoot->VolumeLabel, StrSize (NewFileSystemInfo->VolumeLabel) / sizeof (CHAR16), NewFileSystemInfo->VolumeLabel);
  ...
  StrCpyS (OldFileName, StrSize (PrivateFile->FileName) / sizeof (CHAR16), PrivateFile->FileName);
