...
#define RELASE_MOUSE_PROMPT L" - Press Ctrl-Alt to exit mouse grab"
...
typedef WINUSERAPI HCURSOR (WINAPI *pSetCursor)(IN HCURSOR hCursor);
typedef WINUSERAPI BOOL (WINAPI *pClipCursor)(IN RECT *lpRect);
typedef WINUSERAPI HWND (WINAPI *pSetCapture)(IN HWND hWnd);
typedef WINUSERAPI BOOL (WINAPI *pReleaseCapture)(VOID);
typedef WINUSERAPI BOOL (WINAPI *pGetClientRect)(IN HWND hWnd, IN LPRECT lpRect);
typedef WINUSERAPI BOOL (WINAPI *pClientToScreen)(IN HWND hWnd, IN LPPOINT lpPoint);
typedef WINUSERAPI SHORT (WINAPI *pGetKeyState)(IN INT nVirtKey);
typedef WINUSERAPI INT  (WINAPI *pGetWindowText)(IN HWND hWnd, OUT LPTSTR lpString, IN INT nMaxCount);
typedef WINUSERAPI BOOL (WINAPI *pSetWindowText)(IN HWND hWnd, IN OUT LPCTSTR lpString);


pRegisterRawInputDevices mRegisterRawInputDevices;
pGetRawInputData         mGetRawInputData;
pSetCursor               mSetCursor;
pClipCursor              mClipCursor;
pSetCapture              mSetCapture;
pReleaseCapture          mReleaseCapture;
pGetClientRect           mGetClientRect;
pClientToScreen          mClientToScreen;
pGetKeyState             mGetKeyState;
pGetWindowText           mGetWindowText;
pSetWindowText           mSetWindowText;
  ...
  )
{
  ...
  Private = GOP_PRIVATE_DATA_FROM_SIMPLE_POINTER_THIS (This);

  OldTpl = gBS->RaiseTPL (TPL_NOTIFY);

  Private->HasMouseData = FALSE;

  Private->MouseState.RelativeMovementX = 0;
  Private->MouseState.RelativeMovementY = 0;
  Private->MouseState.RelativeMovementZ = 0;
  Private->MouseState.LeftButton        = FALSE;
  Private->MouseState.RightButton       = FALSE;

  Private->SimplePointer.Mode->ResolutionX = 8;
  Private->SimplePointer.Mode->ResolutionY = 8;
  Private->SimplePointer.Mode->ResolutionZ = 120;
  Private->SimplePointer.Mode->LeftButton  = TRUE;
  Private->SimplePointer.Mode->RightButton = TRUE;
  ...
  )
{
  ...
  OldTpl = gBS->RaiseTPL (TPL_NOTIFY);
  if (Private->HasMouseData) {
    CopyMem (State, &Private->MouseState, sizeof (EFI_SIMPLE_POINTER_STATE));
    Private->HasMouseData = FALSE;
    gBS->RestoreTPL (OldTpl);
...
VOID
EFIAPI
WinNtGopSimplePointerWaitForInput (
    IN EFI_EVENT          Event,
    IN VOID               *Context
  )
{
...
EFI_STATUS
WinNtGopInitializeSimplePointerForWindow (
  IN  GOP_PRIVATE_DATA    *Private
  )
{
  HMODULE                   User32;
  ...
  User32 = Private->WinNtThunk->LoadLibraryEx (L"user32.dll", NULL, DONT_RESOLVE_DLL_REFERENCES);

  mRegisterRawInputDevices  = (pRegisterRawInputDevices)Private->WinNtThunk->GetProcAddress (User32, "RegisterRawInputDevices");
  mGetRawInputData          = (pGetRawInputData)Private->WinNtThunk->GetProcAddress (User32, "GetRawInputData");
  mSetCursor                = (pSetCursor)Private->WinNtThunk->GetProcAddress (User32, "SetCursor");
  mClipCursor               = (pClipCursor)Private->WinNtThunk->GetProcAddress (User32, "ClipCursor");
  mSetCapture               = (pSetCapture)Private->WinNtThunk->GetProcAddress (User32, "SetCapture");
  mReleaseCapture           = (pReleaseCapture)Private->WinNtThunk->GetProcAddress (User32, "ReleaseCapture");
  mGetClientRect            = (pGetClientRect)Private->WinNtThunk->GetProcAddress (User32, "GetClientRect");
  mClientToScreen           = (pClientToScreen)Private->WinNtThunk->GetProcAddress (User32, "ClientToScreen");
  mGetKeyState              = (pGetKeyState)Private->WinNtThunk->GetProcAddress (User32, "GetKeyState");
  mGetWindowText            = (pGetWindowText)Private->WinNtThunk->GetProcAddress (User32, "GetWindowTextW");
  mSetWindowText            = (pSetWindowText)Private->WinNtThunk->GetProcAddress (User32, "SetWindowTextW");

  Private->WinNtThunk->FreeLibrary (User32);

  ZeroMem (&Private->SimplePointer, sizeof (EFI_SIMPLE_POINTER_PROTOCOL));

  Private->SimplePointer.Reset    = WinNtGopSimplePointerReset;
  Private->SimplePointer.GetState = WinNtGopSimplePointerGetState;
  Private->SimplePointer.Mode     = &Private->MouseMode;

  WinNtGopSimplePointerReset (&Private->SimplePointer, TRUE);
...
EFI_STATUS
WinNtGopDestroySimplePointerForWindow (
  IN  GOP_PRIVATE_DATA    *Private
  )
{
  return EFI_SUCCESS;
...
EFI_STATUS
WinNtGopSimplePointerReleaseCapture (
  IN  GOP_PRIVATE_DATA    *Private,
  IN  HWND                hwnd
  )
{
  ...
  //
  // Stop reading mouse raw input,
  // According to msdn; hwndTarget must be NULL
  //
  Rid.usUsagePage = 0x01;
  Rid.usUsage     = 0x02;
  Rid.dwFlags     = RIDEV_REMOVE;
  Rid.hwndTarget  = NULL;
  mRegisterRawInputDevices (&Rid, 1, sizeof (Rid));

  mClipCursor (NULL);
  mSetWindowText (hwnd, Private->WindowTitle);
  mReleaseCapture();
  Private->IsCaptureCursor = FALSE;
  ...
  )
{
  ...
  case WM_CAPTURECHANGED:
    ...
    // According to msdn; hwndTarget must be NULL
    //
    if (Private->IsCaptureCursor) {
      WinNtGopSimplePointerReleaseCapture (Private, hwnd);
      return 0;
    }
    break;
  ...
  case WM_KEYDOWN:
    ...
    if (Private->IsCaptureCursor) {
      ...
      //
      if ((wParam == VK_CONTROL && (lParam & GOP_ALT_KEY_PRESSED) == GOP_ALT_KEY_PRESSED) ||
          (wParam == VK_MENU && mGetKeyState (VK_LCONTROL) < 0)) {
        WinNtGopSimplePointerReleaseCapture (Private, hwnd);
        return 0;
      }
    }
  ...
  case WM_LBUTTONDOWN:
    if (Private->IsCaptureCursor == FALSE) {
      //
      // According to Top-Level Collections Opened by Windows for System Use (Windows Drivers)
      // Mouse type UsagePage is 0x01 and Usage is 0x02.
      //
      Rid.usUsagePage = 0x01;
      Rid.usUsage     = 0x02;
      Rid.dwFlags     = 0;
      ...
      mGetClientRect (hwnd, &Rect);
      mClientToScreen (hwnd, (LPPOINT)&Rect);
      mClientToScreen (hwnd, (LPPOINT)&Rect + 1);
      mClipCursor (&Rect);
      ...
      //
      // Show release mouse prompt
      //
      Private->WindowTitle[0] = '\0';
      mGetWindowText (hwnd, Private->WindowTitle, sizeof (Private->WindowTitle) / sizeof (CHAR16));
      StrCpyS (TitleStr, sizeof (TitleStr) / sizeof (CHAR16), Private->WindowTitle);
      StrCatS (TitleStr, sizeof (TitleStr) / sizeof (CHAR16), RELASE_MOUSE_PROMPT);
      mSetWindowText (hwnd, TitleStr);
      mSetCapture (hwnd);
      Private->IsCaptureCursor = TRUE;
    }
    break;
  ...
  case WM_INPUT:
    ...
    Size  = sizeof (Buffer);
    mGetRawInputData((HRAWINPUT)lParam, RID_INPUT, RawInput, &Size, sizeof(RAWINPUTHEADER));
    ASSERT (RawInput->header.dwType == RIM_TYPEMOUSE);

    if (RawInput->header.dwType != RIM_TYPEMOUSE) {
      break;
    }

    Private->MouseState.RelativeMovementX = RawInput->data.mouse.lLastX;
    Private->MouseState.RelativeMovementY = RawInput->data.mouse.lLastY;
    Private->MouseState.RelativeMovementZ = RawInput->data.mouse.usButtonFlags & RI_MOUSE_WHEEL ? (INT16)RawInput->data.mouse.usButtonData : 0;
    Private->MouseState.LeftButton = RawInput->data.mouse.usButtonFlags & RI_MOUSE_LEFT_BUTTON_DOWN ? TRUE : Private->MouseState.LeftButton;
    Private->MouseState.LeftButton = RawInput->data.mouse.usButtonFlags & RI_MOUSE_LEFT_BUTTON_UP ? FALSE : Private->MouseState.LeftButton;
    Private->MouseState.RightButton = RawInput->data.mouse.usButtonFlags & RI_MOUSE_RIGHT_BUTTON_DOWN ? TRUE : Private->MouseState.RightButton;
    Private->MouseState.RightButton =  RawInput->data.mouse.usButtonFlags & RI_MOUSE_RIGHT_BUTTON_UP ? FALSE : Private->MouseState.RightButton;
    Private->HasMouseData = TRUE;
    return 0;
  ...
  )
{
  ...
  WinNtGopInitializeSimplePointerForWindow (Private);
  ...
  )
{
  if (!Private->HardwareNeedsStarting) {
    ...
    WinNtGopDestroySimplePointerForWindow (Private);
  }
