...
// TODO:    EFI_INVALID_PARAMETER - add return value to function comment
{
  ...
  gWinNt->SystemTimeToFileTime (&SystemTime, &CurrentFileTime);

  *(UINT64 *)&CurrentFileTime = *(UINT64 *)&CurrentFileTime + mCurrentTimeAdjust;
  gWinNt->FileTimeToSystemTime (&CurrentFileTime, &SystemTime);
...
// TODO:    EFI_SUCCESS - add return value to function comment
{
  ...
  //
  // only record current time and set time different,
  // not call SetLocalTime to avoid change host time
  //
  gWinNt->GetLocalTime (&CurrentTime);
  gWinNt->SystemTimeToFileTime (&CurrentTime, &CurrentFileTime);
  gWinNt->SystemTimeToFileTime (&SystemTime, &FileTime);

  mCurrentTimeAdjust = *(UINT64 *)&FileTime - (*(UINT64 *)&CurrentFileTime);
