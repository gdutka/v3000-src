...
--*/
{
  ...
  InitialStackMemory = (EFI_PHYSICAL_ADDRESS) (UINTN) VirtualAlloc (NULL, (SIZE_T) (InitialStackMemorySize), MEM_TOP_DOWN|MEM_COMMIT, PAGE_EXECUTE_READWRITE);
  ...
  //
  FileNamePtr = (CHAR16 *)malloc (StrSize ((CHAR16 *)FirmwareVolumesStr));
  if (FileNamePtr == NULL) {
    SecPrint ("ERROR : Can not allocate memory for firmware volume string\n");
    exit (1);
  }

  StrCpyS (FileNamePtr, StrSize ((CHAR16 *)FirmwareVolumesStr) / sizeof (CHAR16), (CHAR16*)FirmwareVolumesStr);
  ...
  for (Done = FALSE, Index = 0, PeiCoreFile = NULL; !Done; Index++) {
    ...
    //
    FileNameBuf = (CHAR16*) malloc (StrSize (FileName) + StrSize (L"IA32"));
    if (FileNameBuf == NULL) {
      SecPrint ("ERROR : Can not allocate memory for firmware volume string\n");
      exit (1);
    }
    ...
    StrCpyS (FileNameBuf, (StrSize (FileName) + StrSize (L"IA32")) / sizeof (CHAR16), FileName);
    if (StrStr (FileNameBuf, L".fd") != NULL) {
#if defined (MDE_CPU_IA32)
      StrCpyS (StrStr (FileNameBuf, L".fd"), StrSize (L"IA32.fd") / sizeof (CHAR16), L"IA32.fd");
#else
      StrCpyS (StrStr (FileNameBuf, L".fd"), StrSize (L"IA32.fd") / sizeof (CHAR16), L"X64.fd");
#endif
...
--*/
{
  ...
  //
  gSystemMemory[Index].Memory = (EFI_PHYSICAL_ADDRESS) (UINTN) VirtualAlloc (NULL, (SIZE_T) (gSystemMemory[Index].Size), MEM_TOP_DOWN|MEM_COMMIT, PAGE_EXECUTE_READWRITE);
...
--*/
{
  ...
  //
  ImageContext.ImageAddress = (EFI_PHYSICAL_ADDRESS) (UINTN) VirtualAlloc (NULL, (SIZE_T) (ImageContext.ImageSize + (ImageContext.SectionAlignment * 2)), MEM_TOP_DOWN|MEM_COMMIT, PAGE_EXECUTE_READWRITE);
