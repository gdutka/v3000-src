...
#include "SnpNtEmulator.h"

EFI_DRIVER_BINDING_PROTOCOL gSnpNtEmulatorDriverBinding = {
  SnpNtEmulatorDriverBindingSupported,
  SnpNtEmulatorDriverBindingStart,
  SnpNtEmulatorDriverBindingStop,
  0xa,
  NULL,
  NULL
};

SNPNTEMULATOR_GLOBAL_DATA         gSnpNtEmulatorGlobalData = {
  SNP_NTEMULATOR_DRIVER_SIGNATURE,  //  Signature
  {
    NULL,
    NULL
  },                          //  InstanceList
  ...
  //  Private functions
  //
  SnpNtEmulatorInitializeGlobalData,            //  InitializeGlobalData
  SnpNtEmulatorInitializeInstanceData,          //  InitializeInstanceData
  SnpNtEmulatorCloseInstance                    //  CloseInstance
};
...
SNPNTEMULATOR_INSTANCE_DATA gSnpNtEmulatorInstanceTemplate = {
  SNP_NTEMULATOR_INSTANCE_SIGNATURE,            //  Signature
  {
    NULL,
    NULL
  },                                      //  Entry
  ...
  {                                       //  Snp
    ...
    SnpNtEmulatorStart,                         //  Start
    SnpNtEmulatorStop,                          //  Stop
    SnpNtEmulatorInitialize,                    //  Initialize
    SnpNtEmulatorReset,                         //  Reset
    SnpNtEmulatorShutdown,                      //  Shutdown
    SnpNtEmulatorReceiveFilters,                //  ReceiveFilters
    SnpNtEmulatorStationAddress,                //  StationAddress
    SnpNtEmulatorStatistics,                    //  Statistics
    SnpNtEmulatorMcastIptoMac,                  //  MCastIpToMac
    SnpNtEmulatorNvdata,                        //  NvData
    SnpNtEmulatorGetStatus,                     //  GetStatus
    SnpNtEmulatorTransmit,                      //  Transmit
    SnpNtEmulatorReceive,                       //  Receive
    NULL,                                 //  WaitForPacket
    NULL                                  //  Mode
  },
  {                                       //  Mode
  ...
  )
{
  ...
  SNPNTEMULATOR_GLOBAL_DATA   *GlobalData;
  LIST_ENTRY            *Entry;
  SNPNTEMULATOR_INSTANCE_DATA *Instance;

  GlobalData = &gSnpNtEmulatorGlobalData;

  NET_LIST_FOR_EACH (Entry, &GlobalData->InstanceList) {

    Instance = NET_LIST_USER_STRUCT_S (Entry, SNPNTEMULATOR_INSTANCE_DATA, Entry, SNP_NTEMULATOR_INSTANCE_SIGNATURE);
  ...
  )
{
  ...
  Instance    = SNP_NTEMULATOR_INSTANCE_DATA_FROM_SNP_THIS (This);
...
**/
EFI_STATUS
SnpNtEmulatorInitializeGlobalData (
  IN OUT SNPNTEMULATOR_GLOBAL_DATA *This
  )
{
  EFI_STATUS            Status;
  CHAR16                *DllFileNameU;
  ...
  //  And enumerate the interfaces in NTEMULATOR host
  //
  ...
  for (Index = 0; Index < InterfaceCount; Index++) {
    ...
    Instance = AllocatePool (sizeof (SNPNTEMULATOR_INSTANCE_DATA));
    ...
    //
    CopyMem (Instance, &gSnpNtEmulatorInstanceTemplate, sizeof (SNPNTEMULATOR_INSTANCE_DATA));
...
/**
  ...
  @param  This                  Pointer to the SnpNtEmulator global data.
...
**/
EFI_STATUS
SnpNtEmulatorInitializeInstanceData (
  IN SNPNTEMULATOR_GLOBAL_DATA        *This,
  IN OUT SNPNTEMULATOR_INSTANCE_DATA  *Instance
  )
{
  EFI_STATUS    Status;
...
/**
  Close the SnpNtEmulator driver instance.
...
{
  EFI_STATUS            Status;
  SNPNTEMULATOR_GLOBAL_DATA   *This;
  LIST_ENTRY            *Entry;
  SNPNTEMULATOR_INSTANCE_DATA *Instance;

  This    = &gSnpNtEmulatorGlobalData;
  ...
  )
{
  ...
  Status = EfiLibInstallDriverBindingComponentName2 (
             ImageHandle,
             SystemTable,
             &gSnpNtEmulatorDriverBinding,
             ImageHandle,
             &gSnpNtEmulatorDriverComponentName,
             &gSnpNtEmulatorDriverComponentName2
             );
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  //
  Status = SnpNtEmulatorInitializeGlobalData (&gSnpNtEmulatorGlobalData);
  if (EFI_ERROR (Status)) {
    SnpNtEmulatorUnload (ImageHandle);
  }
