  ...
  )
{
  ...
  UnicodeValueToStringS (StringBuffer, DestMax * sizeof (CHAR16), LEFT_JUSTIFY, FreqMhz / 1000, 3);
  Index = StrnLenS (StringBuffer, DestMax);
  StrCatS (StringBuffer, DestMax, L".");
  UnicodeValueToStringS (StringBuffer + Index + 1, sizeof (CHAR16) * (DestMax - (Index + 1)), PREFIX_ZERO, (FreqMhz % 1000) / 10, 2);
  ...
  )
{
  ...
  StringBufferSize = 0x20;
  StringBuffer = AllocateZeroPool (StringBufferSize);
  if (StringBuffer == NULL) {
    return;
  }
  ASSERT (StringBuffer != NULL);
  UnicodeValueToStringS (StringBuffer, StringBufferSize, LEFT_JUSTIFY, MemorySize, 6);
  StrCatS (StringBuffer, StringBufferSize / sizeof (CHAR16), L" MB RAM");
