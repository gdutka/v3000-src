...
/**
  Macro that calls DEBUG() Macro.

  If MDEPKG_NDEBUG is not defined, and the H2O_DEBUG_TRACE_OEM_SVC bit of
  PcdH2ODebugTraceErrorLevel is set, then this macro passes Expression
  to DEBUG().
...
**/
#if !defined (MDEPKG_NDEBUG)
#define DEBUG_OEM_SVC(Expression) \
  do {                              \
      if (DebugOemSvcEnable ()) {   \
        DEBUG (Expression);         \
      }                             \
    } while (FALSE)
#else
...
/**
  ...
  If MDEPKG_NDEBUG is not defined, and the H2O_DEBUG_TRACE_CP bit of
  PcdH2ODebugTraceErrorLevel is set, then this macro passes Expression
  to DEBUG().
