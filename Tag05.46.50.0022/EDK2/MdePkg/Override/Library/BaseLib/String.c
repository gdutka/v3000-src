  ...
  )
{
  ...
  ASSERT (String != NULL);
  MaxStrLen = PcdGet32 (PcdMaximumUnicodeStringLength);
  for (Length = 0; *String != L'\0'; String++, Length++) {
    //
    // If PcdMaximumUnicodeStringLength is not zero,
    // length should not more than PcdMaximumUnicodeStringLength
    //
    if ((MaxStrLen != 0) && (Length > MaxStrLen)) {
      ASSERT (FALSE);
    }
  }
  ...
  )
{
  ...
  //
  ASSERT (PcdGet32 (PcdMaximumUnicodeStringLength) == 0 || StrSize (FirstString) != 0);
  ASSERT (PcdGet32 (PcdMaximumUnicodeStringLength) == 0 || StrSize (SecondString) != 0);
  ...
  )
{
  ...
  // Length tests are performed inside StrLen().
  //
  ASSERT (PcdGet32 (PcdMaximumUnicodeStringLength) == 0 || StrSize (String) != 0);
  ASSERT (PcdGet32 (PcdMaximumUnicodeStringLength) == 0 || StrSize (SearchString) != 0);
  ...
  )
{
  ...
  ASSERT (String != NULL);
  MaxStrLen = PcdGet32 (PcdMaximumAsciiStringLength);
