  ...
  )
{
  ...
  UINT8                    ResetDeviceString[] = { 0x1B, 0x63 };  // <ESC>, 'c'
  ...
  if (PcdGetBool (PcdStatusCodeUseSerial)) {
    ...
    // prevent when console redirection is enabled, and then reboot system during draw line in SCU,
    // EFI_DEBUG message will be scramble.
    //
