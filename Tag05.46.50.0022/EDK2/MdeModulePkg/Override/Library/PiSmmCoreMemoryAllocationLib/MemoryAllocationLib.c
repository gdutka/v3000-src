  ...
  )
{
  ...
  //
  // System may call FreePool () in SmmInitializeMemoryServices (). In this situation, the
  // input buffer is a memory in SMRAM but the BufferInSmram () will return FALSE. Consider
  // mSmmCoreMemoryAllocLibSmramRangeCount == 0 is the case of FreePool () is invoked in
  // SmmInitializeMemoryServices ().
  //
  if (BufferInSmram (Buffer) || (mSmmCoreMemoryAllocLibSmramRangeCount == 0)) {
    //
