...
SCRIPT_TABLE_PRIVATE_DATA  *mS3BootScriptTablePtr = NULL;

//
// Allocate SMM copy because we can not use mS3BootScriptTablePtr after SmmReadyToLock in InSmm.
//
SCRIPT_TABLE_PRIVATE_DATA  *mS3BootScriptTableSmmPtr = NULL;
  ...
  )
{
  ...
  if (!IsInSmm ()) {
    ...
    if (S3TablePtr == 0) {
      ...
      ASSERT_EFI_ERROR (Status);
      if (EFI_ERROR (Status)) {
        Status = gBS->FreePages ((EFI_PHYSICAL_ADDRESS)(UINTN)S3TablePtr, EFI_SIZE_TO_PAGES (sizeof (SCRIPT_TABLE_PRIVATE_DATA)));
        ASSERT_EFI_ERROR (Status);
  ...
  mS3BootScriptTablePtr = (SCRIPT_TABLE_PRIVATE_DATA *)(UINTN)PcdGet64 (PcdS3BootScriptTablePrivateDataPtr);
  ASSERT (mS3BootScriptTablePtr != NULL);
  Status = gBS->LocateProtocol (
                  &gEfiSmmBase2ProtocolGuid,
                  NULL,
                  (VOID **)&SmmBase2
  ...
  if (S3TableSmmPtr == 0) {
    ...
    ASSERT_EFI_ERROR (Status);
    if (EFI_ERROR (Status)) {
      Status = mBootScriptSmst->SmmFreePool (S3TableSmmPtr);
      ASSERT_EFI_ERROR (Status);
      return RETURN_SUCCESS;
    }
  ...
  )
{
  ...
  if (S3TableBase == 0) {
    ...
    Status      = gBS->AllocatePages (
                         AllocateMaxAddress,
                         EfiReservedMemoryType,
                         2 + PcdGet16 (PcdS3BootScriptRuntimeTableReservePageNumber) + PcdGet16 (PcdH2OS3BootScriptTableReservePageNumber),
                         (EFI_PHYSICAL_ADDRESS *)&S3TableBase
                         );
    ...
    mS3BootScriptTablePtr->TableMemoryPageNumber = (UINT16)(2 + PcdGet16 (PcdS3BootScriptRuntimeTableReservePageNumber) + PcdGet16 (PcdH2OS3BootScriptTableReservePageNumber));
  }
  ...
  )
{
  ...
  if (mS3BootScriptTablePtr == NULL) {
    //
    // Constructor was failed. Return NULL.
    //
    DEBUG ((DEBUG_ERROR, "S3BootScriptGetEntryAddAddress - FATAL ERROR: Constructor was failed!\n"));
    return NULL;
  }
  ...
  )
{
  ...
  if (mS3BootScriptTablePtr == NULL) {
    //
    // Constructor was failed. Return 0.
    //
    DEBUG ((DEBUG_ERROR, "S3BootScriptCloseTable - FATAL ERROR: Constructor was failed!\n"));
    return 0;
  }
