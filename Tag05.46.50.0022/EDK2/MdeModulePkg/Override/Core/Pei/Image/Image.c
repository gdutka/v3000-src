...
/**
  ...
  @retval EFI_OUT_OF_RESOURCES     Internal buffer is too small.
  @retval EFI_NOT_FOUND            The EFI_SECTION_USER_INTERFACE section was not found.
  ...
  )
{
  ...
  if ((StringBuffer == NULL) || (StringBufferSize < 4)) {
  ...
  for (Index = 0; Index < StringBufferSize - 4; Index++) {
    StringBuffer[Index] = (CHAR8)NameString[Index];
