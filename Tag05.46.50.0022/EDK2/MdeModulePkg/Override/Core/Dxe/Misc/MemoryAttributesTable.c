  ...
  )
{
  ...
  Status           = CoreGetMemoryMap (MemoryMapSize, MemoryMap, MapKey, &LocalDescriptorSize, DescriptorVersion);
  if (Status == EFI_BUFFER_TOO_SMALL) {
    *MemoryMapSize = *MemoryMapSize + LocalDescriptorSize * AdditionalRecordCount;
  } else if (Status == EFI_SUCCESS) {
    ASSERT (MemoryMap != NULL);
    if (OldMemoryMapSize - *MemoryMapSize < LocalDescriptorSize * AdditionalRecordCount) {
      *MemoryMapSize = *MemoryMapSize + LocalDescriptorSize * AdditionalRecordCount;
      //
      ...
      // Split PE code/data
      //
      SplitTable (MemoryMapSize, MemoryMap, LocalDescriptorSize);
    }
  }
  ...
  )
{
  ...
  //
  // The content of ImageRecord will be changed after calling SortImageRecord (). Need check
  // CodeSegmentCountMax before calling SortImageRecord ().
  //
