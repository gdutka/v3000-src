...
/**
  Get image name from the EFI_SECTION_USER_INTERFACE section of firmware file.

  @param  LoadedImage              Pointer to the loaded image protocol.
  @param  StringBufferSize         The size of the StringBuffer.
  @param  StringBuffer             Output buffer that contains a Null-terminated ASCII image name string.

  @retval EFI_SUCCESS              Get image name successfully.
  @retval EFI_INVALID_PARAMETER    Input pointer is NULL or StringBufferSize < 4.
  @retval EFI_OUT_OF_RESOURCES     An allocation failure occurred.
  @retval Others                   Read section error.

**/
EFI_STATUS
GetImageNameFromUiSection (
  ...
  )
{
  ...
  if ((LoadedImage == NULL) || (StringBuffer == NULL) || (StringBufferSize < 4)) {
    return EFI_INVALID_PARAMETER;
  }

  if ((LoadedImage->FilePath == NULL) || (LoadedImage->FilePath->Type != MEDIA_DEVICE_PATH) ||
  ...
  UnicodeStrToAsciiStrS (NameString, AsciiNameString, StrSize (NameString) / sizeof (CHAR16));

  //
  // Append ".efi" to image name and copy it to the StringBuffer
  //
  for (Index = 0; Index < StringBufferSize - 4; Index++) {
    StringBuffer[Index] = AsciiNameString[Index];
    if (StringBuffer[Index] == 0) {
      StringBuffer[Index] = '.';
    }

    if (StringBuffer[Index] == '.') {
      StringBuffer[Index + 1] = 'e';
      StringBuffer[Index + 2] = 'f';
      StringBuffer[Index + 3] = 'i';
      StringBuffer[Index + 4] = 0;
      break;
    }
  }

  if (Index == StringBufferSize - 4) {
    StringBuffer[Index] = 0;
  }

  gBS->FreePool (NameString);
  FreePool (AsciiNameString);
  ...
  )
{
  ...
  if (Image->ImageContext.PdbPointer != NULL) {
    ...
    //
    // Print Module Name by EFI_SECTION_USER_INTERFACE section
    //
    Status = GetImageNameFromUiSection (
               &Image->Info,
               sizeof (EfiFileName),
               EfiFileName
               );
