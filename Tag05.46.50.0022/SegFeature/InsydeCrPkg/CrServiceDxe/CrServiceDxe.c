  ...
  )
{
  ...
  while (!IsNull (&mCrVarDeviceHead.Link, Link)) {
    ...
    Status = GetProtocolWithLocateDevicePath (&gH2OCrUart16550AccessProtocolGuid, &RemainDevPath, (VOID **)&Uart16550Protocol);
    if (EFI_ERROR (Status)) {
      Link  = GetNextNode (&mCrVarDeviceHead.Link, Link);
      continue;
    }
    ...
    RemainDevPath = GetDevicePathFromCrDevSetting(CrDevNode->CrSetting);
    Status = GetProtocolWithLocateDevicePath (&gEfiDevicePathProtocolGuid, &RemainDevPath, (VOID **)&DevPathProtocol);
    if (!EFI_ERROR (Status)) {
