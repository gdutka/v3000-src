...
EFI_GUID mCrTerminalGuidTable [] = {
  ...
  DEVICE_PATH_MESSAGING_LOG_TERM,
  EFI_TTY_TERM_GUID,
  ...
  )
{
  ...
  Size = sizeof (UINT8);
  Status = gRT->GetVariable (VarName, &gH2OCrConfigurationGuid, NULL, &Size, &Data);
  ...
  )
{
  ...
  //                    0x05 - Space Parity
  //
  Attribute->Parity   = (Uart->Parity >= 1 && Uart->Parity <= 5)? Uart->Parity : 1;
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    Index = CrConfig->GlobalBaudRate;
    //
    // Check BaudRate valid. Make sure it isn't out-of-array boundry.
    //
    if (Index >= ARRAY_SIZE (mCrBaudRateTable) - 1) {
      Index = ARRAY_SIZE (mCrBaudRateTable) - 2;
    }
    mUartNode.BaudRate              = mCrBaudRateTable[Index];

    //
    // Check DataBits vaild. only support 0x07 - 7 Bits
    //                                    0x08 - 8 Bits
    //
    mUartNode.DataBits              = ((CrConfig->GlobalDataBits == 7) || (CrConfig->GlobalDataBits == 8)) ? CrConfig->GlobalDataBits : 8;
    ...
    //                                  0x05 - Space Parity
    //
    mUartNode.Parity                = ((CrConfig->GlobalParity >= 1) && (CrConfig->GlobalParity <= 5)) ? CrConfig->GlobalParity : 1;

    //
    // Check StopBits vaild. only support  0x01 - 1 Stop Bit
    //                                     0x03 - 2 Stop Bits
    //
    mUartNode.StopBits              = ((CrConfig->GlobalStopBits == 1) || (CrConfig->GlobalStopBits == 3)) ? CrConfig->GlobalStopBits : 1;

    Index = CrConfig->GlobalTerminalType;
    //
    // Check TerminalType valid. Make sure it isn't out-of-array boundry.
    //
    if (Index >= ARRAY_SIZE (mCrTerminalGuidTable) - 1) {
      Index = 0;
    }
    mTerminalNode.Guid              = mCrTerminalGuidTable[Index];
    
    //
    // Check FlowControl vaild. only support 0x00 - None
    //                                       0x01 - Hardware
    //                                       0x02 - Software
    //
    mFlowControlNode.FlowControlMap = (CrConfig->GlobalFlowControl <= 2) ? CrConfig->GlobalFlowControl : 0;
    FreePool (CrConfig);
  }
}
  ...
  )
{
  ...
  if (TempDevPath == NULL) {
    ...
    //                                  0x05 - Space Parity
    //
    if (!(Uart->Parity >= 1 && Uart->Parity <= 5)) {
  ...
  )
{
  ...
  for (Index = 0;  Index < MAX_CR_DEVICE_VARIABLE; Index++) {
    ...
    if (!CrDevSetting->Exist) {
      continue;
    }

    if (Index < COMMON_CR_DEVICE_SUPPORT_NUM) {
      CrConfig->CrPortConfig[Index].PortEnable          = CrDevSetting->Attribute.PortEnable;
      CrConfig->CrPortConfig[Index].UseGlobalSetting    = CrDevSetting->Attribute.UseGlobalSetting;
      if (CrDevSetting->Attribute.UseGlobalSetting == 1) {
        CrConfig->CrPortConfig[Index].TerminalType        = CrConfig->GlobalTerminalType;
        CrConfig->CrPortConfig[Index].BaudRate            = CrConfig->GlobalBaudRate;
        CrConfig->CrPortConfig[Index].Parity              = CrConfig->GlobalParity;
        CrConfig->CrPortConfig[Index].DataBits            = CrConfig->GlobalDataBits;
        CrConfig->CrPortConfig[Index].StopBits            = CrConfig->GlobalStopBits;
        CrConfig->CrPortConfig[Index].FlowControl         = CrConfig->GlobalFlowControl;
      } else {
        CrConfig->CrPortConfig[Index].TerminalType        = CrDevSetting->Attribute.TerminalType;
        CrConfig->CrPortConfig[Index].BaudRate            = CrDevSetting->Attribute.BaudRate;
        CrConfig->CrPortConfig[Index].Parity              = CrDevSetting->Attribute.Parity;
        CrConfig->CrPortConfig[Index].DataBits            = CrDevSetting->Attribute.DataBits;
        CrConfig->CrPortConfig[Index].StopBits            = CrDevSetting->Attribute.StopBits;
        CrConfig->CrPortConfig[Index].FlowControl         = CrDevSetting->Attribute.FlowControl;
      }
    } else {
      CrConfig->ExCrPortConfig[Index - COMMON_CR_DEVICE_SUPPORT_NUM].PortEnable          = CrDevSetting->Attribute.PortEnable;
      CrConfig->ExCrPortConfig[Index - COMMON_CR_DEVICE_SUPPORT_NUM].UseGlobalSetting    = CrDevSetting->Attribute.UseGlobalSetting;
      if (CrDevSetting->Attribute.UseGlobalSetting == 1) {
        CrConfig->ExCrPortConfig[Index - COMMON_CR_DEVICE_SUPPORT_NUM].TerminalType        = CrConfig->GlobalTerminalType;
        CrConfig->ExCrPortConfig[Index - COMMON_CR_DEVICE_SUPPORT_NUM].BaudRate            = CrConfig->GlobalBaudRate;
        CrConfig->ExCrPortConfig[Index - COMMON_CR_DEVICE_SUPPORT_NUM].Parity              = CrConfig->GlobalParity;
        CrConfig->ExCrPortConfig[Index - COMMON_CR_DEVICE_SUPPORT_NUM].DataBits            = CrConfig->GlobalDataBits;
        CrConfig->ExCrPortConfig[Index - COMMON_CR_DEVICE_SUPPORT_NUM].StopBits            = CrConfig->GlobalStopBits;
        CrConfig->ExCrPortConfig[Index - COMMON_CR_DEVICE_SUPPORT_NUM].FlowControl         = CrConfig->GlobalFlowControl;
      } else {
        CrConfig->ExCrPortConfig[Index - COMMON_CR_DEVICE_SUPPORT_NUM].TerminalType        = CrDevSetting->Attribute.TerminalType;
        CrConfig->ExCrPortConfig[Index - COMMON_CR_DEVICE_SUPPORT_NUM].BaudRate            = CrDevSetting->Attribute.BaudRate;
        CrConfig->ExCrPortConfig[Index - COMMON_CR_DEVICE_SUPPORT_NUM].Parity              = CrDevSetting->Attribute.Parity;
        CrConfig->ExCrPortConfig[Index - COMMON_CR_DEVICE_SUPPORT_NUM].DataBits            = CrDevSetting->Attribute.DataBits;
        CrConfig->ExCrPortConfig[Index - COMMON_CR_DEVICE_SUPPORT_NUM].StopBits            = CrDevSetting->Attribute.StopBits;
        CrConfig->ExCrPortConfig[Index - COMMON_CR_DEVICE_SUPPORT_NUM].FlowControl         = CrDevSetting->Attribute.FlowControl;
      }
    }
  ...
  )
{
  ...
  if (EFI_ERROR (Status) || (BufferSize < sizeof (CR_CONFIGURATION))) {
    return EFI_DEVICE_ERROR;
  }
  ...
  for (Index = 0;  Index < MAX_CR_DEVICE_VARIABLE; Index++) {
    ...
    if (Index < COMMON_CR_DEVICE_SUPPORT_NUM) {
      CrDevSetting->Attribute.PortEnable       = CrConfig->CrPortConfig[Index].PortEnable;
      CrDevSetting->Attribute.UseGlobalSetting = CrConfig->CrPortConfig[Index].UseGlobalSetting;
      if (CrDevSetting->Attribute.UseGlobalSetting == 1) {
        CrDevSetting->Attribute.TerminalType     = CrConfig->GlobalTerminalType;
        CrDevSetting->Attribute.BaudRate         = CrConfig->GlobalBaudRate;
        CrDevSetting->Attribute.Parity           = CrConfig->GlobalParity;
        CrDevSetting->Attribute.DataBits         = CrConfig->GlobalDataBits;
        CrDevSetting->Attribute.StopBits         = CrConfig->GlobalStopBits;
        CrDevSetting->Attribute.FlowControl      = CrConfig->GlobalFlowControl;
      } else {
        CrDevSetting->Attribute.TerminalType     = CrConfig->CrPortConfig[Index].TerminalType;
        CrDevSetting->Attribute.BaudRate         = CrConfig->CrPortConfig[Index].BaudRate;
        CrDevSetting->Attribute.Parity           = CrConfig->CrPortConfig[Index].Parity;
        CrDevSetting->Attribute.DataBits         = CrConfig->CrPortConfig[Index].DataBits;
        CrDevSetting->Attribute.StopBits         = CrConfig->CrPortConfig[Index].StopBits;
        CrDevSetting->Attribute.FlowControl      = CrConfig->CrPortConfig[Index].FlowControl;
      }
    } else {
      CrDevSetting->Attribute.PortEnable       = CrConfig->ExCrPortConfig[Index - COMMON_CR_DEVICE_SUPPORT_NUM].PortEnable;
      CrDevSetting->Attribute.UseGlobalSetting = CrConfig->ExCrPortConfig[Index - COMMON_CR_DEVICE_SUPPORT_NUM].UseGlobalSetting;
      if (CrDevSetting->Attribute.UseGlobalSetting == 1) {
        CrDevSetting->Attribute.TerminalType     = CrConfig->GlobalTerminalType;
        CrDevSetting->Attribute.BaudRate         = CrConfig->GlobalBaudRate;
        CrDevSetting->Attribute.Parity           = CrConfig->GlobalParity;
        ...
        CrDevSetting->Attribute.TerminalType     = CrConfig->ExCrPortConfig[Index - COMMON_CR_DEVICE_SUPPORT_NUM].TerminalType;
        CrDevSetting->Attribute.BaudRate         = CrConfig->ExCrPortConfig[Index - COMMON_CR_DEVICE_SUPPORT_NUM].BaudRate;
        CrDevSetting->Attribute.Parity           = CrConfig->ExCrPortConfig[Index - COMMON_CR_DEVICE_SUPPORT_NUM].Parity;
        CrDevSetting->Attribute.DataBits         = CrConfig->ExCrPortConfig[Index - COMMON_CR_DEVICE_SUPPORT_NUM].DataBits;
        CrDevSetting->Attribute.StopBits         = CrConfig->ExCrPortConfig[Index - COMMON_CR_DEVICE_SUPPORT_NUM].StopBits;
        CrDevSetting->Attribute.FlowControl      = CrConfig->ExCrPortConfig[Index - COMMON_CR_DEVICE_SUPPORT_NUM].FlowControl;
      }
    }

    UpdateOrDeleteCrDevVarWithDevInfo (Index, CrDevSetting);
