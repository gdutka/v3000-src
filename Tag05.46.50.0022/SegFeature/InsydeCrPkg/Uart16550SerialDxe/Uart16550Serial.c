  ...
  )
{
  ...
  //
  // To avoid the printing string being cut while the key buffer be cleared.
...
//[-start-220209-IB05580778-add]//
  SERIAL_PORT_LSR           Lsr;
  UINTN                     Count;
//[-end-220209-IB05580778-add]//

  SerialDevice = SERIAL_DEV_FROM_THIS (This);

//[-start-220209-IB05580778-add]//
  //
  // wait for efidebug message flush out
  //
  Count = 0;
  Lsr.Data = READ_LSR (SerialDevice->Uart16550);
  while (!Lsr.Bits.Temt) {
    Count++;
    if (Count >= 100) {
      DEBUG ((DEBUG_ERROR, "ERROR : Serial device LSR.TEMT not ready in 100 reads LSR.\n"));
      break;
    }
    gBS->Stall (10);
    Lsr.Data = READ_LSR (SerialDevice->Uart16550);
