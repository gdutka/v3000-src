...
/**
  Use to judge the current Pci device if described in PCD LIST
  
  @param[in]  PciIo        The current Pci device io Protocol
  @retval     TRUE         The device is HsUart
  @retval     FALSE        The device is not HsUart  
*/
BOOLEAN
CheckIfHsUart (
  IN  EFI_PCI_IO_PROTOCOL   *PciIo
  )
{
  ...
  if (EFI_ERROR(Status)) {
    // Return True to caller to make it unsupport
    return TRUE;
  }
  ...
  )
{
  ...
  //
  // Judge if current device is HsUart which declared at PCD list
  // If yes that return unsupported
  //
  if (CheckIfHsUart (PciIo)) {
    //
    // Don't directly return EFI_UNSUPPORTED, need to close protocol first
    // to prevent the situation that HS driver OpenProtocol fail
    //
    Status = EFI_UNSUPPORTED;
    goto Exit;
  }
