...
[PcdsFixedAtBuild]
  ...
  #    Note: To change to use specific terminal type, can append message device path node after uart device path node.
  #          Use VenVt100(),VenVt100Plus(),VenUtf8(),VenPcAnsi() for Vt100,Vt100+,Utf8,PcAnsi terminal type,
  #          or use VenMsg(terminal type guid) for specific terminal type. Ex: VenMsg(F3895AE6-96BA-4478-83A9-47078B6D8187) for Log-Term terminal type.
  #          For supported terminal type guid, please refer "mCrTerminalGuidTable" table in CrDeviceVariableMisc.c
  #          
  ...
  gInsydeCrTokenSpaceGuid.PcdH2OCrPolicy| { \
    ...
    UINT8(0x00),     \  # Reserve (compatiable) |              |
    UINT8(0x00),     \  # Reserve (compatiable) |              |
    UINT8(0x01),     \  # CRForceAsyncForLegacy |  Disable     |  Enable
