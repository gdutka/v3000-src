...
//
// Used to convert TERMINAL_TYPE value to TP_XXXX defined in TerminalEscCode.h file.
//   For Lig term type, change to use VT100 escape code.
//
static UINT16  mTerminalFlagTable [] = {
 TP_PCANSI,   // TerminalTypePcAnsi
 TP_VT100,    // TerminalTypeVt100
 TP_VT100P,   // TerminalTypeVt100Plus
 TP_VTUTF8,   // TerminalTypeVtUtf8
 TP_TTY,      // TerminalTypeTtyTerm
 TP_LINUX,    // TerminalTypeLinux
 TP_XTERM_R6, // TerminalTypeXtermR6
 TP_VT400,    // TerminalTypeVt400
 TP_SCO,      // TerminalTypeSCO
 TP_VT100     // TerminalTypeLogTerm
 };
  ...
  )
{
  ...
  if (!EfiKeyFiFoRemoveOneKey (TerminalDevice, KeyData)) {
    KeyData->KeyState.KeyToggleState = TerminalDevice->KeyState.KeyToggleState;
...
//[-start-220308-IB04880007-add]//
  if ((KeyData.KeyState.KeyShiftState & (EFI_LEFT_CONTROL_PRESSED | EFI_RIGHT_CONTROL_PRESSED)) != 0) {
    if (Key->UnicodeChar >= 'a' && Key->UnicodeChar <= 'z') {
      Key->UnicodeChar = Key->UnicodeChar - 'a' + 1;
    }
  }
//[-end-220308-IB04880007-add]//

  return EFI_SUCCESS;
}

/**
  Convert KeyShiftState to Keyflag for shift, alt and ctrl.

  @param  KeyData     A pointer to a buffer that is filled in with the
                      keystroke state data.

  @return KeyFlag     Key Flag.
  ...
  )
{
  ...
  if (KeyData->KeyState.KeyShiftState & EFI_SHIFT_STATE_VALID) {
    if (KeyData->KeyState.KeyShiftState & (EFI_RIGHT_SHIFT_PRESSED | EFI_LEFT_SHIFT_PRESSED)) {
      KeyFlag |= SHIFT_KEY_ENABLE_FLAG;
    }

    if (KeyData->KeyState.KeyShiftState & (EFI_RIGHT_ALT_PRESSED | EFI_LEFT_ALT_PRESSED)) {
      KeyFlag |= ALT_KEY_ENABLE_FLAG;
    }

    if (KeyData->KeyState.KeyShiftState & (EFI_RIGHT_CONTROL_PRESSED | EFI_LEFT_CONTROL_PRESSED)) {
      KeyFlag |= CTRL_KEY_ENABLE_FLAG;
    }
  }
  ...
  )
{
  ...
  if (RegsiteredData->KeyState.KeyShiftState != 0 &&
      ConvertKeyFlag (RegsiteredData) != ConvertKeyFlag (InputData)) {
    return FALSE;
  }
  ...
  )
{
  ...
  // update toggle state
  //
  KeyState = &TerminalDevice->KeyState;
  ...
  )
{
  ...
  while (TRUE) {
    ...
    //
    if ((TerminalDevice->CrsEnable) && (TerminalDevice->CsmEnable)) {
      SetMonitorKey (Key);
    }
  }
}
  ...
  )
{
  ...
  CopyMem (&EfiKeyFiFo->Data[Tail], Input, sizeof (EFI_KEY_DATA));
  ...
  )
{
  ...
  CopyMem (Output, &EfiKeyFiFo->Data[Head], sizeof (EFI_KEY_DATA));
  ...
  )
{
  ...
  for (Index = 0, BitIndex = 0; FilterKeyList[Index] != 0; Index += 3, BitIndex ++) {
    if ((FilterKeyList[Index] == (UINT8) ScanCode) && (FilterKeyList[Index + 1] == ConvertKeyFlag(&Key))) {
...
/**
  Reset escape code state to default value.
  ...
  )
{
  ...
  Count = gEscSequenceCodeSize / sizeof(ESC_SEQUENCE_CODE) - 1;
  
  for (Index = 0; Index < Count; Index++) {
    pEscSeqCodeTable[Index].EscCodeState = ESC_CODE_STATE_DEFAULT;
  }
}
  ...
  )
{
  ...
  if ((TerminalDevice->TerminalType == TerminalTypeLogTerm) && !PcdGetBool(PcdH2OCrLogTermAcceptSequenceCode)) {
    return ESC_CODE_INVALID;
  }
  ...
  for (Index = 0; pEscSeqCodeTable[Index].TerminalType != 0; Index++) {
    ...
    if ((TerminalDevice->TerminalFeatureFlag & CR_TERMINAL_KEY_FLAG) == CR_TERMINAL_KEY_SENSITIVE) {
      if ((pEscSeqCodeTable[Index].TerminalType & mTerminalFlagTable[TerminalDevice->TerminalType]) == 0) {
        continue;
      }
    }

    //
    // Skip comparied escape code string and doesn't match with exist escape code in the escape code table.
    //
    if (pEscSeqCodeTable[Index].EscCodeState == ESC_CODE_STATE_NOT_MATCH) {
      continue;
    }
    ...
    case ESC_CODE_UNFINISHED :
      pEscSeqCodeTable[Index].EscCodeState = ESC_CODE_STATE_MATCHING;
    ...
    default:
      pEscSeqCodeTable[Index].EscCodeState = ESC_CODE_STATE_NOT_MATCH;
      break;
    }
  }
...
CHAR16
ConvertKeyScancodeToAlphabet (
  IN UINT16   KeyScancode
  )
{
  switch (KeyScancode) {
  // ctrl + M
  ...
  )
{
  ...
  case ESC_CODE_CONTROL :
    Key.Key.ScanCode    = 0;
    Key.Key.UnicodeChar = ConvertKeyScancodeToAlphabet (pEscSeqCodeTable[TableIndex].Data);
    Key.KeyState.KeyShiftState |= (EFI_LEFT_CONTROL_PRESSED | EFI_RIGHT_CONTROL_PRESSED);
    EfiKeyFiFoInsertOneKey (TerminalDevice, Key);
    break;

  case ESC_CODE_ALTCODE :
    if (IsAlphabet((UINT8)pEscSeqCodeTable[TableIndex].Data)) {
      Key.Key.ScanCode = SCAN_NULL;
...
VOID
DealInvalidEscCodeProcess (
  IN TERMINAL_DEV    *TerminalDevice,
  IN BOOLEAN         DoFinalCheck
  )
{
  UNICODE_FIFO    *pTempFiFo;
  EFI_KEY_DATA    Key;

  ResetEscCodeState (gEscSequenceCode);
  ...
  if ((pTempFiFo->Data[0] != 0) && (pTempFiFo->Data[1] == 0)) {
    //
    // TempFifo only has one character, check to see if it is alphabet or digital number.
    // If it is, convert to alt combo key, otherwise clean up TempFifo.
    //
    if (IsAlphabet((UINT8)pTempFiFo->Data[0])) {
      Key.Key.ScanCode = SCAN_NULL;
      Key.Key.UnicodeChar = pTempFiFo->Data[0];
      Key.KeyState.KeyShiftState |= (EFI_LEFT_ALT_PRESSED | EFI_RIGHT_ALT_PRESSED);
      EfiKeyFiFoInsertOneKey (TerminalDevice, Key);
      TempFiFoCleanUp(TerminalDevice);
      TerminalDevice->InputState = INPUT_STATE_DEFAULT;
  ...
  } else if (pTempFiFo->Data[0] == 0) {
    //
    // If TempFifo if empty. Do TempFiFoFlush to send ESC key.
    //
    TempFiFoFlush (TerminalDevice);
  } else {
    //
    // TempFifo has two or more characters, do TempFiFoCleanUp to clean FIFO only to avoid sending ESC key.
    //
    TempFiFoCleanUp(TerminalDevice);
  }
  ...
  )
{
  ...
  case ESC_CODE_INVALID :
    DealInvalidEscCodeProcess (TerminalDevice, TRUE);
    break;
  ...
  while (!IsUnicodeFiFoEmpty(TerminalDevice) && !IsEfiKeyFiFoFull (TerminalDevice)) {
    ...
    if (TerminalDevice->InputState == INPUT_STATE_DEFAULT) {
      ...
      if (UnicodeChar == ESC) {
        ...
        //
        if (gCrSpecialCommandSize > sizeof(CR_SPECIAL_COMMAND)) {
          ProcessCrSpecialCommand (TerminalDevice, gCrSpecialCommand, UnicodeChar);
        }
        if (UnicodeChar == 127) {
          //
          // ASCII code 0x7f for DEL key
          //
          Key.Key.ScanCode    = SCAN_DELETE;
          Key.Key.UnicodeChar = 0;
        } else if ((UnicodeChar < 27) && !TerminalIsValidEfiCntlChar (UnicodeChar)) {
          //
          // Convert ASCII control code to Alphabet code and set control pressed state to EFI key status
          //
          Key.Key.ScanCode = SCAN_NULL;
          Key.Key.UnicodeChar = L'a' + UnicodeChar - 1;
          Key.KeyState.KeyShiftState |= (EFI_LEFT_CONTROL_PRESSED | EFI_RIGHT_CONTROL_PRESSED);
        } else {
          Key.Key.ScanCode    = SCAN_NULL;
          Key.Key.UnicodeChar = UnicodeChar;
      ...
      switch (LookUpStatus) {
      case ESC_CODE_INVALID :
        DealInvalidEscCodeProcess (TerminalDevice, FALSE);
        break;
