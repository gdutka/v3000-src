  ...
  )
{
  ...
  if (mCrPolicyVar->Feature.Bit.CRAsyncTerm == TRUE) {
    //
    // After EndOfBootSelection event, always change Terminal mode to async mode if force async mode enabled.
    //
    mTerminalMode = ASYNC;
  }
  ...
  if (RemainingDevicePath == NULL) {
    ...
    if (!IsDevicePathEnd (RemainingDevicePath)) {
      ...
      if (!CompareGuid (&Node->Guid, &gEfiPcAnsiGuid) &&
          ...
          !CompareGuid (&Node->Guid, &gEdkiiLinuxTermGuid) &&
          !CompareGuid (&Node->Guid, &gEdkiiXtermR6Guid) &&
          !CompareGuid (&Node->Guid, &gEdkiiVT400Guid) &&
          !CompareGuid (&Node->Guid, &gEdkiiSCOTermGuid) &&
...
/**
  Initialize terminal fifo for save EfiKey Notify.

   @param    TerminalDevice  terminal device

**/
VOID
InitializeEfiKeyFiFoForNotify (
  IN  TERMINAL_DEV  *TerminalDevice
  )
{
  //
  // Make the EfiKeyFiFoForNotify fifo empty
  //
  ...
  )
{
  ...
  if (FirstEnter) {
    ...
    if (RemainingDevicePath == NULL) {
      ...
      CopyMem (&DefaultNode->Guid, mTerminalType[TerminalType], sizeof (EFI_GUID));
    ...
    if (!EFI_ERROR (Status)) {
      TerminalDevice->CsmEnable = TRUE;
    } else {
      TerminalDevice->CsmEnable = FALSE;
    }
    ...
    InitializeEfiKeyFiFoForNotify (TerminalDevice);
  ...
  )
{
  ...
  //
  for (TerminalType = TerminalTypePcAnsi; TerminalType <= TerminalTypeLogTerm; TerminalType++) {
