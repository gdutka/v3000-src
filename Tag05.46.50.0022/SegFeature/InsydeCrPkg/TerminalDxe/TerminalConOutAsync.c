...
/**
  ...
  @retval EFI_SUCCESS        Success for updating this row.
  @retval Others             Skip and do nothing for this row.
  ...
  )
{
  ...
  ColumnIndex = ColUpdateHead;
  while (ColumnIndex < ColUpdateTail) {
  ...
  )
{
  ...
  for (; *WString != CHAR_NULL; WString++) {
    ...
    default:
      ...
      //
      if (TerminalDevice->TerminalType == TerminalTypeVtUtf8) {
        if ((AtributeSetting & EFI_WIDE_ATTRIBUTE) == 0) {
          //
          // When string is \narrow "wide char", we need fix glyph width
          //
          GlyphWidth = ConsoleLibGetGlyphWidth (Unicode);
        } else {
          //
          // Convert all narrow characters(ascii code 0x20, 0x21-0x7E) between "\wide' and "\narrow" to
          // wide character(unicode 0x3000, 0xFF01-0xFF5E).
          //
        ...
        //
        // Some unicode char glyph display width less than 8 pixel, ex: U+2019
        // So always puts space char to the position of next cursor column to clear screen.
        //
        if (GlyphWidth >= 2) {
          ScrChar[(Mode->CursorRow * MaxColumn) + Mode->CursorColumn + 1] = 0x0020;
        }
