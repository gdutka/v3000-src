  ...
  )
{
  ...
  UINT8                            MyNameValue0;
  CHAR16                           BackupChar;
//[-start-180508-IB08400589-add]// 
  CHAR16                           *StrPointer;
//[-end-180508-IB08400589-add]//
  ...
  *Progress         = Request;
  ...
  if (Request == NULL) {
    ...
    //
    ConfigRequestHdr = HiiConstructConfigHdr (&gH2OSolConfigUtilVarstoreGuid, mVariableName, mPrivateData->DriverHandle);
    ...
    //
    if (!HiiIsConfigHdrMatch (Request, &gH2OSolConfigUtilVarstoreGuid, NULL)) {
      return EFI_NOT_FOUND;
    }
    //
    ...
    //
    if (!HiiIsConfigHdrMatch (Request, &gH2OSolConfigUtilVarstoreGuid, mVariableName)) {
      return EFI_NOT_FOUND;
    }
    //
...
//[-start-180508-IB08400589-modify]//
...
//[-start-180508-IB08400589-modify]//
  if (!HiiIsConfigHdrMatch(Configuration, &gH2OSolConfigUtilVarstoreGuid, mVariableName)) {
    return EFI_NOT_FOUND;
  }
//[-end-180508-IB08400589-modify]//
...
//[-start-180823-IB10920058-add]//
  SOL_CONFIGURATION                          Configuration = {0};
  UINTN                                      ConfigSize;
...
//[-start-180823-IB10920058-add]//
    if (QuestionId == KEY_LOAD_DEFAULT) {
      ...
      //
      DEBUG ((EFI_D_INFO, "Load VAR default.\n"));
      ConfigSize = sizeof (SOL_CONFIGURATION);
  ...
  )
{
  ...
  //
  ConfigRequestHdr = HiiConstructConfigHdr (&gH2OSolConfigUtilVarstoreGuid, mVariableName, mDriverHandle);
  ...
  BufferSize = sizeof (SOL_CONFIGURATION);
  Status = gRT->GetVariable (mVariableName, &gH2OSolConfigUtilVarstoreGuid, NULL, &BufferSize, SolConfig);
//[-start-180823-IB10920058-add]//
  if (EFI_ERROR (Status) || (Status == EFI_SUCCESS && !IsVariableInVariableStoreRegion (mVariableName, &gH2OSolConfigUtilVarstoreGuid))) {
//[-end-180823-IB10920058-add]//
//[-start-180904-IB10920058-add]//
    BufferSize = sizeof (SOL_CONFIGURATION);
    Status = InitVarDefault (mVariableName, &gH2OSolConfigUtilVarstoreGuid, &BufferSize, SolConfig);
    if (EFI_ERROR (Status)) {
...
//[-start-180823-IB10920058-remove]//
//    //
